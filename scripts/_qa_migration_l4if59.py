# -*- coding: utf-8 -*-
"""L4-IF.59 QA: setup-on-k8s/guides/migration batch (3 files).
...
G = "ingest-from/setup-on-k8s/guides"
M = G + "/migration"

# (rel_dir, filename)
FILES = [
    (G, "migration.md"),
    (M, "migrate-dto-to-tenant.md"),
    (M, "migrate-to-helm.md"),
]
...
FENCE_RE = re.compile(r"^\s*```")  # strip-aware: matches list-indented fences too
CAP_PHRASE = re.compile(r"[A-Z][a-z]+(\s+[a-z]+){2,}")
EN_LOWER_RUN = re.compile(r"\b[a-z]+(\s+[a-z]+){3,}\b")

# Bare product/tab labels that legitimately stay as-is.
ALLOW_EN_LINE = {
    "Kubernetes",
    "OpenShift",
}
