#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Critical review + code-fence sync for Batch L4H (configuration-api/azure-credentials-api, parent + 7 endpoints).
ACTIVE config-api (no deprecated banner, L89/L90), structural twin of aws-credentials-api RU (L4G anchor, L85/L86).
Force-syncs ``` fenced blocks EN->RU (byte-identity L98/L100), then checks line/heading/fence/table-row parity,
em-dash=0, mojibake=0, leftover-EN=0, EN-invariants (title, H1x2, * Reference, * Published).
Canon L99 + k8s-credentials shared objects + L101 periods + L4B/L4G **Deprecated**->**Устарело** / **required**->**обязательно**."""
...
PAIRS = [
    (f"{EN}/azure-credentials-api.md", f"{RU}/azure-credentials-api.md"),
    (
        f"{EN}/azure-credentials-api/get-all.md",
        f"{RU}/azure-credentials-api/get-all.md",
    ),
    (
        f"{EN}/azure-credentials-api/get-credentials.md",
        f"{RU}/azure-credentials-api/get-credentials.md",
    ),
    (
        f"{EN}/azure-credentials-api/get-services.md",
        f"{RU}/azure-credentials-api/get-services.md",
    ),
    (
        f"{EN}/azure-credentials-api/delete-credentials.md",
        f"{RU}/azure-credentials-api/delete-credentials.md",
    ),
    (
        f"{EN}/azure-credentials-api/post-new-credentials.md",
        f"{RU}/azure-credentials-api/post-new-credentials.md",
    ),
    (
        f"{EN}/azure-credentials-api/put-credentials.md",
        f"{RU}/azure-credentials-api/put-credentials.md",
    ),
    (
        f"{EN}/azure-credentials-api/put-services.md",
        f"{RU}/azure-credentials-api/put-services.md",
    ),
]
