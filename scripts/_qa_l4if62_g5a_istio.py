# -*- coding: utf-8 -*-
"""L4-IF.62 g5a QA: istio-deployment.md (single file).
...
# Match fences after stripping leading whitespace so indented code fences
# (`   ```) are tracked the same way the builder treats them. Otherwise the
# leftover-EN scanner mis-toggles and flags verbatim code-fence lines.
FENCE_RE = re.compile(r"^\s*```")
CAP_PHRASE = re.compile(r"[A-Z][a-z]+(\s+[a-z]+){2,}")
EN_LOWER_RUN = re.compile(r"\b[a-z]+(\s+[a-z]+){3,}\b")

# Full lines that legitimately stay EN (product/identifier-only lines).
ALLOW_EN_LINE = {
    "Sidecar",
    "Ambient",
}
