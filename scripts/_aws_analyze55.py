#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Per-file uncovered-line count for the L4-IF.55 AWS iot batch.
Imports _build_aws_l4if55 (applies the iot mislabeled-header TABLE signature +
UNIT extension to B), then dumps the unique skeleton to _aws_missing_l4if55.json.
EN never written."""
...
import _build_aws_l4if55  # noqa: F401  applies B.TABLES + B.UNIT extensions
import _build_aws_l4if44 as B

files = [
    x.strip()
    for x in open(os.path.join(B.HERE, "_aws_batch55.txt"), encoding="utf-8")
    if x.strip()
]
rows = []
allflags = []
...
with open(
    os.path.join(B.HERE, "_aws_missing_l4if55.json"), "w", encoding="utf-8"
) as wf:
    json.dump(skel, wf, ensure_ascii=False, indent=1)
print(f"unique prose+cell skeleton: {len(skel)} -> _aws_missing_l4if55.json")
