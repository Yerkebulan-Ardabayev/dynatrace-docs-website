#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Critical review + code-fence sync for Batch L4K (configuration-api):
notifications-api/post-a-notification (DEPRECATED, Settings API banner, L4E/L4J canon)
+ mobile-symbolication-api parent + 9 endpoints (ACTIVE API, L89/L90, no banner).
No env-api twin -> L103 case (b): anchor = k8s-credentials RU for shared objects
(ErrorEnvelope/Error/ConstraintViolation), L4J alerting-profiles RU for deprecated banner.
Domain terms (corpus-dominant): symbol file->"файл символов" (58x), symbolication->
"символикация", deobfuscation->"деобфускация", stack trace->"трассировка стека",
Instrumentation Wizard->"мастер инструментирования". Canon L99. L101 periods by source
(all 400 = "Сбой. Невалидный ввод." WITH period; notif validate-204 no period).
Force-syncs ``` fenced blocks EN->RU (byte-identity L98/L100), then checks
line/heading/fence/table-row parity, em-dash=0, mojibake=0 (incl. source `â`),
leftover-EN=0, EN-invariants (title, H1x2, * Reference, * Published/* Updated on)."""
...
MS = "mobile-symbolication-api"
PAIRS = [
    (
        f"{EN}/notifications-api/post-a-notification.md",
        f"{RU}/notifications-api/post-a-notification.md",
    ),
    (f"{EN}/{MS}.md", f"{RU}/{MS}.md"),
    (f"{EN}/{MS}/get-all.md", f"{RU}/{MS}/get-all.md"),
    (f"{EN}/{MS}/get-files-app.md", f"{RU}/{MS}/get-files-app.md"),
    (
        f"{EN}/{MS}/get-files-app-version.md",
        f"{RU}/{MS}/get-files-app-version.md",
    ),
    (f"{EN}/{MS}/get-storage-info.md", f"{RU}/{MS}/get-storage-info.md"),
    (
        f"{EN}/{MS}/get-supported-versions.md",
        f"{RU}/{MS}/get-supported-versions.md",
    ),
    (f"{EN}/{MS}/del-files-app.md", f"{RU}/{MS}/del-files-app.md"),
    (
        f"{EN}/{MS}/del-files-app-version.md",
        f"{RU}/{MS}/del-files-app-version.md",
    ),
    (
        f"{EN}/{MS}/put-files-app-version.md",
        f"{RU}/{MS}/put-files-app-version.md",
    ),
    (
        f"{EN}/{MS}/put-files-app-version-pin.md",
        f"{RU}/{MS}/put-files-app-version-pin.md",
    ),
]
