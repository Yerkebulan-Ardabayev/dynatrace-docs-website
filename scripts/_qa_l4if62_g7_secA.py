# -*- coding: utf-8 -*-
"""L4-IF.62 g7-secA QA: networking-security-compliance/security-configurations
batch (3 files). Checks line-parity, frontmatter byte-eq, em-dash=0, mojibake=0,
BOM=0, URL-identity, code-fence byte-identity, heading-level parity,
calque/leftover. Framework copied from _qa_meta_l4if58.py.
"""
...
# (rel_dir, filename)
FILES = [
    (SUB, "security-configurations.md"),
    (SUBC, "enable-app-armor.md"),
    (SUBC, "seccomp.md"),
]
...
# Full lines that legitimately stay EN (identifiers / product names / EN-doc
# link titles for external Kubernetes pages / mode-name tab labels).
ALLOW_EN_LINE = {
    "Helm",
    "Manifest",
...
for rel_dir, rel in FILES:
    ...
    # URL identity: strip BOM/em-dash mojibake from EN before comparing, since
    # those bytes sit inside some EN link targets/anchors but not in clean RU.
    en_clean = en.replace("﻿", "").replace("ï»¿", "").replace("â", "")
    if urls(en_clean) != urls(ru):
        only_en = sorted(set(urls(en_clean)) - set(urls(ru)))
        only_ru = sorted(set(urls(ru)) - set(urls(en_clean)))
