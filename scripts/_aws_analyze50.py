#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Per-file uncovered-line count for the L4-IF.50 AWS streaming batch.
Reuses e44 engine + L4-IF.45 durable UNIT extension, reads _aws_batch50.txt.
Dumps unique skeleton (CELL/prose) to _aws_missing_l4if50.json. EN never written."""
...
with open(
    os.path.join(B.HERE, "_aws_missing_l4if50.json"), "w", encoding="utf-8"
) as wf:
    json.dump(skel, wf, ensure_ascii=False, indent=1)
print(f"unique prose+cell skeleton: {len(skel)} -> _aws_missing_l4if50.json")
# titles needed
