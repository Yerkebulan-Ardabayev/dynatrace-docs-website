#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Per-file uncovered-line count for the L4-IF.53 AWS builtin batch.
Imports _build_aws_l4if53 (applies the builtin TABLE signature + UNIT extension
to the shared B engine), then dumps the unique skeleton (CELL/prose) to
_aws_missing_l4if53.json. EN never written."""

import os, sys, json
from collections import Counter

sys.stdout.reconfigure(encoding="utf-8")
import _build_aws_l4if53  # noqa: F401  applies B.TABLES + B.UNIT extensions
import _build_aws_l4if44 as B

files = [
    x.strip()
    for x in open(os.path.join(B.HERE, "_aws_batch53.txt"), encoding="utf-8")
    if x.strip()
]
rows = []
allflags = []
...
with open(
    os.path.join(B.HERE, "_aws_missing_l4if53.json"), "w", encoding="utf-8"
) as wf:
    json.dump(skel, wf, ensure_ascii=False, indent=1)
print(f"unique prose+cell skeleton: {len(skel)} -> _aws_missing_l4if53.json")
