#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""L4-IF.47 - build RU for the AWS aws-service-*.md DB-family standard batch
(6 files: Aurora, DocumentDB, DynamoDB Accelerator (DAX), ElastiCache,
Keyspaces (Cassandra), Neptune).
...
def main():
    ...
    if B.FLAGS:
        ...
        with open(
            os.path.join(B.HERE, "_aws_missing_l4if47.json"), "w", encoding="utf-8"
        ) as wf:
            json.dump(skel, wf, ensure_ascii=False, indent=1)
        with open(
            os.path.join(B.HERE, "_aws_flags_report47.txt"), "w", encoding="utf-8"
        ) as wf:
            wf.write(f"FLAGS: {len(B.FLAGS)} | unique: {len(uniq)}\n" + "=" * 60 + "\n")
            for t, n in uniq.most_common():
                wf.write(f"{n:3d}  {t}\n")
        print(
            f"FLAGS: {len(B.FLAGS)} | unique: {len(uniq)} -> scripts/_aws_flags_report47.txt"
        )
        return 1
    if mode == "build":
        n = 0
