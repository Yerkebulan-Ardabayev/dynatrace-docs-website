#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Critical review + code-fence sync for Batch L4F (configuration-api/credential-vault, parent + 6 endpoints).
Force-syncs every ``` fenced block EN->RU (code never translated => byte-identity, L98/L100),
then checks: line parity, heading/fence/table-row parity, em-dash=0, mojibake=0, leftover-EN=0,
EN-verbatim invariants (frontmatter title, H1x2, * Reference, * Published). Canon L99 + L4E (twin env-api/credential-vault, L101 periods)."""
...
PAIRS = [
    (f"{EN}/credential-vault.md", f"{RU}/credential-vault.md"),
    (
        f"{EN}/credential-vault/del-credentials.md",
        f"{RU}/credential-vault/del-credentials.md",
    ),
    (f"{EN}/credential-vault/get-all.md", f"{RU}/credential-vault/get-all.md"),
    (
        f"{EN}/credential-vault/get-credentials.md",
        f"{RU}/credential-vault/get-credentials.md",
    ),
    (f"{EN}/credential-vault/models.md", f"{RU}/credential-vault/models.md"),
    (
        f"{EN}/credential-vault/post-credentials.md",
        f"{RU}/credential-vault/post-credentials.md",
    ),
    (
        f"{EN}/credential-vault/put-credentials.md",
        f"{RU}/credential-vault/put-credentials.md",
    ),
]
