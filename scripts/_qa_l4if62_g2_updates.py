# -*- coding: utf-8 -*-
"""L4-IF.62 g2 QA: setup-on-k8s/guides/.../updates-and-maintenance (3 files).
...
# (rel_dir, filename)
FILES = [
    (SUB, "auto-update-components.md"),
    (SUB, "dto-auto-update.md"),
    (SUB, "update-uninstall-operator.md"),
]
...
FENCE_RE = re.compile(r"^\s*```")  # tolerate indented code fences (Helm section)
CAP_PHRASE = re.compile(r"[A-Z][a-z]+(\s+[a-z]+){2,}")
EN_LOWER_RUN = re.compile(r"\b[a-z]+(\s+[a-z]+){3,}\b")

# Full lines that legitimately stay EN (component/option/tab labels, bare image
# rows, EN product/identifier headings, single tech words).
ALLOW_EN_LINE = {
    "Kubernetes",
    "OpenShift",
    "Helm",
