...
Reuses e44 engine + L4-IF.45 durable UNIT extension, reads _aws_batch52.txt.
Dumps unique skeleton (CELL/prose) to _aws_missing_l4if52.json. EN never written."""
...
B.BATCH_PATH = os.path.join(B.HERE, "_aws_batch52.txt")
...
with open(
    os.path.join(B.HERE, "_aws_missing_l4if52.json"), "w", encoding="utf-8"
) as wf:
    json.dump(skel, wf, ensure_ascii=False, indent=1)
print(f"unique prose+cell skeleton: {len(skel)} -> _aws_missing_l4if52.json")
# titles needed
