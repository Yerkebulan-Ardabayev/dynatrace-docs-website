    ...
    for i, (e, r) in enumerate(zip(en, ru)):
        ...
        if es.startswith("|") and rs.startswith("|"):
            ...
            if (
                ...
                # col1 = Description; exempt identifier-echo (desc == name,
                # an EN source quirk: the metric name repeated as description)
                if (
                    ecells[1]
                    and ecells[1] != ecells[0]
...
def main():
    files = [
        x.strip()
        for x in open(os.path.join(HERE, "_aws_batch44.txt"), encoding="utf-8")
        if x.strip()
    ]
    for f in files:
        qa_file(f)
