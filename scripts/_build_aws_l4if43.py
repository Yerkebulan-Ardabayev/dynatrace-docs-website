...
BATCH_PATH = os.path.join(HERE, "_aws_batch.txt")
...
def main():
    ...
    if FLAGS:
        ...
        with open(
            os.path.join(HERE, "_aws_missing_l4if43.json"), "w", encoding="utf-8"
        ) as wf:
            json.dump(skel, wf, ensure_ascii=False, indent=1)
        with open(
            os.path.join(HERE, "_aws_flags_report.txt"), "w", encoding="utf-8"
        ) as wf:
            wf.write(f"FLAGS: {len(FLAGS)} | unique: {len(uniq)}\n" + "=" * 60 + "\n")
            for t, n in uniq.most_common():
                wf.write(f"{n:3d}  {t}\n")
        print(
            f"FLAGS: {len(FLAGS)} | unique: {len(uniq)} -> scripts/_aws_flags_report.txt"
        )
        return 1
    if mode == "build":
        n = 0
