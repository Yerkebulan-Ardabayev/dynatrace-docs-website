#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""QA review for the OneAgent AIX operation RU batch (3 files) against EN sources.

Checks: line-parity, BOM/CRLF/trailing-newline, frontmatter byte-equality (source/scraped),
em-dash + broken-mojibake-em-dash absence in RU, broken-ellipsis absence, ï»¿ count parity,
URL-list identity, fenced code-block byte-identity, heading-count parity,
latin-only link titles (untranslated), and Cyrillic presence."""
...
EN = Path(
    "docs/managed/ingest-from/dynatrace-oneagent/installation-and-operation/aix/operation"
)
RU = Path(
    "docs/managed-ru/ingest-from/dynatrace-oneagent/installation-and-operation/aix/operation"
)
FILES = [
    "stop-restart-oneagent-on-aix.md",
    "enable-auto-injection.md",
    "update-oneagent-on-aix.md",
]

BOM = b"\xef\xbb\xbf"
MOJI_DASH = "â"  # double-encoded em-dash/ellipsis scrape artifact (visible as 'a-hat')
...
for rel in FILES:
    ...
    if ru.count(MOJI_DASH):
        fails.append(f"RU broken-mojibake (a-hat) count={ru.count(MOJI_DASH)}")
    # broken double-encoded ellipsis bytes c3a2 c280 c2a6 must be gone
    if b"\xc3\xa2\xc2\x80\xc2\xa6" in rub or b"\xc3\xa2\xc2\x80\xc2\x94" in rub:
        fails.append("RU still has broken mojibake bytes (ellipsis/em-dash)")

    if ru.count("ï»¿") or ru.count("﻿"):
        fails.append("RU still has BOM mojibake (canon = strip)")
