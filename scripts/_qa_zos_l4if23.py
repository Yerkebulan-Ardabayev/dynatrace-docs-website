"""QA script for L4-IF.23 zOS batch.

Checks for 5 RU files:
- line-parity (RU lines == EN lines)
- em-dash absent
- BOM at start absent / double-encoded BOM absent
- broken-a (mojibake) absent in RU
- URL set identical EN vs RU
- frontmatter: title differs, source/scraped/updated byte-identical
- heading count + level parity per line
- code-fence count parity
- EN-leftover scan: lines with 3+ EN words and no Cyrillic (per feedback_translation_qa_blindspots)
"""
...
FILES = [
    ...
    "ingest-from/dynatrace-oneagent/installation-and-operation/zos/operation/zos-code-module-messages.md",
...
for f in FILES:
    ...
    if en_urls != ru_urls:
        ...
        if diff_en or diff_ru:
            errors.append(
                f"URL diff: missing_in_RU={list(diff_en)[:2]} added_in_RU={list(diff_ru)[:2]}"
            )
