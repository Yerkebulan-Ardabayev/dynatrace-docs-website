# -*- coding: utf-8 -*-
"""L4-IF.60b QA: setup-on-k8s/guides/migration/migrate-to-dto.md (1 file).
...
# list-INDENTED code fences must be detected too.
FENCE_RE = re.compile(r"^\s*```")
CAP_PHRASE = re.compile(r"[A-Z][a-z]+(\s+[a-z]+){2,}")
EN_LOWER_RUN = re.compile(r"\b[a-z]+(\s+[a-z]+){3,}\b")

# Tab labels / bare technical lines that legitimately stay as-is.
ALLOW_EN_LINE = {
    "Kubernetes",
    "OpenShift",
}
