#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Per-file uncovered-line count for the L4-IF.48 compute/integration AWS batch.
Reuses the proven e44 engine (same TABLE/UNIT/TRANS) + the L4-IF.45 durable UNIT
extension, reads _aws_batch48.txt. Reports how many FLAG lines each file still
has, so a coherent, right-sized ship-group can be assembled. Also dumps the
unique skeleton (CELL/prose) to _aws_missing_l4if48.json. EN never written.
"""
...
OUT = os.path.join(B.HERE, "_aws_analyze48_out.txt")
...
json.dump(
    skel,
    open(os.path.join(B.HERE, "_aws_missing_l4if48.json"), "w", encoding="utf-8"),
    ensure_ascii=False,
    indent=1,
)
