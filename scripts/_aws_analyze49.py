#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Per-file uncovered-line count for the L4-IF.49 AWS remainder (all 20 left).
Reuses e44 engine + L4-IF.45 durable UNIT extension, reads _aws_batch49.txt.
Dumps unique skeleton (CELL/prose) to _aws_missing_l4if49.json. EN never written."""
import os, sys, json
from collections import Counter
sys.stdout.reconfigure(encoding="utf-8")
import _build_aws_l4if44 as B
B.UNIT.update({
    "Count/Minute": "Количество в минуту", "Microseconds": "Микросекунда",
    "Microsecond": "Микросекунда", "Bits/Second": "Бит в секунду",
    "Kilobytes/Second": "Килобайт в секунду", "Megabytes/Second": "Мегабайт в секунду",
    "DecibelMilliWatts": "Децибел-милливатт",
})
B.BATCH_PATH = os.path.join(B.HERE, "_aws_batch49.txt")
...
rows.sort()
print(f"{'TOT':>4} {'CELL':>4} {'PROSE':>5} {'UNIT':>4} {'TTL':>3} {'ROW':>4} {'APL':>3}  file")
for r in rows:
    print(f"{r[0]:4d} {r[1]:4d} {r[2]:5d} {r[3]:4d} {r[4]:3d} {r[5]:4d} {r[6]:3d}  {r[7]}")
print("="*70)
print(f"TOTAL flags: {len(allflags)}")
# unique skeleton dump (prose+cells only)
uniq = Counter(t for (_, _, t) in allflags)
skel = {t[6:] if t.startswith('CELL: ') else t: "" for t in uniq
        if not t.startswith(("TITLE:", "UNIT?:", "APPLIC?:", "ROW?:"))}
print(f"unique prose+cell skeleton: {len(skel)}")
...
print(f"\n--- distinct ROW? (unknown table rows) top 15 ---")
for t,n in rowsigs.most_common(15):
    print(f"  {n:3d}  {t[:90]}")
