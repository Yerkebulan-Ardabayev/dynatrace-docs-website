# -*- coding: utf-8 -*-
"""L4-IF.62 g3 QA: setup-on-k8s/guides/deployment-and-configuration/
...
Note: EN sources contain mojibake (`ï»¿` before some `]`, and `â\x80\x94`
em-dash in k8s L16). The RU is written mojibake-clean, so the EN<->RU URL set
and code-fence comparisons normalize EN mojibake before comparing.
"""
...
# Strip the same mojibake codepoints the builder removes, so EN-side
# comparisons (URLs, fences) match the clean RU output.
MOJI_RE = re.compile("[﻿ï»¿â\x80\x94]")

# (rel_dir, filename)
FILES = [
    (SUB, "annotate.md"),
    (SUB, "instrument-nginx.md"),
    (SUB, "k8s-api-monitoring.md"),
]
...
# Full lines that legitimately stay English:
# - deployment-mode / feature identifiers (tab selectors)
# - Kubernetes/OpenShift code-tab labels
# - mode tab labels kept EN (corpus-consistent across managed-ru K8s guides)
# - connection-method tab labels
# - the Step-1 image line (alt-text stays EN per corpus)
ALLOW_EN_LINE = {
...
def urls(text):
    return sorted(URL_RE.findall(MOJI_RE.sub("", text)))
...
for rel_dir, rel in FILES:
    ...
    # normalize EN mojibake inside code blocks before comparing (none expected,
    # but keeps the check robust if a fence interior ever carries mojibake)
    eb = [[MOJI_RE.sub("", x) for x in blk] for blk in code_blocks(el)]
    rb = code_blocks(rl)
    if eb != rb:
