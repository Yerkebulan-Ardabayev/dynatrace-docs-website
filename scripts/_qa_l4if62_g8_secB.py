# -*- coding: utf-8 -*-
"""L4-IF.62 G8 secB QA: setup-on-k8s/guides networking-security-compliance
...
# (rel_dir, filename)
FILES = [
    (SEC, "openshift-configuration.md"),
    (SEC, "pod-security-standards.md"),
    (SEC, "token-rotation.md"),
    (ADV, "injection-readonly-volume.md"),
]
...
FENCE_RE = re.compile(r"^\s*```")  # fences may be indented under list items
CAP_PHRASE = re.compile(r"[A-Z][a-z]+(\s+[a-z]+){2,}")
EN_LOWER_RUN = re.compile(r"\b[a-z]+(\s+[a-z]+){3,}\b")

# Full lines that legitimately stay (mostly) EN: identifier/component table rows
# and mode-name identifier lines (product names / SCC names / mode identifiers).
ALLOW_EN_LINE = {
    "| Operator | `nonroot-v2` |",
    "| Webhook | `nonroot-v2` |",
