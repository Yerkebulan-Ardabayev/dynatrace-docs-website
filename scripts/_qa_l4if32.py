"""QA script for L4-IF.32 technology-support matrix batch.

2 files (support matrices):
- mainframe-technology-support.md (177)
- oneagent-platform-and-capability-support-matrix.md (266)
"""
...
FILES = [
    "ingest-from/technology-support/mainframe-technology-support.md",
    "ingest-from/technology-support/oneagent-platform-and-capability-support-matrix.md",
]
...
# catches untranslated English prose whose first word is lowercase or hyphenated
# (e.g. "Out-of-the-box infrastructure alerting capabilities are not supported ...")
EN_LOWER_RUN_RE = re.compile(r"(?:\b[a-z]{2,}\b[ ,]+){3,}\b[a-z]{2,}\b")
...
for f in FILES:
    ...
    # table-column-count parity (matrix integrity): for each table row, pipe count
    for i, (el, rl) in enumerate(zip(en_lines, ru_lines)):
        if el.startswith("|") and rl.startswith("|"):
            if el.count("|") != rl.count("|"):
                errors.append(
                    f"L{i + 1}: pipe-count EN={el.count('|')} RU={rl.count('|')}"
                )
    ...
    for i, line in enumerate(ru_lines):
        ...
        lower_run = EN_LOWER_RUN_RE.search(cleaned)
        has_cyr = bool(CYR_RE.search(line))
        if (matches or lower_run) and not has_cyr:
            hit = matches[0] if matches else lower_run.group(0)
            warns.append(
                f"L{i + 1}: EN-leftover={hit[:60]!r} | line: {line.strip()[:100]}"
            )
