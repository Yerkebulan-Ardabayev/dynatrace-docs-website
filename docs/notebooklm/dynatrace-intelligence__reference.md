...
Прогностический анализ предсказывает будущие значения любого временного ряда числовых значений. Прогностический анализатор не ограничивается хранимыми метрическими данными — вы можете использовать свои собственные данные, задать метрический запрос или выполнить запрос данных, в результате которого получается временной ряд числовых значений.
...
Вы можете запустить прогностический анализ из своей [тетради](/docs/dynatrace-intelligence/dynatrace-intelligence-integrations/davis-for-notebooks "Запустите анализ ИИ в Dynatrace Notebooks.").
...
* [Прогностический анализатор выборки](#выборка)
* [Прогностический анализатор линейной экстраполяции](#линейная-экстраполяция)

Прогностический анализатор выборки используется, если дисперсия **линейной истории временного ряда** больше максимального значения пары `0,1` и `0,001 × дисперсия входного временного ряда`. Линейная история временного ряда — это X наиболее недавних точек данных, где X находится в диапазоне от `14` до `20`.
...
Прогностический анализатор выборки обеспечивает многошаговые прогнозы на основе сезонных закономерностей. Его архитектура показана на изображении ниже.

![Общий алгоритм прогностического анализатора](https://dt-cdn.net/images/generic-forecaster-analyser-940-f6f15f47fc.png)

Сначала к входному временному ряду применяется преобразователь пути, а затем прогностический анализатор обучается на нем. Если в результате полученная модель недействительна, вместо нее используется прогностический анализатор линейной экстраполяции. Если модель действительна, пути выборки создаются сезонным прогностическим анализатором. Эти пути затем обратно преобразуются до применения квантилей на каждом временном шаге.

### Сезонные закономерности

Прогностический анализатор всегда ищет различные сезонные закономерности во входном временном ряде. Входной временной ряд должен содержать достаточно данных, чтобы обнаружить сезонность надежно.
...
1. Сезонный прогностический анализатор оценивает квартили распределения временного ряда на момент следующей точки данных.

   ![Многошаговый алгоритм прогнозирования - шаг 1](https://dt-cdn.net/images/multi-step-prediction-1-547-f8edff4ed6.png)
2. Распределение значения на следующем временном шаге оценивается из квартилей. Колоколообразная кривая на изображении приведена для иллюстрации и не представляет собой распределение, используемое в прогностическом анализаторе выборки.

   ![Многошаговый алгоритм прогнозирования - шаг 2](https://dt-cdn.net/images/multi-step-prediction-2-553-d30631c97f.png)
3. Значение на следующем временном шаге выбирается из распределения, полученного на шаге 2.

   ![Многошаговый алгоритм прогнозирования - шаг 3](https://dt-cdn.net/images/multi-step-prediction-3-565-fe5b638654.png)
4. Значение, полученное на шаге 3, теперь считается частью временного ряда, и прогностический анализатор повторяет шаги 1-4, пока не достигнет горизонта прогнозирования.
   ...
   ![Многошаговый алгоритм прогнозирования - шаг 4](https://dt-cdn.net/images/multi-step-prediction-4-559-33e251190f.png)
5. Предсказанные точки данных образуют один путь выборки (показан красным на изображении ниже).

   ![Многошаговый алгоритм прогнозирования - шаг 5](https://dt-cdn.net/images/multi-step-prediction-5-1051-f971c5a471.png)
6. Прогностический анализатор повторяет этот процесс N раз, создавая несколько путей выборки.

   ![Многошаговый алгоритм прогнозирования - шаг 6](https://dt-cdn.net/images/multi-step-prediction-6-1049-931db96b7b.png)
7. Прогностический анализатор берет искомый квантиль на каждом временном шаге, образуя интервал прогнозирования (показан светло-фиолетовым на изображении ниже).

   ![Многошаговый алгоритм прогнозирования - шаг 7](https://dt-cdn.net/images/multi-step-prediction-7-1049-68f5d98212.png)

## Прогностический анализатор линейной экстраполяции

Прогностический анализатор линейной экстраполяции — это алгоритм, который использует [простую линейную регрессию](https://en.wikipedia.org/wiki/Simple_linear_regression), чтобы найти лучшую линию, проходящую через набор точек данных, и расширить эту линию в будущее. Количество точек данных, используемых для обучения прогностического анализатора, определяется следующим образом
...
Для обучения прогностического анализатора линейной экстраполяции `nHistory` должно содержать **не менее 14** не-null точек данных.
...
Стандартное отклонение прогноза (SDprediction) рассчитывается как максимальное стандартное отклонение нижней и верхней границ прогноза, а также стандартное отклонение точечного прогноза.
...
* Поля в записи временного ряда, кроме timeframe, duration и числовых массивов данных, считаются размерностями
...
В формате единого значения каждая запись в наборе данных указывает одно значение в временном ряду. Допускается ровно одно числовое поле на запись.

Запись в допустимом формате единого значения должна содержать:
...
* Серия определяется на основе уникальных значений для каждого измерения. Записи с одинаковыми измерениями (строковыми свойствами) считаются принадлежащими к одной и той же временной серии.

DQL ответ в формате единого значения

Следующий JSON описывает структуру формата единого значения.
...
Этот пример показывает прогноз для метрики доступного диска. В данных нет сезонной закономерности, и есть нисходящая тенденция. Здесь используется прогнозист линейной экстраполяции, который дает широкий интервал прогноза.
...
Сезонные данные с шумом на расширенном временном интервале

Этот пример показывает прогноз для временного интервала 6 часов. Помимо обширного временного интервала прогноза, сами данные содержат шум и нисходящую тенденцию, которые влияют на качество прогноза - интервал прогноза становится все шире и шире, когда он продвигается в будущее.

![Сезонный прогноз с шумом за 6-часовой временной интервал](https://dt-cdn.net/images/seasonal-forecast-with-noise-result-6h-1730-75d0e7ceb1.png)
...
Сезонная базовая линия представляет собой динамический подход к базовой линии, где системы имеют разные сезонные закономерности. Примером сезонной закономерности является метрика, которая увеличивается во время рабочих часов и остается низкой вне их. В таком случае сложно настроить конфигурацию оповещения на основе статического или автоадаптивного порога. Если вы установите фиксированный порог на основе поведения во время рабочих часов, вы пропустите аномалию вне рабочих часов. Dynatrace Intelligence **изучает сезонное поведение** ваших метрик и автоматически создает доверительный интервал для него.

Когда конфигурация события метрики включает несколько сущностей, каждая сущность получает свою собственную сезонную базовую линию, и каждая базовая линия оценивается независимо. Например, если область события включает пять сервисов, Dynatrace рассчитывает и оценивает пять независимых базовых линий.

## Пример базовой линии

В этом примере мы рассматриваем количество действий пользователей в приложении. Реальные взаимодействия пользователей часто следуют сезонности рабочих часов и выходных, и наш временной ряд также показывает ежедневные пиковые значения.

Когда оценивается статическим порогом, эти ежедневные пиковые значения приводят к ложноположительным сигналам тревоги (красная аннотация внутри диаграммы).
...
Dynatrace Intelligence автоматически обнаруживает сезонное поведение во временном ряду и создает доверительный интервал на основе этой закономерности. Как вы можете видеть на диаграмме ниже, модель распознает сезонность и не генерирует ложные сигналы тревоги на пиках. Однако она все равно сработает, если скачок произойдет в другое время.
...
Вы можете адаптировать ширину доверительного интервала с помощью параметра **терпимость**. Более высокая терпимость означает более широкий доверительный интервал, что приводит к меньшему количеству срабатываний событий. Разрешенный диапазон для терпимости составляет от `0,1` до `10`, с `4` в качестве значения по умолчанию.

Как следует из названия, сезонная модель полезна для данных с сезонными закономерностями (например, ежедневными пиками). Поскольку она потребляет значительные ресурсы и требует больше времени для изучения поведения, существуют некоторые проверки валидации, чтобы обеспечить оптимальную производительность. [Валидация](#training) оценивает каждую пару (уникальные комбинации метрики-размерности-значения размерности). Если проверка не пройдена, используется простая модель, и соответствующее сообщение появляется в предварительном просмотре.

Простая модель рассчитывает доверительный интервал на основе робастной оценки распределения входных данных и его рассчитанный интервал остается постоянным во времени. Для простой модели параметр **терпимость** контролирует ширину доверительного интервала таким же образом, как и для сезонной модели.
...
Другим важным параметром для сезонных базовых линий является [скользящее окно](/docs/dynatrace-intelligence/anomaly-detection/anomaly-detection-configuration#sliding-window "Как настроить оповещение для пропущенных измерений.") , которое используется для сравнения текущих измерений с доверительным интервалом. Оно определяет, как часто доверительный интервал должен быть нарушен в течение скользящего окна времени, чтобы сработать событие (нарушения не должны быть последовательными). Этот подход помогает уменьшить количество ложных положительных сигналов при оповещении. Вы можете задать скользящее окно до максимума 60 минут.

## Обучение модели базовой линии

Модель сезонной базовой линии использует значения метрик за каждую минуту за последние 14 дней для обучения и обучается по паре (уникальным комбинациям метрики-размерности-значения размерности). Обучение происходит ежедневно.

Ниже приведена схема процесса обучения.

![Обзор модели сезонной базовой линии](https://dt-cdn.net/images/seasonal-baseline-model-training-1187-8bd05de831.png)

Первый шаг, **валидация данных**, гарантирует, что обучение сезонной модели на данных метрик является разумным. Вот некоторые причины для переключения на простую модель:

* данные имеют много пропущенных значений
* данные имеют много выбросов
* в данных нет достаточной изменчивости
* в данных нет обнаруживаемой сезонности.

Если эта проверка не пройдена, используется **простая модель**. Если проверка пройдена успешно, следующим шагом является **предварительная обработка входных данных**. На этом этапе данные готовятся для обучения модели квантильной регрессии, которая изучает сезонное поведение из трех основных характеристик (фич) данных:

* **Авторегрессивные фичи** представляют собой десять наиболее недавних исторических точек данных (маленькие пробелы интерполируются)
...
Следующим шагом является **валидация модели**, которая проверяет надежность прогнозированных квантилей на подмножестве данных. Если эта проверка не пройдена, используется **простая модель**.

После завершения обучения Dynatrace использует сезонную модель для обнаружения аномалий. Каждую минуту модель производит **одношаговый прогноз** доверительного интервала для следующей минуты. Как только фактическая точка данных приходит, предсказанный доверительный интервал сравнивается с фактическим значением, чтобы проверить, находится ли значение в пределах предсказанного интервала.
...
Прочитайте это объяснение](/docs/dynatrace-intelligence/reference/ai-models/causal-correlation-analysis)[#### Прогнозный анализ ИИ интеллекта Dynatrace

Узнайте, как прогнозный ИИ интеллекта Dynatrace генерирует прогнозы.
...
Максимальное количество событий Davis на провайдера, которые можно обработать в течение часа, составляет 100 000. Это ограничение пополняется каждый час.
