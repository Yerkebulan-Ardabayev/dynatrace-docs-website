...
Прогностический анализ прогнозирует будущие значения любого временного ряда числовых значений. Анализатор прогноза не ограничивается хранимыми данными метрик — вы можете использовать свои собственные данные, задать запрос метрики или выполнить запрос данных, в результате которого получится временной ряд числовых значений.
...
Вы можете запустить прогностический анализ из своей [тетради](/docs/dynatrace-intelligence/dynatrace-intelligence-integrations/davis-for-notebooks "Запустить анализ ИИ в Dynatrace Notebooks.").

## Входные данные анализатора

## Методология анализа

Когда вы запускаете прогностический анализ, временной ряд отправляется в подходящий прогностик, который производит прогноз. Затем оценивается [качество прогноза](#quality), и прогноз и оценка возвращаются в качестве результатов анализа.
...
* [Прогностик выборки](#sampling)
* [Прогностик линейной экстраполяции](#linear-extrapolation)

Прогностик выборки используется, если дисперсия **линейной истории временного ряда** больше максимального значения пары `0,1` и `0,001 × дисперсия входного временного ряда`. Линейная история временного ряда — это X наиболее недавних точек данных, где X находится в диапазоне от `14` до `20`.

В любом другом случае (включая случай, когда обучение прогностика выборки завершается неудачно) используется прогностик линейной экстраполяции.
...
Сначала к входному временному ряду применяется преобразователь пути, а затем прогностик обучается на нем. Если полученная модель недействительна, вместо нее используется прогностик линейной экстраполяции. Если модель действительна, пути выборки создаются сезонным прогностиком. Эти пути затем обратно преобразуются до применения квантилей на каждом шаге времени.

### Сезонные закономерности

Прогностик всегда ищет различные сезонные закономерности во входном временном ряде. Входной временной ряд должен содержать достаточно данных, чтобы обнаружить сезонность надежно.
...
Помимо этих естественных закономерностей, прогностик ищет любые повторяющиеся закономерности во временном ряде, разбивая их на основе определенного временного окна.

### Прогноз шаг за шагом

1. Сезонный прогностик оценивает квартили распределения временного ряда на момент следующей точки данных.

   ![Многоступенчатый алгоритм прогнозирования - шаг 1](https://dt-cdn.net/images/multi-step-prediction-1-547-f8edff4ed6.png)
2. Распределение для значения на следующем шаге времени оценивается из квартилей. Колоколообразная кривая на изображении приведена для иллюстрации и не представляет собой распределение, используемое в прогностике выборки.
...
4. Значение, полученное на шаге 3, теперь считается частью временного ряда, и прогностик повторяет шаги 1-4, пока не достигнет горизонта прогноза.
...
7. Прогностик принимает искомый квантиль на каждом шаге времени, образуя интервал прогноза (показан светло-фиолетовым на изображении ниже).
...
Для обучения прогностика линейной экстраполяции `nHistory` должно содержать **не менее 14** не-NULL-значений.
...
После генерации прогнозов мы оцениваем их качество, чтобы обнаружить потенциальные численные проблемы. Для оценки качества прогноза анализатор сравнивает стандартное отклонение прогноза со стандартным отклонением входного временного ряда (SDinput).
...
Чтобы учесть допустимые тенденции в прогнозах, анализатор использует масштабный коэффициент (SCF). Когда длина прогнозных данных (Nprediction) велика по сравнению с входным временным рядом (Ninput), мы разрешаем большее стандартное отклонение прогноза, чем для небольшого прогноза.
...
Когда вы используете прогностический анализ в одной из своих [тетрадей](/docs/analyze-explore-automate/dashboards-and-notebooks/notebooks "Анализируйте, визуализируйте и делитесь идеями из ваших данных наблюдаемости — все в одном совместном, настраиваемом рабочем пространстве."), вы можете записать любой DQL запрос, который возвращает данные временного ряда в [формате записи временного ряда](#time-series-record-format).
...
Размерности можно добавлять в виде дополнительных свойств.

* Дополнительные свойства могут иметь только строковые значения.
* Серия определяется на основе уникальных значений для каждой размерности. Записи с одинаковыми размерностями (строковыми свойствами) считаются принадлежащими одной и той же временной серии.
...
Этот пример показывает прогноз для доступного дискового метрика. В данных нет сезонной закономерности, и есть нисходящая тенденция. Здесь используется прогнозист линейной экстраполяции, производящий широкий интервал прогноза.
...
Этот пример показывает прогноз для временного интервала 6 часов. Помимо обширного временного интервала прогноза, сами данные содержат некоторый шум и нисходящую тенденцию, которые влияют на качество прогноза - интервал прогноза расширяется все больше и больше, когда он продвигается в будущее.
...
Когда оценивается статическим порогом, эти ежедневные пиковые значения приводят к ложноположительным сигналам тревоги (красная аннотация на графике).
...
Модель сезонной базовой линии использует значения метрик для каждой минуты за последние **14 дней** в качестве эталона для расчета базовой линии. Модель учитывает обычное поведение метрики и стремится обнаружить регулярно повторяющиеся закономерности. Она использует **вероятностное прогнозирование** для расчета доверительного интервала, который охватывает ожидаемый диапазон будущей точки.

Вы можете адаптировать ширину доверительного интервала с помощью параметра **допуск**. Более высокий допуск означает более широкий доверительный интервал, что приводит к меньшему количеству срабатываний событий. Разрешенный диапазон для допуска составляет от `0,1` до `10`, с значением `4` по умолчанию.

Как следует из названия, сезонная модель полезна для данных с сезонными закономерностями (например, ежедневными пиками). Поскольку она потребляет значительные ресурсы и требует больше времени для обучения, существуют некоторые проверки валидации для обеспечения оптимальной производительности. [Валидация](#training) оценивает каждую пару (уникальные комбинации метрики - размерность - значение размерности). Если проверка не проходит, используется простая модель, и соответствующее сообщение появляется в предварительном просмотре.

Простая модель рассчитывает доверительный интервал на основе робастной оценки распределения входных данных и его рассчитанный интервал остается постоянным во времени. Для простой модели параметр **допуск** контролирует ширину доверительного интервала таким же образом, как и для сезонной модели.
...
Другим важным параметром для сезонных базовых линий является [скользящее окно](/docs/dynatrace-intelligence/anomaly-detection/anomaly-detection-configuration#sliding-window "Как настроить оповещение для пропущенных измерений.") , которое используется для сравнения текущих измерений с доверительным интервалом. Оно определяет, как часто доверительный интервал должен быть нарушен в течение скользящего окна времени, чтобы сработать событие (нарушения не должны быть последовательными). Этот подход помогает уменьшить количество ложных положительных сигналов при оповещении. Вы можете задать скользящее окно до максимального значения 60 минут.

## Обучение модели базовой линии

Модель сезонной базовой линии использует значения метрик для каждой минуты за последние 14 дней для обучения и обучается по паре (уникальные комбинации метрики - размерность - значение размерности). Обучение происходит ежедневно.

Ниже представлена схема процесса обучения.

![схема обучения модели сезонной базовой линии](https://dt-cdn.net/images/seasonal-baseline-model-training-1187-8bd05de831.png)

Первым шагом является **валидация данных**, которая гарантирует, что обучение сезонной модели на данных метрик является разумным. Вот некоторые причины для переключения на простую модель:

* данные содержат много пропущенных значений
* данные содержат много выбросов
* в данных нет достаточной изменчивости
* в данных не обнаруживается сезонность.

Если эта проверка не проходит, используется **простая модель**. Если проверка проходит успешно, следующим шагом является **предварительная обработка входных данных**. На этом этапе данные готовятся для обучения модели квантильной регрессии, которая учитывает сезонное поведение из трех основных характеристик (фич) данных:

* **авторегрессионные фичи** представляют собой десять последних исторических точек данных (маленькие пробелы интерполируются)
* **фичи времени и дня** идентифицируют день или конкретное время дня, что может помочь в обучении закономерности, возникающей только в конкретное время
