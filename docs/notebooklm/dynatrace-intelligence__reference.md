...
Прогнозный анализ прогнозирует будущие значения любого временного ряда числовых значений. Анализатор прогнозов не ограничивается хранимыми данными метрик — вы можете использовать свои собственные данные, использовать запрос метрик или выполнить запрос данных, который приводит к временному ряду числовых значений.
...
Прогнозист выборки используется, если дисперсия **линейной истории временного ряда** больше максимального значения пары `0.1` и `0.001 × дисперсия входного временного ряда`. Линейная история временного ряда представляет собой X наиболее недавних точек данных, где X находится в диапазоне от `14` до `20`.

В любом другом случае (включая случай, когда обучение прогнозиста выборки не удалось), используется прогнозист линейной экстраполяции.
...
Сначала к входному временному ряду применяется преобразователь пути, а затем прогнозист обучается на нем. Если полученная модель недействительна, вместо нее используется прогнозист линейной экстраполяции. Если модель действительна, пути выборки создаются сезонным прогнозистом. Эти пути затем обратно преобразуются до применения квантилей на каждом временном шаге.

### Сезонные закономерности

Прогнозист всегда ищет различные сезонные закономерности во входном временном ряде. Входной временной ряд должен содержать достаточно данных, чтобы надежно обнаружить сезонность.
...
5. Прогнозируемые точки данных образуют один путь выборки (показан красным на изображении ниже).
...
7. Прогнозист берет искомый квантиль на каждом временном шаге, формируя интервал прогноза (показан светло-фиолетовым на изображении ниже).
...
Прогнозист линейной экстраполяции — это алгоритм, который использует [простую линейную регрессию](https://en.wikipedia.org/wiki/Simple_linear_regression), чтобы найти лучшую линию, проходящую через набор точек данных, и продлить эту линию в будущее. Количество точек данных, используемых для обучения прогнозиста, определяется следующим образом
...
Чтобы обучить прогнозист линейной экстраполяции, `nHistory` должно содержать **не менее 14** не-NULL точек данных.
...
После генерации прогнозов мы оцениваем их качество, чтобы выявить потенциальные численные проблемы. Чтобы оценить качество прогноза, анализатор сравнивает стандартное отклонение прогноза со стандартным отклонением входного временного ряда (SDinput).
...
В формате единичного значения каждая запись задает одно значение в временном ряду. Допускается ровно одно числовое поле на запись.
