...
Прогностический анализ предсказывает будущие значения любого временного ряда числовых значений. Прогностический анализатор не ограничивается хранимыми метрическими данными - вы можете использовать свои собственные данные, использовать запрос метрики или выполнить запрос данных, который приводит к временному ряду числовых значений.
...
Вы можете запустить прогностический анализ из своей [тетради](/docs/dynatrace-intelligence/dynatrace-intelligence-integrations/davis-for-notebooks "Выполните анализ ИИ в Dynatrace Notebooks.").

## Входные данные анализатора

## Методология анализа

Когда вы запускаете прогностический анализ, временной ряд отправляется в подходящий прогностический анализатор, который производит прогноз. Затем оценивается [качество прогноза](#качество), и прогноз и оценка возвращаются в качестве результатов анализа.
...
* [Линейный экстраполяционный прогностический анализатор](#линейная-экстраполяция)
...
В любом другом случае (включая случай, когда обучение прогностического анализатора выборки не удалось) используется линейный экстраполяционный прогностический анализатор.
...
1. Сезонный прогностический анализатор оценивает квартили распределения временного ряда на отметке времени следующей точки данных.
   ...
   ![Алгоритм многоступенчатого прогноза - шаг 1](https://dt-cdn.net/images/multi-step-prediction-1-547-f8edff4ed6.png)
2. Распределение для значения на следующем временном шаге оценивается из квартилей. Колоколообразная кривая на изображении приведена для иллюстрации и не представляет собой распределение, используемое в прогностическом анализаторе выборки.
   ...
   ![Алгоритм многоступенчатого прогноза - шаг 2](https://dt-cdn.net/images/multi-step-prediction-2-553-d30631c97f.png)
3. Значение для следующего временного шага выбирается из распределения, полученного на шаге 2.

   ![Алгоритм многоступенчатого прогноза - шаг 3](https://dt-cdn.net/images/multi-step-prediction-3-565-fe5b638654.png)
4. Значение, полученное на шаге 3, теперь считается частью временного ряда, и прогностический анализатор повторяет шаги 1-4, пока не достигнет горизонта прогноза.

   ![Алгоритм многоступенчатого прогноза - шаг 4](https://dt-cdn.net/images/multi-step-prediction-4-559-33e251190f.png)
5. Предсказанные точки данных образуют один путь выборки (показан красным на изображении ниже).

   ![Алгоритм многоступенчатого прогноза - шаг 5](https://dt-cdn.net/images/multi-step-prediction-5-1051-f971c5a471.png)
6. Прогностический анализатор повторяет этот процесс N раз, создавая несколько путей выборки.

   ![Алгоритм многоступенчатого прогноза - шаг 6](https://dt-cdn.net/images/multi-step-prediction-6-1049-931db96b7b.png)
7. Прогностический анализатор берет искомый квантиль на каждом временном шаге, образуя интервал прогноза (показан светло-фиолетовым на изображении ниже).

   ![Алгоритм многоступенчатого прогноза - шаг 7](https://dt-cdn.net/images/multi-step-prediction-7-1049-68f5d98212.png)

## Линейный экстраполяционный прогностический анализатор

Линейный экстраполяционный прогностический анализатор - это алгоритм, который использует [простую линейную регрессию](https://en.wikipedia.org/wiki/Simple_linear_regression), чтобы найти линию, которая лучше всего подходит к набору точек данных, и продлить эту линию в будущее. Количество точек данных, используемых для обучения прогностического анализатора, определяется следующим образом
...
Чтобы обучить линейный экстраполяционный прогностический анализатор, `nHistory` должно содержать **не менее 14** не-null точек данных.
...
После генерации прогнозов мы оцениваем их качество, чтобы обнаружить потенциальные численные проблемы. Чтобы оценить качество прогноза, анализатор сравнивает стандартное отклонение прогноза с стандартным отклонением входного временного ряда (SDinput).
...
Дополнительным входным в масштабный коэффициент является **Коэффициент стандартного отклонения** (SDfactor) со значением по умолчанию `100`. Масштабный коэффициент рассчитывается следующим образом:
...
* Точно одно поле типа timeframe, содержащее начало и конец [метки времени](/docs/platform/grail/dynatrace-query-language/data-types#timestamp "Список DQL типов данных.")
...
В формате единичного значения каждая запись в наборе данных указывает одно значение в временном ряду. Для каждой записи разрешено ровно одно числовое поле.
...
Наименьший разрешенный интервал (временная разница между двумя записями) составляет одну минуту.
...
* Серия определяется на основе различных значений для каждого измерения. Записи с одинаковыми измерениями (строковые свойства) считаются принадлежащими к одной и той же временной серии.

DQL ответ в формате единичного значения

Следующий JSON описывает структуру формата единичного значения.

* Типы полей записей указаны в разделе `types`.
* Фактическое значение поля типа [duration](/docs/platform/grail/dynatrace-query-language/data-types#duration "Список DQL типов данных.") указано в наносекундах.
...
Этот пример показывает прогноз для доступного дискового метрика. В данных нет сезонной закономерности, и есть нисходящая тенденция. Здесь используется прогнозатор [линейной экстраполяции](#linear-extrapolation), производящий широкий прогнозируемый интервал.
...
Dynatrace Intelligence использует специализированные модели ИИ для анализа вашей среды и предоставления действенных рекомендаций. Эти модели обеспечивают различные возможности, от обнаружения аномалий и анализа причин до прогностического анализа и оптимизации производительности. Каждая модель предназначена для конкретных случаев использования и предоставляет разные типы интеллекта, чтобы помочь вам понять и улучшить ваши системы.

[#### Анализ причинно-следственной корреляции Dynatrace Intelligence

Узнайте, как анализ причинно-следственной корреляции Dynatrace Intelligence находит связанные метрики во всей среде.

* Объяснение

Прочитайте это объяснение](/docs/dynatrace-intelligence/reference/ai-models/causal-correlation-analysis)[#### Прогностический анализ ИИ Dynatrace Intelligence

Узнайте, как прогностический анализ ИИ Dynatrace Intelligence генерирует прогнозы.

* Объяснение

Прочитайте это объяснение](/docs/dynatrace-intelligence/reference/ai-models/forecast-analysis)[#### Сезонный базовый уровень

Как Dynatrace Intelligence предлагает пороги сезонного базового уровня для области сущностей.
