...
Чтобы найти все JSON-модели, которые зависят от типа модели, смотрите [JSON models](/managed/dynatrace-api/configuration-api/management-zones-api/json-models "Изучите вариации моделей в API зон управления.").

| Параметр | Тип | Описание | Где | Обязательный |
| --- | --- | --- | --- | --- |
| body | [ManagementZone](#openapi-definition-ManagementZone) | JSON-тело запроса. Содержит параметры новой зоны управления. | body | Optional |

### Объекты тела запроса

#### Объект `ManagementZone`

Конфигурация зоны управления. Определяет, как применяется зона управления.

| Элемент | Тип | Описание | Обязательный |
| --- | --- | --- | --- |
| description | string | Описание зоны управления. | Optional |
| dimensionalRules | [MzDimensionalRule[]](#openapi-definition-MzDimensionalRule) | Список правил по измерениям для использования зоны управления.  Если указано несколько правил, применяется логика **OR**. | Optional |
| entitySelectorBasedRules | [EntitySelectorBasedMzRule[]](#openapi-definition-EntitySelectorBasedMzRule) | Список правил на основе селектора сущностей для использования зоны управления.  Если указано несколько правил, применяется логика **OR**. | Optional |
| id | string | ID зоны управления. | Optional |
| metadata | [ConfigurationMetadata](#openapi-definition-ConfigurationMetadata) | Метаданные для отладки | Optional |
| name | string | Имя зоны управления. | Required |
| rules | [MzRule[]](#openapi-definition-MzRule) | Список правил для использования зоны управления.  Если указано несколько правил, применяется логика **OR**. | Optional |

#### Объект `MzDimensionalRule`

Правило по измерениям для использования зоны управления. Определяет, как применяется зона управления.

Каждое правило вычисляется независимо от всех остальных правил.

| Элемент | Тип | Описание | Обязательный |
| --- | --- | --- | --- |
| appliesTo | string | Цель правила. Возможные значения: * `ANY` * `LOG` * `METRIC` | Required |
| conditions | [MzDimensionalRuleCondition[]](#openapi-definition-MzDimensionalRuleCondition) | Список условий для зоны управления.  Зона управления применяется, только если выполнены **все** условия. | Required |
| enabled | boolean | Правило включено (`true`) или отключено (`false`). | Required |

#### Объект `MzDimensionalRuleCondition`

Условие правила по измерениям зоны управления.

| Элемент | Тип | Описание | Обязательный |
| --- | --- | --- | --- |
| conditionType | string | Тип условия. Возможные значения: * `DIMENSION` * `LOG_FILE_NAME` * `METRIC_KEY` | Required |
| key | string | Опорное значение для сравнения.  Для условий типа `DIMENSION` укажите ключ здесь. | Required |
| ruleMatcher | string | Как мы сравниваем значения. Возможные значения: * `BEGINS_WITH` * `EQUALS` | Required |
| value | string | Значение измерения.  Применимо только когда **conditionType** установлен в `DIMENSION`. | Optional |

#### Объект `EntitySelectorBasedMzRule`

Правило на основе селектора сущностей для использования зоны управления. Позволяет добавлять сущности в зону управления через селектор сущностей.

| Элемент | Тип | Описание | Обязательный |
| --- | --- | --- | --- |
| enabled | boolean | Правило включено (`true`) или отключено (`false`). | Optional |
| entitySelector | string | Строка селектора сущностей, по которой выбираются сущности. | Required |
...
Правило использования зоны управления. Определяет, как применяется зона управления.

Каждое правило вычисляется независимо от всех остальных правил.

| Элемент | Тип | Описание | Обязательный |
| --- | --- | --- | --- |
| conditions | [EntityRuleEngineCondition[]](#openapi-definition-EntityRuleEngineCondition) | Список правил сопоставления для зоны управления.  Зона управления применяется, только если выполнены **все** условия. | Required |
| enabled | boolean | Правило включено (`true`) или отключено (`false`). | Required |
| propagationTypes | string[] | Как применять зону управления к нижележащим сущностям:  * `SERVICE_TO_HOST_LIKE`: применить к нижележащим хостам подходящих сервисов. * `SERVICE_TO_PROCESS_GROUP_LIKE`: применить к нижележащим группам процессов подходящих сервисов. * `PROCESS_GROUP_TO_HOST`: применить к нижележащим хостам подходящих групп процессов. * `PROCESS_GROUP_TO_SERVICE`: применить ко всем сервисам, предоставляемым подходящими группами процессов. * `HOST_TO_PROCESS_GROUP_INSTANCE`: применить к процессам, работающим на подходящих хостах. * `CUSTOM_DEVICE_GROUP_TO_CUSTOM_DEVICE`: применить к пользовательским устройствам в подходящих группах пользовательских устройств. * `AZURE_TO_PG`: применить к группам процессов, связанным с подходящими сущностями Azure. * `AZURE_TO_SERVICE`: применить к сервисам, предоставляемым подходящими сущностями Azure. Возможные значения: * `AZURE_TO_PG` * `AZURE_TO_SERVICE` * `CUSTOM_DEVICE_GROUP_TO_CUSTOM_DEVICE` * `HOST_TO_PROCESS_GROUP_INSTANCE` * `PROCESS_GROUP_TO_HOST` * `PROCESS_GROUP_TO_SERVICE` * `SERVICE_TO_HOST_LIKE` * `SERVICE_TO_PROCESS_GROUP_LIKE` | Optional |
| type | string | Тип сущностей Dynatrace, к которым может применяться зона управления. Возможные значения: * `APPMON_SERVER` * `APPMON_SYSTEM_PROFILE` * `AWS_ACCOUNT` * `AWS_APPLICATION_LOAD_BALANCER` * `AWS_AUTO_SCALING_GROUP` * `AWS_CLASSIC_LOAD_BALANCER` * `AWS_NETWORK_LOAD_BALANCER` * `AWS_RELATIONAL_DATABASE_SERVICE` * `AZURE` * `BROWSER_MONITOR` * `CLOUD_APPLICATION` * `CLOUD_APPLICATION_NAMESPACE` * `CLOUD_FOUNDRY_FOUNDATION` * `CUSTOM_APPLICATION` * `CUSTOM_DEVICE` * `CUSTOM_DEVICE_GROUP` * `DATA_CENTER_SERVICE` * `ENTERPRISE_APPLICATION` * `ESXI_HOST` * `EXTERNAL_MONITOR` * `HOST` * `HOST_GROUP` * `HTTP_MONITOR` * `KUBERNETES_CLUSTER` * `KUBERNETES_SERVICE` * `MOBILE_APPLICATION` * `MULTIPROTOCOL_MONITOR` * `OPENSTACK_ACCOUNT` * `PROCESS_GROUP` * `QUEUE` * `SERVICE` * `WEB_APPLICATION` | Required |
...
| Элемент | Тип | Описание | Обязательный |
| --- | --- | --- | --- |
| comparisonInfo | [ComparisonBasic](#openapi-definition-ComparisonBasic) | Определяет, как именно выполняется сопоставление: что и как мы сравниваем.  Фактический набор полей и возможные значения поля **operator** зависят от типа сравнения. Найдите список фактических объектов в описании поля **type** или смотрите [JSON models](https://dt-url.net/0b83s6z). | Required |
| key | [ConditionKey](#openapi-definition-ConditionKey) | Ключ для идентификации сопоставляемых данных.  Фактический набор полей и возможные значения зависят от типа ключа. Найдите список фактических объектов в описании поля **type** или смотрите [JSON models](https://dt-url.net/0b83s6z). | Required |
...
| operator | string | Оператор сравнения. Вы можете инвертировать его, установив **negate** в `true`.  Возможные значения зависят от **type** сравнения. Найдите список фактических моделей в описании поля **type** и проверьте описание нужной вам модели. | Required |
...
| Код | Тип | Описание |
| --- | --- | --- |
| **201** | [EntityShortRepresentation](#openapi-definition-EntityShortRepresentation) | Успех. Зона управления создана. Ответ содержит ID новой зоны. |
...
В этом примере запрос создаёт новую зону управления с именем **Mainframe**. Она включает все **services**, работающие на хостах с архитектурой **z/OS**, и **underlying process groups**.

API-токен передаётся в заголовке **Authorization**.

Тело запроса объёмное, поэтому в секции **Curl** оно усечено. Полное тело смотрите в секции **Request body**. Вы можете скачать или скопировать пример тела запроса, чтобы попробовать его самостоятельно.
...
Новая зона управления выглядит в UI так:
