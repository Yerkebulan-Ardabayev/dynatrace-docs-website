...
| --- | --- | --- | --- | --- |
| id | string | ID обновляемого автоматически применяемого тега.  Если вы задаёте ID и в теле, он должен совпадать с этим ID. | path | Required |
| body | [AutoTag](#openapi-definition-AutoTag) | JSON-тело запроса. Содержит обновлённые параметры автоматически применяемого тега. | body | Optional |
...
| --- | --- | --- | --- |
| description | string | Описание автоматически применяемого тега. | Optional |
| entitySelectorBasedRules | [EntitySelectorBasedAutoTagRule[]](#openapi-definition-EntitySelectorBasedAutoTagRule) | Список правил на основе селектора сущностей для применения автотегирования.  Если указано несколько правил, применяется логика **OR**. | Optional |
| id | string | ID автоматически применяемого тега. | Optional |
...
| rules | [AutoTagRule[]](#openapi-definition-AutoTagRule) | Список правил применения тега.  При наличии нескольких правил применяется логика OR. | Optional |
...
| normalization | string | Изменения, применяемые к значению после применения формата значения. По умолчанию LEAVE\_TEXT\_AS\_IS. Возможные значения: * `LEAVE_TEXT_AS_IS` * `TO_LOWER_CASE` * `TO_UPPER_CASE` | Optional |
| valueFormat | string | Значение автоматически применяемого тега на основе селектора сущностей. Если указано, тег используется в формате `name:valueFormat`.  Например, вы можете расширить тег `Infrastructure` до `Infrastructure:Windows` и `Infrastructure:Linux`. | Optional |
...
| Элемент | Тип | Описание | Обязательный |
| --- | --- | --- | --- |
| conditions | [EntityRuleEngineCondition[]](#openapi-definition-EntityRuleEngineCondition) | Список правил сопоставления для автоматически применяемого тега.  Тег применяется только когда выполнены **все** условия. | Required |
...
| Код | Тип | Описание |
| --- | --- | --- |
| **201** | [EntityShortRepresentation](#openapi-definition-EntityShortRepresentation) | Успех. Новый автоматически применяемый тег создан. Тело ответа содержит ID нового автоматически применяемого тега. |
| **204** | - | Успех. Автоматически применяемый тег обновлён. Ответ без тела. |
...
В этом примере запрос обновляет автоматически применяемый тег **Infrastructure - Linux** из примера [GET request](/managed/dynatrace-api/configuration-api/automatically-applied-tags-api/get-auto-tag#example "Просмотр автоматически применяемого тега через Dynatrace API.").

Изначальная конфигурация следующая:

![GET example](https://dt-cdn.net/images/get-example-1036-b0d81737f7.png)

GET example

Запрос добавляет дополнительное условие к существующему правилу для значения тега **prod**, дополнительно требуя, чтобы **host group name** содержало `production`.

Также он добавляет новое правило, которое назначает значение тега **preProd** для хостов **Linux** с именами, начинающимися с **PreProd**.
...
Обновлённый автоматически применяемый тег имеет следующие параметры:
