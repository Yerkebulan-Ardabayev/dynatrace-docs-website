---
title: Automatically applied tags API - JSON-модели
source: https://docs.dynatrace.com/managed/dynatrace-api/configuration-api/automatically-applied-tags-api/models
scraped: 2026-05-12T12:16:09.590408
---

# Automatically applied tags API - JSON-модели
...
Некоторые JSON-модели API **Automatically applied tags** различаются в зависимости от **type** модели. Ниже перечислены JSON-модели для каждого варианта.

## Варианты объекта `ConditionKey`

Объект `ConditionKey` является базовым для всех условий. Конкретный набор полей зависит от **type** условия.
...
| Поле | Тип | Описание |
| --- | --- | --- |
| dynamicKey | [CustomHostMetadataKey](#openapi-definition-CustomHostMetadataKey) | Ключ атрибута, требующий динамических ключей. В остальных случаях не применяется, поскольку сам атрибут выступает в роли ключа. |
...
| source | string | Источник пользовательских метаданных. Поле может содержать следующие значения * `ENVIRONMENT` * `GOOGLE_COMPUTE_ENGINE` * `PLUGIN` |
...
| Поле | Тип | Описание |
| --- | --- | --- |
| dynamicKey | [CustomProcessMetadataKey](#openapi-definition-CustomProcessMetadataKey) | Ключ атрибута, требующий динамических ключей. В остальных случаях не применяется, поскольку сам атрибут выступает в роли ключа. |
...
| --- | --- | --- |
| key | string | Фактический ключ пользовательских метаданных. |
| source | string | Источник пользовательских метаданных. Поле может содержать следующие значения * `AGENT` * `CLOUD_FOUNDRY` * `ENVIRONMENT` * `GOOGLE_CLOUD` * `KUBERNETES` * `PLUGIN` |
...
| Поле | Тип | Описание |
| --- | --- | --- |
| dynamicKey | string | Ключ атрибута, требующий динамических ключей. В остальных случаях не применяется, поскольку сам атрибут выступает в роли ключа. |
...
Объект `ComparisonBasic` является базовым для всех операций сравнения. Конкретный набор полей зависит от **type** сравнения.
...
| Поле | Тип | Описание |
| --- | --- | --- |
| operator | string | Оператор сравнения. Можно инвертировать, установив **negate** в `true`. Возможные значения зависят от **type** сравнения: список доступных моделей смотрите в описании поля **type** и сверяйтесь с описанием нужной модели. Поле может содержать следующие значения * `EQUALS` * `EXISTS` |
| value | string | Значение для сравнения. Поле может содержать следующие значения * `AGENTLESS_MONITORING` * `AUTO_INJECTED` * `DEFAULT` * `SAAS_VENDOR` |
...
| --- | --- | --- |
| operator | string | Оператор сравнения. Можно инвертировать, установив **negate** в `true`. Возможные значения зависят от **type** сравнения: список доступных моделей смотрите в описании поля **type** и сверяйтесь с описанием нужной модели. Поле может содержать следующие значения * `EQUALS` * `EXISTS` |
| value | string | Значение для сравнения. Поле может содержать следующие значения * `DEDICATED` * `SHARED` |
...
| --- | --- | --- |
| operator | string | Оператор сравнения. Можно инвертировать, установив **negate** в `true`. Возможные значения зависят от **type** сравнения: список доступных моделей смотрите в описании поля **type** и сверяйтесь с описанием нужной модели. Поле может содержать следующие значения * `EQUALS` * `EXISTS` |
| value | string | Значение для сравнения. Поле может содержать следующие значения * `BASIC` * `DYNAMIC` * `FREE` * `PREMIUM` * `SHARED` * `STANDARD` |
...
| --- | --- | --- |
| operator | string | Оператор сравнения. Можно инвертировать, установив **negate** в `true`. Возможные значения зависят от **type** сравнения: список доступных моделей смотрите в описании поля **type** и сверяйтесь с описанием нужной модели. Поле может содержать следующие значения * `EQUALS` * `EXISTS` |
| value | string | Значение для сравнения. Поле может содержать следующие значения * `32` * `64` |
...
| --- | --- | --- |
| operator | string | Оператор сравнения. Можно инвертировать, установив **negate** в `true`. Возможные значения зависят от **type** сравнения: список доступных моделей смотрите в описании поля **type** и сверяйтесь с описанием нужной модели. Поле может содержать следующие значения * `EQUALS` * `EXISTS` |
| value | string | Значение для сравнения. Поле может содержать следующие значения * `AZURE` * `EC2` * `GOOGLE_CLOUD_PLATFORM` * `OPENSTACK` * `ORACLE` * `UNRECOGNIZED` |
...
| --- | --- | --- |
| operator | string | Оператор сравнения. Можно инвертировать, установив **negate** в `true`. Возможные значения зависят от **type** сравнения: список доступных моделей смотрите в описании поля **type** и сверяйтесь с описанием нужной модели. Поле может содержать следующие значения * `EQUALS` * `EXISTS` |
| value | string | Значение для сравнения. Поле может содержать следующие значения * `AMAZON_ECHO` * `DESKTOP` * `EMBEDDED` * `IOT` * `MICROSOFT_HOLOLENS` * `UFO` |
...
| --- | --- | --- |
| operator | string | Оператор сравнения. Можно инвертировать, установив **negate** в `true`. Возможные значения зависят от **type** сравнения: список доступных моделей смотрите в описании поля **type** и сверяйтесь с описанием нужной модели. Поле может содержать следующие значения * `EQUALS` * `EXISTS` |
| value | string | Значение для сравнения. Поле может содержать следующие значения * `CLUSTER` * `EMBEDDED` * `FAILOVER` * `IPC` * `LOAD_BALANCING` * `SINGLE_SERVER` * `UNSPECIFIED` |
...
| --- | --- | --- |
| operator | string | Оператор сравнения. Можно инвертировать, установив **negate** в `true`. Возможные значения зависят от **type** сравнения: список доступных моделей смотрите в описании поля **type** и сверяйтесь с описанием нужной модели. Поле может содержать следующие значения * `EQUALS` * `EXISTS` |
| value | string | Значение для сравнения. Поле может содержать следующие значения * `ALL_OTHER` * `CITRIX_APPFLOW` * `CITRIX_ICA` * `CITRIX_ICA_OVER_SSL` * `DB2_DRDA` * `HTTP` * `HTTPS` * `HTTP_EXPRESS` * `INFORMIX` * `MYSQL` * `ORACLE` * `SAP_GUI` * `SAP_GUI_OVER_HTTP` * `SAP_GUI_OVER_HTTPS` * `SAP_HANA_DB` * `SAP_RFC` * `SSL` * `TDS` |
...
| Поле | Тип | Описание |
| --- | --- | --- |
| operator | string | Оператор сравнения. Можно инвертировать, установив **negate** в `true`. Возможные значения зависят от **type** сравнения: список доступных моделей смотрите в описании поля **type** и сверяйтесь с описанием нужной модели. Поле может содержать следующие значения * `EQUALS` |
...
| --- | --- | --- |
| operator | string | Оператор сравнения. Можно инвертировать, установив **negate** в `true`. Возможные значения зависят от **type** сравнения: список доступных моделей смотрите в описании поля **type** и сверяйтесь с описанием нужной модели. Поле может содержать следующие значения * `EQUALS` * `EXISTS` |
| value | string | Значение для сравнения. Поле может содержать следующие значения * `AHV` * `AWS_NITRO` * `GVISOR` * `HYPER_V` * `KVM` * `LPAR` * `QEMU` * `UNRECOGNIZED` * `VIRTUAL_BOX` * `VMWARE` * `WPAR` * `XEN` |
...
| operator | string | Оператор сравнения. Можно инвертировать, установив **negate** в `true`. Возможные значения зависят от **type** сравнения: список доступных моделей смотрите в описании поля **type** и сверяйтесь с описанием нужной модели. Поле может содержать следующие значения * `CONTAINS` * `EQUALS` * `EXISTS` |
| value | string | Значение для сравнения. |
...
| Поле | Тип | Описание |
| --- | --- | --- |
| operator | string | Оператор сравнения. Можно инвертировать, установив **negate** в `true`. Возможные значения зависят от **type** сравнения: список доступных моделей смотрите в описании поля **type** и сверяйтесь с описанием нужной модели. Поле может содержать следующие значения * `EQUALS` * `EXISTS` * `TAG_KEY_EQUALS` |
...
| Поле | Тип | Описание |
| --- | --- | --- |
| context | string | Источник тега, например AWS или Cloud Foundry. Пользовательские теги используют значение `CONTEXTLESS`. Поле может содержать следующие значения * `AWS` * `AWS_GENERIC` * `AZURE` * `CLOUD_FOUNDRY` * `CONTEXTLESS` * `ENVIRONMENT` * `GOOGLE_CLOUD` * `KUBERNETES` |
| key | string | Ключ тега. Для пользовательских тегов здесь располагается значение тега. |
| value | string | Значение тега. К пользовательским тегам не применяется. |
...
| Поле | Тип | Описание |
| --- | --- | --- |
| operator | string | Оператор сравнения. Можно инвертировать, установив **negate** в `true`. Возможные значения зависят от **type** сравнения: список доступных моделей смотрите в описании поля **type** и сверяйтесь с описанием нужной модели. Поле может содержать следующие значения * `EQUALS` * `EXISTS` * `GREATER_THAN` * `GREATER_THAN_OR_EQUAL` * `LOWER_THAN` * `LOWER_THAN_OR_EQUAL` |
...
| --- | --- | --- |
| caseSensitive | boolean | Сравнение чувствительно к регистру (`true`) или нечувствительно (`false`). |
| operator | string | Оператор сравнения. Можно инвертировать, установив **negate** в `true`. Возможные значения зависят от **type** сравнения: список доступных моделей смотрите в описании поля **type** и сверяйтесь с описанием нужной модели. Поле может содержать следующие значения * `BEGINS_WITH` * `CONTAINS` * `ENDS_WITH` * `EQUALS` * `EXISTS` * `IS_IP_IN_RANGE` * `REGEX_MATCHES` |
| value | string | Значение для сравнения. |
...
| --- | --- | --- |
| operator | string | Оператор сравнения. Можно инвертировать, установив **negate** в `true`. Возможные значения зависят от **type** сравнения: список доступных моделей смотрите в описании поля **type** и сверяйтесь с описанием нужной модели. Поле может содержать следующие значения * `EQUALS` * `EXISTS` |
| value | string | Значение для сравнения. Поле может содержать следующие значения * `ANDROID` * `IOS` * `LINUX` * `MAC_OS` * `OTHER` * `TVOS` * `WINDOWS` |
...
| --- | --- | --- |
| operator | string | Оператор сравнения. Можно инвертировать, установив **negate** в `true`. Возможные значения зависят от **type** сравнения: список доступных моделей смотрите в описании поля **type** и сверяйтесь с описанием нужной модели. Поле может содержать следующие значения * `EQUALS` * `EXISTS` |
| value | string | Значение для сравнения. Поле может содержать следующие значения * `ARM` * `IA64` * `PARISC` * `PPC` * `PPCLE` * `S390` * `SPARC` * `X86` * `ZOS` |
...
| --- | --- | --- |
| operator | string | Оператор сравнения. Можно инвертировать, установив **negate** в `true`. Возможные значения зависят от **type** сравнения: список доступных моделей смотрите в описании поля **type** и сверяйтесь с описанием нужной модели. Поле может содержать следующие значения * `EQUALS` * `EXISTS` |
| value | string | Значение для сравнения. Поле может содержать следующие значения * `AIX` * `DARWIN` * `HPUX` * `LINUX` * `SOLARIS` * `WINDOWS` * `ZOS` |
...
| --- | --- | --- |
| operator | string | Оператор сравнения. Можно инвертировать, установив **negate** в `true`. Возможные значения зависят от **type** сравнения: список доступных моделей смотрите в описании поля **type** и сверяйтесь с описанием нужной модели. Поле может содержать следующие значения * `EQUALS` * `EXISTS` |
| value | string | Значение для сравнения. Поле может содержать следующие значения * `AWS_ECS_EC2` * `AWS_ECS_FARGATE` * `AWS_LAMBDA` * `AZURE_FUNCTIONS` * `AZURE_WEBSITES` * `CLOUD_FOUNDRY` * `GOOGLE_APP_ENGINE` * `GOOGLE_CLOUD_RUN` * `HEROKU` * `KUBERNETES` * `OPENSHIFT` |
...
| --- | --- | --- |
| operator | string | Оператор сравнения. Можно инвертировать, установив **negate** в `true`. Возможные значения зависят от **type** сравнения: список доступных моделей смотрите в описании поля **type** и сверяйтесь с описанием нужной модели. Поле может содержать следующие значения * `EQUALS` * `EXISTS` |
| value | string | Значение для сравнения. Поле может содержать следующие значения * `EXTERNAL_SERVICE` * `FULLY_MONITORED` * `OPAQUE_SERVICE` |
...
| Поле | Тип | Описание |
| --- | --- | --- |
| type | string | Предопределённая технология; если технология не предопределена, должен быть задан verbatim-тип Поле может содержать следующие значения * `APPARMOR` * `BOSH` * `BOSHBPM` * `CLOUDFOUNDRY` * `CONTAINERD` * `CRIO` * `DIEGO_CELL` * `DOCKER` * `GARDEN` * `GRSECURITY` * `KUBERNETES` * `OPENSHIFT` * `OPENSTACK_COMPUTE` * `OPENSTACK_CONTROLLER` * `SELINUX` |
...
| --- | --- | --- |
| caseSensitive | boolean | Сравнение чувствительно к регистру (`true`) или нечувствительно (`false`). |
| operator | string | Оператор сравнения. Можно инвертировать, установив **negate** в `true`. Возможные значения зависят от **type** сравнения: список доступных моделей смотрите в описании поля **type** и сверяйтесь с описанием нужной модели. Поле может содержать следующие значения * `BEGINS_WITH` * `CONTAINS` * `ENDS_WITH` * `EQUALS` * `EXISTS` * `REGEX_MATCHES` |
| value | string | Значение для сравнения. |
...
| --- | --- | --- |
| operator | string | Оператор сравнения. Можно инвертировать, установив **negate** в `true`. Возможные значения зависят от **type** сравнения: список доступных моделей смотрите в описании поля **type** и сверяйтесь с описанием нужной модели. Поле может содержать следующие значения * `EQUALS` * `EXISTS` |
| value | string | Значение для сравнения. Поле может содержать следующие значения * `CLASSIC` * `CUSTOM` |
...
| Поле | Тип | Описание |
| --- | --- | --- |
| operator | string | Оператор сравнения. Можно инвертировать, установив **negate** в `true`. Возможные значения зависят от **type** сравнения: список доступных моделей смотрите в описании поля **type** и сверяйтесь с описанием нужной модели. Поле может содержать следующие значения * `EQUALS` * `TAG_KEY_EQUALS` |
| value | [TagInfo](#openapi-definition-TagInfo) | Тег сущности Dynatrace. |
...
* [Определение и применение тегов](/managed/manage/tags-and-metadata/setup/how-to-define-tags "Узнайте, как задавать и применять теги вручную и автоматически.")
* [Теги и метаданные](/managed/manage/tags-and-metadata "Используйте теги и метаданные для упорядочивания данных в окружении Dynatrace.")