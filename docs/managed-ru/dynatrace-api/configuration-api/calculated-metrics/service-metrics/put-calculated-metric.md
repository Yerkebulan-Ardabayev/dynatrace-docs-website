...
| --- | --- | --- | --- | --- |
| metricKey | string | Ключ вычисляемой метрики сервиса, которую нужно обновить.  Ключ вычисляемой метрики сервиса в теле запроса должен совпадать с этим ключом. | path | Required |
| body | [CalculatedServiceMetric](#openapi-definition-CalculatedServiceMetric) | JSON-тело запроса. Содержит обновлённые параметры вычисляемой метрики сервиса. | body | Required |
...
| Код | Тип | Описание |
| --- | --- | --- |
| **201** | [EntityShortRepresentation](#openapi-definition-EntityShortRepresentation) | Успех. Новая вычисляемая метрика сервиса создана. Тело ответа содержит ключ новой метрики. |
| **204** | - | Успех. Вычисляемая метрика сервиса обновлена. Ответ без тела. |
