---
title: Reports API - PUT a report
source: https://docs.dynatrace.com/managed/dynatrace-api/configuration-api/reports-api/put-report
scraped: 2026-05-12T11:15:45.449587
---
...
Обновляет указанный отчёт. Если отчёта с указанным ID не существует, создаётся новый.

Запрос принимает и возвращает payload `application/json`.

|  |  |  |
| --- | --- | --- |
| PUT | ManagedDynatrace for Government | `https://{your-domain}/e/{your-environment-id}/api/config/v1/reports/{id}` |
| PUT | Environment ActiveGate | `https://{your-activegate-domain}/e/{your-environment-id}/api/config/v1/reports/{id}` |
...
| --- | --- | --- | --- | --- |
| id | string | ID обновляемого отчёта.  ID в теле запроса должен совпадать с этим ID. | path | Required |
| body | [DashboardReport](#openapi-definition-DashboardReport) | JSON-тело запроса. Содержит обновлённые параметры отчёта. | body | Optional |
...
| **204** | - | Успех. Отчёт обновлён. Ответ без тела |
...
|  |  |  |
| --- | --- | --- |
| POST | ManagedDynatrace for Government | `https://{your-domain}/e/{your-environment-id}/api/config/v1/reports/{id}/validator` |
| POST | Environment ActiveGate | `https://{your-activegate-domain}/e/{your-environment-id}/api/config/v1/reports/{id}/validator` |
...
В этом примере запрос обновляет отчёт из примера [POST request](/managed/dynatrace-api/configuration-api/reports-api/post-report#example "Создание конфигурации отчёта через Dynatrace API."). Он добавляет следующих подписчиков:

* Еженедельно для адреса **operations@organization.com**.
* Ежемесячно для адреса **cto@organization.com**.

Обратите внимание, что все существующие подписчики должны присутствовать в теле запроса. Существующие подписчики, отсутствующие в теле запроса, удаляются.

API-токен передаётся в заголовке **Authorization**.

Вы можете скачать или скопировать пример тела запроса, чтобы попробовать его самостоятельно. Обязательно используйте ID дашборда и ID пользователей, доступных в вашем окружении
