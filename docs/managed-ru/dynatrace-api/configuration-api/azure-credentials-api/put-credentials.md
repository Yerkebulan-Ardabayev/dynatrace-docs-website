...
Обновляет существующую конфигурацию Azure credentials. Проверьте статус подключения для этих credentials через 10 минут запросом [GET credentials](/managed/dynatrace-api/configuration-api/azure-credentials-api/get-credentials "Просмотр конфигурации Azure credentials через Dynatrace API.").

Если конфигурация credentials с указанным ID не существует, создаётся новая конфигурация.
...
| --- | --- | --- | --- | --- |
| id | string | ID конфигурации Azure credentials, которую нужно обновить. | path | Required |
| body | [AzureCredentials](#openapi-definition-AzureCredentials) | JSON-тело запроса. Содержит обновлённые параметры конфигурации Azure credentials. | body | Optional |
...
| appId | string | ID приложения (также называется client ID).  Поле **обязательно** при создании новой конфигурации credentials.  Поле игнорируется при обновлении, прежнее значение сохраняется. | Optional |
| autoTagging | boolean | Автоматический сбор Azure-тегов включён (`true`) или выключен (`false`). | Required |
| directoryId | string | ID каталога (также называется tenant ID).  Поле **обязательно** при создании новой конфигурации credentials.  Поле игнорируется при обновлении, прежнее значение сохраняется. | Optional |
...
В этом примере запрос обновляет конфигурацию Azure credentials из примера [POST request](/managed/dynatrace-api/configuration-api/azure-credentials-api/post-new-credentials#example "Создание конфигурации Azure credentials через Dynatrace API.").

Он задаёт новый пароль и меняет режим мониторинга с ресурсов с указанными тегами на все ресурсы:

* значение **monitorOnlyTaggedEntities** изменено на `false`
* массив **monitorOnlyTagPairs** пустой.
