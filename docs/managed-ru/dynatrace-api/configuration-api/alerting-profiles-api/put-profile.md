---
title: Alerting profiles API - PUT a profile
source: https://docs.dynatrace.com/managed/dynatrace-api/configuration-api/alerting-profiles-api/put-profile
scraped: 2026-05-12T12:06:40.510933
---
...
Обновляет указанный профиль оповещений. Если профиль оповещений с указанным ID не существует, создаётся новый профиль.

Запрос принимает и возвращает payload `application/json`.

|  |  |  |
| --- | --- | --- |
| PUT | ManagedDynatrace for Government | `https://{your-domain}/e/{your-environment-id}/api/config/v1/alertingProfiles/{id}` |
| PUT | Environment ActiveGate | `https://{your-activegate-domain}/e/{your-environment-id}/api/config/v1/alertingProfiles/{id}` |
...
| --- | --- | --- | --- | --- |
| id | string | ID обновляемого профиля оповещений. | path | Required |
| body | [AlertingProfile](#openapi-definition-AlertingProfile) | JSON-тело запроса. Содержит обновлённые параметры профиля оповещений. | body | Optional |
...
| **204** | - | Успех. Профиль оповещений обновлён. Ответ без тела. |
...
|  |  |  |
| --- | --- | --- |
| POST | ManagedDynatrace for Government | `https://{your-domain}/e/{your-environment-id}/api/config/v1/alertingProfiles/{id}/validator` |
| POST | Environment ActiveGate | `https://{your-activegate-domain}/e/{your-environment-id}/api/config/v1/alertingProfiles/{id}/validator` |
...
В этом примере запрос обновляет профиль оповещений, созданный в примере [POST request](/managed/dynatrace-api/configuration-api/alerting-profiles-api/post-profile#example "Создание профиля оповещений через Dynatrace API."). Он вносит следующие изменения:

* Уменьшает задержку до **1** минуты.
* Добавляет второй тег отслеживаемых сущностей: **MobileApp**.
* Добавляет второй фильтр событий, который требует, чтобы событие **Unexpected low traffic** **не** присутствовало.

API-токен передаётся в заголовке **Authorization**.

Вы можете скачать или скопировать пример тела запроса, чтобы попробовать его самостоятельно. Обязательно замените теги на те, которые есть в вашем окружении.
...
У обновлённого профиля оповещений следующие параметры:

![PUT example](https://dt-cdn.net/images/put-1343-aaa345370c.png)
