...
scraped: 2026-05-12T11:18:33.528913
---
...
| --- | --- | --- | --- | --- |
| position | string | Позиция нового правила:  * `APPEND`: внизу списка правил. * `PREPEND`: вверху списка правил.  Если не задано, используется `APPEND`. Возможные значения: * `APPEND` * `PREPEND` | query | Optional |
| body | [FullWebRequestRule](#openapi-definition-FullWebRequestRule) | JSON-тело запроса. Содержит параметры нового правила обнаружения сервисов.  Не указывайте ID правила!  Поле **order** в этом запросе игнорируется. Чтобы задать определённый порядок, используйте запрос `PUT /service/detectionRules/FULL_WEB_REQUEST/reorder`. | body | Optional |
...
| conditions | [ConditionsFullWebRequestAttributeTypeDto[]](#openapi-definition-ConditionsFullWebRequestAttributeTypeDto) | Список условий правила.  Если указано несколько условий, применяется логика AND. | Optional |
...
| Элемент | Тип | Описание | Обязательный |
| --- | --- | --- | --- |
| attributeType | string | Тип проверяемого атрибута. Возможные значения: * `APPLICATION_ID` * `CONTEXT_ROOT` * `PG_TAG` * `SERVER_NAME` * `URL_HOST_NAME` * `URL_PATH` | Required |
...
| Код | Тип | Описание |
| --- | --- | --- |
| **201** | [EntityShortRepresentation](#openapi-definition-EntityShortRepresentation) | Успех. Новое правило обнаружения сервисов создано. Тело ответа содержит ID правила. |
