...
| --- | --- | --- | --- | --- |
| technology | string | Технология нового пользовательского сервиса. Возможные значения: * `dotNet` * `go` * `java` * `nodeJS` * `php` | path | Required |
| position | string | Порядок нового пользовательского сервиса. Установите `PREPEND`, чтобы добавить его в начало списка, или `APPEND`, чтобы добавить в конец. По умолчанию `APPEND`. Возможные значения: * `APPEND` * `PREPEND` | query | Optional |
| body | [CustomService](#openapi-definition-CustomService) | JSON-тело запроса, содержащее определение нового пользовательского сервиса.  Не указывайте ID для пользовательского сервиса или любого из его правил. Поле *order* также не допускается. | body | Optional |
