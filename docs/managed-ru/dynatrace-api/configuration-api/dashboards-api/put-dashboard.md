...
| --- | --- | --- | --- | --- |
| id | string | ID обновляемого дашборда.  ID в теле запроса должен совпадать с этим ID. | path | Required |
| body | [Dashboard](#openapi-definition-Dashboard) | JSON-тело запроса. Содержит обновлённые параметры дашборда. | body | Optional |
...
| Код | Тип | Описание |
| --- | --- | --- |
| **201** | [EntityShortRepresentation](#openapi-definition-EntityShortRepresentation) | Успех. Новый дашборд создан. Ответ без тела. |
| **204** | - | Успех. Дашборд обновлён. Ответ без тела |
...
В этом примере запрос добавляет плитку **Service health** на дашборд **Sample dashboard** из примера [GET request](/managed/dynatrace-api/configuration-api/dashboards-api/get-dashboard#example "Просмотр дашборда через Dynatrace Classic API.").

Плитка показывает здоровье всех сервисов, относящихся к зоне управления **Easytravel**. Она расположена рядом с существующей плиткой **Host health** и имеет тот же размер (**304x304** пикселя).

API-токен передаётся в заголовке **Authorization**.

Поскольку тело запроса объёмное, в этом примере оно усечено в секции **Curl**. Полное тело смотрите в секции **Request body**. Вы можете скачать или скопировать пример тела запроса, чтобы попробовать его самостоятельно. Обязательно измените ID и имя зоны управления **Easytravel** на зону управления, которая существует в вашем окружении, либо задайте этому полю значение `null`.
