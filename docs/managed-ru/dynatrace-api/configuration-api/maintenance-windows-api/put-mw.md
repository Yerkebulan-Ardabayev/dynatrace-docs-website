---
title: Maintenance windows API - PUT a maintenance window
source: https://docs.dynatrace.com/managed/dynatrace-api/configuration-api/maintenance-windows-api/put-mw
scraped: 2026-05-12T12:06:23.842601
---
...
Обновляет указанное maintenance window. Если maintenance window с указанным ID не существует, создаётся новое.

Запрос принимает и возвращает payload `application/json`.

|  |  |  |
| --- | --- | --- |
| PUT | ManagedDynatrace for Government | `https://{your-domain}/e/{your-environment-id}/api/config/v1/maintenanceWindows/{id}` |
| PUT | Environment ActiveGate | `https://{your-activegate-domain}/e/{your-environment-id}/api/config/v1/maintenanceWindows/{id}` |
...
| --- | --- | --- | --- | --- |
| id | string | ID обновляемого maintenance window. | path | Required |
| body | [MaintenanceWindow](#openapi-definition-MaintenanceWindow) | JSON-тело запроса. Содержит обновлённые параметры maintenance window. | body | Optional |
...
| **204** | - | Успех. Maintenance window обновлено. Ответ без тела. |
...
|  |  |  |
| --- | --- | --- |
| POST | ManagedDynatrace for Government | `https://{your-domain}/e/{your-environment-id}/api/config/v1/maintenanceWindows/{id}/validator` |
| POST | Environment ActiveGate | `https://{your-activegate-domain}/e/{your-environment-id}/api/config/v1/maintenanceWindows/{id}/validator` |
...
В этом примере запрос обновляет maintenance window **infrastructure maintenance** из [примера запроса GET](/managed/dynatrace-api/configuration-api/maintenance-windows-api/get-mw#example "Просмотр maintenance window через Dynatrace API.")
...
Запрос меняет периодичность на **еженедельное** обслуживание по **понедельникам**. Также добавляются хосты с тегами **TestWindows** и **TestLinux** в область действия.

API-токен передаётся в заголовке **Authorization**.

Тело запроса объёмное, поэтому в секции **Curl** оно усечено. Полное тело смотрите в секции **Request body**. Можно скачать или скопировать пример тела запроса, чтобы попробовать у себя.
...
У обновлённого maintenance window следующие параметры:

![PUT example](https://dt-cdn.net/images/put-example-966-a764e5d6a7.png)
