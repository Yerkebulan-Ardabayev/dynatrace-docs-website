...
Модели JSON, зависящие от **type** модели, перечислены в разделе [JSON models](/managed/dynatrace-api/configuration-api/conditional-naming/json-models "Learn what the Dynatrace conditional naming API offers.").

| Параметр | Тип | Описание | Расположение | Обязательный |
| --- | --- | --- | --- | --- |
| type | string | Тип правила, определяемый типом сущностей Dynatrace, к которым оно применяется. Элемент может принимать следующие значения * `processGroup` * `host` * `service` | path | Обязательный |
| id | string | ID правила именования, которое нужно обновить. | path | Обязательный |
| body | [ConditionalNamingRule](#openapi-definition-ConditionalNamingRule) | Тело запроса JSON. Содержит обновлённые параметры правила именования.  Значение поля **type** должно совпадать с path-параметром **type**. | body | Опционально |
...
| --- | --- | --- | --- |
| displayName | string | Название правила | Обязательно |
...
| rules | [EntityRuleEngineCondition](#openapi-definition-EntityRuleEngineCondition)[] | Список условий сопоставления правила.  Правило применяется, только если выполняются **все** условия. | Обязательно |
| type | string | Тип сущностей Dynatrace, к которым применяется правило. Элемент может принимать следующие значения * `HOST` * `PROCESS_GROUP` * `SERVICE` | Обязательно |
...
| Код | Тип | Описание |
| --- | --- | --- |
| **201** | [EntityShortRepresentation](#openapi-definition-EntityShortRepresentation) | Успешно. Правило именования создано. Ответ содержит ID нового правила. |
| **204** | - | Успешно. Правило именования обновлено. Ответ не содержит тела. |
...
Рекомендуется проверять полезную нагрузку перед отправкой её с реальным запросом. Код ответа **204** указывает на валидную полезную нагрузку.
