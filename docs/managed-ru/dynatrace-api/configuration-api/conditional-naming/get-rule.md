...
Возвращает параметры указанного правила условного именования.
...
| id | string | ID запрашиваемого правила именования. | path | Required |
...
| Код | Тип | Описание |
| --- | --- | --- |
| **200** | [ConditionalNamingRule](#openapi-definition-ConditionalNamingRule) | Успех |
...
| --- | --- | --- |
| displayName | string | Имя правила. |
| enabled | boolean | Правило включено (`true`) или выключено (`false`). |
...
| rules | [EntityRuleEngineCondition[]](#openapi-definition-EntityRuleEngineCondition) | Список условий сопоставления правила.  Правило применяется только если выполнены **все** условия. |
| type | string | Тип сущностей Dynatrace, к которым применяется правило. Элемент может принимать значения * `HOST` * `PROCESS_GROUP` * `SERVICE` |
...
| Элемент | Тип | Описание |
| --- | --- | --- |
| comparisonInfo | [ComparisonBasic](#openapi-definition-ComparisonBasic) | Определяет, как фактически выполняется сопоставление: что и как мы сравниваем.  Фактический набор полей и возможные значения поля **operator** зависят от типа сравнения. Список фактических объектов см. в описании поля **type** или см. [JSON models](https://dt-url.net/0b83s6z). |
| key | [ConditionKey](#openapi-definition-ConditionKey) | Ключ для идентификации сопоставляемых данных.  Фактический набор полей и возможные значения зависят от типа ключа. Список фактических объектов см. в описании поля **type** или см. [JSON models](https://dt-url.net/0b83s6z). |
...
| operator | string | Оператор сравнения. Можно инвертировать, установив **negate** в `true`.  Возможные значения зависят от **type** сравнения. Список фактических моделей см. в описании поля **type** и проверьте описание нужной модели. |
