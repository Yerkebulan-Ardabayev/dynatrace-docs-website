...
Этот API-вызов тестирует proxy-конфигурацию кластера.
...
В этом примере тестируется proxy-сервер (`outbound-proxy.dynatrace.com`) на порту 8080, требующий пароль; одновременно исключается внутренний lab-хост (`*.internal.lab.company.com`). В ответ возвращается JSON, указывающий, что подключение в порядке.
