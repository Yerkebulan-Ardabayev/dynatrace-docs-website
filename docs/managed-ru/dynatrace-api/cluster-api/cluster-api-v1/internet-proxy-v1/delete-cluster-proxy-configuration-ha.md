...
Этот API-вызов удаляет proxy-конфигурацию в конкретном датацентре.
...
| **404** | - | Proxy для данного датацентра не был настроен. |
...
В этом примере удаляется proxy-конфигурация Dynatrace Managed развёртывания в конкретном датацентре (`eu-west-1`). В ответ возвращается предыдущая proxy-конфигурация.
