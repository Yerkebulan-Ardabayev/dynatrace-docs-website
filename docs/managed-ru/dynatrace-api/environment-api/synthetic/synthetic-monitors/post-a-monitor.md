---
title: Synthetic monitors API - POST монитор
...
# Synthetic monitors API - POST монитор
...
Создаёт новый синтетический монитор.

Конфигурация нового монитора передаётся через его JSON-скрипт.
...
| --- | --- | --- | --- | --- |
| body | [SyntheticMonitorUpdate](#openapi-definition-SyntheticMonitorUpdate) | JSON-тело запроса, содержащее параметры нового синтетического монитора. | body | Необязательный |
...
| script | object | Скрипт [браузерного](https://dt-url.net/9c103rda) или HTTP-монитора. | Обязательный |
...
| Код | Тип | Описание |
| --- | --- | --- |
| **200** | [EntityIdDto](#openapi-definition-EntityIdDto) | Успех. Новый синтетический монитор создан. Ответ содержит Dynatrace entity ID нового монитора. |
...
В этом примере запрос создаёт простой браузерный монитор, который переходит на **dynatrace.com**.

Монитор выполняется каждые **10 минут** из одной локации, у которой ID равен **GEOLOCATION-0A41430434C388A9**. [Проблема](/managed/dynatrace-intelligence "Познакомьтесь с возможностями Davis AI.") будет создана, если сайт недоступен в течение трёх последовательных запусков. Уведомление отправляется, если загрузка сайта занимает больше **500 миллисекунд**.

API-токен передаётся в заголовке **Authorization**.

Ответ содержит entity ID только что созданного монитора.
