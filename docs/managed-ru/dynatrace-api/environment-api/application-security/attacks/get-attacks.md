...
Возвращает список всех обнаруженных [атак](/managed/secure/application-security/application-protection/manage-attacks "Отслеживайте атаки на код вашего приложения.") на ваши приложения.
...
| GET | Environment and Cluster ActiveGate (порт по умолчанию 9999) | `https://{your-activegate-domain}:9999/e/{your-environment-id}/api/v2/attacks` |
...
| pageSize | integer | Количество атак в одном payload ответа.  Максимально допустимый размер страницы 500.  Если не задано, используется 100. | query | Опциональный |
| attackSelector | string | Определяет область запроса. В ответ включаются только атаки, соответствующие указанным критериям. Можно добавить один или несколько критериев. Значения *не* регистрозависимы, и используется оператор `EQUALS`, если не указано иное.  * State: `state("value")`. Состояние атаки. Возможные значения `EXPLOITED`, `BLOCKED` и `ALLOWLISTED`. * Attack Type: `attackType("value")`. Тип атаки. Возможные значения смотрите в описании поля **attackType** ответа. * Country Code: `countryCode("value")`. Код страны атакующего. Поддерживаются все коды стран ISO-3166-1 alpha-2 (2 буквы). Указание пустого фильтра `countryCode()` вернёт атаки, для которых местоположение недоступно. * Request path contains: `requestPathContains("value")`. Фильтр по подстроке в пути запроса. Используется оператор `CONTAINS`. Допускается максимум 48 символов. * Process group name contains: `processGroupNameContains("value")`. Фильтр по подстроке в имени целевой process group. Используется оператор `CONTAINS`. * Vulnerability ID: `vulnerabilityId("123456789")`. Точный ID уязвимости. * Source IPs: `sourceIps("93.184.216.34", "63.124.6.12")`. Точные IPv4/IPv6 адреса атакующего. * Management zone ID: `managementZoneIds("mzId-1", "mzId-2")`. * Management zone name: `managementZones("name-1", "name-2")`. Значения регистрозависимы. * Technology: `technology("technology-1", "technology-2")`. Возможные значения смотрите в описании поля **technology** ответа. Используется оператор `EQUALS`.  Чтобы задать несколько критериев, разделите их запятой (`,`). В ответ включаются только результаты, соответствующие **всем** критериям.  Указывайте значение критерия как строку в кавычках. Следующие специальные символы должны быть экранированы тильдой (`~`) внутри кавычек:  * Тильда `~` * Кавычка `"` | query | Опциональный |
| sort | string | Указывает одно или несколько полей для сортировки списка атак. Несколько полей можно объединить через запятую (`,`) как разделитель (например, `+state,-timestamp`).  Можно сортировать по следующим свойствам с префиксом знака для порядка сортировки.  * `displayId`: Display ID атаки. * `displayName`: Display name атаки. * `attackType`: Тип атаки (например, SQL\_INJECTION, JNDI\_INJECTION и т.д.). * `state`: Состояние атаки. (`+` низкая severity первая `-` высокая severity первая) * `sourceIp`: IP-адрес атакующего. Сортировка по числовому значению IP. * `requestPath`: Путь запроса, где была начата атака. * `timestamp`: Когда атака была выполнена. (`+` старые атаки первыми или `-` новые атаки первыми)   Если префикс не указан, используется `+`. | query | Опциональный |
| fields | string | Список дополнительных свойств атаки, которые можно добавить в ответ.  Доступны следующие свойства (все остальные свойства всегда включены, и их нельзя убрать из ответа):  * `attackTarget`: Целевой host/database атаки. * `request`: Запрос, отправленный от атакующего. * `entrypoint`: Точка входа, использованная атакующим для начала конкретной атаки. * `vulnerability`: Уязвимость, использованная атакой. * `securityProblem`: Связанная security problem. * `attacker`: Атакующий. * `managementZones`: Связанные management zones. * `affectedEntities`: Затронутые сущности атаки.  Чтобы добавить свойства, укажите их через запятую и поставьте перед каждым свойством знак плюс (например, `+attackTarget,+securityProblem`). | query | Опциональный |
| from | string | Начало запрашиваемого таймфрейма.  Можно использовать один из следующих форматов:  * Временная метка в миллисекундах UTC. * Человекочитаемый формат `2021-01-25T05:57:01.123+01:00`. Если зона не указана, используется UTC. Вместо `T` можно использовать пробел. Секунды и доли секунды опциональны. * Относительный таймфрейм назад от текущего момента. Формат: `now-NU/A`, где `N` это количество времени, `U` это единица измерения, а `A` это выравнивание. Выравнивание округляет все меньшие значения до ближайшего нуля в прошлом. Например, `now-1y/w` это год назад, выровненный по неделе.   Можно также указать относительный таймфрейм без выравнивания: `now-NU`.   Поддерживаемые единицы времени для относительного таймфрейма: + `m`: минуты   + `h`: часы   + `d`: дни   + `w`: недели   + `M`: месяцы   + `y`: годы  Если не задано, используется относительный таймфрейм в тридцать дней (`now-30d`). | query | Опциональный |
...
| Код | Тип | Описание |
| --- | --- | --- |
| **200** | [AttackList](#openapi-definition-AttackList) | Успех |
...
| --- | --- | --- |
| attacks | [Attack[]](#openapi-definition-Attack) | Список атак. |
