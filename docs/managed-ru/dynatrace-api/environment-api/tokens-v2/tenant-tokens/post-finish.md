---
title: Tenant tokens API - POST завершение ротации
...
# Tenant tokens API - POST завершение ротации
...
Завершает ротацию tenant-токена. Старый токен становится недействительным. Выполняйте запрос только после того, как обновили все OneAgent и ActiveGate, как описано в [Ротация tenant-токена](/managed/manage/identity-access-management/access-tokens-and-oauth-clients/access-tokens/rotate-tenant-token "Что такое tenant-токен и как его изменить.").
...
| Код | Тип | Описание |
| --- | --- | --- |
| **200** | [TenantTokenConfig](#openapi-definition-TenantTokenConfig) | Успех. Процесс ротации завершён. Поле **active** ответа содержит новый tenant-токен. |
| **400** | [ErrorEnvelope](#openapi-definition-ErrorEnvelope) | Активного процесса ротации нет. |
...
В этом примере запрос завершает процесс ротации, запущенный в [примере запроса start](/managed/dynatrace-api/environment-api/tokens-v2/tenant-tokens/post-start#example "Запуск ротации tenant-токена Dynatrace.").

Код ответа **200** указывает на успешный запрос. Старый токен **1234567890qrstuvwxyz** больше не действителен.
