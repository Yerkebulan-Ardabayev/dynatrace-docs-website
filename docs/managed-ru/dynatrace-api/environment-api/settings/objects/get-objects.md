...
| scopes | string | Список разделённых запятыми scopes, на которые нацелены запрашиваемые объекты.  Выборка сопоставляет только объекты, напрямую нацеленные на указанные scopes. Например, `environment` не сопоставит объекты, нацеленные на хост внутри окружения. Подробнее см. [Dynatrace Documentation](https://dt-url.net/ky03459).  Для загрузки первой страницы, когда **nextPageKey** не задан, требуется либо этот параметр, либо **schemaIds**.  Чтобы загрузить все объекты, относящиеся к данным scopes, оставьте параметр **schemaIds** пустым. | query | Optional |
...
| filter | string | Параметр filter, как объяснено [здесь](https://dt-url.net/platform-services-filtering).  Фильтрация поддерживается по следующим полям:  * `created` * `modified` * `createdBy` * `modifiedBy` * `author` (deprecated, не будет работать для будущих schemas) * `value` со свойствами и под-свойствами, разделёнными точкой (например, `value.owningApp = 'Notebooks'`)  Если этот параметр опущен, будут возвращены все settings objects. Максимальная глубина вложенности (через скобки): 5. Максимальная длина выражения: 1024 символа.  Обратите внимание, что для фильтрации можно использовать только поля, включённые в ответ через `fields`. | query | Optional |
| sort | string | Параметр sort, как объяснено [здесь](https://dt-url.net/platform-services-filtering).  Сортировка поддерживается по следующим полям:  * `created` * `modified` * `createdBy` * `modifiedBy` * `author` (deprecated, не будет работать для будущих schemas) * `value` со свойствами и под-свойствами, разделёнными точкой (например, `value.owningApp`)  Обратите внимание, что для сортировки можно использовать только поля, включённые в ответ через `fields`. | query | Optional |
...
| scope | string | Scope, на который нацелен объект. Подробнее см. [Dynatrace Documentation](https://dt-url.net/ky03459). |
