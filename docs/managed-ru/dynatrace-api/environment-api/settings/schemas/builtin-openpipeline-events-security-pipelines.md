---
title: Settings API - Ingest pipelines configuration (events.security) schema table
source: https://docs.dynatrace.com/managed/dynatrace-api/environment-api/settings/schemas/builtin-openpipeline-events-security-pipelines
scraped: 2026-05-12T11:42:57.060041
---

# Settings API - Ingest pipelines configuration (events.security) schema table

# Settings API - Ingest pipelines configuration (events.security) schema table

* Опубликовано 25 августа 2025 г.

### Конфигурация ingest pipelines (events.security) (`builtin:openpipeline.events.security.pipelines)`

Содержит конфигурацию pipelines

| Schema ID | Группы схемы | Scope |
| --- | --- | --- |
| `builtin:openpipeline.events.security.pipelines` | * `group:openpipeline.all.pipelines` | `environment` |

Получить schema через Settings API

|  |  |  |
| --- | --- | --- |
| GET | Managed | `https://{your-domain}/e/{your-environment-id}/api/v2/settings/schemas/builtin:openpipeline.events.security.pipelines` |
| GET | SaaS | `https://{your-environment-id}.live.dynatrace.com/api/v2/settings/schemas/builtin:openpipeline.events.security.pipelines` |
| GET | Environment ActiveGate | `https://{your-activegate-domain}/e/{your-environment-id}/api/v2/settings/schemas/builtin:openpipeline.events.security.pipelines` |
...
| --- | --- | --- | --- |
| Список metadata для pipeline `metadataList` | [MetadataEntry](#MetadataEntry)[] | - | Required |
| ID пользовательского pipeline `customId` | text | - | Required |
| Display name `displayName` | text | - | Required |
| Processing stage `processing` | [Stage](#Stage) | - | Optional |
| Security context stage `securityContext` | [Stage](#Stage) | - | Optional |
| Cost allocation stage `costAllocation` | [Stage](#Stage) | - | Optional |
| Product allocation stage `productAllocation` | [Stage](#Stage) | - | Optional |
| Storage stage `storage` | [Stage](#Stage) | - | Optional |
| Smartscape node extraction stage `smartscapeNodeExtraction` | [Stage](#Stage) | - | Optional |
| Smartscape edge extraction stage `smartscapeEdgeExtraction` | [Stage](#Stage) | - | Optional |
| Metrics extraction stage `metricExtraction` | [Stage](#Stage) | - | Optional |
| Davis event extraction stage `davis` | [Stage](#Stage) | - | Optional |
| Data extraction stage `dataExtraction` | [Stage](#Stage) | - | Optional |
| Routing `routing` | enum | Возможные значения: * `routable` * `notRoutable` | Optional |
| Роль в группе `groupRole` | enum | Возможные значения: * `memberPipeline` * `compositionPipeline` | Optional |
