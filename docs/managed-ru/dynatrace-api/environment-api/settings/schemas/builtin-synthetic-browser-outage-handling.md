...
scraped: 2026-05-12T11:42:07.288409
---
...
### Обработка простоев (`builtin:synthetic.browser.outage-handling)`
...
| Автоматическая повторная попытка при ошибке. `retryOnError` | boolean | When enabled, which is the default, failing monitor executions are retried immediately one time to avoid false positives and only the second result is used. When disabled, we use the first result right away. Requires ActiveGate version 1.207+ for private locations. | Required |