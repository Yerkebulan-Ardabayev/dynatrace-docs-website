---
title: Settings API - Data forwarding configuration (security.events) schema table
source: https://docs.dynatrace.com/managed/dynatrace-api/environment-api/settings/schemas/builtin-openpipeline-security-events-data-forwarding
scraped: 2026-05-12T11:38:53.162242
---

# Settings API - Data forwarding configuration (security.events) schema table

# Settings API - Data forwarding configuration (security.events) schema table

* Опубликовано 25 сентября 2025 г.

### Конфигурация data forwarding (security.events) (`builtin:openpipeline.security.events.data-forwarding)`

Содержит конфигурацию data forwarding

| Schema ID | Группы схемы | Scope |
| --- | --- | --- |
| `builtin:openpipeline.security.events.data-forwarding` | * `group:openpipeline.all.data-forwarding` | `environment` |

Получить schema через Settings API

|  |  |  |
| --- | --- | --- |
| GET | Managed | `https://{your-domain}/e/{your-environment-id}/api/v2/settings/schemas/builtin:openpipeline.security.events.data-forwarding` |
| GET | SaaS | `https://{your-environment-id}.live.dynatrace.com/api/v2/settings/schemas/builtin:openpipeline.security.events.data-forwarding` |
| GET | Environment ActiveGate | `https://{your-activegate-domain}/e/{your-environment-id}/api/v2/settings/schemas/builtin:openpipeline.security.events.data-forwarding` |
...
| Имя forwarding `forwardingName` | text | - | Required |
| Тип pipeline `dataForwardingType` | enum | Возможные значения: * `raw` * `processed` | Required |
| Список ingest sources `ingestSources` | set | - | Required |
| Список встроенных ingest sources `builtinIngestSources` | set | - | Required |
| Pipelines `pipelines` | set | - | Required |
| Встроенные pipelines `builtinPipelines` | set | - | Required |
| Query, определяющий, должна ли запись направляться в целевой pipeline этого правила. `matcher` | text | - | Required |
| Тип cloud-провайдера `cloudVendorType` | enum | Возможные значения: * `aws` * `azure` * `gcp` | Required |
| Подключение AWS `awsConnection` | [AwsConnection](#AwsConnection) | - | Required |
| Подключение Azure `azureConnection` | [AzureConnection](#AzureConnection) | - | Required |
| Подключение GCP `gcpConnection` | [GcpConnection](#GcpConnection) | - | Required |
| Сегментация и префикс данных `bulkPattern` | text | - | Required |
| Размер пакета для передачи `bulkSize` | integer | - | Optional |
| Processing `processing` | [Stage](#Stage) | - | Optional |

##### Объект `AwsConnection`

| Свойство | Тип | Описание | Обязательный |
| --- | --- | --- | --- |
| Подключение AWS `connectionId` | text | - | Required |
| ARN S3 bucket `arn` | text | - | Required |

##### Объект `AzureConnection`

| Свойство | Тип | Описание | Обязательный |
| --- | --- | --- | --- |
| Подключение Azure `connectionId` | text | - | Required |
| URL подключения к контейнеру `containerUrl` | text | - | Required |

##### Объект `GcpConnection`

| Свойство | Тип | Описание | Обязательный |
| --- | --- | --- | --- |
| Подключение GCP `connectionId` | text | - | Required |
| Имя GCS bucket `bucketName` | text | - | Required |
