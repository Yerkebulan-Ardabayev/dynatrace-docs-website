...
| --- | --- | --- | --- | --- |
| query | string | Выполняемый запрос пользовательских сессий. Подробности синтаксиса см. на [странице документации USQL](https://dt-url.net/dtusql?dt=m).  Доступные столбцы таблицы **usersession** можно найти в объекте `UserSession`.  Пример запроса: `SELECT country, city, COUNT(*) FROM usersession GROUP BY country, city`. | query | Required |
