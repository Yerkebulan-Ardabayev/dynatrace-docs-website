---
title: Развёртывание OneAgent Operator на OpenShift (устаревшее)
...
scraped: 2026-05-12T11:52:11.430297
---

# Развёртывание OneAgent Operator на OpenShift (устаревшее)
...
* При новой установке используйте [настройку мониторинга OpenShift с помощью Dynatrace Operator](/managed/ingest-from/setup-on-k8s/deployment "Развёртывание Dynatrace Operator в Kubernetes").
...
Инструкции ниже применимы также к **OpenShift Dedicated**. Для OpenShift Dedicated необходимы [права cluster-admin](https://docs.openshift.com/dedicated/4/administering_a_cluster/cluster-admin-role.html).
...
* Поддерживаемые версии OpenShift см. в разделе [Жизненный цикл поддержки](/managed/ingest-from/technology-support/support-model-and-issues "Как Dynatrace поддерживает версии Kubernetes и Red Hat OpenShift и известные проблемы").
...
1. Добавьте новый проект.
   ...
   ```
2. OCP версии 3.11. Укажите секреты для загрузки образов.
   Пропустите этот шаг, если используете более позднюю версию.
   Чтобы использовать сертифицированные образы [OneAgent Operator](https://access.redhat.com/containers/#/registry.connect.redhat.com/dynatrace/dynatrace-oneagent-operator) и [OneAgent](https://access.redhat.com/containers/#/registry.connect.redhat.com/dynatrace/oneagent) из [Red Hat Container Catalog](https://access.redhat.com/containers/) (RHCC), необходимо [предоставить секреты для загрузки образов](https://access.redhat.com/documentation/en-us/openshift_container_platform/3.9/html/developer_guide/dev-guide-managing-images#pulling-private-registries-delegated-auth). Service Accounts в манифесте `openshift.yaml` уже содержат ссылки на секреты, которые будут созданы ниже.
...
3. OCP версии 4.x / OCP версии 3.11. Примените манифест `openshift.yaml` для развёртывания OneAgent Operator.
   ...
   Для версий OpenShift **ранее 3.11.188** необходимо **удалить строку** `type: object` **из раздела обязательной проверки спецификации в** `openshift.yaml` **перед развёртыванием** `CustomResourceDefinition` ([известная ошибка OpenShift](https://github.com/openshift/origin/pull/24540)).
   ...
   ```
4. Создайте секрет с токенами API и PaaS для аутентификации в кластере Dynatrace.
...
5. Сохраните пользовательский ресурс.
   ...
   ```
6. Настройте пользовательский ресурс.
...
| `tolerations` | Необязательный. Оставьте значение по умолчанию, чтобы при возможности развернуть OneAgent на мастер-узлах. Чтобы добавить tolerations для подов OneAgent на узлах с taint, укажите их здесь. Подробнее см. в [документации Kubernetes](https://kubernetes.io/docs/concepts/configuration/taint-and-toleration/). |  |
...
| `trustedCAs` | Необязательный. Имя ConfigMap, содержащего пользовательские CA-сертификаты. ConfigMap должен иметь поле `certs` с содержимым PEM-пакета. Эти сертификаты будут использоваться как OneAgent Operator, так и OneAgent. | Если не задано, используются встроенные сертификаты образов. |
...
* Рекомендуется устанавливать **последнюю версию** Helm chart.

1. Добавьте новый проект с именем `dynatrace`.

   ```
   oc adm new-project --node-selector="" dynatrace
   ```
2. Добавьте репозиторий Helm для Dynatrace OneAgent.
...
Узнайте, как [устранять проблемы](/managed/ingest-from/setup-on-k8s/deployment/troubleshooting#deploy "Эта страница поможет вам справиться с трудностями, которые могут возникнуть при работе с Dynatrace Operator и его компонентами."), которые могут возникнуть при развёртывании OneAgent на OpenShift.

## Развёртывание ActiveGate и подключение Kubernetes API к Dynatrace

Теперь, когда OneAgent запущен на узлах OpenShift, можно отслеживать эти узлы и приложения, работающие в OpenShift. Следующий шаг: развернуть ActiveGate и подключить Kubernetes API к Dynatrace, чтобы получать нативные метрики Kubernetes, например ограничения запросов и расхождение между запрошенными и работающими подами.
Дополнительные инструкции см. в разделе [Развёртывание ActiveGate в OpenShift как StatefulSet](/managed/ingest-from/setup-on-k8s/deployment/other/ag-statefulset "Установка и настройка ActiveGate в Kubernetes как StatefulSet.").

## Обновление OneAgent Operator с помощью oc

OneAgent Operator для OpenShift версии 3.9+ автоматически управляет жизненным циклом развёрнутых экземпляров OneAgent, поэтому обновлять поды OneAgent вручную не требуется.
...
1. Обновите репозитории Helm.
   ...
   Альтернативный способ: добавить репозиторий повторно. Это перезапишет устаревшую версию.
