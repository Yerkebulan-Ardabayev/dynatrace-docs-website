---
title: Мониторинг AWS Certificate Manager Private Certificate Authority (ACM PCA)
...
# Мониторинг AWS Certificate Manager Private Certificate Authority (ACM PCA)
...
Dynatrace принимает метрики для множества предопределённых пространств имён, включая AWS Certificate Manager Private Certificate Authority. Можно просматривать метрики по каждому экземпляру сервиса, разбивать их на несколько измерений и создавать собственные графики, которые можно закреплять на дашбордах.
...
Чтобы [обновить политику AWS IAM](https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies_manage-edit.html#edit-managed-policy-console), используйте приведённый ниже JSON, который содержит политику мониторинга (разрешения) для всех поддерживаемых сервисов.
...
Ниже приведён пример JSON-политики для одного сервиса.
...
Основное измерение: `PrivateCAArn`.

| Имя | Описание | Единица измерения | Статистика | Измерения | Рекомендуется |
| --- | --- | --- | --- | --- | --- |
| CRLGenerated | Список отзыва сертификатов (CRL) был сгенерирован. Эта метрика применяется только к частному CA. | None | Sum | PrivateCAArn | Применимо |
| Failure | Операция завершилась сбоем. Эта метрика применяется только к операции `IssueCertificate`. | None | Sum | Operation, PrivateCAArn | Применимо |
| Failure |  | None | Sum | Operation, Region |  |
| MisconfiguredCRLBucket | Корзина S3, указанная для CRL, настроена неправильно. Проверьте политику корзины. Эта метрика применяется только к частному CA. | None | Sum | PrivateCAArn | Применимо |
| Success | Сертификат был успешно выпущен. Эта метрика применяется только к операции `IssueCertificate`. | None | Sum | Operation, PrivateCAArn | Применимо |
| Success |  | None | Sum | Operation, Region |  |
| Time | Затраченное время в миллисекундах на выпуск сертификата. Эта метрика применяется только к операции `IssueCertificate`. | None | Multi | Operation, PrivateCAArn |  |
