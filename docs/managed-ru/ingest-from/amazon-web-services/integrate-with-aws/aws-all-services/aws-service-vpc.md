---
title: Мониторинг Amazon VPC NAT Gateways
source: https://docs.dynatrace.com/managed/ingest-from/amazon-web-services/integrate-with-aws/aws-all-services/aws-service-vpc
scraped: 2026-05-12T11:28:16.304316
---

# Мониторинг Amazon VPC NAT Gateways
...
Dynatrace принимает метрики для множества предопределённых пространств имён, включая Amazon VPC NAT Gateways. Можно просматривать метрики по каждому экземпляру сервиса, разбивать их на несколько измерений и создавать собственные графики, которые можно закреплять на дашбордах.
...
Основное измерение: `NatGatewayId`.

| Имя | Описание | Единица измерения | Статистика | Измерения | Рекомендуется |
| --- | --- | --- | --- | --- | --- |
| ActiveConnectionCount | Общее количество одновременных активных TCP-соединений через шлюз NAT | Количество | Maximum | NatGatewayId | Применимо |
| BytesInFromDestination | Количество байт, полученных шлюзом NAT из пункта назначения | Байт | Sum | NatGatewayId |  |
| BytesInFromSource | Количество байт, полученных шлюзом NAT от клиентов в вашей VPC | Байт | Sum | NatGatewayId |  |
| BytesOutToDestination | Количество байт, отправленных через шлюз NAT в пункт назначения | Байт | Sum | NatGatewayId |  |
| BytesOutToSource | Количество байт, отправленных через шлюз NAT клиентам в вашей VPC | Байт | Sum | NatGatewayId |  |
| ConnectionAttemptCount | Количество попыток подключения, совершённых через шлюз NAT | Количество | Sum | NatGatewayId | Применимо |
| ConnectionEstablishedCount | Количество подключений, установленных через шлюз NAT | Количество | Sum | NatGatewayId | Применимо |
| ErrorPortAllocation | Количество случаев, когда шлюзу NAT не удалось выделить исходный порт | Количество | Sum | NatGatewayId | Применимо |
| IdleTimeoutCount | Количество подключений, перешедших из активного состояния в состояние простоя | Количество | Sum | NatGatewayId | Применимо |
| PacketsDropCount | Количество пакетов, отброшенных шлюзом NAT | Количество | Sum | NatGatewayId | Применимо |
| PacketsInFromDestination | Количество пакетов, полученных шлюзом NAT из пункта назначения | Количество | Sum | NatGatewayId |  |
| PacketsInFromSource | Количество пакетов, полученных шлюзом NAT от клиентов в вашей VPC | Количество | Sum | NatGatewayId |  |
| PacketsOutToDestination | Количество пакетов, отправленных через шлюз NAT в пункт назначения | Количество | Sum | NatGatewayId |  |
| PacketsOutToSource | Количество пакетов, отправленных через шлюз NAT клиентам в вашей VPC | Количество | Sum | NatGatewayId |  |