---
title: Мониторинг AWS Glue
source: https://docs.dynatrace.com/managed/ingest-from/amazon-web-services/integrate-with-aws/aws-all-services/aws-service-glue
scraped: 2026-05-12T11:29:59.053576
---
...
Dynatrace принимает метрики для множества предопределённых пространств имён, включая AWS Glue. Можно просматривать метрики по каждому экземпляру сервиса, разбивать их на несколько измерений и создавать собственные графики, которые можно закреплять на дашбордах.
...
Основное измерение: `JobName`.

Чтобы принимать дополнительные метрики наблюдаемости

1. В консоли AWS перейдите в раздел **Job details**.
2. Включите опцию **Job observability metrics**.

| Имя | Описание | Статистика | Единица измерения | Измерения | Рекомендуется |
| --- | --- | --- | --- | --- | --- |
| glue.driver.aggregate.bytesRead | Количество байт, прочитанных из всех источников данных всеми завершёнными задачами Spark, выполняющимися во всех исполнителях | Sum | Байт | JobName, JobRunId, Type |  |
| glue.driver.aggregate.elapsedTime | Прошедшее время ETL в миллисекундах (не включает время начальной загрузки задания) | Sum | Миллисекунда | JobName, JobRunId, Type |  |
| glue.driver.aggregate.numCompletedStages | Количество завершённых этапов в задании | Sum | Количество | JobName, JobRunId, Type |  |
| glue.driver.aggregate.numCompletedTasks | Количество завершённых задач в задании | Sum | Количество | JobName, JobRunId, Type |  |
| glue.driver.aggregate.numFailedTasks | Количество неуспешных задач в задании | Sum | Количество | JobName, JobRunId, Type |  |
| glue.driver.aggregate.numKilledTasks | Количество принудительно завершённых задач в задании | Sum | Количество | JobName, JobRunId, Type |  |
| glue.driver.aggregate.recordsRead | Количество записей, прочитанных из всех источников данных всеми завершёнными задачами Spark, выполняющимися во всех исполнителях | Sum | Количество | JobName, JobRunId, Type |  |
| glue.driver.aggregate.shuffleBytesWritten | Количество байт, записанных всеми исполнителями для перемешивания данных между ними с момента предыдущего отчёта (агрегируется дашбордом метрик AWS Glue как количество байт, записанных для этой цели за предыдущую минуту) | Sum | Байт | JobName, JobRunId, Type |  |
| glue.driver.aggregate.shuffleLocalBytesRead | Количество байт, прочитанных всеми исполнителями для перемешивания данных между ними с момента предыдущего отчёта (агрегируется дашбордом метрик AWS Glue как количество байт, прочитанных для этой цели за предыдущую минуту) | Sum | Байт | JobName, JobRunId, Type |  |
| glue.driver.BlockManager.disk.diskSpaceUsed\_MB | Количество мегабайт дискового пространства, используемого по всем исполнителям | Average | Мегабайт | JobName, JobRunId, Type |  |
| glue.driver.ExecutorAllocationManager.executors.numberAllExecutors | Количество активно работающих исполнителей задания | Average | Количество | JobName, JobRunId, Type |  |
| glue.driver.ExecutorAllocationManager.executors.numberMaxNeededExecutors | Максимальное количество исполнителей задания (активно работающих и ожидающих), необходимых для обработки текущей нагрузки | Maximum | Количество | JobName, JobRunId, Type |  |
| glue.driver.jvm.heap.usage | Доля памяти, используемой кучей JVM (шкала: `0`-`1`), для этого драйвера | Average | Процент | JobName, JobRunId, Type | Применимо |
| glue.ALL.jvm.heap.usage | Доля памяти, используемой кучей JVM (шкала: `0`-`1`), для всех исполнителей | Average | Процент | JobName, JobRunId, Type | Применимо |
| glue.driver.jvm.heap.used | Количество байт памяти, используемых кучей JVM для драйвера | Average | Байт | JobName, JobRunId, Type |  |
| glue.ALL.jvm.heap.used | Количество байт памяти, используемых кучей JVM для всех исполнителей | Average | Байт | JobName, JobRunId, Type |  |
| glue.driver.s3.filesystem.read\_bytes | Количество байт, прочитанных из Amazon S3 драйвером с момента предыдущего отчёта (агрегируется дашбордом метрик AWS Glue как количество байт, прочитанных за предыдущую минуту) | Sum | Байт | JobName, JobRunId, Type |  |
| glue.ALL.s3.filesystem.read\_bytes | Количество байт, прочитанных из Amazon S3 всеми исполнителями с момента предыдущего отчёта (агрегируется дашбордом метрик AWS Glue как количество байт, прочитанных за предыдущую минуту) | Sum | Байт | JobName, JobRunId, Type | Применимо |
| glue.driver.s3.filesystem.write\_bytes | Количество байт, записанных в Amazon S3 драйвером с момента предыдущего отчёта (агрегируется дашбордом метрик AWS Glue как количество байт, записанных за предыдущую минуту) | Sum | Байт | JobName, JobRunId, Type |  |
| glue.ALL.s3.filesystem.write\_bytes | Количество байт, записанных в Amazon S3 всеми исполнителями с момента предыдущего отчёта (агрегируется дашбордом метрик AWS Glue как количество байт, записанных за предыдущую минуту) | Sum | Байт | JobName, JobRunId, Type | Применимо |
| glue.driver.system.cpuSystemLoad | Доля используемой системной нагрузки CPU (шкала: `0`-`1`) драйвером | Average | Процент | JobName, JobRunId, Type |  |
| glue.ALL.system.cpuSystemLoad | Доля используемой системной нагрузки CPU (шкала: `0`-`1`) всеми исполнителями | Average | Процент | JobName, JobRunId, Type |  |