...
source: https://docs.dynatrace.com/managed/ingest-from/amazon-web-services/integrate-with-aws/aws-all-services/aws-service-simple-email-service-ses
scraped: 2026-05-12T11:28:25.857783
---

# Мониторинг Amazon SES (Simple Email Service)
...
Dynatrace принимает метрики для множества предопределённых пространств имён, включая Amazon Simple Email Service (Amazon SES). Можно просматривать метрики по каждому экземпляру сервиса, разбивать их на несколько измерений и создавать собственные графики, которые можно закреплять на дашбордах.
...
| --- | --- | --- | --- | --- | --- |
| Bounce | Количество случаев, когда почтовый сервер получателя безвозвратно отклоняет письмо (жёсткие отказы) | Количество | Sum | Region |  |
| Click | Количество случаев, когда получатель получает письмо и переходит по ссылке | Количество | Sum | Region |  |
| Complaint | Количество случаев, когда письма помечаются получателем как спам | Количество | Sum | Region |  |
| Delivery | Количество писем, успешно доставленных на почтовый сервер получателя | Количество | Sum | Region | Применимо |
| Open | Количество случаев, когда получатель получает письмо и открывает его в почтовом клиенте | Количество | Sum | Region |  |
| PublishExpired | Количество случаев, когда Amazon SES сталкивается с ошибкой при попытке выполнить настроенные действия и прекращает повторные попытки доставки письма | Количество | Sum | Region, RuleName |  |
| PublishExpired |  | Количество | Sum | Region, RuleSetName |  |
| PublishFailure | Количество случаев, когда Amazon SES сталкивается с ошибкой при попытке выполнить настроенные действия | Количество | Sum | Region, RuleName |  |
| Reject | Количество случаев, когда Amazon SES принимает запрос отправителя, затем определяет, что сообщение содержит вирус, и прекращает обработку | Количество | Sum | Region | Применимо |
| Rendering Failure | Количество случаев, когда письмо не может быть отправлено из-за проблемы с отрисовкой шаблона | Количество | Sum | Region |  |
| Reputation.BounceRate | Процент отправленных писем, приводящих к жёстким отказам | Процент | Average | Region | Применимо |
| Reputation.ComplaintRate | Процент отправленных писем, помеченных как спам | Процент | Average | Region | Применимо |
| Send | Количество успешных запросов (вызовов API отправки писем) к Amazon SES | Количество | Sum | Region | Применимо |