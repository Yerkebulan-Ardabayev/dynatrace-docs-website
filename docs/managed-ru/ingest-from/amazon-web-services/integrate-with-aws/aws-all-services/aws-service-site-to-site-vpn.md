---
title: Мониторинг AWS Site-to-Site VPN
source: https://docs.dynatrace.com/managed/ingest-from/amazon-web-services/integrate-with-aws/aws-all-services/aws-service-site-to-site-vpn
scraped: 2026-05-12T11:30:10.298623
---
...
Dynatrace принимает метрики для множества предопределённых пространств имён, включая AWS Site-to-Site VPN. Можно просматривать графики по каждому экземпляру сервиса с набором измерений и создавать собственные графики, которые можно закреплять на дашбордах.
...
Основное измерение: `VpnId`.

| Имя | Описание | Единица измерения | Статистика | Измерения | Рекомендуется |
| --- | --- | --- | --- | --- | --- |
| TunnelDataIn | Байты, полученные через VPN-туннель | Байт | Sum | VpnId | Применимо |
| TunnelDataIn |  | Байт | Sum | Region | Применимо |
| TunnelDataIn |  | Байт | Sum | Region, TunnelIpAddress |  |
| TunnelDataOut | Байты, отправленные через VPN-туннель | Байт | Sum | VpnId | Применимо |
| TunnelDataOut |  | Байт | Sum | Region | Применимо |
| TunnelDataOut |  | Байт | Sum | Region, TunnelIpAddress |  |
| TunnelState | Состояние туннеля. Для статических VPN 0 означает DOWN, а 1 означает UP. Для BGP VPN 1 означает ESTABLISHED, а 0 используется для всех остальных состояний | Количество | Multi | Region | Применимо |
| TunnelState |  | Количество | Multi | Region, TunnelIpAddress |  |
| TunnelState |  | Количество | Multi | VpnId | Применимо |