...
Dynatrace принимает метрики для множества предопределённых пространств имён, включая Amazon AppStream 2.0. Можно просматривать метрики по каждому экземпляру сервиса, разбивать их на несколько измерений и создавать собственные графики, которые можно закреплять на дашбордах.
...
Основное измерение: `Fleet`.

| Имя | Описание | Единица измерения | Статистика | Измерения | Рекомендуется |
| --- | --- | --- | --- | --- | --- |
| ActualCapacity | Общее количество экземпляров, доступных для потоковой передачи или ведущих её в данный момент. `ActualCapacity` = `AvailableCapacity` + `InUseCapacity`. | Количество | Multi | Fleet | Применимо |
| AvailableCapacity | Количество простаивающих экземпляров, доступных в данный момент для пользовательских сессий. `AvailableCapacity` = `ActualCapacity` - `InUseCapacity`. | Количество | Multi | Fleet | Применимо |
| CapacityUtilization | Процент используемых экземпляров во флоте, рассчитываемый по следующей формуле. `CapacityUtilization` = (`InUseCapacity`/`ActualCapacity`) × 100. | Процент | Multi | Fleet | Применимо |
| DesiredCapacity | Общее количество экземпляров, которые работают или ожидают запуска. Это общее количество одновременных потоковых сессий, которые ваш флот может поддерживать в устойчивом состоянии. `DesiredCapacity` = `ActualCapacity` + `PendingCapacity`. | Количество | Multi | Fleet | Применимо |
| InUseCapacity | Количество экземпляров, используемых в данный момент для потоковых сессий. Одна единица `InUseCapacity` соответствует одной потоковой сессии. | Количество | Multi | Fleet | Применимо |
| InsufficientCapacityError | Количество запросов на сессию, отклонённых из-за нехватки ёмкости. | Количество | Sum | Fleet |  |
| PendingCapacity | Количество экземпляров, подготавливаемых AppStream 2.0. Представляет дополнительное количество потоковых сессий, которые флот сможет поддерживать после завершения подготовки. После начала подготовки экземпляр обычно становится доступным для потоковой передачи через 10-20 минут. | Количество | Multi | Fleet | Применимо |