---
title: Мониторинг Amazon EBS (Elastic Block Store)
...
Dynatrace принимает метрики для множества предопределённых пространств имён, включая Amazon EBS. Можно просматривать метрики по каждому экземпляру сервиса, разбивать их на несколько измерений и создавать собственные графики, которые можно закреплять на дашбордах.
...
Этот сервис отслеживает часть Amazon EBS (AWS/EBS). Пока этот сервис настроен, нельзя включить сервис Amazon EBS (built-in).

Основное измерение: `VolumeId`.

| Имя | Описание | Единица измерения | Статистика | Измерения | Рекомендуется |
| --- | --- | --- | --- | --- | --- |
| BurstBalance | Предоставляет информацию о проценте кредитов ввода-вывода (для `gp2`) или кредитов пропускной способности (для `st1` и `sc1`), оставшихся в корзине всплеска. | Процент | Multi | VolumeId |  |
| EBSByteBalance% | Предоставляет информацию о проценте кредитов пропускной способности, оставшихся в корзине всплеска. | Процент | Multi | VolumeId |  |
| EBSIOBalance% | Предоставляет информацию о проценте кредитов ввода-вывода, оставшихся в корзине всплеска. | Процент | Multi | VolumeId |  |
| EBSReadBytes | Количество байт, прочитанных со всех томов EBS, подключённых к экземпляру, за указанный период времени. | Байт | Multi | VolumeId |  |
| EBSReadOps | Количество завершённых операций чтения со всех томов Amazon EBS, подключённых к экземпляру, за указанный период времени. | Количество | Sum | VolumeId |  |
| EBSWriteBytes | Количество байт, записанных на все тома EBS, подключённые к экземпляру, за указанный период времени. | Байт | Multi | VolumeId |  |
| EBSWriteOps | Количество завершённых операций записи на все тома EBS, подключённые к экземпляру, за указанный период времени. | Количество | Sum | VolumeId |  |
| FastSnapshotRestoreCreditsBalance | Количество доступных кредитов создания тома. Эта метрика регистрируется для каждого моментального снимка в каждой зоне доступности. | None | Multi | AvailabilityZone, SnapshotId |  |
| FastSnapshotRestoreCreditsBucketSize | Максимальное количество кредитов создания тома, которое может быть накоплено. Эта метрика регистрируется для каждого моментального снимка в каждой зоне доступности. | None | Multi | AvailabilityZone, SnapshotId |  |
| VolumeConsumedReadWriteOps | Общее количество операций чтения и записи (нормализованных к единицам ёмкости по 256 КБ), потреблённых за указанный период времени. | Количество | Sum | VolumeId |  |
| VolumeIdleTime | Общее количество секунд за указанный период времени, когда не было отправлено ни одной операции чтения или записи. | Секунда | Multi | VolumeId | Применимо |
| VolumeQueueLength | Количество запросов на операции чтения и записи, ожидающих завершения за указанный период времени. | Количество | Sum | VolumeId | Применимо |
| VolumeReadBytes | Предоставляет информацию об операциях чтения за указанный период времени. | Байт | Count | VolumeId |  |
| VolumeReadBytes |  | Байт | Multi | VolumeId |  |
| VolumeReadBytes |  | Байт | Sum | VolumeId | Применимо |
| VolumeReadOps | Общее количество операций чтения за указанный период времени. | Количество | Sum | VolumeId | Применимо |
| VolumeThroughputPercentage | Процент операций ввода-вывода в секунду (IOPS), обеспеченных от общего числа IOPS, подготовленных для тома Amazon EBS. | Процент | Multi | VolumeId |  |
| VolumeTotalReadTime | Общее количество секунд, затраченных всеми операциями чтения, завершившимися за указанный период времени. | Секунда | Multi | VolumeId |  |
| VolumeTotalWriteTime | Общее количество секунд, затраченных всеми операциями записи, завершившимися за указанный период времени. | Секунда | Multi | VolumeId |  |
| VolumeWriteBytes | Предоставляет информацию об операциях записи за указанный период времени. | Байт | Count | VolumeId |  |
| VolumeWriteBytes |  | Байт | Multi | VolumeId |  |
| VolumeWriteBytes |  | Байт | Sum | VolumeId | Применимо |
| VolumeWriteOps | Общее количество операций записи за указанный период времени. | Количество | Sum | VolumeId | Применимо |