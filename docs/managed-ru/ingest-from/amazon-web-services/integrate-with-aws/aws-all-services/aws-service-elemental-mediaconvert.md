...
source: https://docs.dynatrace.com/managed/ingest-from/amazon-web-services/integrate-with-aws/aws-all-services/aws-service-elemental-mediaconvert
scraped: 2026-05-12T11:28:23.932485
---
...
Dynatrace принимает метрики для множества предопределённых пространств имён, включая AWS Elemental MediaConvert. Можно просматривать графики по каждому экземпляру сервиса с набором измерений и создавать собственные графики, которые можно закреплять на дашбордах.
...
Основное измерение: `Queue`.

| Имя | Описание | Единица измерения | Статистика | Измерения | Рекомендуется |
| --- | --- | --- | --- | --- | --- |
| AudioOutputDuration | Количество секунд вывода только аудио для очереди | Миллисекунда | Multi | Queue |  |
| AudioOutputDuration |  | Миллисекунда | Sum | Queue | Применимо |
| Errors | Количество ошибок, вызванных недопустимыми параметрами операции, например запросом статуса задания без идентификатора задания | Количество | Sum | Region, Operation | Применимо |
| HDOutputDuration | Количество секунд вывода высокой чёткости (HD) для очереди | Миллисекунда | Multi | Queue |  |
| HDOutputDuration |  | Миллисекунда | Sum | Queue | Применимо |
| JobsCanceledCount | Количество заданий, отменённых в этой очереди | Количество | Sum | Queue | Применимо |
| JobsCompletedCount | Количество заданий, выполненных в этой очереди | Количество | Sum | Queue | Применимо |
| JobsErroredCount | Количество заданий, завершившихся сбоем из-за недопустимых входных данных, например запроса на перекодирование файла, отсутствующего в указанной входной корзине | Количество | Sum | Queue | Применимо |
| 8KOutputDuration | Количество секунд вывода 8K для очереди | Миллисекунда | Multi | Queue |  |
| 8KOutputDuration |  | Миллисекунда | Sum | Queue | Применимо |
| SDOutputDuration | Количество секунд вывода стандартной чёткости (SD) для очереди | Миллисекунда | Multi | Queue |  |
| SDOutputDuration |  | Миллисекунда | Sum | Queue | Применимо |
| StandbyTime | Количество секунд до того, как AWS Elemental MediaConvert начинает перекодирование задания | Миллисекунда | Multi | Region | Применимо |
| StandbyTime |  | Миллисекунда | Multi | Queue | Применимо |
| TranscodingTime | Количество секунд, необходимых AWS Elemental MediaConvert для завершения перекодирования | Миллисекунда | Multi | Queue | Применимо |
| UHDOutputDuration | Количество секунд вывода сверхвысокой чёткости (UHD) для очереди | Миллисекунда | Multi | Queue |  |
| UHDOutputDuration |  | Миллисекунда | Sum | Queue | Применимо |

## Ограничения

В настоящее время Dynatrace не собирает атрибуты экземпляров для AWS Elemental MediaConvert.