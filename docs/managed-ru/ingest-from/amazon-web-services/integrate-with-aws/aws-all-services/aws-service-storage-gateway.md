...
source: https://docs.dynatrace.com/managed/ingest-from/amazon-web-services/integrate-with-aws/aws-all-services/aws-service-storage-gateway
scraped: 2026-05-12T11:31:18.800674
---
...
Dynatrace принимает метрики для множества предопределённых пространств имён, включая AWS Storage Gateway. Можно просматривать метрики по каждому экземпляру сервиса, разбивать их на несколько измерений и создавать собственные графики, которые можно закреплять на дашбордах.
...
Основное измерение: `GatewayName`.

| Имя | Описание | Единица измерения | Статистика | Измерения | Рекомендуется |
| --- | --- | --- | --- | --- | --- |
| CacheFree | Общее количество байт, доступных в кэш-хранилище шлюза | Байт | Sum | GatewayName, GatewayId | Применимо |
| CacheHitPercent | Процент операций чтения приложений, обслуженных из кэша | Процент | Multi | Region, ShareId | Применимо |
| CacheHitPercent |  | Процент | Multi | Region, VolumeId | Применимо |
| CacheHitPercent |  | Процент | Multi | GatewayName, GatewayId | Применимо |
| CachePercentDirty | Процент кэша, являющегося «грязным», то есть содержащего данные, которые ещё не были выгружены в AWS | Процент | Multi | Region, ShareId | Применимо |
| CachePercentDirty |  | Процент | Multi | Region, VolumeId | Применимо |
| CachePercentDirty |  | Процент | Multi | GatewayName, GatewayId | Применимо |
| CachePercentUsed | Процент используемого кэш-хранилища шлюза | Процент | Multi | Region, ShareId | Применимо |
| CachePercentUsed |  | Процент | Multi | Region, VolumeId | Применимо |
| CachePercentUsed |  | Процент | Multi | GatewayName, GatewayId | Применимо |
| CacheUsed | Общее количество байт, используемых в кэш-хранилище шлюза | Байт | Sum | GatewayName, GatewayId | Применимо |
| CloudBytesDownloaded | Общее количество байт, отправленных шлюзом в облако за отчётный период. Эта метрика включает активность всех томов на шлюзе. | Байт | Sum | Region, ShareId | Применимо |
| CloudBytesDownloaded |  | Байт | Count | Region, ShareId |  |
| CloudBytesDownloaded |  | Байт | Sum | GatewayName, GatewayId | Применимо |
| CloudBytesDownloaded |  | Байт | Count | GatewayName, GatewayId |  |
| CloudBytesUploaded | Общее количество байт, отправленных шлюзом в облако за отчётный период | Байт | Sum | Region, ShareId | Применимо |
| CloudBytesUploaded |  | Байт | Count | Region, ShareId |  |
| CloudBytesUploaded |  | Байт | Sum | GatewayName, GatewayId |  |
| CloudBytesUploaded |  | Байт | Count | GatewayName, GatewayId |  |
| CloudDownloadLatency | Задержка при скачивании данных из облака на шлюз | Миллисекунда | Multi | GatewayName, GatewayId |  |
| IndexFetches | Количество файлов, для которых были получены метаданные | Количество | Sum | Region, ShareId | Применимо |
| IndexFetches |  | Количество | Sum | GatewayName, GatewayId | Применимо |
| IoWaitPercent | Процент времени, в течение которого шлюз ожидает ответа от локального диска | Процент | Multi | GatewayName | Применимо |
| MemTotalBytes | Объём ОЗУ, выделенного виртуальной машине шлюза, в байтах | Байт | Sum | GatewayName | Применимо |
| MemUsedBytes | Объём ОЗУ, используемого в данный момент виртуальной машиной шлюза, в байтах | Байт | Sum | GatewayName | Применимо |
| NfsSessions | Количество сеансов NFS, активных на шлюзе | Количество | Sum | GatewayName | Применимо |
| QueuedWrites | Количество байт, ожидающих записи в AWS, замеренное в конце отчётного периода по всем томам шлюза | Байт | Sum | Region, VolumeId | Применимо |
| QueuedWrites |  | Байт | Sum | GatewayName, GatewayId | Применимо |
| ReadBytes | Общее количество байт, прочитанных из ваших локальных приложений за отчётный период по всем томам шлюза | Байт | Sum | Region, VolumeId | Применимо |
| ReadBytes |  | Байт | Count | Region, VolumeId |  |
| ReadBytes |  | Байт | Sum | GatewayName, GatewayId | Применимо |
| ReadBytes |  | Байт | Count | GatewayName, GatewayId |  |
| ReadTime | Общее количество миллисекунд, затраченных на операции чтения из ваших локальных приложений за отчётный период по всем томам шлюза | Миллисекунда | Multi | Region, VolumeId | Применимо |
| ReadTime |  | Миллисекунда | Multi | GatewayNam, GatewayIde | Применимо |
| SmbV1Sessions | Количество сеансов Server Message Block (SMB) версии 1, активных на шлюзе | Количество | Sum | GatewayName |  |
| SmbV2Sessions | Количество сеансов SMB версии 2, активных на шлюзе | Количество | Sum | GatewayName |  |
| SmbV3Sessions | Количество сеансов SMB версии 3, активных на шлюзе | Количество | Sum | GatewayName |  |
| TimeSinceLastRecoveryPoint | Время, прошедшее с момента последней доступной точки восстановления | Секунда | Multi | Region, VolumeId | Применимо |
| TimeSinceLastRecoveryPoint |  | Секунда | Multi | GatewayName, GatewayId |  |
| TotalCacheSize | Общий размер кэша в байтах | Байт | Sum | GatewayName, GatewayId | Применимо |
| UploadBufferFree |  | Байт | Sum | GatewayName, GatewayId | Применимо |
| UploadBufferPercentUsed | Процент использования буфера выгрузки шлюза | Процент | Multi | GatewayName, GatewayId | Применимо |
| UploadBufferUsed | Общее количество байт, используемых в буфере выгрузки шлюза | Байт | Sum | GatewayName, GatewayId | Применимо |
| UserCpuPercent | Процент времени CPU, затраченного на обработку шлюзом, усреднённый по всем ядрам | Процент | Multi | GatewayName |  |
| WorkingStorageFree | Общий объём неиспользуемого пространства в рабочем хранилище шлюза | Байт | Sum | GatewayName, GatewayId | Применимо |
| WorkingStoragePercentUsed | Процент использования буфера выгрузки шлюза | Процент | Multi | GatewayName, GatewayId | Применимо |
| WorkingStorageUsed | Общее количество байт, используемых в буфере выгрузки шлюза | Байт | Sum | GatewayName, GatewayId | Применимо |
| WriteBytes | Общее количество байт, записанных в ваши локальные приложения за отчётный период по всем томам шлюза | Байт | Sum | Region, VolumeId | Применимо |
| WriteBytes |  | Байт | Count | Region, VolumeId |  |
| WriteBytes |  | Байт | Sum | GatewayName, GatewayId | Применимо |
| WriteBytes |  | Байт | Count | GatewayName, GatewayId |  |
| WriteTime | Общее количество миллисекунд, затраченных на операции записи из ваших локальных приложений за отчётный период по всем томам шлюза | Миллисекунда | Multi | Region, VolumeId | Применимо |
| WriteTime |  | Миллисекунда | Multi | GatewayName, GatewayId | Применимо |