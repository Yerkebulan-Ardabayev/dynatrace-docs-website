...
source: https://docs.dynatrace.com/managed/ingest-from/amazon-web-services/integrate-with-aws/aws-all-services/aws-service-quantum-ledger-database-qldb
scraped: 2026-05-12T11:29:12.174538
---
...
Dynatrace принимает метрики для множества предопределённых пространств имён, включая Amazon QLDB. Можно просматривать графики по каждому экземпляру сервиса с набором измерений и создавать собственные графики, которые можно закреплять на дашбордах.
...
Сервис Amazon QLDB отображается на странице группы пользовательских устройств. Сервис состоит из экземпляров (пользовательских устройств), называемых **реестрами** (ledgers). Метрики каждого пользовательского устройства можно просматривать в разделе **Further details** на странице обзора пользовательского устройства, с разбивкой по **Streams** и **Command types**.
...
* **LedgerName** ограничивает данные конкретным реестром. Этим значением может быть любое имя реестра в текущем регионе AWS и текущем аккаунте AWS.
* **StreamId** ограничивает данные конкретным потоком журнала. Этим значением может быть любой идентификатор потока для реестра в текущем регионе AWS и текущем аккаунте AWS.
* **CommandType** ограничивает данные одной из следующих команд API данных QLDB: `AbortTransaction`, `CommitTransaction`, `EndSession`, `ExecuteStatement`, `FetchPage`, `StartSession`, `StartTransaction`.

Основное измерение: `LedgerName`.

| Имя | Описание | Единица измерения | Статистика | Измерения | Рекомендуется |
| --- | --- | --- | --- | --- | --- |
| CommandLatency | Время, затраченное на операции с данными (регистрируется с интервалом в одну минуту) | Миллисекунда | Multi | LedgerName, CommandType | Применимо |
| IndexedStorage | Общий объём дискового пространства, используемого таблицами, индексами и индексированной историей реестра (регистрируется с интервалом в 15 минут) | Байт | Multi | LedgerName | Применимо |
| IsImpaired | Флаг, указывающий, нарушена ли работа потока журнала в Kinesis Data Streams (регистрируется с интервалом в одну минуту) | Количество | Maximum | LedgerName, StreamId | Применимо |
| JournalStorage | Общий объём дискового пространства, используемого журналом реестра (регистрируется с интервалом в 15 минут) | Байт | Multi | LedgerName | Применимо |
| OccConflictExceptions | Количество запросов к QLDB, генерирующих исключение `OccConflictException` | Количество | Sum | LedgerName | Применимо |
| ReadIOs | Количество операций ввода-вывода чтения с диска (регистрируется с интервалом в одну минуту) | Количество | Sum | LedgerName | Применимо |
| ReadIOs |  | Количество | Multi | LedgerName | Применимо |
| SessionRateExceededExceptions | Количество запросов к QLDB, генерирующих исключение `SessionRateExceededException` | Количество | Sum | LedgerName | Применимо |
| Session4xxExceptions | Количество запросов к QLDB, генерирующих ошибку HTTP 4xx | Количество | Sum | LedgerName | Применимо |
| Session5xxExceptions | Количество запросов к QLDB, генерирующих ошибку HTTP 5xx | Количество | Sum | LedgerName | Применимо |
| WriteIOs | Количество операций ввода-вывода записи на диск (регистрируется с интервалом в одну минуту) | Количество | Sum | LedgerName | Применимо |
| WriteIOs |  | Количество | Multi | LedgerName | Применимо |