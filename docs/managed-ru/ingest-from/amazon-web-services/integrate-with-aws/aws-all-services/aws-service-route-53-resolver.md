...
source: https://docs.dynatrace.com/managed/ingest-from/amazon-web-services/integrate-with-aws/aws-all-services/aws-service-route-53-resolver
scraped: 2026-05-12T11:30:33.562466
---
...
Dynatrace принимает метрики для множества предопределённых пространств имён, включая Amazon Route 53 Resolver. Можно просматривать метрики по каждому экземпляру сервиса, разбивать их на несколько измерений и создавать собственные графики, которые можно закреплять на дашбордах.
...
| InboundQueryVolume | Количество DNS-запросов, перенаправленных из вашей сети в ваши VPC через конечную точку, заданную `EndpointId` (для входящих конечных точек) | Количество | Sum | EndpointId | Применимо |
| InboundQueryVolume |  | Количество | Sum | Region | Применимо |
| InboundQueryVolume |  | Количество | Sum | Region, RniId |  |
| OutboundQueryAggregateVolume | Общее количество DNS-запросов, перенаправленных из Amazon VPC в вашу сеть (для исходящих конечных точек) | Количество | Sum | Region |  |
| OutboundQueryAggregateVolume |  | Количество | Sum | Region, RniId |  |
| OutboundQueryAggregateVolume |  | Количество | Sum | EndpointId | Применимо |
| OutboundQueryVolume | Количество DNS-запросов, перенаправленных из ваших VPC в вашу сеть через конечную точку, заданную `EndpointId` (для исходящих конечных точек) | Количество | Sum | EndpointId | Применимо |
| OutboundQueryVolume |  | Количество | Sum | Region | Применимо |