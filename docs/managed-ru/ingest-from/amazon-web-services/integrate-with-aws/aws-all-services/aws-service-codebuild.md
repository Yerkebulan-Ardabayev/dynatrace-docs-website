...
source: https://docs.dynatrace.com/managed/ingest-from/amazon-web-services/integrate-with-aws/aws-all-services/aws-service-codebuild
scraped: 2026-05-12T11:30:43.251929
---

# Мониторинг AWS CodeBuild
...
Dynatrace принимает метрики для множества предопределённых пространств имён, включая AWS CodeBuild. Можно просматривать метрики по каждому экземпляру сервиса, разбивать их на несколько измерений и создавать собственные графики, которые можно закреплять на дашбордах.
...
Основное измерение: `ProjectName`.

| Имя | Описание | Единица измерения | Статистика | Измерения | Рекомендуется |
| --- | --- | --- | --- | --- | --- |
| BuildDuration | Измеряет длительность фазы `BUILD` сборки | Секунда | Multi | Region |  |
| BuildDuration |  | Секунда | Multi | ProjectName | Применимо |
| Builds | Измеряет количество запущенных сборок | Количество | Sum | Region | Применимо |
| Builds |  | Количество | Sum | ProjectName | Применимо |
| CPUUtilized | Количество единиц CPU выделенной вычислительной мощности, используемых контейнером сборки | None | Multi | ProjectName |  |
| CPUUtilized |  | None | Multi | ProjectName, BuildId, BuildNumber |  |
| CPUUtilizedPercent | Процент выделенной вычислительной мощности, используемой контейнером сборки | Процент | Multi | ProjectName |  |
| CPUUtilizedPercent |  | Процент | Multi | ProjectName, BuildId, BuildNumber | Применимо |
| DownloadSourceDuration | Измеряет длительность фазы `DOWNLOAD_SOURCE` сборки | Секунда | Multi | Region |  |
| DownloadSourceDuration |  | Секунда | Multi | ProjectName |  |
| Duration | Измеряет длительность всех сборок за период времени | Секунда | Multi | Region |  |
| Duration |  | Секунда | Multi | ProjectName | Применимо |
| FailedBuilds | Измеряет количество сборок, завершившихся сбоем из-за ошибки клиента или тайм-аута | Количество | Sum | Region | Применимо |
| FailedBuilds |  | Количество | Sum | ProjectName | Применимо |
| FinalizingDuration | Измеряет длительность фазы `FINALIZING` сборки | Секунда | Multi | Region |  |
| FinalizingDuration |  | Секунда | Multi | ProjectName |  |
| InstallDuration | Измеряет длительность фазы `INSTALL` сборки | Секунда | Multi | Region |  |
| InstallDuration |  | Секунда | Multi | ProjectName |  |
| MemoryUtilized | Количество мегабайт памяти, используемых контейнером сборки | Мегабайт | Multi | ProjectName |  |
| MemoryUtilized |  | Мегабайт | Multi | ProjectName, BuildId, BuildNumber |  |
| MemoryUtilizedPercent | Процент выделенной памяти, используемой контейнером сборки | Процент | Multi | ProjectName |  |
| MemoryUtilizedPercent |  | Процент | Multi | ProjectName, BuildId, BuildNumber | Применимо |
| PostBuildDuration | Измеряет длительность фазы `POST_BUILD` сборки | Секунда | Multi | Region |  |
| PostBuildDuration |  | Секунда | Multi | ProjectName |  |
| PreBuildDuration | Измеряет длительность фазы `PRE_BUILD` сборки | Секунда | Multi | Region |  |
| PreBuildDuration |  | Секунда | Multi | ProjectName |  |
| ProvisioningDuration | Измеряет длительность фазы `PROVISIONING` сборки | Секунда | Multi | Region |  |
| ProvisioningDuration |  | Секунда | Multi | ProjectName |  |
| QueuedDuration | Измеряет длительность фазы `QUEUED` сборки | Секунда | Multi | Region |  |
| QueuedDuration |  | Секунда | Multi | ProjectName | Применимо |
| StorageReadBytes | Скорость чтения из хранилища, используемая контейнером сборки | Байт в секунду | Multi | ProjectName |  |
| StorageReadBytes |  | Байт в секунду | Multi | ProjectName, BuildId, BuildNumber | Применимо |
| StorageWriteBytes | Скорость записи в хранилище, используемая контейнером сборки | Байт в секунду | Multi | ProjectName |  |
| StorageWriteBytes |  | Байт в секунду | Multi | ProjectName, BuildId, BuildNumber | Применимо |
| SubmittedDuration | Измеряет длительность фазы `SUBMITTED` сборки | Секунда | Multi | Region |  |
| SubmittedDuration |  | Секунда | Multi | ProjectName |  |
| SucceededBuilds | Измеряет количество успешных сборок | Количество | Sum | Region | Применимо |
| SucceededBuilds |  | Количество | Sum | ProjectName | Применимо |
| UploadArtifactsDuration | Измеряет длительность фазы `UPLOAD_ARTIFACTS` сборки | Секунда | Multi | Region |  |
| UploadArtifactsDuration |  | Секунда | Multi | ProjectName |  |