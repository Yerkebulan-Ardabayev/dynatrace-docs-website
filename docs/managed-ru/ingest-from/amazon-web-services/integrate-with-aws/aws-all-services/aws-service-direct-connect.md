---
title: Мониторинг AWS Direct Connect
source: https://docs.dynatrace.com/managed/ingest-from/amazon-web-services/integrate-with-aws/aws-all-services/aws-service-direct-connect
scraped: 2026-05-12T11:30:44.747769
---

# Мониторинг AWS Direct Connect
...
Dynatrace принимает метрики для множества предопределённых пространств имён, включая AWS Direct Connect. Можно просматривать метрики по каждому экземпляру сервиса, разбивать их на несколько измерений и создавать собственные графики, которые можно закреплять на дашбордах.
...
Основное измерение: `ConnectionId`.

| Имя | Описание | Единица измерения | Статистика | Измерения | Рекомендуется |
| --- | --- | --- | --- | --- | --- |
| ConnectionBpsEgress | Битрейт исходящих данных со стороны AWS этого соединения | Бит в секунду | Multi | ConnectionId | Применимо |
| ConnectionBpsIngress | Битрейт входящих данных на сторону AWS этого соединения | Бит в секунду | Multi | ConnectionId | Применимо |
| ConnectionErrorCount | Общее количество ошибок всех типов уровня MAC на устройстве AWS | Количество | Sum | ConnectionId | Применимо |
| ConnectionLightLevelRx | Состояние оптоволоконного соединения для входящего (ingress) трафика на сторону AWS этого соединения | Децибел-милливатт | Multi | ConnectionId, OpticalLaneNumber | Применимо |
| ConnectionLightLevelTx | Состояние оптоволоконного соединения для исходящего (egress) трафика со стороны AWS этого соединения | Децибел-милливатт | Multi | ConnectionId, OpticalLaneNumber | Применимо |
| ConnectionPpsEgress | Частота пакетов исходящих данных со стороны AWS этого соединения | Количество в секунду | Multi | ConnectionId |  |
| ConnectionPpsIngress | Частота пакетов входящих данных на сторону AWS этого соединения | Количество в секунду | Multi | ConnectionId |  |
| ConnectionState | Состояние соединения (активно или неактивно) | None | Multi | ConnectionId | Применимо |
| VirtualInterfaceBpsEgress | Битрейт исходящих данных со стороны AWS виртуального интерфейса | Бит в секунду | Multi | ConnectionId, VirtualInterfaceId | Применимо |
| VirtualInterfaceBpsIngress | Битрейт входящих данных на сторону AWS виртуального интерфейса | Бит в секунду | Multi | ConnectionId, VirtualInterfaceId | Применимо |
| VirtualInterfacePpsEgress | Частота пакетов исходящих данных со стороны AWS виртуального интерфейса | Количество в секунду | Multi | ConnectionId, VirtualInterfaceId |  |
| VirtualInterfacePpsIngress | Частота пакетов входящих данных на сторону AWS виртуального интерфейса | Количество в секунду | Multi | ConnectionId, VirtualInterfaceId |  |