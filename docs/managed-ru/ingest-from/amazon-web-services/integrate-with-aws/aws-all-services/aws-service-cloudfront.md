...
Dynatrace принимает метрики для множества предопределённых пространств имён, включая Amazon CloudFront. Можно просматривать метрики по каждому экземпляру сервиса, разбивать их на несколько измерений и создавать собственные графики, которые можно закреплять на дашбордах.
...
CloudFront, это глобальный сервис: метрики доступны только в регионе `us-east-1`. Учётные данные должны иметь как минимум следующий набор разрешений для региона `us-east-1`:
...
Основное измерение: `DistributionId`.

| Имя | Описание | Единица измерения | Статистика | Измерения | Рекомендуется |
| --- | --- | --- | --- | --- | --- |
| 4xxErrorRate | Процент всех запросов зрителей, для которых HTTP-код ответа равен `4xx` | Процент | Average | DistributionId, Region |  |
| 5xxErrorRate | Процент всех запросов зрителей, для которых HTTP-код ответа равен `5xx` | Процент | Average | DistributionId, Region |  |
| BytesDownloaded | Общее количество байт, загруженных зрителями для запросов `GET`, `HEAD` и `OPTIONS` | None | Sum | DistributionId, Region |  |
| BytesUploaded | Общее количество байт, отправленных зрителями на ваш источник через CloudFront с помощью запросов `POST` и `PUT` | None | Sum | DistributionId, Region |  |
| Requests | Общее количество запросов зрителей, полученных CloudFront по всем методам HTTP и для запросов как HTTP, так и HTTPS | None | Sum | DistributionId, Region | Применимо |
| TotalErrorRate | Процент всех запросов зрителей, для которых HTTP-код ответа равен `4xx` или `5xx` | Процент | Average | DistributionId, Region | Применимо |