...
Dynatrace принимает метрики для множества предопределённых пространств имён, включая Amazon MQ. Можно просматривать метрики по каждому экземпляру сервиса, разбивать их на несколько измерений и создавать собственные графики, которые можно закреплять на дашбордах.
...
Основное измерение: `Broker`.

### ActiveMQ

| Имя | Описание | Единица измерения | Статистика | Измерения | Рекомендуется |
| --- | --- | --- | --- | --- | --- |
| ConsumerCount | Количество потребителей, подписанных на место назначения | Количество | Average | Broker, Topic |  |
| ConsumerCount |  | Количество | Average | Broker, Queue |  |
| CpuCreditBalance | Количество заработанных кредитов CPU, которые экземпляр накопил с момента его запуска или старта (включая количество кредитов запуска) | Количество | Average | Broker | Применимо |
| CpuUtilization | Процент выделенных вычислительных единиц Amazon EC2, которые в данный момент использует брокер | Процент | Average | Broker | Применимо |
| CurrentConnectionsCount | Текущее количество активных подключений на текущем брокере | Количество | Average | Broker | Применимо |
| DequeueCount | Количество сообщений, подтверждённых потребителями, в минуту | Количество | Average | Broker, NetworkConnector, RemoteBroker |  |
| DequeueCount |  | Количество | Average | Broker, Topic |  |
| DequeueCount |  | Количество | Average | Broker, Queue |  |
| DispatchCount | Количество сообщений, отправленных потребителям, в минуту | Количество | Average | Broker, Topic |  |
| DispatchCount |  | Количество | Average | Broker, Queue |  |
| EnqueueCount | Количество сообщений, отправленных в место назначения, в минуту | Количество | Average | Broker, NetworkConnector, RemoteBroker |  |
| EnqueueCount |  | Количество | Average | Broker, Topic |  |
| EnqueueCount |  | Количество | Average | Broker, Queue |  |
| EnqueueTime | Сквозная задержка от момента поступления сообщения на брокер до момента его доставки потребителю | Миллисекунда | Multi | Broker, Topic |  |
| EnqueueTime |  | Миллисекунда | Multi | Broker, Queue |  |
| EstablishedConnectionsCount | Общее количество подключений, активных и неактивных, установленных на брокере | Количество | Average | Broker |  |
| ExpiredCount | Количество сообщений, которые не удалось доставить из-за истечения их срока действия, в минуту | Количество | Average | Broker, Topic |  |
| ExpiredCount |  | Количество | Average | Broker, Queue |  |
| HeapUsage | Процент лимита памяти JVM ActiveMQ, который в данный момент использует брокер | Процент | Average | Broker | Применимо |
| InFlightCount | Количество сообщений, отправленных потребителям, которые не были подтверждены | Количество | Average | Broker, Topic |  |
| InFlightCount |  | Количество | Average | Broker, Queue |  |
| InactiveDurableTopicSubscribersCount | Количество неактивных устойчивых подписчиков топика, до максимума в 2000 | Количество | Average | Broker |  |
| JobSchedulerStorePercentUsage | Процент дискового пространства, используемого хранилищем планировщика задач | Процент | Average | Broker |  |
| JournalFilesForFastRecovery | Количество файлов журнала, которые будут воспроизведены после штатного завершения работы | Количество | Average | Broker |  |
| JournalFilesForFullRecovery | Количество файлов журнала, которые будут воспроизведены после нештатного завершения работы | Количество | Average | Broker |  |
| MemoryUsage | Процент лимита памяти, который в данный момент использует место назначения. | Процент | Average | Broker, Topic |  |
| MemoryUsage |  | Процент | Average | Broker, Queue |  |
| NetworkIn | Объём входящего трафика брокера | Байт | Average | Broker |  |
| NetworkIn |  | Байт | Sum | Broker | Применимо |
| NetworkOut | Объём исходящего трафика брокера | Байт | Average | Broker |  |
| NetworkOut |  | Байт | Sum | Broker | Применимо |
| OpenTransactionCount | Общее количество выполняемых транзакций | Количество | Average | Broker |  |
| ProducerCount | Количество производителей для места назначения | Количество | Average | Broker, Topic |  |
| ProducerCount |  | Количество | Average | Broker, Queue |  |
| QueueSize | Количество сообщений в очереди. Эта метрика применяется только к очередям. | Количество | Average | Broker, Queue |  |
| ReceiveCount | Количество сообщений, полученных от удалённого брокера для дуплексного сетевого коннектора | Количество | Average | Broker, NetworkConnector, RemoteBroker |  |
| StorePercentUsage | Процент, использованный от лимита хранилища. Если он достигнет 100, брокер начнёт отклонять сообщения. | Процент | Average | Broker |  |
| TempPercentUsage | Процент доступного временного хранилища, используемого непостоянными сообщениями | Процент | Average | Broker |  |
| TotalConsumerCount | Количество потребителей сообщений, подписанных на места назначения на текущем брокере | Количество | Average | Broker | Применимо |
| TotalDequeueCount | Общее количество сообщений, потреблённых клиентами | Количество | Average | Broker |  |
| TotalEnqueueCount | Общее количество сообщений, отправленных на брокер | Количество | Average | Broker |  |
| TotalMessageCount | Количество сообщений, хранящихся на брокере | Количество | Average | Broker | Применимо |
| TotalProducerCount | Количество производителей сообщений, активных на местах назначения на текущем брокере | Количество | Average | Broker | Применимо |

### RabbitMQ

| Имя | Описание | Единица измерения | Статистика | Измерения | Рекомендуется |
| --- | --- | --- | --- | --- | --- |
| AckRate | Скорость, с которой сообщения подтверждаются потребителями. | Количество в секунду | Multi | Broker | Применимо |
| ChannelCount | Общее количество каналов, установленных на брокере. | Количество | Average | Broker |  |
| ConfirmRate | Скорость, с которой сервер RabbitMQ подтверждает опубликованные сообщения. | Количество в секунду | Multi | Broker | Применимо |
| ConnectionCount | Общее количество подключений, установленных на брокере. | Количество | Average | Broker |  |
| ConsumerCount | Общее количество потребителей, подключённых к брокеру. | Количество | Average | Broker, Queue, VirtualHost |  |
| ConsumerCount |  | Количество | Average | Broker | Применимо |
| ExchangeCount | Общее количество обменников, настроенных на брокере. | Количество | Average | Broker |  |
| MessageCount | Общее количество сообщений в очередях. | Количество | Average | Broker, Queue, VirtualHost |  |
| MessageCount |  | Количество | Average | Broker | Применимо |
| MessageReadyCount | Общее количество готовых сообщений в очередях. | Количество | Average | Broker, Queue, VirtualHost |  |
| MessageReadyCount |  | Количество | Average | Broker | Применимо |
| MessageUnacknowledgedCount | Общее количество неподтверждённых сообщений в очередях. | Количество | Average | Broker, Queue, VirtualHost |  |
| MessageUnacknowledgedCount |  | Количество | Average | Broker | Применимо |
| PublishRate | Скорость, с которой сообщения публикуются на брокере. | Количество в секунду | Multi | Broker | Применимо |
| QueueCount | Общее количество очередей, настроенных на брокере. | Количество | Average | Broker |  |
| RabbitMQDiskFreeLimit | Дисковый лимит для узла RabbitMQ. | Байт | Multi | Broker, Node | Применимо |
| RabbitMQDiskFree |  | Байт | Multi | Broker, Node | Применимо |
| RabbitMQFdUsed | Количество используемых файловых дескрипторов. | Количество | Average | Broker, Node |  |
| RabbitMQMemLimit |  | Байт | Multi | Broker, Node | Применимо |
| RabbitMQMemUsed |  | Байт | Multi | Broker, Node | Применимо |
| SystemCpuUtilization | Процент выделенных вычислительных единиц Amazon EC2, которые в данный момент использует брокер. | Процент | Multi | Broker, Node | Применимо |
