...
source: https://docs.dynatrace.com/managed/ingest-from/amazon-web-services/integrate-with-aws/aws-all-services/aws-service-connect
scraped: 2026-05-12T11:30:57.567539
---

# Мониторинг Amazon Connect
...
Dynatrace принимает метрики для множества предопределённых пространств имён, включая Amazon Connect. Можно просматривать метрики по каждому экземпляру сервиса, разбивать их на несколько измерений и создавать собственные графики, которые можно закреплять на дашбордах.
...
Основное измерение: `InstanceId`.

| Имя | Описание | Единица измерения | Статистика | Измерения | Рекомендуется |
| --- | --- | --- | --- | --- | --- |
| CallBackNotDialableNumber | Количество случаев, когда поставленный в очередь обратный вызов клиенту не удалось набрать, поскольку номер клиента находится в стране, для которой исходящие вызовы для экземпляра не разрешены | Количество | Sum | InstanceId, ContactFlowName, MetricGroup |  |
| CallRecordingUploadError | Количество записей вызовов, которые не удалось загрузить в корзину Amazon S3, настроенную для вашего экземпляра | Количество | Sum | InstanceId, MetricGroup | Применимо |
| CallsBreachingConcurrencyQuota | Общее количество голосовых вызовов, превысивших квоту одновременных вызовов для экземпляра | Количество | Sum |  |  |
| CallsPerInterval | Количество голосовых вызовов, как входящих, так и исходящих, принятых или совершённых в секунду в экземпляре | Количество | Multi | InstanceId, MetricGroup | Применимо |
| ConcurrentCalls | Количество одновременных активных голосовых вызовов в экземпляре на момент отображения данных на дашборде | Количество | Multi | InstanceId, MetricGroup | Применимо |
| ConcurrentCallsPercentage | Процент использованной в экземпляре сервисной квоты на одновременные активные голосовые вызовы | Процент | Multi | InstanceId, MetricGroup | Применимо |
| ContactFlowErrors | Количество случаев выполнения ветви ошибки для потока обращений | Количество | Sum | InstanceId, ContactFlowName, MetricGroup | Применимо |
| ContactFlowFatalErrors | Количество случаев, когда поток обращений не удалось выполнить из-за системной ошибки | Количество | Sum | InstanceId, ContactFlowName, MetricGroup |  |
| LongestQueueWaitTime | Наибольшее время в секундах, в течение которого обращение ожидало в очереди | Секунда | Multi | InstanceId, MetricGroup, QueueName | Применимо |
| MisconfiguredPhoneNumbers | Количество вызовов, завершившихся сбоем, поскольку номер телефона не связан с потоком обращений | Количество | Sum | InstanceId, MetricGroup |  |
| MissedCalls | Количество голосовых вызовов, пропущенных операторами в течение выбранного интервала обновления, например 1 минуты или 5 минут. Пропущенным считается вызов, на который оператор не ответил в течение 20 секунд. | Количество | Multi | InstanceId, MetricGroup | Применимо |
| PublicSigningKeyUsage | Количество случаев использования ключа безопасности потока обращений (открытого ключа подписи) для шифрования вводимых клиентом данных в потоке обращений | Количество | Sum | InstanceId, SigningKeyId |  |
| QueueCapacityExceededError | Количество вызовов, отклонённых из-за заполненности очереди | Количество | Sum | InstanceId, MetricGroup, QueueName | Применимо |
| QueueSize | Количество обращений в очереди | Количество | Multi | InstanceId, MetricGroup, QueueName | Применимо |
| ThrottledCalls | Количество голосовых вызовов, отклонённых из-за того, что частота вызовов в секунду превысила максимально поддерживаемую квоту | Количество | Sum | InstanceId, MetricGroup |  |
| ToInstancePacketLossRate | Доля потерь пакетов для вызовов в экземпляре; регистрируется каждые 10 секунд. Каждая точка данных находится в диапазоне от 0 до 1 и представляет долю потерянных пакетов для экземпляра. | None | Multi | Region, Instance ID, Participant, Stream Type, Type of Connection | Применимо |