---
title: Мониторинг Amazon Redshift
source: https://docs.dynatrace.com/managed/ingest-from/amazon-web-services/integrate-with-aws/aws-all-services/aws-service-redshift
scraped: 2026-05-12T11:30:22.126331
---

# Мониторинг Amazon Redshift
...
Dynatrace принимает метрики для множества предопределённых пространств имён, включая Amazon Redshift. Можно просматривать метрики по каждому экземпляру сервиса, разбивать их на несколько измерений и создавать собственные графики, которые можно закреплять на дашбордах.
...
| --- | --- | --- | --- | --- | --- |
| CPUUtilization | Процент использования CPU. Для кластеров эта метрика представляет агрегацию значений использования CPU по всем узлам (ведущим и вычислительным). | Процент | Multi | ClusterIdentifier | Применимо |
| CPUUtilization |  | Процент | Multi | ClusterIdentifier, NodeID | Применимо |
| DatabaseConnections | Количество подключений к базе данных кластера | Количество | Multi | ClusterIdentifier, NodeID |  |
| DatabaseConnections |  | Количество | Multi | ClusterIdentifier |  |
| DatabaseConnections |  | Количество | Sum | ClusterIdentifier, NodeID |  |
| DatabaseConnections |  | Количество | Sum | ClusterIdentifier |  |
| HealthStatus | Указывает на работоспособность кластера. Каждую минуту кластер подключается к своей базе данных и выполняет простой запрос. Если эта операция выполняется успешно, кластер считается работоспособным. В противном случае кластер неработоспособен. Статус неработоспособности может возникать, когда база данных кластера находится под крайне высокой нагрузкой или при наличии проблемы с конфигурацией базы данных на кластере. | Количество | Average | ClusterIdentifier | Применимо |
| Healthstatus |  | Количество | Average | ClusterIdentifier, NodeID |  |
| MaintenanceMode | Указывает, находится ли кластер в режиме обслуживания. | Количество | Multi | ClusterIdentifier, NodeID |  |
| MaintenanceMode |  | Количество | Multi | ClusterIdentifier |  |
| NetworkReceiveThroughput | Скорость, с которой узел или кластер получает данные. | Байт в секунду | Multi | ClusterIdentifier | Применимо |
| NetworkReceiveThroughput |  | Байт в секунду | Multi | ClusterIdentifier, NodeID | Применимо |
| NetworkTransmitThroughput | Скорость, с которой узел или кластер записывает данные. | Байт в секунду | Multi | ClusterIdentifier, NodeID |  |
| NetworkTransmitThroughput |  | Байт в секунду | Multi | ClusterIdentifier |  |
| PercentageDiskSpaceUsed | Процент использованного дискового пространства | Процент | Multi | ClusterIdentifier | Применимо |
| PercentageDiskSpaceUsed |  | Процент | Multi | ClusterIdentifier, NodeID |  |