...
source: https://docs.dynatrace.com/managed/ingest-from/amazon-web-services/integrate-with-aws/aws-all-services/aws-service-robomaker
scraped: 2026-05-12T11:28:03.862330
---

# Мониторинг AWS RoboMaker
...
Dynatrace принимает метрики для множества предопределённых пространств имён, включая AWS RoboMaker. Можно просматривать графики по каждому экземпляру сервиса с набором измерений и создавать собственные графики, которые можно закреплять на дашбордах.
...
Основное измерение: `SimulationJobId`.

| Имя | Описание | Единица измерения | Статистика | Измерения | Рекомендуется |
| --- | --- | --- | --- | --- | --- |
| Memory | Объём памяти (в ГБ), используемый заданием SimulationJob | Гигабайт | Multi | SimulationJobId | Применимо |
| RealTimeFactor | Отношение симулированного времени к реальному (по настенным часам) | Количество | Multi | SimulationJobId | Применимо |
| SimulationUnit | SimulationUnit рассчитывается на основе потребления vCPU и памяти заданием Simulation Job | Количество | Multi | SimulationJobId | Применимо |
| SimulationUnit |  | Количество | Sum | SimulationJobId | Применимо |
| vCPU | Количество виртуальных ядер CPU, используемых заданием симуляции | Количество | Multi | SimulationJobId | Применимо |

## Ограничения

Метрики `vCPU`, `Memory` и `SimulationUnit` предназначены для оценки. AWS RoboMaker отправляет метрики во время подготовки к запуску задания симуляции. Плата не начисляется, пока задание симуляции не перейдёт в состояние **Running**. Подробнее см. [документацию AWS](https://docs.aws.amazon.com/robomaker/latest/dg/monitoring-aws-robomaker-cloudwatch.html).