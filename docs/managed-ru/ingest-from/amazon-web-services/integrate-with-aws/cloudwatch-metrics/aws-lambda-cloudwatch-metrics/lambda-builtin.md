---
title: Мониторинг AWS Lambda (встроенный)
source: https://docs.dynatrace.com/managed/ingest-from/amazon-web-services/integrate-with-aws/cloudwatch-metrics/aws-lambda-cloudwatch-metrics/lambda-builtin
scraped: 2026-05-12T11:31:13.707946
---

# Мониторинг AWS Lambda (встроенный)
...
Сведения о различиях между классическими сервисами и другими сервисами см. в разделе [Миграция с классических (ранее «встроенных») сервисов AWS на облачные сервисы](/managed/ingest-from/amazon-web-services/integrate-with-aws/cloudwatch-metrics/aws-migration-guide "Переход с классических сервисов AWS на их новые версии.").

Dynatrace получает метрики для множества предварительно выбранных namespace, включая AWS Lambda. Можно просматривать метрики для каждого экземпляра сервиса, разделять метрики по нескольким измерениям и создавать пользовательские графики, которые можно закрепить на дашбордах.
...
* Обновлённая [политика мониторинга AWS](/managed/ingest-from/amazon-web-services/integrate-with-aws/cloudwatch-metrics#aws-policy-and-authentication "Приём метрик Amazon CloudWatch.") для включения дополнительных сервисов AWS.
...
Если не нужно добавлять разрешения для всех сервисов, а требуется выбрать разрешения только для определённых, обратитесь к таблице ниже. Таблица содержит набор разрешений, необходимых для [всех облачных сервисов AWS](/managed/ingest-from/amazon-web-services/integrate-with-aws/aws-all-services "Мониторинг всех облачных сервисов AWS в Dynatrace и просмотр доступных метрик."), а также для каждого облачного сервиса, список необязательных разрешений, специфичных для него.
...
* Чтобы отключить мониторинг встроенных сервисов, требуются Environment ActiveGate версии 1.245+ и Dynatrace версии 1.247+.
...
Сведения о включении мониторинга сервиса см. в разделе [Включение мониторинга сервиса](/managed/ingest-from/amazon-web-services/integrate-with-aws/aws-metrics-ingest/aws-enable-service-monitoring "Включение мониторинга AWS в Dynatrace.").

## Просмотр метрик сервиса

Метрики сервиса можно просматривать в окружении Dynatrace на странице **учётной записи AWS** или на странице **Дашборды**.
...
1. Перейдите в **AWS**.
2. Выберите учётную запись AWS, метрики которой требуется проверить.
3. Выберите блок **Сервис**. Метрики выбранного сервиса отображаются под инфографикой в разделе сервиса.

### Просмотр метрик на дашборде

Также можно создать собственный дашборд. Подробнее о создании дашбордов см. в разделе [Создание и редактирование дашбордов Dynatrace](/managed/analyze-explore-automate/dashboards-classic/dashboards/create-dashboards "Узнайте, как создавать и редактировать дашборды Dynatrace.")
...
Это встроенный сервис. Мониторинг начинается автоматически после создания нового экземпляра интеграции AWS. Для встроенных сервисов все метрики являются рекомендуемыми (изменение конфигурации невозможно).
