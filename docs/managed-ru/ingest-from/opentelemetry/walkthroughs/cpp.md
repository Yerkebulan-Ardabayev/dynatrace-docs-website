---
title: Инструментирование C++-приложения с OpenTelemetry
...
В этом руководстве показано, как добавить наблюдаемость в ваше C++-приложение с помощью библиотек и инструментов OpenTelemetry для C++.
...
* [Поддерживаемый](https://github.com/open-telemetry/opentelemetry-cpp/blob/main/README.md#supported-development-platforms) компилятор C++ (C++ 11 и новее)
...
1. Добавьте следующие директивы в конфигурацию сборки CMake в `CMakeLists.txt`:
   ...
   ```
2. Создайте файл с именем `otel.h` в каталоге приложения и сохраните в него следующее содержимое:
...
3. Настройте `DT_API_URL` и `DT_API_TOKEN` на [URL Dynatrace](#base-url) и [токен доступа](#access-token) в `otel.h`.

## Шаг 3 Инструментирование приложения

Чтобы использовать OpenTelemetry, сначала нужно выполнить эти два шага:

1. Добавьте необходимые заголовочные файлы в ваш код.

   Чтобы добавить заголовочные файлы, подключите `otel.h` везде, где требуется использовать OpenTelemetry.
...
2. Инициализируйте OpenTelemetry.
   ...
   Для инициализации используйте функцию `initOpenTelemetry` из `otel.h` и вызовите её на раннем этапе кода запуска приложения.

### Добавление трассировки

1. Получите ссылку на tracer provider.

   ```
   auto provider = opentelemetry::trace::Provider::GetTracerProvider();
   ```
2. Получите объект tracer.
   ...
   ```
3. С помощью `tracer` теперь можно запускать новые спаны и устанавливать их для текущей области выполнения.
   ...
   * Добавляем `TODO` вместо итоговой бизнес-логики
   * Вызываем метод `End()` спана, чтобы завершить спан

### Сбор метрик

1. Получите ссылку на meter provider.

   ```
   auto provider = metrics_api::Provider::GetMeterProvider();
   ```
2. Получите объект meter.
...
3. С помощью `meter` теперь можно создавать отдельные инструменты, например счётчик.
   ...
   ```
4. Теперь можно вызвать метод `Add()` объекта `request_counter`, чтобы записать новые значения счётчиком и сохранить дополнительные атрибуты (например, `action.type`).
...
1. Получите ссылку на logger provider.

   ```
   auto provider = logs_api::Provider::GetLoggerProvider();
   ```
2. Вызовите метод `GetLogger()` провайдера, чтобы получить экземпляр логгера.

   ```
   auto logger = provider->GetLogger("scope_name", "", OPENTELEMETRY_SDK_VERSION);
   ```
3. Вызовите любой из доступных методов логирования, чтобы записать запись в лог. В следующем примере записывается отладочное сообщение.
...
В следующих примерах предполагается, что мы обрабатываем context propagation с помощью стандартных заголовков [W3C trace context](https://www.w3.org/TR/trace-context/), и мы получаем и устанавливаем HTTP-заголовки с помощью объекта OpenTelemetry `http_client::Headers`.

Для этой цели мы используем экземпляр класса `HttpTextMapCarrier`, который мы определили при настройке и который основан на классе OpenTelemetry [`TextMapCarrier`](https://opentelemetry-cpp.readthedocs.io/en/latest/otel_docs/classopentelemetry_1_1context_1_1propagation_1_1TextMapCarrier.html#exhale-class-classopentelemetry-1-1context-1-1propagation-1-1textmapcarrier).

#### Извлечение контекста при получении запроса

Чтобы извлечь информацию о существующем контексте, мы вызываем метод `Extract` синглтона глобального пропагатора и передаём ему экземпляр `HttpTextMapCarrier`, а также текущий контекст. Это возвращает новый объект контекста (`new_context`), который позволяет нам продолжить предыдущую трассировку нашими спанами.
...
Для внедрения информации о текущем контексте в исходящий запрос мы вызываем метод `Inject` синглтона глобального пропагатора и передаём ему экземпляр `HttpTextMapCarrier`, а также текущий контекст. Это добавляет соответствующие заголовки к экземпляру `carrier`, который мы затем используем на следующем шаге с нашим HTTP-запросом.
