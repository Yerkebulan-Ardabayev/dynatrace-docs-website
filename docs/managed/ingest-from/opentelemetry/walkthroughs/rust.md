...
4. Configure the environment variables `DT_API_URL` and `DT_API_TOKEN`:

   * **Platform token**: Set `DT_API_TOKEN` to your platform token and replace `"Api-Token "` with `"Bearer "` in the `map.insert("Authorization"...)` line.
