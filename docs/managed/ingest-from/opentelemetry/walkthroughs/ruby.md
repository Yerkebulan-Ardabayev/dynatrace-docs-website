...
3. Add the `init_opentelemetry` function to startup code and provide the variables `DT_API_URL` and `DT_API_TOKEN` with the values for the [Dynatrace URL](#base-url) and [access token](#access-token).
   ...
   When exporting to OneAgent, add `compression: "none"` to the `Exporter.new()` call to disable that feature. Otherwise, [export to ActiveGate](/managed/ingest-from/opentelemetry/otlp-api#export-to-activegate "Learn about the OTLP API endpoints that your application uses to export OpenTelemetry data to Dynatrace.") instead.
