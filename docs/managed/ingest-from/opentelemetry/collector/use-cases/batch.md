...
Under `exporters`, we specify an [`otlp_http` exporter﻿](https://github.com/open-telemetry/opentelemetry-collector/tree/v0.157.0/exporter/otlphttpexporter) for each signal and configure it with the appropriate batching settings as well as our Dynatrace API URL and the required authentication token.
...
Under the `sending_queue` section of the exporter's config, we specify a different [`batch` section﻿](https://github.com/open-telemetry/opentelemetry-collector/tree/v0.157.0/exporter/exporterhelper#sending-queue-batch-settings)
