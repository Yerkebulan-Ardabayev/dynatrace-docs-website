...
* the OTel Collector deployed in [agent mode](/managed/ingest-from/opentelemetry/collector/deployment#dynatrace-docs--agent "How to deploy the Dynatrace OpenTelemetry Collector.")
...
We also configure the [`transform` processor﻿](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/v0.160.0/processor/transformprocessor) to have Kubernetes cluster information automatically added as resource attributes for all telemetry signals.
