...
1. [Create an access token with `InstallerDownload`](/managed/manage/identity-access-management/access-tokens-and-oauth-clients/access-tokens#paas-token "Learn the concept of an access token and its scopes.") scope
...
A Dynatrace ActiveGate image is supported on a variety of Kubernetes and OpenShift versions. For a complete list, see [Technology support - Kubernetes](/managed/ingest-from/technology-support/support-model-and-issues "How Dynatrace supports Kubernetes and Red Hat OpenShift versions and known issues").
...
For verification details, see [Verify Software Bill of Materials (SBOM) Attestation](/managed/ingest-from/setup-on-k8s/guides/container-registries/verify-image-signature#sbom-attestation-verification "Verify Dynatrace image signatures").
...
4. Modify your deployment YAML file.
   ...
   * Optional Apply resource limits according to sizing hints.
   ...
   For additional configuration options, see [Containerized ActiveGate configuration](/managed/ingest-from/dynatrace-activegate/activegate-in-container/configuration "Learn how to configure containerized ActiveGate.").
5. Deploy ActiveGate.
...
2. Reduce the number of ActiveGate threads:
   ...
   * Create custom properties as described in [Advanced configuration](/managed/ingest-from/dynatrace-activegate/activegate-in-container/configuration#advanced-configuration "Learn how to configure containerized ActiveGate.")
...
* To monitor Kubernetes/Openshift, select one of the following:
  ...
  + Use [Dynatrace Operator](/managed/ingest-from/setup-on-k8s/deployment "Deploy Dynatrace Operator on Kubernetes")
  + Deploy [ActiveGate directly as a StatefulSet](/managed/ingest-from/setup-on-k8s/deployment/other/ag-statefulset "Install and configure ActiveGate in Kubernetes as a StatefulSet.")
* To collect logs from Kubernetes, use [Log Monitoring](/managed/upgrade/unavailable-in-managed "Your selection is unavailable in Dynatrace Managed.").

## FIPS-compliant images

ActiveGate version 1.315+

There is a dedicated, FIPS-compliant ActiveGate image available. See [ActiveGate FIPS compliance](/managed/ingest-from/dynatrace-activegate/activegate-fips-compliance "Learn about ActiveGate FIPS compliance") for information on requirements, limitations, where to get the image, and how to verify the deployment.