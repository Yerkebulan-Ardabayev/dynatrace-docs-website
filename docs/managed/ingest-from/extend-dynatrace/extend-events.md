...
scraped: 2026-05-12T12:30:28.598745
---
...
The full API documentation is available at [Events API v2](/managed/dynatrace-api/environment-api/events-v2 "Find out what you can do with the Dynatrace Events API v2.").
...
| --- | --- |
| Cluster ActiveGate | `https://{your-domain}/e/{your-environment-id}/api/v2/events` |
...
Be sure to specify your [environment ID](/managed/discover-dynatrace/get-started/monitoring-environment "Understand and learn how to work with monitoring environments.") at the correct location in the URL.
...
See [Events API v2](/managed/dynatrace-api/environment-api/events-v2 "Find out what you can do with the Dynatrace Events API v2.") for a full list of ActiveGate examples for the different request types.
...
OneAgent uses by default the TCP port 14499 for this endpoint. You can change the port with [`oneagentctl`](/managed/ingest-from/extend-dynatrace/extend-metrics/ingestion-methods/oneagent-metric-api#communication-port "Use the Dynatrace API to retrieve the metrics of monitored entities.").
...
The following curl command sends a `POST` request to the local OneAgent endpoint at `/v2/events/ingest`, indicates a JSON content type, and provides the [JSON event data](/managed/dynatrace-api/environment-api/events-v2/post-event#request-body-objects "Ingests an event via the Dynatrace API.") using the `--data` parameter.
