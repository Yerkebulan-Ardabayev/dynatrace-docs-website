...
Both `[Options]` and `[Metrics]` are optional. The syntax of metrics passed to the `[Metrics]` arguments must comply with the [Metrics ingestion protocol](/managed/ingest-from/extend-dynatrace/extend-metrics/reference/metric-ingestion-protocol "Learn how the data ingestion protocol for Dynatrace Metrics API works.").
