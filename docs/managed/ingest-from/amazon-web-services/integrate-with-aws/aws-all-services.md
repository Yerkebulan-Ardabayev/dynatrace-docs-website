...
As a result of [AWS monitoring integration](/managed/ingest-from/amazon-web-services/integrate-with-aws/cloudwatch-metrics "Integrate metrics from Amazon CloudWatch."), some services are monitored out-of-the-box. Such services are marked as classic.
...
To list all AWS-supported services on your cluster by the current version, use the [AWS-supported services API](/managed/dynatrace-api/configuration-api/aws-supported-services "Fetch a list of AWS supported services via the Dynatrace API.").
