...
The table below lists all AWS metrics that Dynatrace collects by default when you enable [AWS monitoring](/managed/observe/infrastructure-observability/cloud-platform-monitoring/aws-monitoring "Monitor AWS with Dynatrace").

For all other metrics collected by Dynatrace per configurable AWS service, see the [cloud services](/managed/ingest-from/amazon-web-services/integrate-with-aws/aws-all-services "Monitor all AWS cloud services with Dynatrace and view available metrics.") pages.
