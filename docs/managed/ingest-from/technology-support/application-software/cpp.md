---
title: C++
source: https://docs.dynatrace.com/managed/ingest-from/technology-support/application-software/cpp
---

# C++

# C++

* Reference
* 1-min read
* Published Dec 20, 2022

You can send data from your C++ application to Dynatrace via OpenTelemetry. See also

* [OpenTelemetry support﻿](https://github.com/open-telemetry/opentelemetry-cpp) for capturing traces.

  + [Instrument your C++ application with OpenTelemetry](/managed/ingest-from/opentelemetry/walkthroughs/cpp "Learn how to instrument your C++ application using OpenTelemetry and Dynatrace.")
* [OneAgent SDK](/managed/ingest-from/extend-dynatrace/extend-tracing/oneagent-sdk "The Dynatrace OneAgent SDK enables you to instrument your application manually to extend end-to-end visibility for frameworks and technologies for which there is no code module yet available.") for custom tracing