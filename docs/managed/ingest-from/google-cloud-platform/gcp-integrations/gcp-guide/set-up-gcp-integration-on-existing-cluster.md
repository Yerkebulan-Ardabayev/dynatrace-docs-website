...
**Run the deployment script**](/managed/ingest-from/google-cloud-platform/gcp-integrations/gcp-guide/set-up-gcp-integration-on-existing-cluster#script "Deploy log and metric monitoring for Google Cloud services on an existing standard GKE or GKE Autopilot cluster")
...
Monitoring of following services will be enabled during deployment of Google Cloud Monitor, provided that the appropriate extensions from Dynatrace Hub are also installed:
...
* View and analyze Google Cloud logs: in Dynatrace, go to **Logs** and, to look for Google Cloud logs, filter by `cloud.provider: gcp`.
