...
This guide describes the steps required to migrate your Dynatrace deployment from cloud-native full-stack to the [application monitoring mode](/managed/ingest-from/setup-on-k8s/how-it-works#auto "In-depth description on how the deployment on Kubernetes works.").
...
| Component | cloud-native full-stack | Application monitoring |
| --- | --- | --- |
| Dynatrace OneAgent | * Deployed as a DaemonSet * Collect host metrics on nodes | * Not present |
| Dynatrace Webhook Server | * Validates DynaKube definitions * Inject code modules into application pods by modifying pod definitions | * Validates DynaKube definitions * Inject code modules into application pods by modifying pod definitions |
| [Dynatrace Operator CSI driver](/managed/ingest-from/setup-on-k8s/how-it-works#csi-driver "In-depth description on how the deployment on Kubernetes works.") | Required  * Deployed as a DaemonSet * Provides volume storage for OneAgents * Manages and provides code modules used for pod injection and optimizes storage consumption | Optional  * Deployed as a DaemonSet * Manages and provides code modules used for pod injection and optimizes storage consumption |