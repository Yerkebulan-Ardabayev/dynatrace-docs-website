...
* Use [ArgoCD﻿](https://dt-url.net/hi037z9) to deploy the configuration from the Git repository into the Kubernetes environment.
* Implement [Renovate﻿](https://dt-url.net/vn237h6) to automatically update the Git repository with the latest Dynatrace Operator configurations.
...
[Argo﻿](https://dt-url.net/wt4379d) offers a suite of open-source tools for Kubernetes app deployment and management. ArgoCD, a continuous delivery tool, is used to keep the Dynatrace Operator configuration in sync with the Kubernetes cluster.
...
Renovate automates the updating of dependencies in Git repositories. Integrating Renovate into your workflow ensures that the Dynatrace Operator version specified in your `ApplicationSet` is always up to date. Use the [Renovate guide﻿](https://dt-url.net/67637gq) for instructions on updating ArgoCD configurations.

## Related topics

* [Manage Dynatrace deployments using GitOps](/managed/ingest-from/setup-on-k8s/guides/deployment-and-configuration/using-gitops "How to deploy Dynatrace Operator and DynaKube using GitOps.")