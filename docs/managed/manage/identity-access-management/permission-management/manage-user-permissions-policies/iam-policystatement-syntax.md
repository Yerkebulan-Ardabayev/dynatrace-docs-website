...
| `<condition>` | Add a restriction to the permission based on a particular attribute or context. | * `WHERE` `<condition name>` `<condition operator>` = "`<condition value>`". + Example: `WHERE settings:schemaId = "builtin:container.monitoring-rule" AND storage:table-name = "events"` * To add multiple conditions to a `statement`, use `AND`. |
