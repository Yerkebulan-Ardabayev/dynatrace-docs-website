...
Use the Account Management **Budgets** tab to customize your Dynatrace Platform Subscription (DPS) budgets. You can:
...
Budgets complement the existing [Cost monitors](/managed/manage-your-costs/control/cost-monitors "Learn how to use the Cost Monitors feature to make forecasts and cost events.") by allowing you to define custom thresholds at the environment and capability level.
...
Budgets are spending limits tied to your annual DPS commitment.
You set them at the account, environment, or capability level, and Dynatrace notifies you as actual or forecasted consumption approaches each threshold (defaults: 75%, 90%, 100%).
Budgets are planned guardrails, complementary to cost monitors, which detect anomalies as they happen.

## Use cases

* Enforce a spending ceiling at the account level matching your annual commitment.
* Cap individual environments to a share of the total commitment.
* Apply per-capability budgets when a single capability (for example, logs or metrics) drives most cost.
* Notify finance, FinOps, or capability owners when actual costs approach a threshold.
...
Until then, it will display the message **We’re creating your budget. Check back later to see cost data.**
...
[Cost notifications](/managed/manage/account-management/notifications "Learn about Account Management notifications, for example budget alerts and cost monitor events.") inform you when budget thresholds are reached.
You can get notifications in the **Notification Center** and via email.
