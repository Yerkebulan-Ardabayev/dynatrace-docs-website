...
* One of the 32 GiB RAM servers is running RVA and RAP and consumes 35,040 ASUs per year.  
  `4 ASUs for RVA and RAP for a 32 GiB host Ã 24 (hours) Ã 365 (days)`
* The other 32 GiB RAM server is running only RVA and consumes 17,520 ASUs per year.  
  `2 ASUs for RVA for a 32 GiB host Ã 24 (hours) Ã 365 (days)`
* The 4 GiB RAM server runs only RVA and consumes 2,190 ASUs per year.  
  `0.25 ASUs for RVA for a 4 GiB host Ã 24 (hours) Ã 365 (days)`
* When the environment is no longer able to handle the load, an additional server with 32 GiB RAM running RVA and RAP spins up to handle the spikes, but it only runs a total of 250 hours during the year. So, the consumption is increased by 1,000 ASUs.  
  `4 ASUs for RVA and RAP for a 32 GiB host Ã 250 (hours)`
...
RunecastÂ® Security Posture Management (SPM) provides continuous monitoring and automated assessment for VMwareÂ® and cloud environments. With insights into configuration, compliance issues, and exposure risk assessment, customers can maintain a strong security posture.
...
* The unit of measure for VSPM is CPU sockets per year (CPU sockets of the licensed VMwareÂ® ESXi hosts), regardless of the number of cores or VMs.
...
You can view details about your current, expired, and upcoming licenses on the [licensing portalï»¿](https://dt-url.net/bj03ua2) (after logging in to your profile, you can see your licenses on the left).
