...
## Consumption generated by undefined host

When a host has been active for any time within a 15 minute billing window, tracked usage is generated. In certain cases, a host may be active long enough to generate tracked usage but not long enough for the host name to be stored in Dynatrace. In these instances, the host will be displayed as **Undefined**.

This affects built-in host monitoring metrics that are available per host:
