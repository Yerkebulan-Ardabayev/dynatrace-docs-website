...
![The priority of timestamp configuration rules](https://cdn.bfldr.com/B686QPH3/as/pzgvqxqw3xwmxb5vkmnjzjg/Supported_scopes_-_Light_Mode?auto=webp&format=png&position=1)

The priority of timestamp configuration rules
...
4. **Timestamp search limit**
   ...
   Use this field to define the number of characters in every log line where timestamp is searched. If you want to ignore timestamps and split logs using the [default rules](/managed/upgrade/unavailable-in-managed "Your selection is unavailable in Dynatrace Managed."), set this value to `0`. Use this field to overwrite the global [timestamp search limit](/managed/upgrade/unavailable-in-managed "Your selection is unavailable in Dynatrace Managed.") (default 64 bytes).
5. **Time zone**
...
6. Select **Add matcher** to create a specific match for this rule and narrow down the scope for that rule.
   ...
   | Matcher attribute | Matching is based on |
   | --- | --- |
   | **Process group** | The process group ID. |
   | **Log source** | A log path; wildcards are supported in form of an asterisk. |
   | **K8s container name** | The name of the Kubernetes container. |
   | **K8s namespace name** | The name of the Kubernetes namespace. |
   | **K8s deployment name** | The name of the Kubernetes deployment. |
   | **Container name** | The name of the container. |
   | **DT entity container group ID** | The DT entity container group ID. |
   | **Process technology** | The technology name. |
