...
1. Go to **Logs**.
2. Create a log viewer query or use log viewer **Available attributes** to extract the log data that interests you. For details, see [Log viewer](/managed/analyze-explore-automate/log-monitoring/analyze-log-data/log-viewer#query-syntax "Learn how to use Dynatrace log viewer to analyze log data.").
...
4. Optionally, add dimensions for your query result.  
   ...
   For details, see [Log viewer](/managed/analyze-explore-automate/log-monitoring/analyze-log-data/log-viewer#log-dimensions "Learn how to use Dynatrace log viewer to analyze log data.").
5. Append the metric name to the metric key `log.` and save changes to create the log metric.

### Unique dimensions limits

If you create metrics with high cardinality (a lot of unique dimensions), you can reach the limit of 1,000,000 dimensions per metric, or the limit of 50,000,000 for all metrics. To mitigate these limitations, you can:
