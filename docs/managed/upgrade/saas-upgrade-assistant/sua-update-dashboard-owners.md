...
* **Automatic adjustments** tells you what changes will be applied during the deployment automatically and where you can
  ...
  + Check the number of users in the mapping who don’t have an email address configured. Use the link to go to Settings and define an email domain that can be automatically appended to the user ID.
...
* **Review errors** tab—one warning is produced for each dashboard. You can review them and navigate to configuration details.
* **Upgrade details: configuration** tab—a warning icon  next to the `dashboard` config type indicates that some action is required in one or more dashboards. The same icon is displayed next to the affected dashboard configurations names.
...
2. In the **Update dashboard owners** step
   ...
   3. If the number of warnings doesn’t go down after the update, ensure you have invited Managed users to the SaaS account using their Managed email as a login.
...
Email domain wasn’t added to some owners.

There are two possible reasons:

* The user has been removed from the Managed cluster and is missing from the mapping.
* The user ID contains invalid characters, so appending the email domain doesn’t form a correct email address.
...
2. Choose configurations to include in the deployment step.
   ...
   2. Review configurations and exclude the ones you don’t want.
...
What if I don’t see the Update dashboard owners step?
...
What if the owners were automatically updated, but warnings still say they’re missing?
...
* Make sure the affected user’s email address in the Managed cluster is correct.
