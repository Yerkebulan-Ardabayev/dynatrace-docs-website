...
| includeProcessGroupReferences | boolean | Flag to include process group references to the response.  Process group references aren't compatible across environments.  When this flag is set to `false`, conditions with process group references are removed. If that results in a rule having no conditions, the entire rule is removed. | query | Optional |
...
| --- | --- | --- |
| **200** | [AutoTag](#openapi-definition-AutoTag) | Success |
...
| entitySelectorBasedRules | [EntitySelectorBasedAutoTagRule](#openapi-definition-EntitySelectorBasedAutoTagRule)[] | A list of entity-selector based rules for auto tagging usage.  If several rules are specified, the **OR** logic applies. |
...
| rules | [AutoTagRule](#openapi-definition-AutoTagRule)[] | The list of rules for tag usage.  When there are multiple rules, the OR logic applies. |
...
| normalization | string | Changes applied to the value after applying the value format. Default is LEAVE\_TEXT\_AS\_IS. The element can hold these values * `LEAVE_TEXT_AS_IS` * `TO_LOWER_CASE` * `TO_UPPER_CASE` |
| valueFormat | string | The value of the entity-selector-based auto-tag. If specified, the tag is used in the `name:valueFormat` format.  For example, you can extend the `Infrastructure` tag to `Infrastructure:Windows` and `Infrastructure:Linux`. |
...
| Element | Type | Description |
| --- | --- | --- |
| conditions | [EntityRuleEngineCondition](#openapi-definition-EntityRuleEngineCondition)[] | A list of matching rules for the auto-tag.  The tag applies only when **all** conditions are fulfilled. |
...
| Element | Type | Description |
| --- | --- | --- |
| comparisonInfo | [ComparisonBasic](#openapi-definition-ComparisonBasic) | Defines how the matching is actually performed: what and how are we comparing.  The actual set of fields and possible values of the **operator** field depend on the type of the comparison. Find the list of actual objects in the description of the **type** field or see [JSON models﻿](https://dt-url.net/0b83s6z?dt=m). |
| key | [ConditionKey](#openapi-definition-ConditionKey) | The key to identify the data we're matching.  The actual set of fields and possible values depend on the type of the key. Find the list of actual objects in the description of the **type** field or see [JSON models﻿](https://dt-url.net/0b83s6z?dt=m). |
...
| value | [AnyValue](#openapi-definition-AnyValue) | The value to compare to. |
