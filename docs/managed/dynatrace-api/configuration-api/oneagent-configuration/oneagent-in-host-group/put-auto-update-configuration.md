...
| body | [HostGroupAutoUpdateConfig](#openapi-definition-HostGroupAutoUpdateConfig) | The JSON body of the request. Contains OneAgent auto-update parameters. | body | Optional |
...
| --- | --- | --- | --- |
| effectiveSetting | string | The actual state of the auto-update on the hosts in a host group.  Applicable only if the **setting** parameter is set to `INHERITED`. In that case the value is taken from the environment-wide setting. The element can hold these values * `ENABLED` * `DISABLED` | Optional |
| effectiveVersion | string | The actual version to which the OneAgent must be updated.  Applicable only if the **setting** parameter is set to `INHERITED`. In that case the value is taken from the environment-wide setting. | Optional |
