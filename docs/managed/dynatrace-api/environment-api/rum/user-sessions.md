...
The GET tree request executes a USQL query and returns results as a tree structure of the requested columns—a flat list containing the requested columns.](/managed/dynatrace-api/environment-api/rum/user-sessions/tree "View user sessions data in a tree form via the Dynatrace API.")[### User session structure
