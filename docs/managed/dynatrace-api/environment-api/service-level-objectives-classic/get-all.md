...
| filter | string | The entity filter for the SLO evaluation. The total length of the entitySelector string in SLOs is limited to 1,000 characters. Use the [syntax of entity selectorï»¿](https://dt-url.net/entityselector). |
...
| ~~useRateMetric~~ | boolean | DEPRECATED  The type of the metric to use for SLO calculation:  * `true`: An existing percentage-based metric. * `false`: A ratio of two metrics.  For a list of available metrics, see [Built-in metric pageï»¿](https://dt-url.net/be03kow) or try the [GET metricsï»¿](https://dt-url.net/8e43kxf) API call. |
