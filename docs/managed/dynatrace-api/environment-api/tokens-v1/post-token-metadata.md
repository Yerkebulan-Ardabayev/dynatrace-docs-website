...
Alternatively, you can retrieve metadata by submitting the ID of the token with the [GET token metadata](/managed/dynatrace-api/environment-api/tokens-v1/get-token-metadata "Learn how use the Dynatrace API to get metadata for a Dynatrace API authentication token.") call.
...
| personalAccessToken | boolean | The token is a [personal access token﻿](https://dt-url.net/wm03sop?dt=m) (`true`) or an API token (`false`). |
