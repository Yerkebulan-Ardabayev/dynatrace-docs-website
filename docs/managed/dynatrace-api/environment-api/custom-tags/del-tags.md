...
| --- | --- | --- | --- | --- |
| key | string | The key of the tag to be deleted.  If **deleteAllWithKey** is `true`, then all tags with this key are deleted. Otherwise, only tags with exact match of key **and** value are deleted.  For value-only tags, specify the value here. | query | Required |
| value | string | The value of the tag to be deleted. The value is ignored if **deleteAllWithKey** is `true`.  For value-only tags, specify the value in the **key** parameter. | query | Optional |
| deleteAllWithKey | boolean | * If `true`, all tags with the specified key are deleted, regardless of the value. * If `false`, only tags with exact match of key **and** value are deleted.  If not set, `false` is used. | query | Optional |
...
| --- | --- | --- |
| **200** | [DeletedEntityTags](#openapi-definition-DeletedEntityTags) | Success |
...
In this example, the request deletes the **REST-test** custom tag created in the [POST request](/managed/dynatrace-api/environment-api/custom-tags/post-tags#example "Assign custom tags to monitored entities via Dynatrace API.") example (`key=REST-test`). Again the **entitySelector** query parameter is set to `type("HOST"),tag("easyTravel")`.
