...
| Connect containerized ActiveGate to local Kubernetes API endpoint `clusterIdEnabled` | boolean | For more information on local Kubernetes API monitoring, see the [documentation﻿](https://dt-url.net/6q62uep).  Enable this toggle when the ActiveGate is deployed to the same Kubernetes clusters you want to monitor. Disable it if you want to monitor a different Kubernetes cluster. | Required |
...
| Kubernetes API URL Target `endpointUrl` | text | Get the API URL for [Kubernetes﻿](https://dt-url.net/kz23snj "Kubernetes") or [OpenShift﻿](https://dt-url.net/d623xgw "OpenShift"). | Required |
| Kubernetes Bearer Token `authToken` | secret | Create a bearer token for [Kubernetes﻿](https://dt-url.net/og43szq "Kubernetes") or [OpenShift﻿](https://dt-url.net/7l43xtp "OpenShift"). | Required |
