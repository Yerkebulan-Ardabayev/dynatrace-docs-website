...
Enabling this setting merges processes of similar workloads into process groups, and consequently, services. Please note that [fine-grained process detection rulesï»¿](https://www.dynatrace.com/support/help/shortlink/process-groups) will still be applied, while ignoring container or platform specific properties.
...
| `filters` | [FilterComplex](#FilterComplex)[] | Define rules to merge similar Kubernetes workloads into process groups.  You can use workload properties like namespace name, base pod name or container name as well as the [environment variables DT\_RELEASE\_STAGE and DT\_RELEASE\_PRODUCTï»¿](https://dt-url.net/sb02v2a) for grouping processes of similar workloads. The first applicable rule will be applied. If no rule matches, âNamespace nameâ + âBase pod nameâ + âContainer nameâ is used as fallback. | Required |
