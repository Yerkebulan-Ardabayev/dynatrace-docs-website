...
| --- | --- | --- | --- |
| Field name `fieldName` | text | - | Required |
| Operator `operator` | enum | The element has these enums * `EQUALS` * `NOT_EQUAL` * `IS_NULL` * `IS_NOT_NULL` * `STARTS_WITH` * `LIKE` * `NOT_LIKE` * `LESS_THAN` * `LESS_THAN_OR_EQUAL_TO` * `GREATER_THAN` * `GREATER_THAN_OR_EQUAL_TO` * `IN` | Required |
