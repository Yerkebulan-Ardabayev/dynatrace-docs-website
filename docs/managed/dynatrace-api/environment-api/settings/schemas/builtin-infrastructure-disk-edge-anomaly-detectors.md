...
To learn more about Disk Edge visit its [official documentationï»¿](https://dt-url.net/diskEdgeDoc).
...
| Disk name filters `diskNameFilters` | set | Disk will be included in this policy if **any** of the filters match  Disk name filter has to match a required format.  * `$match(/zSecure/snapshot?/*)` â Matches string with wildcards: `*` any number (including zero) of characters and `?` exactly one character. * `$contains(/log/)` â Matches if `/log/` appears anywhere in disk name. * `$eq(/)` â Matches if `/` matches the disk name exactly. * `$prefix(/srv/)` â Matches if `/srv/` matches the prefix of disk name. * `$suffix(/backup)` â Matches if `/backup` matches the suffix of disk name.  Available logic operations:  * `$not($eq(/usr))` â Matches if the disk name is different from `/usr`. * `$and($prefix(/var),$suffix(/backup))` â Matches if disk name starts with `/var` and ends with `/backup`. * `$or($prefix(/home/),$eq(/root))` â Matches if disk name starts with `/home` or equals `/root`.  Brackets **(** and **)** that are part of the matched disk name **must be escaped with a tilde (~)** | Required |
...
| Properties `eventProperties` | Set<[MetadataItem](#MetadataItem)> | Set of additional key-value properties to be attached to the triggered event. You can retrieve the available property keys using the [Events API v2ï»¿](https://dt-url.net/9622g1w). Additionally any Host resource attribute can be dynamically substituted (agent 1.325+) | Required |
...
| Filesystem condition `diskFilesystemCondition` | text | Disk filesystem will be included in this policy if **any** of the filters match  Disk filesystem has to match a required format.  * `$match(ext*)` â Matches string with wildcards: `*` any number (including zero) of characters and `?` exactly one character. * `$contains(fs)` â Matches if `fs` appears anywhere in the filesystem type. * `$eq(ext4)` â Matches if `ext4` matches the filesystem type exactly. * `$prefix(ext)` â Matches if `ext` matches the prefix of the filesystem type. * `$suffix(fs)` â Matches if `fs` matches the suffix of the filesystem type.  Available logic operations:  * `$not($eq(tmpfs))` â Matches if the filesystem type is different from `tmpfs`. * `$and($prefix(ext),$suffix(4))` â Matches if filesystem type starts with `ext` and ends with `4`. * `$or($eq(xfs),$eq(btrfs))` â Matches if filesystem type equals `xfs` or `btrfs`.  Brackets **(** and **)** that are part of the matched filesystem type **must be escaped with a tilde (~)** | Required |
