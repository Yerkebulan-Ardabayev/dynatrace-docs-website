...
Define custom [Service-level objectivesï»¿](https://dt-url.net/slos) (SLOs) to assist in fulfilling your organizationâs service-level agreements. Create up to 10000 custom SLOs for this Dynatrace environment.
...
| Entity selector `filter` | text | Set a filter parameter (entitySelector) on any GET call to evaluate this SLO against specific services only (for example, type("SERVICE")). For details, see the [Entity Selector documentationï»¿](https://dt-url.net/entityselector). | Required |
