...
| Source `source` | text | The source must follow the syntax of Kubernetes annotation/label keys as defined in the [Kubernetes documentation﻿](https://dt-url.net/2c02sbn).  `source := (prefix/)?name`  `prefix := a-z0-9 (`/[-a-z0-9]*[a-z0-9]`)?(\.a-z0-9 (`/[-a-z0-9]*[a-z0-9]`)?)*`  `name := ([A-Za-z0-9][-A-Za-z0-9_.]*)?[A-Za-z0-9]`  Additionally, the name can have at most 63 characters, and the overall length of the source must not exceed 75 characters. | Required |
