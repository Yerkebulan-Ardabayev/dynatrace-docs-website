...
| **400** | - | Operation failed. The input is invalid. Possible reasons:  * No group information received for the request to create a group * Group ID is not set * Group name cannot be null or empty * At least one of the specified environments doesn't exist |
...
In this example, you update the `Sales Group` user group to only map to `sales` LDAP group membership. This group will allow access Cluster Management Console and Account Management full rights. As a response, you'll receive back the entity's current state.
