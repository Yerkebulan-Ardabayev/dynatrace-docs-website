...
To view the resources that are excluded from the default export, run the provider with the `-list-exclusions` flag. To export any of the excluded resources, see the [Usage Examples](/managed/deliver/configuration-as-code/terraform/guides/export-utility#usage-examples "Export existing Dynatrace configurations using Dynatrace Configuration as Code via Terraform.").
