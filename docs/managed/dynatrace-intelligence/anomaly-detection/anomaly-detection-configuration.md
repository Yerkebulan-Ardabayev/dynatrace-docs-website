...
* The preview of a metric event visually represents your event's behavior. You can adjust the settings to see how they affect the configuration.

## Data source

Data source provides a time series that is evaluated by Davis:

* A metric defines the time series. It can be a single metric defined by a metric key or a [metric expression](/managed/dynatrace-api/environment-api/metric-v2/metric-expressions "Use metric expressions to apply arithmetic operations in a data points query via the Metrics API v2.").
...
| Threshold | This parameter defines the value of a static threshold and, if applicable, its unit. Select **Suggest values** to use a value calculated by Davis based on the previous data. |
| Tolerance | This parameter defines the [tolerance](/managed/dynatrace-intelligence/ai-models/seasonal-baseline#parameters "How Davis suggests Seasonal baseline thresholds for a scope of entities.") of the seasonal model. The higher the tolerance, the broader the confidence band, leading to fewer triggered events. |
