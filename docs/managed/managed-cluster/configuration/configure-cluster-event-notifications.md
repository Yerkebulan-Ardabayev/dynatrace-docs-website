---
title: Configure Cluster event notifications
...
You can manage the following notifications in a Managed Cluster:

* **Insufficient disk space on a Managed Cluster node**  
  Triggered when a disk partition on a Managed Cluster node has less disk space than required for a given storage type. In that case, you need to extend your disk or review cluster settings. Otherwise, data retention might be reduced.
* **Insufficient hardware on a Managed Cluster node**  
  Triggered when a node doesn't have enough CPU cores and RAM to meet recommended requirements. For details, see [System requirements](/managed/managed-cluster/installation/managed-hardware-requirements "Review the hardware sizing, storage, and multi-node cluster requirements before installing Dynatrace Managed on your infrastructure.").
* **Metrics storage exceeds supported size (4 TiB)**  
  Triggered when metrics storage for a node is too high. In that case, you need to review your monitoring settings or add additional nodes to the Managed Cluster.
* **Transaction storage retention period truncation**  
  Triggered when data retention has been automatically reduced to store new data. In that case, Dynatrace recommends that you adjust the target retention time, review monitoring settings, or extend a disk.
* **Adaptive Load Reduction activity**  
  Triggered when a node is considered overloaded and unable to keep up with processing incoming requests. In that case, you need to review monitoring settings or increase CPU cores and RAM on the node. For details, see [Adaptive Load Reduction](/managed/ingest-from/dynatrace-oneagent/adaptive-traffic-management/adaptive-traffic-management-managed#alr "Improve your Dynatrace Managed environment health and performance with the adaptive features of traffic management, load reduction, and capture control.").

## UI workflows

Use the Cluster Management Console (CMC) to configure who receives cluster event notifications and emergency notifications.

### Configure email notification recipients

Configure who receives cluster event email notifications in the CMC:

1. In CMC, go to **Settings** > **Emails** > **Email notifications**.
2. In **Notifications recipients**, enter the email addresses that should receive cluster, environment, and account-related notifications. Separate multiple email addresses with commas.
3. Optional: Turn on **Also notify all active users with cluster admin rights**.

### Configure emergency contacts

Emergency contacts receive notifications from Dynatrace product experts in the event of catastrophic Managed Cluster states or critical security vulnerabilities that must be addressed.

1. In the CMC, go to **Settings** > **Emails** > **Email notifications**.
...
To configure which cluster events trigger email notifications, send your configuration as a JSON payload. Use the [POST an object](/managed/dynatrace-api/environment-api/settings/objects/post-object "Create or validate a settings object via the Dynatrace API.") endpoint with a cluster token that has the appropriate [access scopes](#token-authentication):
...
To delete an existing notification settings object, make a [`DELETE`](/managed/dynatrace-api/environment-api/settings/objects/del-object "Delete a settings object via the Dynatrace API.") API call to the `/api/cluster/v2/settings/objects/<objectId>` endpoint. Use a cluster token with the appropriate [access scopes](#token-authentication) and provide the `objectId` obtained when you created the initial event notification settings object. After deletion, notification behavior falls back to the default: all notifications trigger an email to configured recipients.

## Cluster events and email notifications

The following tables list Managed Cluster event notifications by event category.
...
| Severity | Summary | Email notification | MC notification |
| --- | --- | --- | --- |
| SEVERE | "License `'%s'` will expire within one day." | Yes | Yes |
...
| SEVERE | "Dynatrace Managed has stopped collecting monitored data…" | No | Yes |
...
| SEVERE | "Cluster traffic control: OneAgent monitoring was disabled on recently connected hosts to avoid cluster overload." | No | Yes |
...
| WARNING | "There is lack of connection to Dynatrace Mission Control." | Yes | Yes |
...
| WARNING | "LDAP connection problems." | No | Yes |
...
| Severity | Summary | Email notification | MC notification |
| --- | --- | --- | --- |
| SEVERE | "Responsibility cluster nodes override set on node: `%d`." | No | Yes |

### `CONFIGURATION_AUDIT`

| Severity | Summary | Email notification | MC notification |
| --- | --- | --- | --- |
| INFO | "Min Agent Version updated to `%d` by Session Replay." | No | Yes |
...
| Severity | Summary | Email notification | MC notification |
| --- | --- | --- | --- |
| INFO | "Elasticsearch storage service on your Dynatrace Managed cluster might be overloaded!" | No | Yes |

### `ERROR`

| Severity | Summary | Email notification | MC notification |
| --- | --- | --- | --- |
| WARNING | "ElasticSearch update transient settings failed." | No | Yes |
...
| Severity | Summary | Email notification | MC notification |
| --- | --- | --- | --- |
| WARNING | "Ingested log data is trimmed." | No | Yes |
...
| Severity | Summary | Email notification | MC notification |
| --- | --- | --- | --- |
| INFO | "Internal: Cassandra has old files." | No | Yes |

### `MANAGED_NODE_ADD`

| Severity | Summary | Email notification | MC notification |
| --- | --- | --- | --- |
| WARNING | "Adding new node is taking more than expected." | No | Yes |
...
| Severity | Summary | Email notification | MC notification |
| --- | --- | --- | --- |
| WARNING | "Node removal finished with error `%s` (id=`%d`)." | No | Yes |
...
| Severity | Summary | Email notification | MC notification |
| --- | --- | --- | --- |
| INFO | "ActiveGate (host=`%s`) registered on cluster." | No | Yes |
...
Configurable means that you can configure the notifications via Settings API. Setting id: `builtin:cluster-events-notification-settings`. To use this REST API, you need to authorize with settings-specific token with `settings.read` and `settings.write` permissions.