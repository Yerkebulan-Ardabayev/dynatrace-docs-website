...
All cluster preferences settings are turned on by default. Proper configuration of these settings depends on the unique needs of your organization. In addition to proactive support settings, you'll also find settings related to new Community-user setup and domain name management.

To configure data privacy settings within your Managed environment, go to **Settings** > **Preferences**. You can also manage them using [Cluster API](/managed/dynatrace-api/cluster-api/cluster-api-v2 "Find out about managing environments, network zones, Synthetic locations, nodes, and tokens in Dynatrace Managed using the Cluster API v2.") and [Cluster API v1](/managed/dynatrace-api/cluster-api/cluster-api-v1 "Find out about API for managing environments, network zones, synthetic locations, nodes, and tokens.").

You must have cluster administrator privileges to access the **Preferences** page.

## UI workflows

Use the Cluster Management Console to configure cluster preferences for proactive support, remote access, privacy, Dynatrace Community access, and domain name management.

### Proactive support

#### Dynatrace Managed Mission Control Support Services

Dynatrace Managed provides fully automated self-management capabilities that keep your system secure, reliable, and up-to-date. To achieve this, Dynatrace needs to send certain information to Dynatrace Mission Control (MC). You can adjust these settings by modifying below UI options.

* **Report usage and billing information**

  Each Managed Cluster reports license-relevant consumption data such as number of host units, custom metrics or log monitoring for each environment. The **Report usage and billing information** UI label reflects this setting and can't be changed to ensure proper billing and license verification of your deployment.
* **Report Dynatrace Cluster health**

  Managed Clusters report status information, including cluster IDs, privacy flags, time zones, traffic levels, and maintenance windows. Server state, including number of CPU cores, CPU load, and used/free storage are reported on a per-cluster basis. The **Report Dynatrace Cluster health** UI label reflects this setting and can't be changed to ensure Dynatrace support of your deployment.
* **Report cluster and OneAgent events to Dynatrace Mission Control**

  For each event, Managed Clusters report type, severity level, timestamp, and description detail so that Dynatrace product experts can remotely analyze and address problems or incompatibilities in your environment. When turned off, your organization is responsible for monitoring system events and collecting log files necessary for problem resolution prior to contacting Dynatrace product experts. This setting is controlled by the **Report cluster and OneAgent events to Dynatrace Managed Mission Control Support Services** UI option.
...
* Report information about monitored technologies and feature usage.
  ...
  Dynatrace proactively sends alerts for incompatibilities or technology-specific risks related to your environment. The Dynatrace platform can report information about installed OneAgent versions, process technologies, hosts, ActiveGates, and other related entities and configurations. The retrieved information may be used for support and to improve Dynatrace offerings. Dynatrace may use this data (if aggregated and it can't be used to identify end users) for industry analyses, benchmarking, and analytics. Learn more about how Dynatrace [sends information about monitored technologies in your environment](/managed/managed-cluster/basics/mission-control-data-exchange "Review the data your Managed Cluster exchanges with Mission Control and the available opt-out options for each data category."). This setting is controlled by the **Send information about monitored technologies and feature usage** UI option.
* Use MC as email notifications sender.

  Enable your own SMTP server to determine how the Dynatrace platform delivers email notifications, reports, and other communications to users and administrators. To configure your own SMTP server, go to **Settings** > **Emails** > **SMTP server**. You can find more information on configuring your SMTP server in [Configure an SMTP server connection](/managed/managed-cluster/configuration/configure-smtp-server-connection "Learn how to configure an SMTP server connection on your Managed Cluster, including connection security, mail server settings, and delivery method options.").

### Dynatrace Community

The Dynatrace Community provides an Internet forum for customers and digital performance experts to connect with each other and share ideas. You can ask questions and view answers in the [Dynatrace Community﻿](https://community.dynatrace.com/) and create support tickets. Before allowing your users access to the Dynatrace Community, make sure your users have a valid email address set.

To create a Dynatrace Community user account upon sign-in:

1. Sign in as an admin to your Cluster Management Console.
2. Go to **Settings** > **Preferences**.
3. Enable **Send users invitation to create a Dynatrace account on first login**.

New and existing users of your Managed Cluster will receive an email with instructions on how to create a Dynatrace Community user account. New users will get the email after the first successful sign-in to the Managed Cluster whereas existing users will get the email a few moments after you've enabled it.

This setting is controlled by the **Send users invitation to create a Dynatrace account on first login**. With an account, users will have access to the Dynatrace Community, Dynatrace University, and support tickets. For a better self-service support experience, turn this on. If you turn this off, you can still manually send users an invitation to create an account from each user's details page.
...
Enable this setting to generate a domain name (a subdomain of `dynatrace-managed.com`) with a trusted certificate for your Managed Cluster. All users in your environment can then access Dynatrace at `<prefix>.dynatrace-managed.com`. The Certificate Authority (CA) is [Let's Encrypt﻿](https://letsencrypt.org/). Certificates are downloaded by HTTPS (REST API) via MC.

**Enable management of domain name and SSL certificates**

Note that this process may take a few minutes. Once complete, you'll be able to access the new URL. Turning off this option results in SSL certificates and the cluster URL being rolled back to the previous version. Remember to update your SSO IdP settings with this URL.

## API workflows

Use the [Cluster API](/managed/dynatrace-api/cluster-api/cluster-api-v2 "Find out about managing environments, network zones, Synthetic locations, nodes, and tokens in Dynatrace Managed using the Cluster API v2.") to configure synchronization of users and permissions and to manage the cluster privacy settings object. For most other cluster preferences, use [Cluster API v1](/managed/dynatrace-api/cluster-api/cluster-api-v1 "Find out about API for managing environments, network zones, synthetic locations, nodes, and tokens.").

[**Authenticate API requests**](/managed/managed-cluster/configuration/configure-cluster-preferences#token-authentication "Configure cluster preferences to manage proactive support reporting, remote access, data privacy, domain name, and community settings for your Managed Cluster.")[**Configure synchronization of users and permissions**](/managed/managed-cluster/configuration/configure-cluster-preferences#config-users-and-permissions-sync "Configure cluster preferences to manage proactive support reporting, remote access, data privacy, domain name, and community settings for your Managed Cluster.")[**Read configuration schema**](/managed/managed-cluster/configuration/configure-cluster-preferences#read-config-schema "Configure cluster preferences to manage proactive support reporting, remote access, data privacy, domain name, and community settings for your Managed Cluster.")[**Read current configuration**](/managed/managed-cluster/configuration/configure-cluster-preferences#read-current-config "Configure cluster preferences to manage proactive support reporting, remote access, data privacy, domain name, and community settings for your Managed Cluster.")[**Create cluster privacy settings object**](/managed/managed-cluster/configuration/configure-cluster-preferences#create-settings-object "Configure cluster preferences to manage proactive support reporting, remote access, data privacy, domain name, and community settings for your Managed Cluster.")[**Update cluster privacy settings**](/managed/managed-cluster/configuration/configure-cluster-preferences#update-settings-object "Configure cluster preferences to manage proactive support reporting, remote access, data privacy, domain name, and community settings for your Managed Cluster.")[**Delete cluster privacy settings**](/managed/managed-cluster/configuration/configure-cluster-preferences#delete-settings-object "Configure cluster preferences to manage proactive support reporting, remote access, data privacy, domain name, and community settings for your Managed Cluster.")

### Authenticate API requests

To generate a cluster token with the **Write settings** and **Read settings** scopes, follow these steps.
...
In this example, using a `POST` API call to the `/api/cluster/v2/settings/objects` endpoint and `builtin:cluster-privacy-settings` schema, you create a cluster privacy settings object in your Managed Cluster settings:
