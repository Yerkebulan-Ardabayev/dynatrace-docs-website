...
* Connect the Managed Cluster to [Dynatrace Mission Control](/managed/managed-cluster/basics/mission-control-proactive-support "Mission Control proactively monitors your Managed Cluster, provides software updates, and keeps your installation secure and reliable."). PHA is available only for online Managed Clusters.
* Deploy at least six nodes, with three nodes per DC. In a rack-aware PHA deployment, six nodes leave one node per rack, where rack awareness gains no failover tolerance.
* Deploy no more than 30 nodes, with no more than 15 nodes per DC.
* Size both DCs symmetrically.
* Plan the migration. You cannot reverse a migration to PHA.
...
Regional fault tolerance means that all Cluster nodes in one location can fail without downtime. To achieve it, distribute Cluster nodes across separate physical locations using one of the following options:

* Use PHA for two locations that need geographic redundancy and automatic failover.
* Use [rack awareness](/managed/managed-cluster/high-availability/rack-awareness "Learn how rack awareness groups Cluster nodes into three fault domains to ensure resilience against a full rack outage and data loss.") for three low-latency locations.
* Combine PHA with [rack awareness](/managed/managed-cluster/high-availability/rack-awareness "Learn how rack awareness groups Cluster nodes into three fault domains to ensure resilience against a full rack outage and data loss.") in each DC for six locations. For the procedure, go to [Combine Premium High Availability with rack awareness](/managed/managed-cluster/high-availability/pha-rack-aware "Combine Premium High Availability with rack awareness so each data center holds three racks and the cluster spans six independent fault domains.").
...
PHA stores three copies of all configuration data, metrics, and user sessions in each DC. Three-copy storage provides optimal performance and reliability in failover scenarios.
