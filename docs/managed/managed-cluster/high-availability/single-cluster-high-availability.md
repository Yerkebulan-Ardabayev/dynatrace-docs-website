...
[Log Monitoring](/managed/analyze-explore-automate/log-monitoring "Learn how to enable Log Monitoring, the insights that Log Monitoring can provide, and more.") event data replicates across the Elasticsearch store to two copies. This means if one node goes down, another node still holds the data. However, the failure of two nodes makes some log events unavailable. If the nodes come back up, the data becomes available again. Otherwise, Dynatrace Managed loses the data.
