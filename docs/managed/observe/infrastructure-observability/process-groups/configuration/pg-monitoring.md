...
5. Define the **Condition**:
   ...
   * The condition target (see [process group detection rules](/managed/observe/infrastructure-observability/process-groups/configuration/pg-detection "Ways to customize process-group detection")).
...
6. Set the **Monitoring state** (`Monitor`, `Do not monitor`, or `Default`).
   ...
   | **Do not monitor** | Process group settings at the host level override the corresponding settings at the host group and environment levels. |
