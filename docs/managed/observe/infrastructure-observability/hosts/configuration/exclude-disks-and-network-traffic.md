...
1. Go to the **Disk options** page for the correct level:
   ...
   * Host
     ...
     1. Go to **Hosts** and select the host for which you want to create a disk exclusion rule.
...
| Operating system | Type | Excluded disks |
| --- | --- | --- |
| All supported OS | File systems | `hsfs`  `devtmpfs`  `sysfs`  `rootfs`  `ramfs`  `proc`  `procfs`  `devpts`  `securityfs`  `cgroup`  `cpuset`  `pstore`  `mqueue`  `debugfs`  `autofs`  `hugetlbfs`  `fusectl`  `fuse.gvfsd-fuse`  `binfmt_misc`  `iso9660`  `none`  `rpc_pipefs` |
| Linux | File systems | `tmpfs`  `udf`  `squashfs` |
| Linux | Mount point | `/dev` |
| AIX | File systems | `cdrfs` |
| Solaris | Network interface | `mac` |