---
title: Create USQL custom metrics for mobile applications
...
# Create USQL custom metrics for mobile applications
...
4. Under **Value type to be extracted**, select one of the following options.
   ...
   * For user session metric events:
     ...
     + **User session counter** to count the number of user sessions, which is similar to `COUNT(*)` when using [USQL](/managed/observe/digital-experience/session-segmentation/custom-queries-segmentation-and-aggregation-of-session-data "Learn how you can access and query user session data based on keywords, syntax, functions, and more.").
   ...
   * For user action metric events:
     ...
     + **User action counter** to count the number of user actions, which is similar to `COUNT(*)` when using [USQL](/managed/observe/digital-experience/session-segmentation/custom-queries-segmentation-and-aggregation-of-session-data "Learn how you can access and query user session data based on keywords, syntax, functions, and more.").
...
Alternatively, you can use [USQL](/managed/observe/digital-experience/session-segmentation/custom-queries-segmentation-and-aggregation-of-session-data#convert-usql-into-custom-metrics "Learn how you can access and query user session data based on keywords, syntax, functions, and more.") to create USQL metric events.

## Configure metrics via API

You can also use the [Settings API](/managed/dynatrace-api/environment-api/settings "Find out what the Dynatrace Settings API offers.") to configure USQL metric events.
...
| `value.type` | * To count the number of user sessions or user actions, which is similar to `COUNT(*)` when using [USQL](/managed/observe/digital-experience/session-segmentation/custom-queries-segmentation-and-aggregation-of-session-data "Learn how you can access and query user session data based on keywords, syntax, functions, and more."), set to `COUNTER`. * To extract a value of a user session or user action field, set to `FIELD`. | `COUNTER` or `FIELD` |
...
| `dimensions` | Lists the fields used as dimensions. | See [Dimensions for user session metric events](#dimensions-uscm) and [Dimensions for user action metric events](#dimensions-uacm). |
| `filters` | Specifies filters. |  |
| `filter.fieldName` | Specifies the name of the filter field. | See [Filters for user session metric events](#filters-uscm) and [Filters for user action metric events](#filters-uacm). |
...
| `filter.value` | Provides the second argument for binary operators (such as `EQUALS` or `GREATER_THAN`). |  |
...
3. Create a metric event based on `uscm.average_duration_of_sessions_by_browser_family_and_version` metric. For details, see [Metric events](/managed/dynatrace-intelligence/anomaly-detection/metric-events "Learn about metric events in Dynatrace").
...
Starting with Dynatrace 1.232, USQL metric events are subject to the [Davis data unit (DDU) licensing](/managed/license/monitoring-consumption-classic/davis-data-units "Understand how Dynatrace monitoring consumption is calculated based on Davis data units (DDU)."). These metrics are billed as regular schemaless metrics.

To estimate the cost per metric, the sessions for the last 7 days are evaluated and the cost in DDUs per metric is calculated according to the expected ingestion per minute. For details, see [How do we calculate DDU consumption for metric events?](/managed/license/monitoring-consumption-classic/davis-data-units/metric-cost-calculation#calculation-details "Understand how to calculate Davis data unit consumption and costs related to monitored metrics.").
...
For details, see [Data retention periods > Metrics](/managed/manage/data-privacy-and-security/data-privacy/data-retention-periods#metrics-classic "Check retention times for various data types.")
