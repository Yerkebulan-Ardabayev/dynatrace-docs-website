...
| --- | --- |
| Green circle | The capacity usage of each type of synthetic monitor is below 80%, which is desirable. |
| Yellow circle | This icon can mean any of the following:  * The capacity usage of a least one type of synthetic monitor is more than 80%. * There is no failover because: + Failure of any ActiveGate or [Synthetic engine](/managed/manage/credential-vault#security-arch "Store and manage credentials in the credential vault.") can cause usage requirements to exceed maximum capacity.   + There is only one ActiveGate assigned to the location. |
| Red circle | * Capacity usage of at least one type of synthetic monitor at this location is greater than 90%. Or, not all synthetic monitors assigned to this location are being executed. Note that some monitor executions may be dropped when there are spikes in capacity usage for a monitor type, for example, when multiple monitors are updated at the same time. * This icon is also displayed when all ActiveGates or all Synthetic engines at the location are down.  If a location is down or a Synthetic engine/ActiveGate is offline, synthetic monitors assigned to that location won't be executed. |
| Grey circle | There is no data, as when ActiveGate versions are lower than 1.217 and capacity usage data cannot be gathered. |
...
Synthetic monitors metrics in Data Explorer Classic
...
Also, check our information on [installing the browser and other dependencies manually (Linux only)](/managed/observe/digital-experience/synthetic-monitoring/private-synthetic-locations/create-a-private-synthetic-location#manual "Learn how to create a private location for synthetic monitoring.").
...
4. Enable/disable the switch below the checkbox. This will overwrite the corresponding setting for the selected locations.
   ...
   Bulk edit location outage handling
5. **Save changes**.
...
3. Remove the old Chrome for Testing directory, extract the new version, and clean up:
   ...
   If you configured a custom Chrome for Testing directory via the `CFT_DIR` environment variable or the `synthetic_chrome_for_testing_path` property in `custom.properties`, replace `/usr/lib/chrome_for_testing` with your custom path in the commands above.
