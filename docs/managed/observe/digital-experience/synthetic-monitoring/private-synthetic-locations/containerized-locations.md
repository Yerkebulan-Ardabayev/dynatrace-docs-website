...
* If the location is monitored by [OneAgent](/managed/ingest-from/dynatrace-oneagent/oa-requirements "OneAgent code module requirements") or another deep monitoring solution, memory (RAM) requirements will increase.
...
7. Optional Turn on [problem generation if all ActiveGates go offline](/managed/observe/digital-experience/synthetic-monitoring/private-synthetic-locations/manage-private-synthetic-locations#outage-handling "Analyze and manage capacity usage at your private Synthetic locations.").
...
3. Provide a **Location namespace** name or use the default. (Leave the **Metric adapter namespace** as is. This field is only necessary for generating the [template for the Synthetic metric adapter](#deploy-adapter).)
   ...
   Required fields for the location template
...
7. Optional Run the following command to list all pods in a given namespace (`dynatrace` in the sample below) and verify their deployment.
   ...
   ActiveGate deployment status filters
...
2. Provide a **Metric adapter namespace** name or use the default. (Leave the **Location namespace** and **ActiveGate name** as is. These fields are only necessary for generating the [template for the location](#deploy-location).)
   ...
   Required fields for the adapter template
...
1. Set location to FIPS mode
   ...
   Currently this is only possible using the [REST API](/managed/dynatrace-api/environment-api/synthetic-v2/synthetic-locations-v2 "Manage synthetic locations via the Synthetic v2 API."), by setting the `fipsMode` property in the request JSON.

   * To create a new location, use a [POST location](/managed/dynatrace-api/environment-api/synthetic-v2/synthetic-locations-v2/post-a-location "Create a private synthetic location via the Synthetic v2 API.") call
   * To update an existing location, use a [PUT location](/managed/dynatrace-api/environment-api/synthetic-v2/synthetic-locations-v2/put-a-location "Update a private synthetic location via the Synthetic v2 API.") call
...
* [Synthetic locations API v2](/managed/dynatrace-api/environment-api/synthetic-v2/synthetic-locations-v2 "Manage synthetic locations via the Synthetic v2 API.")
