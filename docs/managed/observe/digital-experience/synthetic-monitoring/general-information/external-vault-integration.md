...
Before setting up credentials synchronized with Azure Key Vault, you need to define the required **client (application) ID** and **client secret** as [token credentials](/managed/manage/credential-vault#token "Store and manage credentials in the credential vault.") stored in the Dynatrace [credential vault](/managed/manage/credential-vault "Store and manage credentials in the credential vault."). We recommend naming such prerequisite tokens so that they're easily identifiable as companion credentials for synchronization. If your vault doesn't contain any tokens that you have access to, you'll see a warning.
...
1. The first request (POST) fetches an access token.
   ...
   * A client token is returned in the response body. A [post-execution script](/managed/observe/digital-experience/synthetic-monitoring/http-monitors-classic/pre-and-post-scripting-for-http-monitors-classic "Learn how to apply pre and post scripts to your requests") saves the token in a [global variable](/managed/observe/digital-experience/synthetic-monitoring/http-monitors-classic/pre-and-post-scripting-for-http-monitors-classic#variables "Learn how to apply pre and post scripts to your requests").
...
2. The second request (GET) fetches the username value.
   ...
   * The [Authorization header](/managed/observe/digital-experience/synthetic-monitoring/http-monitors-classic/configure-http-monitors-classic#headers "Learn about configuring HTTP monitors.") contains the access token retrieved in the first request.
   ...
   * The username value is returned in the response body. A [post-execution script](/managed/observe/digital-experience/synthetic-monitoring/http-monitors-classic/pre-and-post-scripting-for-http-monitors-classic "Learn how to apply pre and post scripts to your requests") saves the value in a [global variable](/managed/observe/digital-experience/synthetic-monitoring/http-monitors-classic/pre-and-post-scripting-for-http-monitors-classic#variables "Learn how to apply pre and post scripts to your requests").
...
* Before using [certificate authentication](#certificate), you need to store the required **TLS [certificate](/managed/manage/credential-vault#certificate "Store and manage credentials in the credential vault.")** in the Dynatrace [credential vault](/managed/manage/credential-vault "Store and manage credentials in the credential vault."). If your vault doesn't contain any certificates you have access to, you'll see a warning.
...
When you have set up your synchronized credential, Dynatrace automatically creates and executes an [HTTP monitor](/managed/observe/digital-experience/synthetic-monitoring/http-monitors-classic/create-an-http-monitor-classic "Learn how to set up an HTTP monitor to check the performance and availability of your site.") that synchronizes the credential with HashiCorp Vault.
...
1. The first request (POST) fetches a client token.
   ...
   * The vault namespace, referenced as an attribute of the synchronized credential, is passed as a [request header](/managed/observe/digital-experience/synthetic-monitoring/http-monitors-classic/configure-http-monitors-classic#headers "Learn about configuring HTTP monitors."); the vault namespace is not displayed.
...
2. The second request (GET) fetches the username and password values. It also uses `api.saveCredential()` in s post-execution script to write the fetched values to the [synchronized username-password credential](#hashicorp-set-up) defined above.
   ...
   * The username and password values are returned in the JSON response. A [post-execution script](/managed/observe/digital-experience/synthetic-monitoring/http-monitors-classic/pre-and-post-scripting-for-http-monitors-classic "Learn how to apply pre and post scripts to your requests") saves the values in [global variables](/managed/observe/digital-experience/synthetic-monitoring/http-monitors-classic/pre-and-post-scripting-for-http-monitors-classic#variables "Learn how to apply pre and post scripts to your requests"). It also uses `api.saveCredential()` to write the retrieved values to the synchronized username-password credential.
...
2. The second request (GET) fetches the token value. It also uses `api.saveToken()` in a post-execution script to write the fetched values to the [synchronized token credential](#hashicorp-set-up) defined above.
   ...
   * A [request header](/managed/observe/digital-experience/synthetic-monitoring/http-monitors-classic/configure-http-monitors-classic#headers "Learn about configuring HTTP monitors.") contains the client token retrieved in the first request. The vault namespace (not displayed, but referenced as an attribute of the synchronized credential) is also passed as a request header.
     ...
     HashiCorp AppRole request 2 headers
   * The token value is returned in the JSON response. A [post-execution script](/managed/observe/digital-experience/synthetic-monitoring/http-monitors-classic/pre-and-post-scripting-for-http-monitors-classic "Learn how to apply pre and post scripts to your requests") saves the value in a [global variable](/managed/observe/digital-experience/synthetic-monitoring/http-monitors-classic/pre-and-post-scripting-for-http-monitors-classic#variables "Learn how to apply pre and post scripts to your requests"). It also uses `api.saveToken()` to write the retrieved value to the synchronized token credential.
...
2. The second request (GET) fetches the username and password values. It also uses `api.saveCredential()` in a post-execution script to write the fetched values to the [synchronized username-password credential](#hashicorp-set-up) defined above.
   ...
   * The request URL references the vault URL and the path to credentials as attributes of the synchronized credential; the vault URL and path to credentials are not displayed.
     ...
     HashiCorp AppRole request 2 URL
   * A [request header](/managed/observe/digital-experience/synthetic-monitoring/http-monitors-classic/configure-http-monitors-classic#headers "Learn about configuring HTTP monitors.") contains the client token retrieved in the first request.
...
1. The first request (POST) fetches an access token.
   ...
   * A token is returned in the response body. A [post-execution script](/managed/observe/digital-experience/synthetic-monitoring/http-monitors-classic/pre-and-post-scripting-for-http-monitors-classic "Learn how to apply pre and post scripts to your requests") saves the token in a [global variable](/managed/observe/digital-experience/synthetic-monitoring/http-monitors-classic/pre-and-post-scripting-for-http-monitors-classic#variables "Learn how to apply pre and post scripts to your requests").
...
2. The second request (GET) fetches the username and password values from CyberArk Vault. It also uses `api.saveCredential()` in a post-execution script to write the fetched values to the [synchronized username-password credential](#cyberark-set-up) defined above.
   ...
   * The second request also contains any authentication certificate and the access token retrieved in the first request in the [Authorization header](/managed/observe/digital-experience/synthetic-monitoring/http-monitors-classic/configure-http-monitors-classic#headers "Learn about configuring HTTP monitors.").
   ...
   * The username and password values are returned in the response body. A [post-execution script](/managed/observe/digital-experience/synthetic-monitoring/http-monitors-classic/pre-and-post-scripting-for-http-monitors-classic "Learn how to apply pre and post scripts to your requests") saves the values in [global variables](/managed/observe/digital-experience/synthetic-monitoring/http-monitors-classic/pre-and-post-scripting-for-http-monitors-classic#variables "Learn how to apply pre and post scripts to your requests"). It also uses `api.saveCredential()` to write the retrieved values to the synchronized username-password credential.
