---
title: Instrument mobile apps with Dynatrace .NET MAUI NuGet package in RUM Classic
...
# Instrument mobile apps with Dynatrace .NET MAUI NuGet package in RUM Classic
...
The Dynatrace .NET MAUI NuGet package helps auto-instrument your .NET MAUI mobile application with OneAgent for Android and iOS as well as provides an API for [manual instrumentation](#usage-mobile-agent).
...
**Install the Dynatrace .NET MAUI NuGet package**](/managed/observe/digital-experience/rum-classic/mobile-applications/cross-platform-frameworks/maui#install-package "Monitor .NET MAUI applications with Dynatrace OneAgent.")[![Step 2](https://dt-cdn.net/images/step-2-1a1384627e.svg "Step 2")

**Create an application and get the config file**](/managed/observe/digital-experience/rum-classic/mobile-applications/cross-platform-frameworks/maui#installation-dynatrace "Monitor .NET MAUI applications with Dynatrace OneAgent.")[![Step 3](https://dt-cdn.net/images/step-3-350cf6c19a.svg "Step 3")

**Add the config file to your project**](/managed/observe/digital-experience/rum-classic/mobile-applications/cross-platform-frameworks/maui#configure-app "Monitor .NET MAUI applications with Dynatrace OneAgent.")[![Step 4](https://dt-cdn.net/images/step-4-3f89d67d41.svg "Step 4")

**Add the OneAgent start method**](/managed/observe/digital-experience/rum-classic/mobile-applications/cross-platform-frameworks/maui#start-method "Monitor .NET MAUI applications with Dynatrace OneAgent.")[![Step 5 optional](https://dt-cdn.net/images/dotted-step-5-52040ae237.svg "Step 5 optional")

**Enable automatic web request instrumentation**](/managed/observe/digital-experience/rum-classic/mobile-applications/cross-platform-frameworks/maui#http-client "Monitor .NET MAUI applications with Dynatrace OneAgent.")
...
2. Find [**Dynatrace.OneAgent.MAUI** from nuget.org﻿](https://www.nuget.org/packages/Dynatrace.OneAgent.MAUI) and select **Add Package**.
...
Add the [`dynatrace.config.json` file](#config-file), which you downloaded in the previous step, to your project. You can do it in any of the following ways, but we have listed them in order of most to least recommended.

* Add the `dynatrace.config.json` file to the custom location. Specify the custom location via the [`DynatraceConfigurationFile` property](#config-file).
* Add the `dynatrace.config.json` file to:

  + `Platforms/Android/Assets` directory of your project (Android). If the `Assets` directory doesn't exist, create it inside the `Platforms/Android` directory.
  + `Platforms/iOS/Resources` directory of your project (iOS). If the `Resources` directory doesn't exist, create it inside the `Platforms/iOS` directory.
* Add the `dynatrace.config.json` file to the root directory of your project. MSBuild automatically sets the root directory via the `ProjectDir` property.

When you migrate your Xamarin project to a .NET project and switch to the Dynatrace .NET MAUI NuGet package, add the `dynatrace.config.json` file to the custom location or to the root directory of your project.
...
### Step 5 optional Enable automatic web request instrumentation Optional
...
Check the following links for information on user action naming: [Android](/managed/observe/digital-experience/rum-classic/mobile-applications/instrument-android-app/instrumentation-via-plugin/monitoring-capabilities#user-action-naming "Configure the Dynatrace Android Gradle plugin to adjust the monitoring capabilities of OneAgent.") and [iOS](/managed/observe/digital-experience/rum-classic/mobile-applications/instrument-ios-app/customization/oneagent-sdk-for-ios#user-action-naming "Enrich mobile user experience monitoring using OneAgent SDK for iOS.").
...
Besides generating standalone custom actions, you can also create [child actions](/managed/observe/digital-experience/rum-classic/rum-concepts/user-actions#child-actions "Learn what user actions are and how they help you understand what users do with your application.").
...
To report an [error](/managed/observe/digital-experience/rum-classic/rum-concepts/user-and-error-events#error "Learn about user and error events and the types of user and error events captured by Dynatrace."), use the `ReportError` method.
...
To report a [crash](/managed/observe/digital-experience/rum-classic/rum-concepts/user-and-error-events#crash "Learn about user and error events and the types of user and error events captured by Dynatrace."), use the following API call.
...
To activate the user opt-in mode, set the `userOptIn` property (Android) or [`DTXUserOptIn` configuration key](/managed/observe/digital-experience/rum-classic/mobile-applications/instrument-ios-app/customization/ios-configuration-keys#privacy-and-security "With configuration keys, you can fine-tune the auto-instrumentation of your iOS apps.") (iOS) to `true` in the [`dynatrace.config.json` file](#config-file).
...
| --- | --- | --- | --- | --- | --- |
| Android | Default new app | net8.0-android | 31.8 MB | 32.4 MB | 0.6 MB |
| iOS | Default new app | net8.0-ios | 421 MB | 426.2 MB | 5.2 MB |
...
2. Change the build output verbosity to `Diagnostic`. For details, see the Microsoft documentation on how to [change the amount of information included in the build log﻿](https://docs.microsoft.com/en-us/visualstudio/ide/how-to-view-save-and-configure-build-log-files?view=vs-2019#to-change-the-amount-of-information-included-in-the-build-log).
3. Rebuild your project.
4. Attach the build logs to the support ticket so that we can further analyze your issue.

## Troubleshooting

If you can't resolve a problem, check [Mobile applications: Issues with Dynatrace .NET MAUI NuGet package﻿](https://dt-url.net/sy038sw) in the Dynatrace Community.

## Related topics

* [Instrument Android apps in RUM Classic](/managed/observe/digital-experience/rum-classic/mobile-applications/instrument-android-app "Learn how to instrument mobile application monitoring on Android, how to customize instrumentation and more.")
* [Instrument iOS apps in RUM Classic](/managed/observe/digital-experience/rum-classic/mobile-applications/instrument-ios-app "Instrument mobile application monitoring for iOS apps, customize the auto-instrumentation, and capture additional data via manual instrumentation.")