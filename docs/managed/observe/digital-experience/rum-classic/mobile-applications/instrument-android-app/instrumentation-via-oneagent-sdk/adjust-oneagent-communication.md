---
title: Adjust communication with OneAgent SDK for Android in RUM Classic
...
# Adjust communication with OneAgent SDK for Android in RUM Classic
...
If your Android app has [network security configured﻿](https://developer.android.com/training/articles/security-config), ensure that the HTTP traffic to the `beaconUrl` endpoint is not blocked by the network security configuration.
...
You can deactivate the certificate validation via the [`certificateValidation` property﻿](https://docs.dynatrace.com/javadoc/oneagent/android/gradle-plugin/dsl/com.dynatrace.tools.android.dsl.DebugOptions.html#com.dynatrace.tools.android.dsl.DebugOptions:certificateValidation).
...
You can also deactivate the certificate validation with the [`ConfigurationBuilder.withCertificateValidation(boolean)`﻿](https://docs.dynatrace.com/javadoc/oneagent/android/agent/com/dynatrace/android/agent/conf/ConfigurationBuilder.html#withCertificateValidation(boolean)) method.
...
To use certificate pinning, follow the instructions provided by Android at [Network security configuration > Pin certificates﻿](https://developer.android.com/training/articles/security-config#CertificatePinning).
