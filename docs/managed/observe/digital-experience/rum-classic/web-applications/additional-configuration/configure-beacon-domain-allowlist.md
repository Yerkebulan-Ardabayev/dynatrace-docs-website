---
title: Configure beacon origin allowlist for web applications in RUM Classic
...
# Configure beacon origin allowlist for web applications in RUM Classic
...
* **Auto-injected applications > same-origin beacons**
  ...
  When the [RUM JavaScript is injected automatically](/managed/observe/digital-experience/rum-classic/web-applications/initial-setup/rum-injection "Configure automatic injection of the RUM JavaScript into the pages of your applications"), RUM beacons are sent back to the web or application server that hosts the auto-injected application; OneAgent provides a beacon endpoint.
  ...
  If you opted for one of the [alternative beacon endpoint setups](/managed/observe/digital-experience/rum-classic/web-applications/additional-configuration/beacon-endpoint "Change the default beacon endpoint URL and send RUM beacons to Dynatrace infrastructure or another instrumented web server.")—where the beacons of an auto-injected application are sent to a Cluster ActiveGate or an instrumented server on a different domain—RUM beacons are **cross-origin beacons**.
* **Agentless applications > cross-origin beacons**

  When [agentless monitoring](/managed/observe/digital-experience/rum-classic/web-applications/initial-setup/set-up-agentless-real-user-monitoring "Set up agentless monitoring for your web applications.") is used, RUM beacons are sent to a beacon endpoint that is part of a Cluster ActiveGate.
