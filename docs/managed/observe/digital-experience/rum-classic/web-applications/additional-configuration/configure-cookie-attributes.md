...
The `SameSite` cookie attribute controls whether the browser sends the cookie with cross-site requests. For a detailed explanation of this attribute and its values, see [SameSite cookies explained﻿](https://web.dev/samesite-cookies-explained/).
...
Browsers do not allow you to set cookies with a `Domain` attribute that exceeds a single organization's boundary. Therefore, setting the cookie domain to a [public suffix﻿](https://publicsuffix.org/list/) does not work.
