---
title: Create calculated metrics for custom applications in RUM Classic
...
# Create calculated metrics for custom applications in RUM Classic
...
In this example, let's analyze `Price`, which is a [user action property](/managed/observe/digital-experience/rum-classic/web-applications/additional-configuration/define-user-action-and-session-properties#custom-properties "Define custom string, numeric, and date properties for your monitored web applications."), and filter it by `Loyalty status`, which is another user action property.
