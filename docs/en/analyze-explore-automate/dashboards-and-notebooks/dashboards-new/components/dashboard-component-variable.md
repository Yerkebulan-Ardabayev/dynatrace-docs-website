...
2. Set **Type** to `DQL`.
3. In **Data**, enter a query. Be sure to use `summarize` and `collectDistinct` to get distinct values from data sources such as logs.
...
As a workaround, you can use the duration function together with a DQL [conversion](../../../../platform/grail/dynatrace-query-language/functions.md#conversion-and-casting-functions "A list of DQL functions.") function. This provides the required output based on your `$resolution` value.
