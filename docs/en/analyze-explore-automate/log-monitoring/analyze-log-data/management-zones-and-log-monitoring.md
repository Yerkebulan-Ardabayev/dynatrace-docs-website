...
* Use log ingestion via API, which requires certain attributes to detect the topological context.

If you use the Log ingestion API via the Log Monitoring API v2, you can send an object representing a single event or an array of objects representing multiple events. The object must contain at least one of the following attribute keys, which define the entity types for which log data is ingested.
