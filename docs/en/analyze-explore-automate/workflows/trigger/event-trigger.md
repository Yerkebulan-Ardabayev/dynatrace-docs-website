...
Davis problems are created by Dynatrace Intelligence based on monitoring data. The **Davis problem trigger** allows selectively triggering a workflow in response. The trigger will happen once, when a Davis problem becomes active (on open and re-open), and optionally once when a problem is closed.
