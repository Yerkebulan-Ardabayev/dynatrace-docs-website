...
* Dynatrace SaaS with a Dynatrace Platform Subscription (DPS), the licensing model for all Dynatrace capabilities.") license that has Traces powered by Grail model."), Metrics powered by Grail, and Log Analytics enabled.
* OTLP ingestion enabled, see OpenTelemetry and Dynatrace into Dynatrace.").
