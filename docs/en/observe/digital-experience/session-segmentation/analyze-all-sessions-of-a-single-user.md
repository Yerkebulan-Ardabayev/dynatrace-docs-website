...
* When you turn off **Extended users**, information displayed on the user detail page consists exclusively of the data from the specific user tag and the devices used by that user tag. If there's no user tag, then the device identifier is used and only for that particular device. For details on how to configure user tagging, see [User tagging](../rum-concepts/user-and-error-events.md#user-tagging "Learn about user and error events and the types of user and error events captured by Dynatrace.").
