...
After that, use the [`DTXHybridApplication`, `DTXSetCookiesForDomain`, and `DTXSetSecureCookiesForDomain` configuration keys](instrument-ios-app/customization/ios-configuration-keys.md#hybrid "With configuration keys, you can fine-tune the auto-instrumentation of your iOS apps.") to allow OneAgent to pass cookies to `WKWebView` and domains specified in your app's `Info.plist` file.
