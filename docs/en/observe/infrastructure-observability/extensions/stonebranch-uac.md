...
There is no charge to use the extension. You are only charged for the data that the extension ingests. The license consumption details depend on which licensing model you're using. This can either be Dynatrace classic licensing or the Dynatrace Platform Subscription (DPS), the licensing model for all Dynatrace capabilities.") model.
