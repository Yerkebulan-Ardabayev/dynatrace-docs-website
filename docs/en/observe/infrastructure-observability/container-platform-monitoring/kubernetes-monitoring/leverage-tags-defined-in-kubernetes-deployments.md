...
While you can also use tags (manual, automated, and via API) to apply ownership information to Kubernetes objects, this approach has its limitationsâread more in Best practices for entity ownership.
...
Additionally Dynatrace detects the following properties that can be used for automated rule-based tags and [property-based process group detection rules](../../process-groups/configuration/pg-detection.md#detection-rules "Ways to customize process-group detection").
