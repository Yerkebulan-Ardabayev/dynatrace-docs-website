...
To use the [`OTEL_RESOURCE_ATTRIBUTES`](../../deliver/release-monitoring/version-detection-strategies.md#otel_resource_attributes "Metadata for version detection in different technologies") environment variable, just directly set the `dt.security_context` as a resource attribute. You can also use any resource attribute in OpenPipeline as a source for your `dt.security_context`.

### Security context via OneAgent metadata file

Dynatrace OneAgent provides enrichment files or environment variables to [add attributes directly in your application code](../extend-dynatrace/extend-data.md#operator-enrichment-directory "Learn how to automatically enrich your telemetry data with Dynatrace-specific fields.").

To read more about enrichment options and setup, see how to [enrich via environment variable](../setup-on-k8s/guides/metadata-automation/k8s-metadata-telemetry-enrichment.md#expand--enrich-via-environment-variable--4 "Guides for telemetry enrichment on Kubernetes").