...
Regardless of which option you decide to use, you need to tailor the DSN by replacing `<hlq>` with the high-level qualifier you set during Download z/OS product datasets.
...
This step is optional for test installations because the DTAX transaction can be used instead to enable the module after CICS initialization. We recommend that you place the `ZDTPLT` entry immediately before the `TYPE=FINAL` specification.
...
You can control the CICS module log level either by using the DTAX transaction or by specifying an optional `INITPARM` at CICS region startup.
...
2. Use the DTAX transaction to disable the CICS module in the CICS region with the `DISABLE` command.
