...
A regular platform user is only granted read-only access for EdgeConnect configurations through `app-engine:edge-connects:read` permission bound to the Standard and Pro user default policies.
...
The Admin default policy already contains the necessary scopes, so an admin user can fully manage EdgeConnect configurations by default.
...
To adjust the policies and group memberships of users, go to [**Account Management**ï»¿](https://myaccount.dynatrace.com/) > **Identity & access management** and select **People**, **Groups**, or **Policies**. For more information, see Manage IAM policies.
...
If you run [EdgeConnect in Kubernetes via Dynatrace Operator](#kubernetes), the above fields are configured using the parameters of a custom Kubernetes resource as explained in EdgeConnect parameters for Dynatrace Operator.
...
For verification details, see Verify Software Bill of Materials (SBOM) Attestation.
...
For details on this example, see Set up manually EdgeConnect for Kubernetes Connector.
