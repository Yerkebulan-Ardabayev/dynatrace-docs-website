...
Collect Micrometer metrics from JVM applications](extend-metrics/ingestion-methods/micrometer.md "Learn how to send Micrometer metrics to Dynatrace.")[![Prometheus](https://dt-cdn.net/images/prometheus-b1fab729ac.svg "Prometheus")

### Prometheus

Send Prometheus metrics to Dynatrace](extend-metrics/ingestion-methods/prometheus.md "Learn how to extend observability in Dynatrace with Prometheus metrics.")[![StatsD](https://dt-cdn.net/images/statsd-icon-bigger-800-72b34b3823.png "StatsD")

### StatsD

Send StatsD metrics to Dynatrace](extend-metrics/ingestion-methods/statsd.md "Ingest metrics into Dynatrace using OneAgent and the ActiveGate StatsD client.")[![Telegraf](https://dt-cdn.net/images/techn-icon-telegraf-ba9e70e8d6.svg "Telegraf")

### Telegraf

Send Telegraf metrics to Dynatrace](extend-metrics/ingestion-methods/telegraf.md "Ingest Telegraf metrics into Dynatrace.")[### Oracle Database
...
Extend observability of your Java applications with JMX metrics.](extend-metrics/ingestion-methods/jmx-extensions.md "Learn how to extend Dynatrace monitoring to include applications you've instrumented with JMX.")[### Scripting integration

Extend metric observability via Dynatrace' scripting integration.](extend-metrics/ingestion-methods/oneagent-pipe.md "Learn how to ingest metrics using local scripting integration.")[### Metric ingestion API

Extend metric observability via Dynatrace's open Metric APIs.](extend-metrics/ingestion-methods/oneagent-metric-api.md "Use the Dynatrace API to retrieve the metrics of monitored entities.")
...
Use the [GET metric data points](../../dynatrace-api/environment-api/metric-v2/get-data-points.md "Read data points of one or multiple metrics via Metrics v2 API.") call of the Metrics API v2 to retrieve ingested data points.

### Data Explorer

Select **Create custom chart** and then select **Try it out** in the top banner. For more information, see [Data Explorer](../../analyze-explore-automate/explorer.md "Query for metrics and transform results to gain desired insights.").
...
* **Tuples**: Unique combinations of metric-dimension pairs (see examples below).
  ...
  + Metrics powered by Grail
    Each tuple counts toward your environment's [cardinality limit](../../analyze-explore-automate/metrics/limits.md "Reference of metrics powered by Grail").
...
Each dimensional value (in this example, each network card) generates an individual time series within the chart. Therefore, for purposes of [calculating custom-metric consumption](../../license/monitoring-consumption-classic/davis-data-units/metric-cost-calculation.md "Understand how to calculate Davis data unit consumption and costs related to monitored metrics."), each dimensional value is counted as a separate custom metric.
