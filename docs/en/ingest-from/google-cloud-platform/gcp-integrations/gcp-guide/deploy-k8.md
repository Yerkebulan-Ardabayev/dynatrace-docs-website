...
To see what metrics are included for specific service, check Google Cloud supported services. By default, only `defaultMetrics` feature set is enabled. To enable additional feature sets, you have to uncomment them in `values.yaml` file and redeploy whole GCP Monitor.
