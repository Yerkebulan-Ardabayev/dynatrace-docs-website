...
Read more about defining tags and metadata for hosts and the OneAgent `oneagentctl` command-line interface.
...
This example for Unix-like systems uses `--set-host-property` to set ownership with the [key-value pair](#format) `owner-1=team-automation`, where `team-automation` is the main team identifier or a supplementary identifier.
...
This example for Unix-like systems sets host ownership with the [key-value pair](#format) `dt.owner-1=team-automation`, where `team-automation` is the main team identifier or a supplementary identifier.
...
Read more about defining process group metadata, for example, setting up the `DT_CUSTOM_PROP` environment variable for IIS and Windows.
...
You can use tags to apply ownership in [key-value pairs](#format) to any monitored entityâread more about tagging.

![Ownership via a manual tag](https://dt-cdn.net/images/ownership-manual-tag-1004-f7c90a9b42.png)

* Manual tags using the defined keys or key prefixes (like `owner` and `dt.owner`) are simple to apply via the web UI.
* You can also set up automatic tagging rules via the web UI.
...
* The `entities.write` token permission
* The `settings.read` token permission or the IAM policy `ALLOW settings:objects:read WHERE settings:schemaId = "builtin:ownership.teams";`
...
Ownership information is only available for unified analysis pages.

For hosts and all Kubernetes entities, select **Owners** on the entity details page to view ownership information.

This example shows a Kubernetes workload mapping to the `CLOUD_APPLICATION` entity. Expand a team name to view its details. Select ![Edit](https://dt-cdn.net/images/edit-icon-a083c62c49.svg "Edit") in the **Ownership** card to edit team details in **Settings**.
