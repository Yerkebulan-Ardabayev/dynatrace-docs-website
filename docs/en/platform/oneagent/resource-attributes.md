...
* `dt.security_context` - attribute to manage data access using policies. See Grant access to entities with security context.
* `dt.cost.costcenter` - attribute to assign usage to a Cost Center. See Allocate your DPS costs.
* `dt.cost.product` - attribute to assign usage to a Product or Application ID. See Allocate your DPS costs.
