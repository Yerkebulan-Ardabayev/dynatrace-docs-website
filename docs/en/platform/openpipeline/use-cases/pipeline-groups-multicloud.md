...
* Settings API - Pipeline Groups configuration (events) schema table
* Settings API - Pipeline Groups configuration (bizevents) schema table
* Settings API - Pipeline Groups configuration (spans) schema table
...
* Settings API - Pipeline Groups configuration (metrics) schema table
* Settings API - Pipeline Groups configuration (security.events) schema table
* Settings API - Pipeline Groups configuration (events.security) schema table
* Settings API - Pipeline Groups configuration (davis.events) schema table
* Settings API - Pipeline Groups configuration (events.sdlc) schema table
* Settings API - Pipeline Groups configuration (system.events) schema table
* Settings API - Pipeline Groups configuration (usersessions) schema table
* Settings API - Pipeline Groups configuration (user.events) schema table

### Prior knowledge

* You know how to create pipelines via the Settings API.
* You know how to set access control in OpenPipeline.
* You are familiar with pipeline groups and [pipeline group limits](../reference/limits.md#pipeline-groups "Reference limits of Dynatrace OpenPipeline.").
...
* Two cloud-specific cost and permission assignment pipelines called **AWS cost and permission assignment** and **Azure cost and permission assignment** to set `dt.cost.product` and security context for the respective cloud provider. They're owned and accessed by the mid-level team only.
  ...
  ```
* Use the `POST /api/v2/settings/objects` request with the pipeline schema of the configuration scope (`builtin:openpipeline.<configuration.scope>.pipelines`).

### 2. Create member pipelines

1. To create a member pipeline, create a custom pipeline with the member role (`"groupRole": "memberPipeline"` and `"routing": "routable"`). Use the OpenPipeline settings Web UI or the `POST /api/v2/settings/objects` request with the pipeline schema of the configuration scope (`builtin:openpipeline.<configuration.scope>.pipelines`).
   ...
   ```
2. Create the associated route. Use the OpenPipeline settings Web UI or the `POST /api/v2/settings/objects` request with the route schema of the configuration scope (`builtin:openpipeline.<configuration.scope>.routing`).
...
1. Define two pipeline groups, one for AWS (`AWS pipelines`) and one for Azure (`Azure pipelines`).
   ...
   ```
2. Use the `POST /api/v2/settings/objects` request with the pipeline group schema of the configuration scope (`builtin:openpipeline.<configuration.scope>.pipeline-groups`).
...
You can create new member pipelines and then add them to the group any time. Use the `PUT /api/v2/settings/objects/{objectId}` request with the pipeline group schema of the configuration scope (`builtin:openpipeline.<configuration-scope>.pipeline-groups`).
