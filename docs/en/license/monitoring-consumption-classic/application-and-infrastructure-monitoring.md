...
* AWS Fargate
* Azure App Service including Azure Functions on App-Service (Dedicated) Plan
...
| Serverless functions | Serverless functions consume DDUs for serverless functions. |
...
The source distributed trace data is Dynatrace OneAgent. OneAgent automatically manages the volume of captured trace data via Adaptive Traffic Management. It automatically and continuously adjusts the sampling rate in an intelligent way and keeps the ingested trace data volume roughly within your included trace data volume.
