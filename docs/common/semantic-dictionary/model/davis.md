...
| `dt.query` | string | experimental A DQL query associated with the event, see Dynatrace Query Language. If the query returns time series data, the time series is charted in the event details in the Problems app. | `timeseries avg(dt.host.cpu.idle)` |
...
| Attribute | Type | Description | Examples |
| --- | --- | --- | --- |
| `dt.query` | string | experimental A DQL query associated with the event, see Dynatrace Query Langauge. | `timeseries avg(dt.host.cpu.idle)` |
