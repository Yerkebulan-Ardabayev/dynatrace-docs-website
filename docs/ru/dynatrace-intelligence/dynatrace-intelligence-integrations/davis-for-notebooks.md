...
Используя ![Anomaly Detection - new](https://dt-cdn.net/images/davis-anomalydetection-256-105da91594.png "Anomaly Detection - new") **Anomaly Detection** в сочетании с возможностями DQL, можно создавать мощные пользовательские конфигурации для обнаружения аномалий в метриках, логах, бизнес-данных или их комбинации. Для предварительного просмотра и тонкой настройки конфигурации обнаружения аномалий перед её развёртыванием используйте параметр анализа Dynatrace Intelligence for Notebooks:

* Auto-adaptive threshold — Dynatrace автоматически вычисляет порог и динамически адаптирует его к поведению данных.
* Seasonal baseline — Dynatrace создаёт доверительную полосу для данных с сезонными паттернами.
* Static threshold — порог, не изменяющийся со временем.
...
Предиктивный ИИ-анализ Dynatrace Intelligence прогнозирует будущие значения любого временного ряда числовых значений на основе накопленных данных. Чтобы запустить анализ прогноза
