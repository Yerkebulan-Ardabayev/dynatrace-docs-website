...
* Full-Stack Monitoring на основе хостов:
  ...
  + Дополнительно предлагает все функции Kubernetes Platform Monitoring.")[1](#fn-1-1-def) [2](#fn-1-2-def).

  + Расширения Dynatrace Extensions могут быть включены на хостах с Full-Stack Monitoring на основе хостов и могут потреблять [пользовательские точки данных метрик Full-Stack](#full-stack-metrics) и Log Analytics.
...
Full-Stack Monitoring включает все метрики Infrastructure Monitoring, метрики мониторинга производительности приложений и другие встроенные метрики.
...
Каждый вносящий вклад ГиБ памяти хоста или приложения добавляет 200 КиБ данных трассировок в минуту, что составляет в общей сложности 3000 КиБ данных трассировок в каждом 15-минутном интервале Среды мониторинга.
...
* Второй источник данных — данные трассировок, отправленные через OneAgent Trace API или, в более общем смысле, трассировки OpenTelemetry, исходящие от хоста или приложения с Full-Stack Monitoring и отправленные через OTLP API.
...
Full-Stack Monitoring включает профилирование CPU, памяти и потоков для таких технологий, как Java, .NET, Go, Node.js и PHP.
...
* Пользовательских метрик и спанов OpenTelemetry, исходящих от любого:
  ...
  + Ноды Kubernetes с Cloud-native Full-Stack мониторингом, при использовании Dynatrace Operator с включённым обогащением метаданных.
...
Владельцы платформ и кластеров могут отслеживать свои кластеры Kubernetes с помощью Kubernetes Platform Monitoring.").
