...
* Справочник
* Чтение: 3 мин
* Опубликовано 03 сен 2020

Каждая поддерживаемая Dynatrace технология предлагает несколько встроенных метрик. Встроенные метрики включены в продукт из коробки, в некоторых случаях — в составе встроенных расширений.

Метрики, основанные на расширениях OneAgent или ActiveGate (префикс `ext:`), и вычисленные метрики (префикс `calc:`) являются пользовательскими метриками, а не встроенными; [потребление DDU](../../license/monitoring-consumption-classic/davis-data-units/metric-cost-calculation.md "Понять, как рассчитывается потребление Davis data unit и расходы на отслеживаемые метрики.") для этих метрик может существенно варьироваться в зависимости от того, как вы используете Dynatrace.

Префикс `ext:` используется метриками из [расширений OneAgent](../../ingest-from/extensions/develop-your-extensions.md "Разработайте собственные расширения в Dynatrace.") и [расширений ActiveGate](../../ingest-from/extensions/develop-your-extensions.md "Разработайте собственные расширения в Dynatrace."), а также [классическими метриками для интеграции AWS](../../ingest-from/amazon-web-services/integrate-with-aws/cloudwatch-metrics.md "Интеграция метрик из Amazon CloudWatch.").

Несмотря на сходство в названиях, метрики интеграции AWS **не** основаны на расширениях.

Для просмотра всех метрик, доступных в **вашей** среде, используйте вызов API [GET metrics](../../dynatrace-api/environment-api/metric-v2/get-all-metrics.md "Список всех метрик, доступных в вашей среде мониторинга через Metrics v2 API."). Мы рекомендуем следующие параметры запроса:

* `pageSize=500` — для получения максимально возможного числа метрик в одном ответе.
* `fields=displayName,unit,aggregationTypes,dduBillable` — для получения того же набора полей, который отображается в этих таблицах.
* В зависимости от того, какие метрики вы хотите запросить, используйте одно из следующих значений для параметра **metricSelector**:

  + `metricSelector=ext:*` — для получения всех метрик из расширений.
  + `metricSelector=calc:*` — для получения всех вычисленных метрик.
  + Опустите параметр для получения **всех** метрик вашей среды.
...
Раздел [Другие метрики приложений](#other-applications-metrics) содержит метрики, собираемые для мобильных и пользовательских приложений. Эти метрики, начинающиеся с `builtin:apps.other`, собираются без указания того, является ли приложение мобильным или пользовательским. Однако [биллинговые метрики приложений](#apps), начинающиеся с `builtin:billing.apps`, разделены для следующих типов приложений:
...
Следующие биллинговые метрики для подсчёта сессий фактически представляют собой сумму оплачиваемых **и неоплачиваемых** пользовательских сессий.
...
Пользовательские метрики определяются или устанавливаются пользователем, тогда как встроенные метрики по умолчанию являются частью продукта. Некоторые встроенные метрики по умолчанию отключены и при включении будут потреблять [DDU](../../license/monitoring-consumption-classic/davis-data-units.md "Понять, как рассчитывается потребление мониторинга Dynatrace на основе Davis data unit (DDU)."). Эти метрики охватывают широкий спектр поддерживаемых технологий, включая Apache Tomcat, NGINX, Couchbase, RabbitMQ, Cassandra, Jetty и многие другие.

Пользовательская метрика — это новый тип метрики с идентификатором и единицей измерения, задаваемыми пользователем. Семантика пользовательских метрик определяется вами и не входит в стандартную установку OneAgent. Пользовательские метрики отправляются в Dynatrace через [различные интерфейсы](../../ingest-from/extend-dynatrace/extend-metrics.md "Узнайте, как расширить наблюдаемость метрик в Dynatrace."). После определения пользовательской метрики она может передаваться для нескольких отслеживаемых компонентов. Пользовательская метрика каждого компонента приводит к отдельному временному ряду.

Например, если вы определите новую пользовательскую метрику `Files count`, считающую вновь созданные файлы в каталоге, эта метрика может собираться как для одного хоста, так и для двух отдельных хостов. Сбор одной и той же метрики для двух отдельных хостов приводит к двум временным рядам одного и того же типа пользовательской метрики, как показано в примере ниже:

![Custom metrics](https://dt-cdn.net/images/custommetrics2-1329-59422c6592.png)

В целях [расчёта потребления мониторинга](../../license/monitoring-consumption-classic/davis-data-units/metric-cost-calculation.md "Понять, как рассчитывается потребление Davis data unit и расходы на отслеживаемые метрики."), сбор одной и той же пользовательской метрики для двух хостов считается двумя отдельными пользовательскими метриками.

## Приложения

### Пользовательские

| Ключ метрики | Имя и описание | Единица | Агрегации |
| --- | --- | --- | --- |
...
### Пользовательские трейсы (классические)
...
### Мониторинг журналов (классический)
