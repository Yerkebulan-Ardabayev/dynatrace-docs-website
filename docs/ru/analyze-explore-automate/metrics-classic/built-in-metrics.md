...
Метрики, основанные на расширениях OneAgent или ActiveGate (префикс `ext:`), и вычисленные метрики (префикс `calc:`) являются пользовательскими метриками, а не встроенными; [потребление DDU](/docs/license/monitoring-consumption-classic/davis-data-units/metric-cost-calculation "Понять, как рассчитывается потребление Davis data unit и расходы на отслеживаемые метрики.") для этих метрик может существенно варьироваться в зависимости от того, как вы используете Dynatrace.

Префикс `ext:` используется метриками из [расширений OneAgent](/docs/ingest-from/extensions/develop-your-extensions "Разработайте собственные расширения в Dynatrace.") и [расширений ActiveGate](/docs/ingest-from/extensions/develop-your-extensions "Разработайте собственные расширения в Dynatrace."), а также [классическими метриками для интеграции AWS](/docs/ingest-from/amazon-web-services/integrate-with-aws/cloudwatch-metrics "Интеграция метрик из Amazon CloudWatch.").

Несмотря на сходство в названиях, метрики интеграции AWS **не** основаны на расширениях.

Для просмотра всех метрик, доступных в **вашей** среде, используйте вызов API [GET metrics](/docs/dynatrace-api/environment-api/metric-v2/get-all-metrics "Список всех метрик, доступных в вашей среде мониторинга через Metrics v2 API."). Мы рекомендуем следующие параметры запроса:
...
В целях [расчёта потребления мониторинга](/docs/license/monitoring-consumption-classic/davis-data-units/metric-cost-calculation "Понять, как рассчитывается потребление Davis data unit и расходы на отслеживаемые метрики."), сбор одной и той же пользовательской метрики для двух хостов считается двумя отдельными пользовательскими метриками.
