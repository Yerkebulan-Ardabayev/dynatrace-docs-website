...
Метрики на Grail позволяют точно находить и извлекать любые данные метрик с помощью Dynatrace Query Language. Ознакомившись с [основами запросов DQL](../../platform/grail/dynatrace-query-language/dql-guide.md#metrics "Узнайте, как работает DQL и каковы ключевые концепции DQL.") и командой timeseries, используйте примеры на этой странице, чтобы начать получать ответы из ваших метрик.
...
В этом примере вы узнаете, как использовать команду `entityAttr` для анализа использования CPU хоста по размеру хоста.
...
В этом примере вы узнаете, как использовать команду `append` для получения нескольких метрик CPU одним запросом.
...
Вы можете использовать команду timeseries с параметром `nonempty` для расчёта доступности хостов. Этот параметр гарантирует получение результата, даже когда данные не соответствуют фильтру — например, когда ни один хост не работает. Это обеспечивает более точное представление доступности хостов.
...
В этом примере вы запросите метрики логов для подсчёта успешных и неудачных проб готовности по хостам.

Вы можете использовать параметр `union` для охвата всех хостов, включая те, у которых нет ошибок или нет успешных проб.
...
В этом примере вы запросите частоту ошибок в секунду для конкретного эндпоинта ("/api/accounts"). Используя параметр `rate`, вы можете нормализовать данные временного ряда к определённой длительности.
...
В этом примере вы запросите текущую доступность дискового пространства хоста и используете параметр `shift` для сравнения с использованием 7 дней назад.
...
В этом примере вы используете агрегацию `count` для отслеживания количества отслеживаемых хостов в каждой зоне доступности региона AWS us-east-1.
...
В этом примере вы используете агрегацию `percentile` для отслеживания 90-го процентиля времени отклика условного эндпоинта /api/accounts.
...
В этом примере вы используете функцию `if` для маркировки недоиспользованных пар хост-диск.
...
Многие функции команды `summarize` принимают итеративные выражения, такие как `cpu_usage[]`, для сохранения временного ряда.
