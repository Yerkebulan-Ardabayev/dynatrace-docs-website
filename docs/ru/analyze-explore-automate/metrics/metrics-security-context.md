...
В Dynatrace существует модель разрешений для Grail. Она применяется ко всем данным телеметрии, таким как метрики, события, span-ы и логи.
...
Для развёртываний на базе Kubernetes убедитесь, что в Dynatrace Operator включено обогащение метаданными.
...
Вы можете определить контекст безопасности в источнике через OneAgent, OpenTelemetry или метки и аннотации Kubernetes. Это позволяет использовать существующие метки и теги для управления разрешениями в Dynatrace.
...
* Настройка разрешений Grail для распределённой трассировки
