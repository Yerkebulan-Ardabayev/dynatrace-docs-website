...
Для новейшей версии Dynatrace см. [Потоковая передача логов Kubernetes с помощью Dynatrace Log Module](../../logs/lma-log-ingestion/lma-log-ingestion-via-oa/lma-logs-from-kubernetes.md "Dynatrace поддерживает сбор данных логов из систем оркестрации контейнеров Kubernetes через OneAgent Log Module или Kubernetes Log Module.").

Dynatrace Log Monitoring поддерживает сбор логов из систем оркестрации контейнеров Kubernetes через OneAgent.

В качестве альтернативы сбору логов на основе OneAgent вы можете передавать логи в Dynatrace через [API приёма логов](../../logs/lma-log-ingestion/lma-log-ingestion-via-api.md "Потоковая передача данных логов в Dynatrace с помощью API для преобразования их в осмысленные сообщения логов.") с использованием интеграции, такой как [Fluent Bit](../../logs/lma-log-ingestion/lma-stream-logs-with-fluent-bit.md "Интеграция Fluent Bit для потоковой передачи логов в Dynatrace."), [Fluentd](../../logs/lma-log-ingestion/lma-stream-logs-fluentd-k8s.md "Интеграция Fluentd с Dynatrace для потоковой передачи логов из узлов и подов в Dynatrace."), [Logstash](../../logs/lma-log-ingestion/lma-stream-logs-with-logstash.md "Интеграция Logstash для потоковой передачи логов из узлов и подов в Dynatrace.") или [Dynatrace Collector](../../../ingest-from/opentelemetry/collector.md#dt-collector-dist "Узнайте о Dynatrace OTel Collector.").
...
Подробнее о поддерживаемых версиях Kubernetes см. [Поддержка и известные проблемы Dynatrace Operator](../../../ingest-from/technology-support/support-model-and-issues.md "Как Dynatrace поддерживает версии Kubernetes и Red Hat OpenShift, а также известные проблемы").
...
Подробнее см. [Обогащение метаданными для Kubernetes](../../../ingest-from/setup-on-k8s/guides/metadata-automation/k8s-metadata-telemetry-enrichment.md "Руководства по обогащению телеметрии в Kubernetes").
...
Стандартные функции обработки логов OneAgent, включая [маскирование конфиденциальных данных](../methods-of-masking-sensitive-data.md "Выберите оптимальный метод для маскирования конфиденциальных данных."), [настройку временных меток](../log-monitoring-configuration/timestamp-configuration.md "Определение правил мониторинга логов, управляющих временными метками данных логов.") и [автоматическое обогащение](lm-log-data-transformation.md "Универсальный приём логов автоматически преобразует данные логов в выходные значения для атрибута loglevel.") записей логов, также доступны и включены здесь.
...
Атрибут уровня записи лога, преобразованный модулем логов OneAgent, отличается от атрибута `status` лога, преобразованного сервером Dynatrace. Подробнее на странице [Автоматическое обогащение логов](lm-log-data-transformation.md#transform-all-types-of-logs "Универсальный приём логов автоматически преобразует данные логов в выходные значения для атрибута loglevel.").
...
Атрибут уровня записи лога, преобразованный OneAgent, отличается от атрибута `status` лога, преобразованного сервером Dynatrace. Подробнее на странице [Автоматическое обогащение логов](../../logs/lma-log-ingestion/lma-log-ingestion-via-oa/lma-log-data-transformation-oa.md#transform-all-types-of-logs "Универсальный приём логов автоматически преобразует данные логов в выходные значения для атрибута loglevel.").
...
Ознакомьтесь с [Областями конфигурации](../../logs/lma-log-ingestion/lma-log-ingestion-via-oa/lma-log-storage-configuration.md#configuration-scopes "Включение и исключение определённых источников логов, уже известных OneAgent, для хранения и анализа.") для четырёх областей иерархии конфигурации.
...
2. Используйте конечную точку [GET a schema](../../../dynatrace-api/environment-api/settings/schemas/get-schema.md "Просмотр схемы настроек через Dynatrace API."), чтобы узнать формат JSON, необходимый для отправки конфигурации. Идентификатор схемы правил приёма логов (`schemaId`) — `builtin:logmonitoring.log-storage-settings`. Вот пример полезной нагрузки JSON с правилами приёма логов:
...
* Процесс, работающий в контейнере, является [важным процессом](../../../observe/infrastructure-observability/process-groups/basic-concepts/which-are-the-most-important-processes.md "Отображение наиболее важных процессов для мониторинга и группировки процессов.");
...
Для получения дополнительных часто задаваемых вопросов о приёме данных ознакомьтесь со страницей [Правила приёма логов](../../logs/lma-log-ingestion/lma-log-ingestion-via-oa/lma-log-storage-configuration.md#faq "Включение и исключение определённых источников логов, уже известных OneAgent, для хранения и анализа.").

## Устранение неполадок

Посетите сообщество Dynatrace для получения руководств по устранению неполадок, а также см. [Устранение неполадок Log Monitoring (Logs Classic)](../lmc-troubleshooting.md "Исправление проблем, связанных с настройкой и конфигурацией Log Monitoring Classic.").
