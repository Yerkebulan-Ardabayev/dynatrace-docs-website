...
Для самой новой версии Dynatrace см. Ввод журналов API.
...
Ввод журналов API позволяет передавать журнальные записи в систему. Он доступен через Мониторинг журналов API v2 - POST ввод журналов для JSON и текстового формата или через OTLP-конечную точку для бинарного формата protobuf OTLP.

* Для Dynatrace конечная точка ввода журналов доступна в вашей среде.
...
Чтобы просмотреть все предопределенные атрибуты Key-Values, такие как поддерживаемые семантические ключи атрибутов, проверьте Мониторинг журналов API v2 - POST ввод журналов.

## Очередь журнальных данных на Environment ActiveGate

Вы можете настроить свойства очереди журнальных данных, редактируя файл `custom.properties` (см. Свойства и параметры конфигурации ActiveGate) на вашем ActiveGate, чтобы задать следующие значения:
...
Посетите сообщество Dynatrace для руководств по устранению неполадок, а также см. Устранение неполадок Мониторинга журналов (Классические журналы).
...
* Мониторинг журналов API v2 - POST ввод журналов