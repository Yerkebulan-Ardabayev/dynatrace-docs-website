...
Для последней версии Dynatrace см. [Передача журналов в Dynatrace с помощью Logstash](../../logs/lma-log-ingestion/lma-stream-logs-with-logstash.md "Интегрируйте Logstash для передачи журналов из узлов и подов в Dynatrace.").

[Мониторинг журналов Dynatrace](../../log-monitoring.md "Узнайте, как включить мониторинг журналов, какие сведения может предоставить мониторинг журналов и многое другое.") использует OneAgent DaemonSet, который включает модуль журнала. Это рекомендуемый способ передачи журналов из узлов и подов в Dynatrace.
...
Поддерживает отправку журналов в [точку входа журналов Dynatrace API v2](../../../dynatrace-api/environment-api/log-monitoring-v2/post-ingest-logs.md "Отправьте пользовательские журналы в Dynatrace через API мониторинга журналов API v2.").
