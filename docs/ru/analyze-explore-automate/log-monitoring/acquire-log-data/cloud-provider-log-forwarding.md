...
Для новейшей версии Dynatrace см. Пересылка логов от облачных провайдеров.

Потребление DDU для Log Monitoring

Для облачного Log Monitoring применяется ценообразование на основе DDU. Подробности см. в разделе DDU для Log Monitoring.
...
Подробнее см. Потоковая передача логов через Amazon Data Firehose (Logs Classic).
