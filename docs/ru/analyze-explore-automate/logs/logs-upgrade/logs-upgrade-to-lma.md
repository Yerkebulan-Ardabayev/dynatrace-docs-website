...
Log Management and Analytics — это новейшее решение Dynatrace для мониторинга логов. С появлением Dynatrace Platform и Grail мы рекомендуем перейти на новейшее решение для мониторинга логов.
...
* Принимаемые данные логов
  ...
  + Принимаемые данные логов сохраняются в базе данных Grail.
  + Принимаемые данные логов могут маршрутизироваться в корзины с различными периодами хранения.
* Потребление DDU

  + При активации Log Management and Analytics вы начинаете потреблять DDU по новой модели с тремя измерениями: Приём и обработка, Хранение, Запросы.
...
* API
  ...
  + API экспорта логов будет недоступен. Мы рекомендуем прекратить использование Log GET search и Log GET aggregate. Если вы продолжите их использовать, для них потребуется токен OAuth2 с областями `storage:logs:read` и `storage:buckets:read`.
...
* Конфигурация приёма, включая конфигурацию OneAgent и приём через общий API.
...
Тем не менее, мы рекомендуем преобразовать ваши LQL-сопоставители для обработки логов, метрик и событий в высокопроизводительные DQL-сопоставители.
...
* Назначение политики новым пользователям
  ...
  В Dynatrace только пользователи с правами администратора могут управлять политиками (пользователи с разрешением учётной записи `Manage users`).
...
* Лучшие практики обновления до последней версии Dynatrace
