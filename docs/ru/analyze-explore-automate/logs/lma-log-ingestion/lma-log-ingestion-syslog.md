...
* Environment ActiveGate версии 1.295+ на Linux, установленный для [мониторинга удалённых технологий](/docs/ingest-from/dynatrace-activegate/capabilities/routing-monitoring-purpose "Узнайте о возможностях маршрутизации и мониторинга ActiveGate.").
...
1. **Разверните Environment ActiveGate**.
   ...
   Разверните Environment ActiveGate в месте, обеспечивающем связность между ActiveGate и отслеживаемыми устройствами. См. инструкции для [Linux](/docs/ingest-from/dynatrace-activegate/installation/linux "Узнайте, как установить ActiveGate на Windows, настроить установку и многое другое."), используя назначение [мониторинга удалённых технологий](/docs/ingest-from/dynatrace-activegate/capabilities/routing-monitoring-purpose "Узнайте о возможностях маршрутизации и мониторинга ActiveGate.").
...
3. Необязательно. **Отредактируйте конфигурацию приёмника syslog**.
   ...
   Если ваши источники syslog отправляют события на пользовательские порты или используют протокол syslog RFC 3164, измените конфигурацию приёмника syslog. Подробности см. в [Приём данных syslog с помощью OpenTelemetry Collector](/docs/ingest-from/opentelemetry/collector/use-cases/syslog "Настройка OpenTelemetry Collector для приёма данных syslog в Dynatrace.").
   ...
   Не изменяйте [конфигурацию экспортёра](/docs/ingest-from/opentelemetry/collector/use-cases/syslog#exporters "Настройка OpenTelemetry Collector для приёма данных syslog в Dynatrace."). Она предварительно настроена для пересылки ваших syslog-записей в среду Dynatrace.
...
Дополнительную информацию см. в [Обработка журналов с помощью парсеров технологических пакетов](/docs/platform/openpipeline/use-cases/tutorial-technology-processor "Настройка конвейера обработки для структурирования технологических журналов в соответствии с Dynatrace Semantic Dictionary.").
...
Когда есть отличия от стандарта syslog, OpenTelemetry Collector не может правильно разобрать такие записи syslog при приёме. Они всё равно пересылаются в Dynatrace, но не разбираются на конечной точке syslog. Из-за этого необработанные сообщения syslog видны в Dynatrace, например, в [![Logs](https://dt-cdn.net/images/logs-256-ae0a9ca67f.png "Logs") **Logs**](/docs/analyze-explore-automate/logs/lma-logs-app "Поиск, фильтрация и анализ журналов с помощью приложения Dynatrace Logs.") или [![Notebooks](https://dt-cdn.net/images/notebooks-768-046137830a.webp "Notebooks") **Notebooks**](/docs/analyze-explore-automate/dashboards-and-notebooks/notebooks "Анализ, визуализация и обмен данными наблюдаемости.").
...
Подробнее о динамической маршрутизации см. в [Маршрутизация данных](/docs/platform/openpipeline/getting-started/how-to-routing "Узнайте, как маршрутизировать данные в конвейер обработки OpenPipeline.").
...
Дополнительные инструкции и информацию об анализе структурированных журналов см. в [Обработка журналов с помощью парсеров технологических пакетов](/docs/platform/openpipeline/use-cases/tutorial-technology-processor "Настройка конвейера обработки для структурирования технологических журналов в соответствии с Dynatrace Semantic Dictionary.").

## Устранение неполадок

Посетите Dynatrace Community для руководств по устранению неполадок, а также см. [Устранение неполадок Log Management and Analytics](/docs/analyze-explore-automate/logs/lma-troubleshooting "Исправление проблем, связанных с настройкой и конфигурацией Log Management and Analytics.").
