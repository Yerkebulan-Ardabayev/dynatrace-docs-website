...
* Токен API Dynatrace с областью действия `logs.ingest`.
  Подробнее о токенах, их генерации и областях действия см. [Dynatrace API — токены и аутентификация](/docs/dynatrace-api/basics/dynatrace-api-authentication "Узнайте, как пройти аутентификацию для использования Dynatrace API.").
...
Подробнее об API приёма логов Dynatrace см. [Log Monitoring API v2 — POST ingest logs](/docs/dynatrace-api/environment-api/log-monitoring-v2/post-ingest-logs "Отправляйте пользовательские логи в Dynatrace через Log Monitoring API v2.").
