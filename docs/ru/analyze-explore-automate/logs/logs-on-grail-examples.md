...
Log Management and Analytics на основе Grail позволяет точно находить и извлекать любые данные логов с помощью [Dynatrace Query Language](/docs/platform/grail/dynatrace-query-language "Как использовать Dynatrace Query Language."). Ознакомившись с [основами запросов DQL](/docs/platform/grail/dynatrace-query-language/dql-guide "Узнайте, как работает DQL и каковы ключевые концепции DQL."), используйте примеры на этой странице, чтобы начать получать ответы из данных ваших логов.
...
* [Пример 1](/docs/analyze-explore-automate/logs/logs-on-grail-examples#logexample1 "Изучите базовые примеры Log Management and Analytics по использованию данных логов в Dynatrace на основе Grail.") — Получение распределения HTTP-кодов состояния и количества по типам ошибок.
* [Пример 2](/docs/analyze-explore-automate/logs/logs-on-grail-examples#logexample2 "Изучите базовые примеры Log Management and Analytics по использованию данных логов в Dynatrace на основе Grail.") — Определение среднего размера корзины на основе логов.
* [Пример 3](/docs/analyze-explore-automate/logs/logs-on-grail-examples#logexample3 "Изучите базовые примеры Log Management and Analytics по использованию данных логов в Dynatrace на основе Grail.") — Отслеживание изменений пользователей с помощью журналов аудита.
* [Пример 4](/docs/analyze-explore-automate/logs/logs-on-grail-examples#logexample4 "Изучите базовые примеры Log Management and Analytics по использованию данных логов в Dynatrace на основе Grail.") — Создание метрики на основе логов.
* [Пример 5](/docs/analyze-explore-automate/logs/logs-on-grail-examples#logexample5 "Изучите базовые примеры Log Management and Analytics по использованию данных логов в Dynatrace на основе Grail.") — Создание оповещения на основе логов.
...
2. Извлечение метрики из поля content.
   Поле content лога содержит HTTP\_STATUS коды, которые вам нужны. Теперь используем команду `parse` для создания шаблона [Dynatrace Pattern Language](/docs/platform/grail/dynatrace-pattern-language "Используйте Dynatrace Pattern Language для описания шаблонов с помощью матчеров.") со следующими элементами:

   * `LD`: начинаем с сопоставления любых [строковых данных](/docs/platform/grail/dynatrace-pattern-language/log-processing-lines-strings#line-data "Изучите синтаксис DPL для обработки строк.") в начале поля
   * `'HTTP_STATUS '`: [литеральное выражение](/docs/platform/grail/dynatrace-pattern-language/log-processing-literal-expression "Изучите синтаксис DPL для обработки литеральных выражений."), непосредственно предшествующее числовому HTTP-статусу, с учётом пробела
   * `INT:httpstatus`: [целое число](/docs/platform/grail/dynatrace-pattern-language/log-processing-numeric#int-integer "Изучите синтаксис DPL для обработки числовых данных."), которое будет извлечено как новое поле `httpstatus`
...
3. Извлечение продуктов и соответствующих количеств.
   ...
   Используя [Dynatrace Pattern Language](/docs/platform/grail/dynatrace-pattern-language "Используйте Dynatrace Pattern Language для описания шаблонов с помощью матчеров."), создайте шаблон и сопоставьте следующие части поля `content`:

   * `LD`: начинаем с сопоставления любых [строковых данных](/docs/platform/grail/dynatrace-pattern-language/log-processing-lines-strings#line-data "Изучите синтаксис DPL для обработки строк.") в начале поля
   * `'userId='`: [литеральное выражение](/docs/platform/grail/dynatrace-pattern-language/log-processing-literal-expression "Изучите синтаксис DPL для обработки литеральных выражений."), непосредственно предшествующее идентификатору пользователя
   ...
   * `INT:productQuantity`: [целое число](/docs/platform/grail/dynatrace-pattern-language/log-processing-numeric#int-integer "Изучите синтаксис DPL для обработки числовых данных."), которое будет извлечено как новое поле с именем `productQuantity`
...
* [Создание метрики отказов в соединении](/docs/analyze-explore-automate/logs/lma-use-cases/lma-e2e-create-log-metric#lma-uc-create-connections-refused-metric "Изучите сценарий использования Log Management and Analytics для создания метрики на основе логов.")
...
* [Создание метрики атрибутов логов](/docs/analyze-explore-automate/logs/lma-use-cases/lma-e2e-create-log-metric#lma-uc-create-log-attribute-metric "Изучите сценарий использования Log Management and Analytics для создания метрики на основе логов.")
...
* [Настройка оповещений на основе событий, извлечённых из логов](/docs/analyze-explore-automate/logs/lma-use-cases/lma-alert-log-based-events "Как создавать и настраивать проблемы и оповещения Davis на основе событий из логов.")
...
* [Настройка пользовательских оповещений на основе метрик, извлечённых из логов](/docs/analyze-explore-automate/logs/lma-use-cases/lma-alert-log-based-metrics "Как создавать и настраивать проблемы и пользовательские оповещения Davis на основе метрик из логов.")