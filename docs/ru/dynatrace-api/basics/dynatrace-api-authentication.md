...
scraped: 2026-02-21T21:24:44.457138
---
...
Чтобы быть аутентифицированным для использования Dynatrace API, вам необходим действительный [токен доступа](/docs/manage/identity-access-management/access-tokens-and-oauth-clients/access-tokens "Изучите концепцию токена доступа и его области.") или действительный [личный токен доступа](/docs/manage/identity-access-management/access-tokens-and-oauth-clients/access-tokens/personal-access-token "Изучите концепцию личного токена доступа и его области."). Доступ к API является тонко зернистым, что означает, что вам также необходимо иметь правильные области, назначенные токену. См. описание каждого запроса, чтобы узнать, какие области необходимы для его использования.
...
**Публичная часть** токена представляет собой 24-символьный публичный идентификатор.

В нашем примере: `ST2EY72KQINMH574WMNVI7YN`

идентификатор токена

**Идентификатор токена** представляет собой комбинацию **префикса** и **публичной части**. Идентификатор токена можно безопасно отображать в интерфейсе пользователя и использовать для целей ведения журнала.

В нашем примере: `<DYNATRACE_TOKEN_PLACEHOLDER>`

секретная часть

**Секретная часть** токена представляет собой 64-символьную строку, которая должна быть обработана как пароль:
...
3. Введите имя для вашего токена.  
   Dynatrace не обеспечивает уникальность имен токенов. Вы можете создать несколько токенов с одинаковым именем. Убедитесь, что вы предоставили осмысленное имя для каждого сгенерированного токена. Правильное именование помогает вам эффективно управлять токенами и, возможно, удалять их, когда они больше не нужны.
...
Предоставляет доступ к запросу [POST Встроенные общие события](/docs/platform/openpipeline/reference/openpipeline-ingest-api/generic-events/events-generic-builtin "Ингестируйте общие события из встроенных конечных точек через OpenPipeline Ingest API.") API ингеста OpenPipeline.

OpenPipeline - Ингестируемые события, Жизненный цикл разработки программного обеспечения

`openpipeline.events_sdlc`

Предоставляет доступ к запросу [POST Встроенные события SDLC](/docs/platform/openpipeline/reference/openpipeline-ingest-api/sdlc-events/events-sdlc-builtin "Ингестируйте события SDLC из встроенных конечных точек через OpenPipeline Ingest API.") API ингеста OpenPipeline.
...
Предоставляет доступ к запросу [POST Пользовательские события SDLC](/docs/platform/openpipeline/reference/openpipeline-ingest-api/sdlc-events/events-sdlc-custom-endpoint "Настройте пользовательскую конечную точку события SDLC через OpenPipeline Ingest API.") API ингеста OpenPipeline.

OpenPipeline - Ингестируемые события безопасности (Встроенные)

`openpipeline.events_security`

Предоставляет доступ к запросу [POST Встроенные события безопасности](/docs/platform/openpipeline/reference/openpipeline-ingest-api/security-events/events-security-builtin "Ингестируйте события безопасности из встроенных конечных точек через OpenPipeline Ingest API.") API ингеста OpenPipeline.

OpenPipeline - Ингестируемые события безопасности (Пользовательские)

`openpipeline.events_security.custom`

Предоставляет доступ к запросу [POST Пользовательские события безопасности](/docs/platform/openpipeline/reference/openpipeline-ingest-api/security-events/events-security-custom-endpoint "Настройте пользовательскую конечную точку события безопасности через OpenPipeline Ingest API.") API ингеста OpenPipeline.

OpenPipeline - Ингестируемые события (Пользовательские)

`openpipeline.events.custom`

Предоставляет доступ к запросу [POST Пользовательская конечная точка общего события](/docs/platform/openpipeline/reference/openpipeline-ingest-api/generic-events/events-generic-custom-endpoint "Настройте пользовательскую конечную точку общего события через OpenPipeline Ingest API.") API ингеста OpenPipeline.
...
Предоставляет доступ к GET-запросам [Токенов доступа API](/docs/dynatrace-api/environment-api/tokens-v2/api-tokens "Управляйте Dynatrace API аутентификационными токенами.").

Запись токенов API

`apiTokens.write`

Предоставляет доступ к POST-, PUT- и DELETE-запросам [Токенов доступа API](/docs/dynatrace-api/environment-api/tokens-v2/api-tokens "Управляйте Dynatrace API аутентификационными токенами.").

Чтение атак

`attacks.read`

Предоставляет доступ к GET-запросам Атак API и [Настройки API](/docs/dynatrace-api/environment-api/settings "Узнайте, что предлагает Dynatrace Настройки API.") для Application Protection (`builtin:appsec.attack-protection-settings`, `builtin:appsec.attack-protection-advanced-config` и `builtin:appsec.attack-protection-allowlist-config` схем).

Запись настроек Application Protection

`attacks.write`

Предоставляет доступ к POST-, PUT- и DELETE-запросам [Настройки API](/docs/dynatrace-api/environment-api/settings "Узнайте, что предлагает Dynatrace Настройки API.") для Application Protection (`builtin:appsec.attack-protection-settings`, `builtin:appsec.attack-protection-advanced-config` и `builtin:appsec.attack-protection-allowlist-config` схем).
...
Предоставляет доступ к GET-запросам из раздела **Конфигурация мониторинга расширений** [Расширения 2.0 API](/docs/dynatrace-api/environment-api/extensions-20 "Узнайте, как управлять расширениями с помощью Dynatrace Расширения 2.0 API.").

Запись конфигурации мониторинга расширений

`extensionConfigurations.write`

Предоставляет доступ к POST-, PUT- и DELETE-запросам из раздела **Конфигурация мониторинга расширений** [Расширения 2.0 API](/docs/dynatrace-api/environment-api/extensions-20 "Узнайте, как управлять расширениями с помощью Dynatrace Расширения 2.0 API.").

Чтение конфигурации окружения расширений

`extensionEnvironment.read`

Предоставляет доступ к GET-запросам из раздела **Конфигурация окружения расширений** [Расширения 2.0 API](/docs/dynatrace-api/environment-api/extensions-20 "Узнайте, как управлять расширениями с помощью Dynatrace Расширения 2.0 API.").

Запись конфигурации окружения расширений

`extensionEnvironment.write`

Предоставляет доступ к POST-, PUT- и DELETE-запросам из раздела **Конфигурация окружения расширений** [Расширения 2.0 API](/docs/dynatrace-api/environment-api/extensions-20 "Узнайте, как управлять расширениями с помощью Dynatrace Расширения 2.0 API.").

Чтение расширений

`extensions.read`

Предоставляет доступ к GET-запросам из раздела **Расширения** [Расширения 2.0 API](/docs/dynatrace-api/environment-api/extensions-20 "Узнайте, как управлять расширениями с помощью Dynatrace Расширения 2.0 API.").

Запись расширений

`extensions.write`

Предоставляет доступ к POST-, PUT- и DELETE-запросам из раздела **Расширения** [Расширения 2.0 API](/docs/dynatrace-api/environment-api/extensions-20 "Узнайте, как управлять расширениями с помощью Dynatrace Расширения 2.0 API.").
...
Предоставляет доступ к [DELETE пользовательской метрики](/docs/dynatrace-api/environment-api/metric-v2/delete-metric "Удалите метрику, введенную через Metrics v2 API.") запроса Metrics API.
...
Предоставляет доступ к GET-запросам [Проблемы API v2](/docs/dynatrace-api/environment-api/problems-v2 "Узнайте, что предлагает Dynatrace Проблемы v2 API.").

Запись проблем

`problems.write`

Предоставляет доступ к POST-, PUT- и DELETE-запросам [Проблемы API v2](/docs/dynatrace-api/environment-api/problems-v2 "Узнайте, что предлагает Dynatrace Проблемы v2 API.").
...
Предоставляет доступ к GET-запросам [Целей сервисного уровня API](/docs/dynatrace-api/environment-api/service-level-objectives-classic "Узнайте, что предлагают классические цели сервисного уровня Dynatrace SLO API.").

Запись SLO

`slo.write`

Предоставляет доступ к POST-, PUT- и DELETE-запросам [Целей сервисного уровня API](/docs/dynatrace-api/environment-api/service-level-objectives-classic "Узнайте, что предлагают классические цели сервисного уровня Dynatrace SLO API.").
...
Поиск одного трассировки

`traces.lookup`

Проверяет наличие трассировки в межсредовой трассировке.

Чтение единой страницы анализа

`unifiedAnalysis.read`

Предоставляет доступ к схеме единого анализа в [Настройках API](/docs/dynatrace-api/environment-api/settings "Узнайте, что предлагают настройки Dynatrace API.")
...
Управление файлами символов для мобильных устройств

`DssFileManagement`

Предоставляет доступ к [Символизации мобильных устройств API](/docs/dynatrace-api/configuration-api/mobile-symbolication-api "Управляйте файлами символов для мобильных устройств через Dynatrace API.").

Real User Monitoring Управление тегами JavaScript

`RumJavaScriptTagManagement`

Предоставляет доступ к [Тегам JavaScript API](/docs/dynatrace-api/environment-api/rum/real-user-monitoring-javascript-code "Узнайте, как использовать Dynatrace API для настройки и поддержки ваших вручную внедренных приложений с помощью Real User Monitoring тегов JavaScript.").
...
Предоставляет разрешение на получение данных из [удаленных сред Dynatrace](/docs/dynatrace-api/configuration-api/remote-environments "Управляйте конфигурациями удаленных сред Dynatrace через Dynatrace конфигурацию API.") для панелей инструментов с несколькими средами.
...
Мы рекомендуем использовать заголовок **Authorization**, поскольку URL (вместе с токенами, переданными в них) могут быть записаны в различных местах. Пользователи также могут добавлять закладки URL или делиться ими в открытом тексте. Следовательно, размещение токенов аутентификации в URL увеличивает риск их перехвата злоумышленником.
...
* [Классические токены доступа](/docs/manage/identity-access-management/access-tokens-and-oauth-clients/access-tokens "Изучите концепцию токена доступа и его области.")