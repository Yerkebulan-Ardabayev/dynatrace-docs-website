...
[Events API v1](/docs/dynatrace-api/environment-api/events-v1 "Узнайте, что можно делать с помощью Dynatrace Events API.") был объявлен устаревшим в [Dynatrace версии 1.243](/docs/whats-new/dynatrace-api/sprint-243 "Журнал изменений Dynatrace API версии 1.243"). Его заменой является [Events API v2](/docs/dynatrace-api/environment-api/events-v2 "Узнайте, что можно делать с помощью Dynatrace Events API v2."). Мы рекомендуем перейти на новый API при первой возможности.
...
* [Селектор сущностей](/docs/dynatrace-api/environment-api/entity-v2/entity-selector "Настройте селектор сущностей для конечных точек Environment API.") помогает фильтровать события в запросах на чтение и принимать события, затрагивающие несколько сущностей.
...
[Приём](/docs/dynatrace-api/environment-api/events-v2/post-event "Приём события через API Dynatrace.") пользовательских событий потребляет [единицы данных Davis (DDU)](/docs/license/monitoring-consumption-classic/davis-data-units "Узнайте, как рассчитывается потребление мониторинга Dynatrace на основе единиц данных Davis (DDU).") из пула событий.
...
Чтобы узнать о новых параметрах запросов/тела, см. документацию отдельных запросов в [Events API v2](/docs/dynatrace-api/environment-api/events-v2 "Узнайте, что можно делать с помощью Dynatrace Events API v2.").
...
* [Events API v2](/docs/dynatrace-api/environment-api/events-v2 "Узнайте, что можно делать с помощью Dynatrace Events API v2.")
* [Events API v1](/docs/dynatrace-api/environment-api/events-v1 "Узнайте, что можно делать с помощью Dynatrace Events API.")