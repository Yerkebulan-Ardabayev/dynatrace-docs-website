...
Events API v1 был объявлен устаревшим в Dynatrace версии 1.243. Его заменой является Events API v2. Мы рекомендуем перейти на новый API при первой возможности.
...
* Селектор сущностей помогает фильтровать события в запросах на чтение и принимать события, затрагивающие несколько сущностей.
...
Приём пользовательских событий потребляет единицы данных Davis (DDU).") из пула событий.
...
Чтобы узнать о новых параметрах запросов/тела, см. документацию отдельных запросов в Events API v2.
