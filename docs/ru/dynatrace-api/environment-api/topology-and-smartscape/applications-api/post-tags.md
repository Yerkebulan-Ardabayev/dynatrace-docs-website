...
Присваивает пользовательские теги указанному приложению. Достаточно указать только значение тега. Контекст `CONTEXTLESS` будет присвоен автоматически.

Использование этого API ограничено тегами, содержащими только значение. Чтобы присвоить теги формата ключ:значение, используйте Custom tags API.

Запрос принимает полезную нагрузку типа `application/json`.

|  |  |  |
| --- | --- | --- |
| POST | Managed | `https://{your-environment-id}.live.dynatrace.com/api/v1/entity/applications/{meIdentifier}` |
