...
Назначает пользовательские теги указанному сервису. Необходимо указать только значение тега. Контекст `CONTEXTLESS` будет назначен автоматически.

Использование этого API ограничено тегами с только значением. Для назначения тегов формата ключ:значение используйте Custom tags API.
