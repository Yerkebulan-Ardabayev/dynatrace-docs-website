...
* В качестве операнда можно использовать любую метрику, включая метрики, модифицированные любой цепочкой трансформаций, а также применять трансформации к результату выражения.
...
Не используйте метрическое выражение для преобразования единиц данных. Используйте вместо этого [трансформацию **toUnit**](metric-selector.md#to-unit "Настройка селектора метрик для Metric v2 API."). Единственное исключение — единицы измерения, которые Dynatrace не поддерживает. Используйте запрос GET all units, чтобы получить список поддерживаемых единиц.
