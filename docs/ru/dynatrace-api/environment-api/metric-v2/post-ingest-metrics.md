...
* [Data Explorer](/docs/analyze-explore-automate/explorer "Запрос метрик и преобразование результатов для получения нужных сведений.")
* Запрос [GET точек данных метрик](/docs/dynatrace-api/environment-api/metric-v2/get-data-points "Чтение точек данных одной или нескольких метрик через Metrics v2 API.") API Metric v2.

Предоставленные точки данных должны соответствовать [протоколу приёма метрик](/docs/ingest-from/extend-dynatrace/extend-metrics/reference/metric-ingestion-protocol "Узнайте, как работает протокол приёма данных для Dynatrace Metrics API."). Вам не нужно предварительно регистрировать метрику. После того как Dynatrace примет и обработает данные, вы сможете использовать их так же, как любые другие метрики в Dynatrace, например, в [графиках](/docs/analyze-explore-automate/explorer "Запрос метрик и преобразование результатов для получения нужных сведений.") или [событиях метрик](/docs/dynatrace-intelligence/anomaly-detection/metric-events "Узнайте о событиях метрик в Dynatrace"). Вы также можете предоставить [метаданные](/docs/ingest-from/extend-dynatrace/extend-metrics/reference/custom-metric-metadata "Предоставьте метаданные для вашей пользовательской метрики.") для принятой метрики через Settings API.
...
Вы можете использовать измерение `dt.process.id=<PID>` для добавления идентификатора группы процессов. Когда идентификатор группы процессов предоставлен, к данной метрике будет добавлено измерение `dt.entity.process_group_instance`. Это работает только при приёме метрик OneAgent через API [`dynatrace_ingest`](/docs/ingest-from/extend-dynatrace/extend-metrics/ingestion-methods/oneagent-pipe "Узнайте, как принимать метрики с помощью локальной скриптовой интеграции.").

Подробнее см. в разделе [OneAgent metric API](/docs/ingest-from/extend-dynatrace/extend-metrics/ingestion-methods/oneagent-metric-api "Используйте Dynatrace API для получения метрик отслеживаемых сущностей.").
...
* [Протокол приёма метрик](/docs/ingest-from/extend-dynatrace/extend-metrics/reference/metric-ingestion-protocol "Узнайте, как работает протокол приёма данных для Dynatrace Metrics API.")
* [Метаданные пользовательских метрик](/docs/ingest-from/extend-dynatrace/extend-metrics/reference/custom-metric-metadata "Предоставьте метаданные для вашей пользовательской метрики.")
