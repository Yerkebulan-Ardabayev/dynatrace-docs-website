...
1. (Предварительное условие) Результаты проверок безопасности AWS из [AWS Security Hub](https://dt-url.net/5l239zz)
   загружаются в Grail посредством загрузки логов Dynatrace.
2. (Собственно автоматизация) Предопределённый рабочий процесс оркестрирует процессы запросов, фильтрации и группировки данных и преобразует полученные результаты в тикеты Jira для устранения. См. раздел [Начало работы](#start) для описания шагов рабочего процесса.
...
* Настройте загрузку логов (загружает результаты проверок безопасности AWS в Grail).
...
**Триггер фиксированного времени**: Устанавливает время запуска рабочего процесса (в данном случае рабочий процесс запускается каждый день в 9:00). Дополнительную информацию о планировании рабочего процесса см. в разделе Планирование рабочих процессов.
...
1. **Получение результатов по учётной записи**: Выполняет DQL-запрос, который получает результаты. Некоторая информация, получаемая для текущего случая и отображаемая в Jira, включает
...
1. **Получение результатов по ресурсу**: Выполняет DQL-запрос, который получает результаты. Некоторая информация, получаемая для текущего случая и отображаемая в Jira, включает
