...
Dynatrace интегрируется с Atlassian Rovo Dev через Model Context Protocol (MCP), обогащая опыт разработчика контекстом среды выполнения и валидацией Runtime Vulnerability Analytics (RVA). Это позволяет Rovo Dev предпринимать целенаправленные действия прямо из IDE:
...
* Настроить мониторинг с помощью Dynatrace OneAgent для продуктовых сервисов.
* Включить Runtime Vulnerability Analytics.
* Создать токен платформы Dynatrace со следующими правами:
...
3. Рабочий процесс использует Davis CoPilot для обобщения результатов и автоматически создаёт задачи Jira для подтверждённых уязвимостей.
...
Сервер Dynatrace MCP предоставляет дополнительные инструменты, которые Rovo Dev может использовать, обеспечивая дополнительные сценарии использования, такие как:
