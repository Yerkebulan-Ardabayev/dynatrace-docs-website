...
* Обновлено 23 сентября 2025 г.

Dynatrace вводит новую таблицу `security.events` в [Grail](/docs/platform/grail "Информация о том, что и как можно запрашивать в данных Dynatrace."), улучшая способ потребления, хранения и запросов данных событий безопасности. Это руководство объясняет преимущества, изменения в управлении доступом, изменения в импорте и запросах, а также необходимые действия пользователей.
...
* **Упрощённых запросов событий безопасности** — вы можете напрямую получать события безопасности с помощью команды `fetch security.events` (см. [Примеры DQL для данных безопасности](/docs/secure/threat-observability/dql-examples "Примеры DQL для данных безопасности на базе Grail.")).
* **Поддержки запросов без кода** — Dashboards и Notebooks позволяют анализировать события безопасности с помощью удобных фильтров, не требуя предварительных знаний DQL (см. [Исследование событий безопасности](/docs/analyze-explore-automate/dashboards-and-notebooks/explore-data#explore-security-events "Исследуйте свои данные с помощью нашего интерфейса.")).
* **Усиленного контроля доступа** — отдельные разрешения обеспечивают безопасность на уровне таблиц и записей без влияния на другие таблицы событий (см. [Разрешения в Grail](/docs/platform/grail/organize-data/assign-permissions-in-grail "Узнайте, как назначать разрешения бакетам и таблицам в Grail.")).
...
* **Миграция подключений для приложений/расширений интеграции** — если вы уже установили и настроили [интеграции безопасности](/docs/secure/threat-observability/security-events-ingest#ingest "Импорт внешних данных безопасности в Grail."), необходимо выполнить одно из следующих действий:
