...
Dynatrace вводит новую таблицу `security.events` в Grail, улучшая способ потребления, хранения и запросов данных событий безопасности. Это руководство объясняет преимущества, изменения в управлении доступом, изменения в импорте и запросах, а также необходимые действия пользователей.
...
* **Упрощённых запросов событий безопасности** — вы можете напрямую получать события безопасности с помощью команды `fetch security.events` (см. Примеры DQL для данных безопасности).
...
* **Усиленного контроля доступа** — отдельные разрешения обеспечивают безопасность на уровне таблиц и записей без влияния на другие таблицы событий (см. Разрешения в Grail).
