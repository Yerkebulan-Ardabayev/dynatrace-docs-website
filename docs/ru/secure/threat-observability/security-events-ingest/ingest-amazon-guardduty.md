...
* Токены:
  ...
  + Сгенерируйте токен доступа с областью `openpipeline.events_security` и сохраните его. Подробности см. в Dynatrace API - Токены и аутентификация.
...
2. Функция Lambda предварительно обрабатывает события и отправляет их в Dynatrace через выделенную конечную точку загрузки безопасности OpenPipeline.
...
Информацию о биллинге см. в События на базе Grail.
