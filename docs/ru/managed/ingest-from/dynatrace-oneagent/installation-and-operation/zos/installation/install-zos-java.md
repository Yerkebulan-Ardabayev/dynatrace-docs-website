...
Анализируйте производительность ваших транзакций из конца в конец с помощью [service flow](/docs/observe/application-observability/services-classic/service-backtrace "Отследите последовательность вызовов сервисов вплоть до клика в браузере, который инициировал последовательность вызовов.").

![zOS Java 1](https://dt-cdn.net/images/zos-java-1-2772-facc3b2740.png)

Используйте [распределённые трассировки PurePath](/docs/observe/application-observability/distributed-traces "Получите наблюдаемость высокораспределённых облачных архитектур для эффективной трассировки и анализа транзакций в реальном времени.") для углубления до уровня кода и оптимизации ваших программ.

![zOS Java 2](https://dt-cdn.net/images/zos-java-2-2729-43a263828d.png)

Быстро обнаруживайте аномалии с помощью [горячих точек времени отклика сервиса](/docs/observe/application-observability/services-classic/service-response-time-hotspots "Определите действия, которые потребляют наибольшее время отклика для каждого сервиса.").
...
Да, вы можете использовать модуль z/OS Java для мониторинга пользовательских метрик JMX. Для получения дополнительной информации см. [Мониторинг метрик JMX на z/OS](/docs/ingest-from/dynatrace-oneagent/installation-and-operation/zos/monitoring/zos-java-custom-jmx-metrics "Узнайте, как настроить мониторинг метрик JMX для ваших Java-приложений на z/OS.").

## Предварительные требования

Активируйте [функцию OneAgent](/docs/ingest-from/dynatrace-oneagent/oneagent-features "Управляйте функциями OneAgent глобально и по группам процессов.") **Forward Tag 4 trace context extension**.

## Загрузка

1. [Загрузите наборы данных продукта z/OS](/docs/ingest-from/dynatrace-oneagent/installation-and-operation/zos/installation/zosmf-installer/download-zos-datasets "Загрузите и установите наборы данных продукта Dynatrace для z/OS.") и извлеките JAR-файл (`dynatrace-oneagent-zos-java.jar`).
...
3. Создайте новый файл с именем `dtconfig.json` в папке USS z/OS, где расположен модуль.
   ...
   Вы можете найти имя подсистемы zDC в [параметрах SYSIN](/docs/ingest-from/dynatrace-oneagent/installation-and-operation/zos/installation/install-zdc#sysin-params "Настройка подсистемы z/OS Data Collection (zDC).") вашего JCL-члена SYSIN `ZDCSYSIN`. Имя подсистемы zDC по умолчанию — `MEPC`.
   ...
   Если `DEFAULT` установлен в `YES` в [параметрах SYSIN](/docs/ingest-from/dynatrace-oneagent/installation-and-operation/zos/installation/install-zdc#sysin-params "Настройка подсистемы z/OS Data Collection (zDC)."), вы также можете использовать `DEFAULT` в файле `dtconfig.json`.
...
4. Для провайдера сервисов CICS активируйте [функцию OneAgent](/docs/ingest-from/dynatrace-oneagent/oneagent-features "Управляйте функциями OneAgent глобально и по группам процессов.") **z/OS CICS z/OS Connect**.
5. Для провайдера сервисов IMS:

   1. Добавьте модуль IMS в **IMS Connect**, как описано в [Установка модуля IMS](/docs/ingest-from/dynatrace-oneagent/installation-and-operation/zos/installation/install-ims#install "Установка модуля Dynatrace IMS.").
   2. Активируйте [функцию OneAgent](/docs/ingest-from/dynatrace-oneagent/oneagent-features "Управляйте функциями OneAgent глобально и по группам процессов.") **z/OS IMS z/OS Connect**.
...
1. [Загрузите наборы данных продукта z/OS](/docs/ingest-from/dynatrace-oneagent/installation-and-operation/zos/installation/zosmf-installer/download-zos-datasets#download-pax "Загрузите и установите наборы данных продукта Dynatrace для z/OS.") и перенесите JAR-файл (`dynatrace-oneagent-zos-java.jar`) в среду USS z/OS в бинарном режиме.
