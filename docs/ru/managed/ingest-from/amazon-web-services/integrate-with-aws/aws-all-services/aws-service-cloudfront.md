---
title: Мониторинг Amazon CloudFront
...
scraped: 2026-03-05T21:31:16.509914
---

# Мониторинг Amazon CloudFront
...
Dynatrace собирает метрики для множества предварительно выбранных пространств имен, включая Amazon CloudFront. Вы можете просматривать метрики для каждого экземпляра сервиса, разделять метрики по нескольким измерениям и создавать пользовательские графики, которые можно закрепить на дашбордах.
...
CloudFront является глобальным сервисом; метрики доступны только в регионе `us-east-1`. Учетные данные должны иметь как минимум следующие разрешения для региона `us-east-1`:
...
Вы также можете просматривать метрики в веб-интерфейсе Dynatrace на дашбордах. Для этого сервиса нет готового дашборда, но вы можете [создать собственный дашборд](/docs/analyze-explore-automate/dashboards-classic/dashboards/create-dashboards "Узнайте, как создавать и редактировать дашборды Dynatrace.").
...
`DistributionId` является основным измерением.

| Название | Описание | Единица измерения | Статистика | Измерения | Рекомендуемая |
| --- | --- | --- | --- | --- | --- |
| 4xxErrorRate | Процент всех запросов зрителей, для которых HTTP-код ответа равен `4xx` | Процент | Среднее | DistributionId, Region |  |
| 5xxErrorRate | Процент всех запросов зрителей, для которых HTTP-код ответа равен `5xx` | Процент | Среднее | DistributionId, Region |  |
| BytesDownloaded | Общее количество байт, загруженных зрителями для запросов `GET`, `HEAD` и `OPTIONS` | Нет | Сумма | DistributionId, Region |  |
| BytesUploaded | Общее количество байт, отправленных зрителями на ваш источник через CloudFront с помощью запросов `POST` и `PUT` | Нет | Сумма | DistributionId, Region |  |
| Requests | Общее количество запросов зрителей, полученных CloudFront для всех HTTP-методов и для запросов HTTP и HTTPS | Нет | Сумма | DistributionId, Region | Доступна |
| TotalErrorRate | Процент всех запросов зрителей, для которых HTTP-код ответа равен `4xx` или `5xx` | Процент | Среднее | DistributionId, Region | Доступна |
