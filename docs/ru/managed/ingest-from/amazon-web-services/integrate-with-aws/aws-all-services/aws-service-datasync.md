...
scraped: 2026-03-06T21:33:15.477959
---

# Мониторинг AWS DataSync
...
Dynatrace собирает метрики для множества предварительно выбранных пространств имен, включая AWS DataSync. Вы можете просматривать метрики для каждого экземпляра сервиса, разделять метрики по нескольким измерениям и создавать пользовательские графики, которые можно закрепить на дашбордах.
...
* Обновленная [политика мониторинга AWS](/docs/ingest-from/amazon-web-services/integrate-with-aws/cloudwatch-metrics#monitoring-policy "Интеграция метрик из Amazon CloudWatch.") для включения дополнительных сервисов AWS.
  Для [обновления политики AWS IAM](https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies_manage-edit.html#edit-managed-policy-console) используйте приведенный ниже JSON, содержащий политику мониторинга (разрешения) для всех поддерживаемых сервисов.
...
| Название | Разрешения |
| --- | --- |
| Все контролируемые сервисы Amazon (обязательно) | `cloudwatch:GetMetricData`, `cloudwatch:GetMetricStatistics`, `cloudwatch:ListMetrics`, `sts:GetCallerIdentity`, `tag:GetResources`, `tag:GetTagKeys`, `ec2:DescribeAvailabilityZones` |
...
См. пример политики JSON для одного отдельного сервиса ниже.
...
* минимум [один агент](https://docs.aws.amazon.com/datasync/latest/userguide/create-agent-cli.html)
...
Чтобы узнать, как включить мониторинг сервиса, см. раздел [Включение мониторинга сервиса](/docs/ingest-from/amazon-web-services/integrate-with-aws/aws-metrics-ingest/aws-enable-service-monitoring "Включение мониторинга AWS в Dynatrace.").
...
После добавления сервиса в мониторинг готовый дашборд, содержащий все рекомендуемые метрики, автоматически появляется на странице **Дашборды**. Для поиска конкретных дашбордов отфильтруйте по **Preset**, а затем по **Name**.

![AWS presets](https://dt-cdn.net/images/image-26-1645-389f58aa89.png)

Для существующих контролируемых сервисов может потребоваться повторное сохранение учетных данных, чтобы готовый дашборд появился на странице **Дашборды**. Для повторного сохранения учетных данных перейдите в **Settings** > **Cloud and virtualization** > **AWS**, выберите нужный экземпляр AWS и нажмите **Save**.

Вы не можете вносить изменения в готовый дашборд напрямую, но можете клонировать и редактировать его. Чтобы клонировать дашборд, откройте меню (**...**) и выберите **Clone**.
...
Чтобы проверить доступность готовых дашбордов для каждого сервиса AWS, см. список ниже.

### Список доступности готовых дашбордов

| Сервис AWS | Готовый дашборд |
| --- | --- |
...
| Все контролируемые сервисы Amazon | Недоступен |
...
`TaskId` является основным измерением.

| Название | Описание | Единица измерения | Статистика | Измерения | Рекомендуемая |
| --- | --- | --- | --- | --- | --- |
| BytesPreparedDestination | Общее количество байт данных, подготовленных в целевом расположении | Байты | Сумма | TaskId |  |
| BytesPreparedSource | Общее количество байт данных, подготовленных в исходном расположении | Байты | Сумма | TaskId |  |
| BytesTransferred | Общее количество байт, переданных по сети при чтении агентом из исходного расположения в целевое | Байты | Сумма | TaskId | Доступна |
| BytesTransferred |  | Байты | Сумма | Region, AgentId |  |
| BytesVerifiedDestination | Общее количество байт данных, проверенных в целевом расположении | Байты | Сумма | TaskId |  |
| BytesVerifiedSource | Общее количество байт данных, проверенных в исходном расположении | Байты | Сумма | TaskId |  |
| BytesWritten | Общий логический размер всех файлов, перенесенных в целевое расположение | Байты | Сумма | TaskId | Доступна |
| BytesWritten |  | Байты | Сумма | Region, AgentId |  |
| FilesPreparedDestination | Общее количество файлов, подготовленных в целевом расположении | Количество | Сумма | TaskId |  |
| FilesPreparedSource | Общее количество файлов, подготовленных в исходном расположении | Количество | Сумма | TaskId |  |
| FilesTransferred | Фактическое количество файлов или метаданных, переданных по сети | Количество | Сумма | TaskId | Доступна |
| FilesTransferred |  | Количество | Сумма | Region, AgentId |  |
| FilesVerifiedDestination | Общее количество файлов, проверенных в целевом расположении | Количество | Сумма | TaskId | Доступна |
| FilesVerifiedSource | Общее количество файлов, проверенных в исходном расположении | Количество | Сумма | TaskId |  |
