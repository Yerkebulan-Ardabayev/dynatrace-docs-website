---
title: Мониторинг Amazon EFS (Elastic File System)
...
scraped: 2026-03-06T21:26:58.555496
---

# Мониторинг Amazon EFS (Elastic File System)
...
Метрики также можно просматривать в веб-интерфейсе Dynatrace на панелях мониторинга. Для этого сервиса стандартная панель недоступна, но вы можете [создать собственную панель](/docs/analyze-explore-automate/dashboards-classic/dashboards/create-dashboards "Узнайте, как создавать и редактировать панели мониторинга Dynatrace.").
...
`FileSystemId` — основное измерение.

| Название | Описание | Единица | Статистика | Измерения | Рекомендуется |
| --- | --- | --- | --- | --- | --- |
| BurstCreditBalance | Количество кредитов на всплески производительности файловой системы | Байты | Multi | FileSystemId | Доступна |
| ClientConnections | Количество клиентских подключений к файловой системе. При использовании стандартного клиента создаётся одно подключение на каждый смонтированный экземпляр Amazon EC2. | Количество | Sum | FileSystemId | Доступна |
| DataReadIOBytes | Количество байт для каждой операции чтения файловой системы | Байты | Multi | FileSystemId |  |
| DataReadIOBytes |  | Байты | Sum | FileSystemId |  |
| DataWriteIOBytes | Количество байт для каждой операции записи в файловую систему | Байты | Multi | FileSystemId |  |
| DataWriteIOBytes |  | Байты | Sum | FileSystemId |  |
| MetadataIOBytes | Количество байт для каждой операции с метаданными | Байты | Multi | FileSystemId |  |
| MetadataIOBytes |  | Байты | Sum | FileSystemId |  |
| PercentIOLimit | Показывает, насколько близко файловая система к достижению лимита ввода/вывода для режима производительности общего назначения | Процент | Multi | FileSystemId |  |
| PermittedThroughput | Максимально допустимая пропускная способность файловой системы | Байты/Секунда | Multi | FileSystemId |  |
| TotalIOBytes | Количество байт для каждой операции файловой системы, включая операции чтения данных, записи данных и работы с метаданными | Байты | Multi | FileSystemId |  |
| TotalIOBytes |  | Байты | Sum | FileSystemId | Доступна |
