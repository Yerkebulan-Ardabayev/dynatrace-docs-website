...
Dynatrace собирает метрики для множества предварительно выбранных пространств имён, включая AWS IoT Analytics. Вы можете просматривать метрики для каждого экземпляра сервиса, разделять метрики по нескольким измерениям и создавать пользовательские диаграммы, которые можно закрепить на ваших дашбордах.
...
* ActiveGate версии 1.197+
  ...
  + Для развёртываний Dynatrace Managed можно использовать любой тип ActiveGate.
    ...
    Для доступа на основе ролей (будь то развёртывание [SaaS](/docs/ingest-from/amazon-web-services/integrate-with-aws/cloudwatch-metrics#role-based-access "Integrate metrics from Amazon CloudWatch.") или [Managed](https://docs.dynatrace.com/managed/shortlink/aws-managed-deployment)) требуется [Environment ActiveGate](/docs/ingest-from/dynatrace-activegate/installation "Learn how to configure ActiveGate"), установленный на хосте Amazon EC2.
* Dynatrace версии 1.203+
* Обновлённая [политика мониторинга AWS](/docs/ingest-from/amazon-web-services/integrate-with-aws/cloudwatch-metrics#monitoring-policy "Integrate metrics from Amazon CloudWatch.") для включения дополнительных сервисов AWS.
...
Чтобы узнать, как включить мониторинг сервиса, см. [Включение мониторинга сервиса](/docs/ingest-from/amazon-web-services/integrate-with-aws/aws-metrics-ingest/aws-enable-service-monitoring "Enable AWS monitoring in Dynatrace.").
...
Для AWS IoT Analytics на странице обзора группы пользовательских устройств нет экземпляров (пользовательских устройств). Метрики сервиса находятся в разделе **Further details** на странице обзора группы пользовательских устройств.
...
| --- | --- | --- | --- | --- | --- |
| ActionExecution | Количество выполненных действий | Count | Sum | Region, ActionType, DatasetName | Доступен |
| ActionExecutionThrottled | Количество отклонённых (throttled) действий | Count | Sum | Region, ActionType, DatasetName | Доступен |
| ActivityExecutionError | Количество ошибок, возникших при выполнении активности конвейера | Count | Sum | Region, PipelineActivityName, PipelineActivityType, PipelineName | Доступен |
| IncomingMessages | Количество сообщений, поступающих в канал | Count | Sum | Region, ChannelName | Доступен |
| PipelineConcurrentExecutionCount | Количество активностей конвейера, выполняемых одновременно | Count | Sum | Region, PipelineName | Доступен |