...
scraped: 2026-03-02T21:31:36.137885
---
...
Dynatrace собирает метрики для множества предварительно выбранных пространств имён, включая Amazon Keyspaces. Вы можете просматривать метрики для каждого экземпляра сервиса, разделять метрики по нескольким измерениям и создавать пользовательские графики, которые можно закрепить на панелях мониторинга.
...
`Keyspace` является основным измерением.

| Название | Описание | Единица | Статистика | Измерения | Рекомендуемая |
| --- | --- | --- | --- | --- | --- |
| AccountMaxReads | The maximum number of read capacity units that can be used by an account | Count | Maximum | Region | Доступна |
| AccountMaxTableLevelReads | The maximum number of read capacity units that can be used by a table of an account | Count | Maximum | Region | Доступна |
| AccountMaxTableLevelWrites | The maximum number of write capacity units that can be used by a table of an account | Count | Maximum | Region | Доступна |
| AccountMaxWrites | The maximum number of write capacity units that can be used by an account | Count | Maximum | Region | Доступна |
| AccountProvisionedReadCapacityUtilization | The percentage of provisioned read capacity units utilized by an account | Percent | Multi | Region | Доступна |
| AccountProvisionedWriteCapacityUtilization | The percentage of provisioned write capacity units utilized by an account | Percent | Multi | Region | Доступна |
| ConditionalCheckFailed | The number of failed lightweight transaction (LWT) write requests | Count | Multi | Keyspace, Operation, TableName | Доступна |
| ConditionalCheckFailed |  | Count | Sum | Keyspace, Operation, TableName | Доступна |
| ConditionalCheckFailed |  | Count | Count | Keyspace, Operation, TableName | Доступна |
| ConsumedReadCapacityUnits | The number of read capacity units consumed over the specified time period | Count | Multi | Keyspace, TableName | Доступна |
| ConsumedReadCapacityUnits |  | Count | Sum | Keyspace, TableName | Доступна |
| ConsumedReadCapacityUnits |  | Count | Count | Keyspace, TableName | Доступна |
| ConsumedWriteCapacityUnits | The number of write capacity units consumed over the specified time period | Count | Multi | Keyspace, TableName | Доступна |
| ConsumedWriteCapacityUnits |  | Count | Sum | Keyspace, TableName | Доступна |
| ConsumedWriteCapacityUnits |  | Count | Count | Keyspace, TableName | Доступна |
| MaxProvisionedTableReadCapacityUtilization | The percentage of provisioned read capacity units utilized by the highest provisioned read table of an account | Percent | Multi | Region | Доступна |
| MaxProvisionedTableWriteCapacityUtilization | The percentage of provisioned write capacity utilized by the highest provisioned write table of an account | Percent | Multi | Region | Доступна |
| ProvisionedReadCapacityUnits | The number of provisioned read capacity units for a table | Count | Multi | Keyspace, TableName | Доступна |
| ProvisionedWriteCapacityUnits | The number of provisioned write capacity units for a table | Count | Multi | Keyspace, TableName | Доступна |
| ReturnedItemCountBySelect | The number of rows returned by SELECT operations during the specified time period | Count | Multi | Keyspace, Operation, TableName | Доступна |
| ReturnedItemCountBySelect |  | Count | Sum | Keyspace, Operation, TableName | Доступна |
| ReturnedItemCountBySelect |  | Count | Count | Keyspace, Operation, TableName | Доступна |
| SuccessfulRequestCount | The number of successful requests | Count | Multi | Keyspace, Operation, TableName | Доступна |
| SuccessfulRequestCount |  | Count | Sum | Keyspace, Operation, TableName | Доступна |
| SuccessfulRequestCount |  | Count | Count | Keyspace, Operation, TableName | Доступна |
| SuccessfulRequestLatency | The successful requests to Amazon Keyspaces during the specified time period | Milliseconds | Multi | Keyspace, Operation, TableName | Доступна |
| SuccessfulRequestLatency |  | Milliseconds | Sum | Keyspace, Operation, TableName | Доступна |
| SuccessfulRequestLatency |  | Milliseconds | Count | Keyspace, Operation, TableName | Доступна |
| SystemErrors | The requests to Amazon Keyspaces that generate a ServerError during the specified time period | Count | Sum | Keyspace, Operation, TableName | Доступна |
| SystemErrors |  | Count | Count | Keyspace, Operation, TableName | Доступна |
| UserErrors | Requests to Amazon Keyspaces that generate an `InvalidRequest` error during the specified time period | Count | Sum | Keyspace, Operation, TableName | Доступна |
| UserErrors |  | Count | Count | Keyspace, Operation, TableName | Доступна |

## Ограничения

* Для данного сервиса нет API. Тегирование доступно на пользовательских устройствах (keyspaces), так как используется отдельный API Amazon.