---
title: Мониторинг Amazon Translate
...
scraped: 2026-03-06T21:27:02.235534
---

# Мониторинг Amazon Translate
...
Dynatrace собирает метрики для множества предварительно выбранных пространств имен, включая Amazon Translate. Вы можете просматривать метрики для каждого экземпляра сервиса, разделять метрики по нескольким измерениям и создавать пользовательские графики, которые можно закрепить на дашбордах.
...
Если вы не хотите добавлять разрешения для всех сервисов, а хотите выбрать разрешения для определенных сервисов, обратитесь к таблице ниже. Таблица содержит набор разрешений, необходимых для [всех облачных сервисов AWS](../../../../../ingest-from/amazon-web-services/integrate-with-aws/aws-all-services.md "Мониторинг всех облачных сервисов AWS с помощью Dynatrace и просмотр доступных метрик."), а также для каждого поддерживаемого сервиса список дополнительных разрешений, специфичных для этого сервиса.
...
Для Amazon Translate на странице обзора группы пользовательских устройств отсутствуют экземпляры (пользовательские устройства). Метрики сервиса находятся в разделе **Further details** на странице обзора группы пользовательских устройств.
...
| --- | --- | --- | --- | --- | --- |
| CharacterCount | Количество оплачиваемых символов в запросах | Count | Sum | Region, LanguagePair, Operation | Доступна |
| ResponseTime | Время, затраченное на ответ на запрос | Milliseconds | Multi | Region, LanguagePair, Operation | Доступна |
| ServerErrorCount | Количество серверных ошибок. Диапазон HTTP-кодов ответа для серверной ошибки: `500`-`599` | Count | Sum | Region, Operation | Доступна |
| SuccessfulRequestCount | Количество успешных запросов на перевод. Код ответа для успешного запроса: `200`-`299` | Count | Sum | Region, Operation | Доступна |
| ThrottledCount | Количество запросов, подвергшихся ограничению | Count | Sum | Region, Operation | Доступна |
| UserErrorCount | Количество пользовательских ошибок. Диапазон HTTP-кодов ответа для пользовательской ошибки: `400`-`499` | Count | Sum | Region, Operation | Доступна |