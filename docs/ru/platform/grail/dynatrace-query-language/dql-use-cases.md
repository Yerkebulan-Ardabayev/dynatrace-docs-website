...
Можно использовать команду `parse` в сочетании с Dynatrace Pattern Language для разбора JSON-объектов. Запрос извлекает поле `payment_provider` из JSON-данных и группирует количество транзакций по каждому провайдеру с помощью команды `summarize`, а также вычисляет общее количество транзакций. Доля отображается в отдельном столбце рядом с количеством транзакций каждого провайдера.
...
В DQL-запросе необходимо использовать матчер DPL для извлечения всего XML-элемента:
