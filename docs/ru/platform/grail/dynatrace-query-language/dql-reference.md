...
Запрос DQL содержит одну или несколько [команд](commands.md "Список команд DQL."), каждая из которых возвращает табличный результат, содержащий записи (строки) и поля (столбцы). Все команды последовательно соединяются символом | (pipe). Данные передаются (или «пайпятся») от одной команды к следующей. На каждом шаге данные фильтруются или обрабатываются, а затем передаются на следующий шаг.
...
* **Строка 1**
  ...
  Вы извлекаете данные логов с помощью команды [`fetch`](commands/data-source-commands.md#fetch "Команды источников данных DQL"). Кроме того, необязательный параметр `from:` указывает начальную метку времени запроса.
* **Строка 2**
...
* **Строка 3**
  ...
  Команда [`filter`](commands/filtering-commands.md#filter "Команды фильтрации и поиска DQL") фильтрует записи логов на основе функции [`endsWith`](functions/string-functions.md#endsWith "Список строковых функций DQL."), которая извлекает файлы логов, имена которых заканчиваются заданной строкой (строка `pgi.log`).
* **Строка 4**
  ...
  Мы используем команду [`parse`](commands/extraction-and-parsing-commands.md#parse "Команды извлечения DQL") для извлечения пар ключ-значение, содержащих статистику выполнения, из необработанной текстовой строки лога. В данном случае она добавляет поля `IP address`, `payload` и `http_status` к результату и преобразует их типы данных в требуемые форматы.
* **Строки 5, 6, 7, 8**
  ...
  Команда [`summarize`](commands/aggregation-commands.md#summarize "Команды агрегации DQL") является ключевым элементом DQL, поскольку позволяет выполнять множественные агрегации по одному или нескольким полям. Этот запрос группирует результаты по `ip` и `host.name`. Полученные записи включают общее значение payload, вычисленное с помощью функции [`sum`](functions/aggregation-functions.md#sum "Список функций агрегации DQL."), и два столбца, вычисленных с помощью функции [`countif`](functions/aggregation-functions.md#countIf "Список функций агрегации DQL."):
...
* **Строка 10**
  ...
  С помощью команды [`fields`](commands/selection-and-modification-commands.md#fields "Команды выбора и модификации DQL") вы определяете, какие поля необходимо извлечь.
* **Строка 11**
  ...
  Команда [`sort`](commands/ordering-commands.md#sort "Команды сортировки DQL") используется для финализации результата. В данном случае результаты сортируются по количеству неудачных запросов от наибольшего к наименьшему.

## Основные строительные блоки DQL

* [Команды](commands.md "Список команд DQL.")
* [Функции](functions.md "Список функций DQL.")
  Функции могут использоваться для выполнения любых необходимых вычислений над полями [команд DQL](commands.md "Список команд DQL.").

* [Типы данных](data-types.md "Список типов данных DQL.")
...
* [Сравнение DQL с SQL и другими языками](dql-comparison.md "Сравнение DQL с другими языками запросов.")
