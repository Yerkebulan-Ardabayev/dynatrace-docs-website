...
3. Пройдите аутентификацию с помощью вашего API-токена.
   ...
   Подробнее см. в разделе [Аутентификация](/docs/manage/identity-access-management/access-tokens-and-oauth-clients/platform-tokens "Create personalised platform tokens to access Dynatrace services via the API in your user context.").
...
См., когда следует [создавать пользовательские бакеты и как предоставлять к ним доступ](/docs/platform/upgrade#organize-your-data "Use the power of Grail, AppEngine, and AutomationEngine to take advantage of improvements in storing and analyzing observability and security data.").
