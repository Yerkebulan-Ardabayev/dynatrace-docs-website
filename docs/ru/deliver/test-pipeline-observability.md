...
* Вы знаете, как настроить дашборды Dynatrace и системы оповещения.
...
* Вы знаете, как настроить автоматизированные рабочие процессы в Dynatrace для автоматизации реагирования на события тестового пайплайна.
...
1. Сбор и загрузка данных о выполнении тестов
   ...
   На основе ваших исходных инструментов и фреймворков тестовых пайплайнов соберите данные о выполнении тестов. Загрузите эти данные как бизнес-события в Dynatrace с помощью Business events API. Например, вы можете написать вспомогательные функции или отправлять запросы напрямую в Business events API.
...
3. Настройка рабочего процесса
   ...
   Рабочий процесс может быть запущен при отправке бизнес-события после выполнения теста. Затем вы можете создавать последующие действия на основе загруженных данных тестирования. Например, вы можете определить владельцев неисправного ресурса с помощью действия Ownership, а затем уведомить владельцев тестов о результатах через действие Slack в рабочих процессах.
...
4. Настройка обнаружения аномалий
   ...
   Вы также можете использовать действие ИИ в рабочих процессах — Прогнозное обслуживание облачных дисков в вашем рабочем процессе для обнаружения аномалий в выполнении тестов/сборок, таких как необычно длительное время выполнения по сравнению с историческими данными. Эта информация помогает поддерживать и оптимизировать тестовые среды.
