---
title: Terraform tutorials overview
source: https://www.dynatrace.com/docs/deliver/configuration-as-code/terraform/tutorials
scraped: 2026-03-05T21:40:31.962704
---

# Обзор руководств по Terraform

# Обзор руководств по Terraform

* Последняя версия Dynatrace
* Пояснение
* 1 минута чтения
* Опубликовано 03 ноября 2025 г.

Terraform — это многофункциональный инструмент, который можно сочетать с Configuration as Code для достижения отличных результатов.

Мы предоставляем руководство, которое поможет вам [настроить автоматические уведомления с помощью Terraform и Configuration as Code](/docs/deliver/configuration-as-code/terraform/tutorials/terraform-tutorial-set-up-automated-notification "Настройка автоматических уведомлений с помощью Terraform и Configuration as Code.").
