...
Оно должно быть уникальным для одного и того же [configType](../reference/commands-saas.md "Как использовать CLI-приложение Monaco, включая аргументы и опции.") и [проекта](monaco-manage-resources.md#projects "Список ресурсов Monaco.").
...
Подробный список см. в разделе [YAML-файл конфигурации Monaco — список полей типов](yaml-configuration-saas-type-fields.md "Список полей типов в YAML-файле конфигурации Monaco.").
...
* `Settings API`
  ...
  + Необязательный `insertAfter`
    ...
    Дополнительную информацию см. в разделе [Объект SettingsObjectUpdate](../../../../dynatrace-api/environment-api/settings/objects/put-object.md#openapi-parameter-body-objects-openapienv2 "Редактирование объекта настроек через Dynatrace API.").
...
* `schema` и `schemaVersion`: применимо для типа `settings`.
  Определяет конкретную схему настроек, например [пользовательские оповещения](../../../../dynatrace-intelligence/anomaly-detection/anomaly-detection-app.md "Изучите конфигурации обнаружения аномалий с помощью приложения Anomaly Detection.").
...
* `id`: применимо для типа `document`.
  ...
  Если он указан, его также необходимо напрямую использовать в [файле удаления](../configuration.md#file-structure-for-direct-reference "Управление файлами конфигурации Dynatrace с помощью Monaco с набором проектов и манифестом развёртывания."), если вы хотите удалить документ позже.
...
Дополнительную информацию см. в описании исключительных случаев для JSON-шаблонов [Работа с командами Dynatrace Monaco CLI для Latest Dynatrace](../reference/commands-saas.md "Как использовать CLI-приложение Monaco, включая аргументы и опции.") и обратитесь к документации API в случае сомнений.
...
* [YAML-файл конфигурации Monaco — список полей типов](yaml-configuration-saas-type-fields.md "Список полей типов в YAML-файле конфигурации Monaco.")
