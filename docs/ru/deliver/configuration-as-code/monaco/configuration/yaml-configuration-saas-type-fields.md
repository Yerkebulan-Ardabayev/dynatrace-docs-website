...
Settings API определяются через `schema`, `scope` и необязательный `schemaVersion`.
...
Конфигурации типа `automation` представляют ресурсы, связанные с рабочими процессами (Workflows).
...
Помимо определения и создания нового бакета хранения, необходимо дополнительное правило бакета через Settings API для указания того, какие данные в нём хранятся.
...
Начиная с CLI Monaco Dynatrace версии 2.15.0+, поддерживается тип `document`, который представляет API для дашбордов и ноутбуков через API.").
...
Если это поле задано, в файле удаления необходима прямая ссылка через `objectId`, если вы захотите удалить документ позже.
...
Monaco не скачивает готовые документы.
...
Дополнительную информацию о совместном использовании документов или смене владельца см. в разделах Дашборды, Ноутбуки или Лаунчпады.
...
Этот ресурс устарел и перемещён в ![Settings](https://dt-cdn.net/images/settings-icon-256-38e1321b51.webp "Settings") **Settings**. Дополнительную информацию см. в разделе OpenPipeline API.
...
Сегменты используются для структурирования и фильтрации данных для создания адаптированных представлений в Dynatrace.
...
Настройка и мониторинг целей уровня обслуживания с помощью Dynatrace, основанных на Grail и использующих DQL, управляются через `type` `slo-v2`.

Тип конфигурации `slo-v2` отличается от существующего типа `slo` и представляет SLO, использующие Grail, как описано в обзоре Настройка и мониторинг целей уровня обслуживания с помощью Dynatrace.
...
* Структура конфигурационного YAML-файла Monaco
