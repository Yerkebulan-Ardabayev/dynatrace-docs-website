...
3. Определите конечные точки связи ActiveGate и аутентификацию. Используйте API GET информация о подключении для ActiveGate.
...
Образ Dynatrace ActiveGate поддерживается на различных версиях Kubernetes и OpenShift. Полный список см. в разделе Поддержка технологий - Kubernetes.
...
Обратите внимание, что мультиархитектурные контейнерные образы Dynatrace, обеспечивающие совместимость с различными платформами, доступны только из публичных реестров. Встроенный реестр Dynatrace предоставляет только образы для x86-64.
...
Подробности о проверке см. в разделе Проверка аттестации Software Bill of Materials (SBOM).
...
4. Измените YAML-файл развертывания.
   ...
   * `<REPOSITORY_URL>` на один из поддерживаемых реестров
...
* Для мониторинга Kubernetes/OpenShift выберите один из следующих вариантов:
  ...
  + Используйте Dynatrace Operator
  + Разверните ActiveGate непосредственно как StatefulSet
* Для сбора логов из Kubernetes используйте Log Monitoring.

## Образы, совместимые с FIPS

ActiveGate версии 1.315+

Доступен специальный образ ActiveGate, совместимый с FIPS. См. Совместимость ActiveGate с FIPS для получения информации о требованиях, ограничениях, где получить образ и как проверить развертывание.
