...
1. Задайте следующие переменные окружения, заменив заполнители (`<...>`) своими значениями.
   ...
   * Для `TARGET_URL` введите URL вашей среды: `https://<your_environment_ID>.live.dynatrace.com`. Чтобы узнать, как определить идентификатор вашей среды для развёртывания SaaS или Managed, см. идентификатор среды.
