...
scraped: 2026-03-06T21:28:55.302593
---
...
Dynatrace собирает метрики для нескольких предварительно выбранных пространств имён, включая Amazon EC2 Auto Scaling. Вы можете просматривать метрики для каждого экземпляра сервиса, разделять метрики по нескольким измерениям и создавать пользовательские графики, которые можно закрепить на дашбордах.
...
Этот сервис осуществляет мониторинг Amazon EC2 Auto Scaling. Для данного сервиса отображаются только экземпляры с метриками.

Помните, что если у вас настроен сервис Amazon EC2 Auto Scaling, вы не можете одновременно включить сервис Amazon EC2 Auto Scaling (built-in). Для мониторинга расширенных метрик необходимо выбрать **Amazon EC2 Auto Scaling**, в противном случае Amazon EC2 Auto Scaling (built-in) будет предоставлять только базовый подсчёт экземпляров EC2 в группе масштабирования.

По умолчанию метрики Auto Scaling Group не передаются. Чтобы они передавались, необходимо включить их в консоли **AWS Auto Scaling Group**.
...
| --- | --- | --- | --- | --- | --- |
| GroupAndWarmPoolDesiredCapacity | Желаемая ёмкость группы Auto Scaling и пула прогрева вместе. | Count | Average | AutoScalingGroupName |  |
| GroupAndWarmPoolTotalCapacity | Общая ёмкость группы Auto Scaling и пула прогрева вместе. Включает экземпляры, которые запущены, остановлены, ожидают, завершаются или находятся в эксплуатации. | Count | Average | AutoScalingGroupName |  |
| GroupDesiredCapacity | Количество экземпляров, которое группа Auto Scaling пытается поддерживать | Count | Average | AutoScalingGroupName | Применимо |
| GroupInServiceCapacity | Количество единиц ёмкости, работающих в составе группы Auto Scaling | Count | Average | AutoScalingGroupName |  |
| GroupInServiceInstances | Количество экземпляров, работающих в составе группы Auto Scaling. Эта метрика не включает экземпляры, которые ожидают или завершаются. | Count | Average | AutoScalingGroupName | Применимо |
| GroupMaxSize | Максимальный размер группы Auto Scaling | Count | Average | AutoScalingGroupName | Применимо |
| GroupMinSize | Минимальный размер группы Auto Scaling | Count | Average | AutoScalingGroupName | Применимо |
| GroupPendingCapacity | Количество единиц ёмкости, ожидающих запуска | Count | Average | AutoScalingGroupName |  |
| GroupPendingInstances | Количество ожидающих экземпляров. Ожидающий экземпляр ещё не введён в эксплуатацию. Эта метрика не включает экземпляры, которые находятся в эксплуатации или завершаются. | Count | Average | AutoScalingGroupName | Применимо |
| GroupStandbyCapacity | Количество единиц ёмкости в режиме ожидания | Count | Average | AutoScalingGroupName |  |
| GroupStandbyInstances | Количество экземпляров в режиме ожидания. Экземпляры в этом состоянии по-прежнему работают, но не находятся в активной эксплуатации. | Count | Average | AutoScalingGroupName | Применимо |
| GroupTerminatingCapacity | Количество единиц ёмкости в процессе завершения | Count | Average | AutoScalingGroupName |  |
| GroupTerminatingInstances | Количество экземпляров в процессе завершения. Эта метрика не включает экземпляры, которые находятся в эксплуатации или ожидают. | Count | Average | AutoScalingGroupName | Применимо |
| GroupTotalCapacity | Общее количество единиц ёмкости в группе Auto Scaling | Count | Average | AutoScalingGroupName |  |
| GroupTotalInstances | Общее количество экземпляров в группе Auto Scaling. Эта метрика определяет количество экземпляров, находящихся в эксплуатации, ожидающих и завершающихся. | Count | Average | AutoScalingGroupName | Применимо |
| WarmPoolDesiredCapacity | Объём ёмкости, который Amazon EC2 Auto Scaling пытается поддерживать в пуле прогрева. | Count | Average | AutoScalingGroupName |  |
| WarmPoolMinSize | Минимальный размер пула прогрева. | Count | Average | AutoScalingGroupName |  |
| WarmPoolPendingCapacity | Объём ёмкости в пуле прогрева, ожидающей запуска. Эта метрика не включает экземпляры, которые запущены, остановлены или завершаются. | Count | Average | AutoScalingGroupName |  |
| WarmPoolTerminatingCapacity | Объём ёмкости в пуле прогрева в процессе завершения. Эта метрика не включает экземпляры, которые запущены, остановлены или ожидают. | Count | Average | AutoScalingGroupName |  |
| WarmPoolTotalCapacity | Общая ёмкость пула прогрева, включая экземпляры, которые запущены, остановлены, ожидают или завершаются. | Count | Average | AutoScalingGroupName |  |
| WarmPoolWarmedCapacity | Объём ёмкости, доступной для включения в группу Auto Scaling при масштабировании. Эта метрика не включает экземпляры, которые ожидают или завершаются. | Count | Average | AutoScalingGroupName |  |

## Ограничения

Тегирование недоступно для Amazon EC2 Auto Scaling.
