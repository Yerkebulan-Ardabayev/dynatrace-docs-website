...
Dynatrace собирает метрики для нескольких предварительно выбранных пространств имён, включая Amazon Elastic Kubernetes Service (EKS). Вы можете просматривать графики для каждого экземпляра сервиса, с набором измерений, и создавать пользовательские графики, которые можно закрепить на дашбордах.
...
* Для развёртываний Dynatrace SaaS требуется Environment ActiveGate или Multi-environment ActiveGate.
  ...
  Для доступа на основе ролей в развёртывании [SaaS](../cloudwatch-metrics.md#role-based-access "Интеграция метрик из Amazon CloudWatch.") необходим Environment ActiveGate, установленный на хосте Amazon EC2.
...
Если вы не хотите добавлять разрешения для всех сервисов и предпочитаете выбрать разрешения для определённых сервисов, обратитесь к таблице ниже. Таблица содержит набор разрешений, необходимых для всех облачных сервисов AWS, а также для каждого поддерживаемого сервиса список дополнительных разрешений, специфичных для этого сервиса.
...
* [Настройка Container Insights для Amazon EKS или Kubernetes](https://dt-url.net/bc03sar).
...
Container Insights необходимо настроить в кластере для получения отчётов о метриках. Подробности см. в [документации AWS](https://dt-url.net/aw030yi).
...
| --- | --- | --- | --- | --- | --- |
| cluster\_failed\_node\_count | Количество отказавших рабочих узлов в кластере | Count | Average | ClusterName | Применимо |
| cluster\_node\_count | Общее количество рабочих узлов в кластере | Count | Average | ClusterName | Применимо |
| namespace\_number\_of\_running\_pods | Количество запущенных подов в пространстве имён для ресурса, указанного используемыми измерениями | Count | Average | ClusterName, Namespace | Применимо |
| node\_cpu\_limit | Максимальное количество единиц CPU, которое может быть назначено одному узлу в этом кластере | None | Multi | ClusterName | Применимо |
| node\_cpu\_reserved\_capacity | Процент единиц CPU, зарезервированных для компонентов узла, таких как kubelet, kube-proxy и Docker | Percent | Multi | ClusterName, InstanceId, NodeName |  |
| node\_cpu\_reserved\_capacity |  | Percent | Multi | ClusterName | Применимо |
| node\_cpu\_usage\_total | Количество используемых единиц CPU на узлах кластера | None | Multi | ClusterName | Применимо |
| node\_cpu\_utilization | Общий процент используемых единиц CPU на узлах кластера | Percent | Multi | ClusterName, InstanceId, NodeName | Применимо |
| node\_cpu\_utilization |  | Percent | Multi | ClusterName |  |
| node\_filesystem\_utilization | Общий процент используемой ёмкости файловой системы на узлах кластера | Percent | Multi | ClusterName, InstanceId, NodeName | Применимо |
| node\_filesystem\_utilization |  | Percent | Multi | ClusterName |  |
| node\_memory\_limit | Максимальный объём памяти в байтах, который может быть назначен одному узлу в этом кластере | Bytes | Multi | ClusterName | Применимо |
| node\_memory\_reserved\_capacity | Процент памяти, используемой в данный момент на узлах кластера | Percent | Multi | ClusterName, InstanceId, NodeName |  |
| node\_memory\_reserved\_capacity |  | Percent | Multi | ClusterName | Применимо |
| node\_memory\_utilization | Процент памяти, используемой в данный момент узлом или узлами | Percent | Multi | ClusterName, InstanceId, NodeName | Применимо |
| node\_memory\_utilization |  | Percent | Multi | ClusterName |  |
| node\_memory\_working\_set | Объём памяти в байтах, используемой в рабочем наборе узлов кластера | Bytes | Multi | ClusterName | Применимо |
| node\_network\_total\_bytes | Общее количество байт в секунду, переданных и полученных по сети на узел в кластере | Bytes/Second | Multi | ClusterName, InstanceId, NodeName | Применимо |
| node\_network\_total\_bytes |  | Bytes/Second | Multi | ClusterName | Применимо |
| node\_number\_of\_running\_containers | Количество запущенных контейнеров на узел в кластере | Count | Average | ClusterName, InstanceId, NodeName | Применимо |
| node\_number\_of\_running\_containers |  | Count | Average | ClusterName |  |
| node\_number\_of\_running\_pods | Количество запущенных подов на узел в кластере | Count | Average | ClusterName, InstanceId, NodeName | Применимо |
| node\_number\_of\_running\_pods |  | Count | Average | ClusterName |  |
| pod\_cpu\_reserved\_capacity | Ёмкость CPU, зарезервированная для каждого пода в кластере | Percent | Multi | ClusterName, Namespace, PodName |  |
| pod\_cpu\_reserved\_capacity |  | Percent | Multi | ClusterName |  |
| pod\_cpu\_utilization | Процент используемых единиц CPU подами | Percent | Multi | ClusterName, Namespace | Применимо |
| pod\_cpu\_utilization |  | Percent | Multi | ClusterName, Namespace, PodName |  |
| pod\_cpu\_utilization |  | Percent | Multi | ClusterName |  |
| pod\_cpu\_utilization\_over\_pod\_limit | Процент используемых единиц CPU подами сверх лимита пода | Percent | Multi | ClusterName, Namespace |  |
| pod\_cpu\_utilization\_over\_pod\_limit |  | Percent | Multi | ClusterName, Namespace, PodName | Применимо |
| pod\_cpu\_utilization\_over\_pod\_limit |  | Percent | Multi | ClusterName |  |
| pod\_memory\_reserved\_capacity | Процент памяти, зарезервированной для подов | Percent | Multi | ClusterName, Namespace, PodName |  |
| pod\_memory\_reserved\_capacity |  | Percent | Multi | ClusterName |  |
| pod\_memory\_utilization | Процент памяти, используемой в данный момент подом или подами | Percent | Multi | ClusterName, Namespace | Применимо |
| pod\_memory\_utilization |  | Percent | Multi | ClusterName, Namespace, PodName |  |
| pod\_memory\_utilization |  | Percent | Multi | ClusterName |  |
| pod\_memory\_utilization\_over\_pod\_limit | Процент памяти, используемой подами сверх лимита пода | Percent | Multi | ClusterName, Namespace |  |
| pod\_memory\_utilization\_over\_pod\_limit |  | Percent | Multi | ClusterName, Namespace, PodName | Применимо |
| pod\_memory\_utilization\_over\_pod\_limit |  | Percent | Multi | ClusterName |  |
| pod\_network\_rx\_bytes | Количество байт в секунду, получаемых по сети подом | Bytes/Second | Multi | ClusterName, Namespace |  |
| pod\_network\_rx\_bytes |  | Bytes/Second | Multi | ClusterName, Namespace, PodName | Применимо |
| pod\_network\_rx\_bytes |  | Bytes/Second | Multi | ClusterName |  |
| pod\_network\_tx\_bytes | Количество байт в секунду, передаваемых по сети подом | Bytes/Second | Multi | ClusterName, Namespace |  |
| pod\_network\_tx\_bytes |  | Bytes/Second | Multi | ClusterName, Namespace, PodName | Применимо |
| pod\_network\_tx\_bytes |  | Bytes/Second | Multi | ClusterName |  |
| pod\_number\_of\_container\_restarts | Общее количество перезапусков контейнеров в поде | Count | Sum | ClusterName, Namespace, PodName | Применимо |
| service\_number\_of\_running\_pods | Количество подов, обслуживающих сервис или сервисы в кластере | Count | Average | ClusterName, Namespace, Service |  |
