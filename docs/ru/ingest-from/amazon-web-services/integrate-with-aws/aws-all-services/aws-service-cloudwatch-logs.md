---
title: Мониторинг Amazon CloudWatch Logs
...
`LogGroupName` является основным измерением.

| Название | Описание | Единица | Статистика | Измерения | Рекомендуется |
| --- | --- | --- | --- | --- | --- |
| DeliveryErrors | Количество событий журнала, для которых CloudWatch Logs получил ошибку при пересылке данных в целевое назначение подписки | Count | Sum | LogGroupName, DestinationType, FilterName | Применимо |
| DeliveryThrottling | Количество событий журнала, для которых CloudWatch Logs был ограничен при пересылке данных в целевое назначение подписки | Count | Sum | LogGroupName, DestinationType, FilterName | Применимо |
| ForwardedBytes | Объем событий журнала в сжатых байтах, перенаправленных в целевое назначение подписки | Bytes | Sum | LogGroupName, DestinationType, FilterName | Применимо |
| ForwardedLogEvents | Количество событий журнала, перенаправленных в целевое назначение подписки | Count | Sum | LogGroupName, DestinationType, FilterName | Применимо |
| IncomingBytes | Объем событий журнала в несжатых байтах, загруженных в CloudWatch Logs. При использовании с измерением `LogGroupName` это объем событий журнала в несжатых байтах, загруженных в группу журналов. | Bytes | Sum | LogGroupName | Применимо |
| IncomingBytes |  | Bytes | Sum | Region | Применимо |
| IncomingLogEvents | Количество событий журнала, загруженных в CloudWatch Logs. При использовании с измерением `LogGroupName` это количество событий журнала, загруженных в группу журналов. | Count | Sum | LogGroupName | Применимо |
| IncomingLogEvents |  | Count | Sum | Region | Применимо |