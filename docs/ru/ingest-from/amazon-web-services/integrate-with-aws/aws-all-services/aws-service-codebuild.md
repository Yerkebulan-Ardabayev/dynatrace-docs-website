---
title: Мониторинг AWS CodeBuild
...
scraped: 2026-03-02T21:22:20.312939
---

* 4-min read

Dynatrace собирает метрики для множества предварительно выбранных пространств имен, включая AWS CodeBuild. Вы можете просматривать метрики для каждого экземпляра сервиса, разделять метрики по нескольким измерениям и создавать пользовательские графики, которые можно закрепить на панелях мониторинга.
...
`ProjectName` является основным измерением.

| Название | Описание | Единица | Статистика | Измерения | Рекомендуется |
| --- | --- | --- | --- | --- | --- |
| BuildDuration | Измеряет продолжительность фазы `BUILD` сборки | Seconds | Multi | Region |  |
| BuildDuration |  | Seconds | Multi | ProjectName | Применимо |
| Builds | Измеряет количество запущенных сборок | Count | Sum | Region | Применимо |
| Builds |  | Count | Sum | ProjectName | Применимо |
| CPUUtilized | Количество единиц ЦП выделенной обработки, используемых контейнером сборки | None | Multi | ProjectName |  |
| CPUUtilized |  | None | Multi | ProjectName, BuildId, BuildNumber |  |
| CPUUtilizedPercent | Процент выделенной обработки, используемый контейнером сборки | Percent | Multi | ProjectName |  |
| CPUUtilizedPercent |  | Percent | Multi | ProjectName, BuildId, BuildNumber | Применимо |
| DownloadSourceDuration | Измеряет продолжительность фазы `DOWNLOAD_SOURCE` сборки | Seconds | Multi | Region |  |
| DownloadSourceDuration |  | Seconds | Multi | ProjectName |  |
| Duration | Измеряет продолжительность всех сборок за время | Seconds | Multi | Region |  |
| Duration |  | Seconds | Multi | ProjectName | Применимо |
| FailedBuilds | Измеряет количество сборок, завершившихся неудачей из-за ошибки клиента или тайм-аута | Count | Sum | Region | Применимо |
| FailedBuilds |  | Count | Sum | ProjectName | Применимо |
| FinalizingDuration | Измеряет продолжительность фазы `FINALIZING` сборки | Seconds | Multi | Region |  |
| FinalizingDuration |  | Seconds | Multi | ProjectName |  |
| InstallDuration | Измеряет продолжительность фазы `INSTALL` сборки | Seconds | Multi | Region |  |
| InstallDuration |  | Seconds | Multi | ProjectName |  |
| MemoryUtilized | Количество мегабайт памяти, используемых контейнером сборки | Megabytes | Multi | ProjectName |  |
| MemoryUtilized |  | Megabytes | Multi | ProjectName, BuildId, BuildNumber |  |
| MemoryUtilizedPercent | Процент выделенной памяти, используемый контейнером сборки | Percent | Multi | ProjectName |  |
| MemoryUtilizedPercent |  | Percent | Multi | ProjectName, BuildId, BuildNumber | Применимо |
| PostBuildDuration | Измеряет продолжительность фазы `POST_BUILD` сборки | Seconds | Multi | Region |  |
| PostBuildDuration |  | Seconds | Multi | ProjectName |  |
| PreBuildDuration | Измеряет продолжительность фазы `PRE_BUILD` сборки | Seconds | Multi | Region |  |
| PreBuildDuration |  | Seconds | Multi | ProjectName |  |
| ProvisioningDuration | Измеряет продолжительность фазы `PROVISIONING` сборки | Seconds | Multi | Region |  |
| ProvisioningDuration |  | Seconds | Multi | ProjectName |  |
| QueuedDuration | Измеряет продолжительность фазы `QUEUED` сборки | Seconds | Multi | Region |  |
| QueuedDuration |  | Seconds | Multi | ProjectName | Применимо |
| StorageReadBytes | Скорость чтения хранилища, используемая контейнером сборки | Bytes/Second | Multi | ProjectName |  |
| StorageReadBytes |  | Bytes/Second | Multi | ProjectName, BuildId, BuildNumber | Применимо |
| StorageWriteBytes | Скорость записи хранилища, используемая контейнером сборки | Bytes/Second | Multi | ProjectName |  |
| StorageWriteBytes |  | Bytes/Second | Multi | ProjectName, BuildId, BuildNumber | Применимо |
| SubmittedDuration | Измеряет продолжительность фазы `SUBMITTED` сборки | Seconds | Multi | Region |  |
| SubmittedDuration |  | Seconds | Multi | ProjectName |  |
| SucceededBuilds | Измеряет количество успешных сборок | Count | Sum | Region | Применимо |
| SucceededBuilds |  | Count | Sum | ProjectName | Применимо |
| UploadArtifactsDuration | Измеряет продолжительность фазы `UPLOAD_ARTIFACTS` сборки | Seconds | Multi | Region |  |
