---
title: Мониторинг Amazon Route 53 Resolver
...
Dynatrace собирает метрики для множества предварительно выбранных пространств имен, включая Amazon Route 53 Resolver. Вы можете просматривать метрики для каждого экземпляра сервиса, разделять метрики по различным измерениям и создавать пользовательские графики, которые можно закрепить на дашбордах.
...
`EndpointId` является основным измерением.

| Название | Описание | Единица измерения | Статистика | Измерения | Рекомендуется |
| --- | --- | --- | --- | --- | --- |
| EndpointHealthyENICount | Количество эластичных сетевых интерфейсов в статусе `OPERATIONAL`. | Количество | Multi | EndpointId |  |
| EndpointUnHealthyENICount | Количество эластичных сетевых интерфейсов в статусе `AUTO_RECOVERING`. | Количество | Multi | EndpointId |  |
| InboundQueryVolume | Количество DNS-запросов, перенаправленных из вашей сети в VPC через эндпоинт, указанный в `EndpointId` (для входящих эндпоинтов) | Количество | Sum | EndpointId | Доступен |
| InboundQueryVolume |  | Количество | Sum | Region | Доступен |
| InboundQueryVolume |  | Количество | Sum | Region, RniId |  |
| OutboundQueryAggregateVolume | Общее количество DNS-запросов, перенаправленных из Amazon VPC в вашу сеть (для исходящих эндпоинтов) | Количество | Sum | Region |  |
| OutboundQueryAggregateVolume |  | Количество | Sum | Region, RniId |  |
| OutboundQueryAggregateVolume |  | Количество | Sum | EndpointId | Доступен |
| OutboundQueryVolume | Количество DNS-запросов, перенаправленных из ваших VPC в вашу сеть через эндпоинт, указанный в `EndpointId` (для исходящих эндпоинтов) | Количество | Sum | EndpointId | Доступен |
| OutboundQueryVolume |  | Количество | Sum | Region | Доступен |
