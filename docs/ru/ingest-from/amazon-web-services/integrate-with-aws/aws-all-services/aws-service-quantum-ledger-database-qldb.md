---
title: Мониторинг Amazon QLDB (Quantum Ledger Database)
...
# Мониторинг Amazon QLDB (Quantum Ledger Database)
...
Dynatrace собирает метрики для нескольких предварительно выбранных пространств имён, включая Amazon QLDB. Вы можете просматривать графики для каждого экземпляра сервиса с набором измерений, а также создавать пользовательские графики и закреплять их на панелях управления.
...
* Обновлённая [политика мониторинга AWS](/docs/ingest-from/amazon-web-services/integrate-with-aws/cloudwatch-metrics#monitoring-policy "Интеграция метрик из Amazon CloudWatch.") для включения дополнительных AWS-сервисов.
...
Если вы не хотите добавлять разрешения для всех сервисов, а только для отдельных, воспользуйтесь таблицей ниже. В ней указан набор разрешений, необходимых для [всех AWS-сервисов](/docs/ingest-from/amazon-web-services/integrate-with-aws/aws-all-services "Мониторинг всех AWS-сервисов с помощью Dynatrace и просмотр доступных метрик."), а также список дополнительных разрешений для каждого поддерживаемого сервиса.
...
Сервис Amazon QLDB отображается на странице группы пользовательских устройств. Сервис состоит из экземпляров (пользовательских устройств), называемых **реестрами**. Метрики для каждого пользовательского устройства можно просмотреть в разделе **Дополнительные сведения** страницы обзора, разделив их по **потокам** и **типам команд**.
...
* **LedgerName** ограничивает данные конкретным реестром. Значением может быть любое имя реестра в текущем регионе AWS и текущей учётной записи AWS.
* **StreamId** ограничивает данные конкретным журнальным потоком. Значением может быть любой идентификатор потока для реестра в текущем регионе AWS и текущей учётной записи AWS.
* **CommandType** ограничивает данные одной из следующих команд QLDB data API: `AbortTransaction`, `CommitTransaction`, `EndSession`, `ExecuteStatement`, `FetchPage`, `StartSession`, `StartTransaction`.

`LedgerName` — основное измерение.

| Название | Описание | Единица измерения | Статистика | Измерения | Рекомендуется |
| --- | --- | --- | --- | --- | --- |
| CommandLatency | Время выполнения операций с данными, отображаемое с интервалом в одну минуту | Миллисекунды | Несколько | LedgerName, CommandType | Применимо |
| IndexedStorage | Общий объём дискового пространства, используемого таблицами, индексами и индексированной историей реестра, с интервалом в 15 минут | Байты | Несколько | LedgerName | Применимо |
| IsImpaired | Флаг, указывающий на нарушение работы журнального потока в Kinesis Data Streams, отображаемый с интервалом в одну минуту | Количество | Максимум | LedgerName, StreamId | Применимо |
| JournalStorage | Общий объём дискового пространства, используемого журналом реестра, с интервалом в 15 минут | Байты | Несколько | LedgerName | Применимо |
| OccConflictExceptions | Количество запросов к QLDB, вызвавших исключение `OccConflictException` | Количество | Сумма | LedgerName | Применимо |
| ReadIOs | Количество операций дискового ввода-вывода при чтении, с интервалом в одну минуту | Количество | Сумма | LedgerName | Применимо |
| ReadIOs |  | Количество | Несколько | LedgerName | Применимо |
| SessionRateExceededExceptions | Количество запросов к QLDB, вызвавших исключение `SessionRateExceededException` | Количество | Сумма | LedgerName | Применимо |
| Session4xxExceptions | Количество запросов к QLDB, вернувших HTTP-ошибку 4xx | Количество | Сумма | LedgerName | Применимо |
| Session5xxExceptions | Количество запросов к QLDB, вернувших HTTP-ошибку 5xx | Количество | Сумма | LedgerName | Применимо |
| WriteIOs | Количество операций дискового ввода-вывода при записи, с интервалом в одну минуту | Количество | Сумма | LedgerName | Применимо |
| WriteIOs |  | Количество | Несколько | LedgerName | Применимо |
