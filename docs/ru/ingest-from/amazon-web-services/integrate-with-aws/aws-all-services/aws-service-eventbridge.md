---
title: Мониторинг Amazon EventBridge
...
scraped: 2026-03-06T21:29:31.613508
---

# Мониторинг Amazon EventBridge
...
Dynatrace собирает метрики для нескольких предварительно выбранных пространств имён, включая Amazon EventBridge. Вы можете просматривать графики для каждого экземпляра сервиса с набором измерений и создавать пользовательские графики, которые можно закрепить на панелях мониторинга.
...
Если вы не хотите добавлять разрешения для всех сервисов, а только для определённых, обратитесь к таблице ниже. В таблице приведены разрешения, необходимые для [всех облачных сервисов AWS](/docs/ingest-from/amazon-web-services/integrate-with-aws/aws-all-services "Мониторинг всех облачных сервисов AWS с помощью Dynatrace и просмотр доступных метрик."), а также для каждого поддерживаемого сервиса — список дополнительных разрешений, специфичных для него.
...
Сервис Amazon EventBridge состоит из одной или нескольких шин событий. Существует **шина событий по умолчанию** с метриками уровня всего сервиса, а также дополнительные шины событий, называемые **пользовательскими шинами событий**.
Метрики шины событий по умолчанию доступны в разделе **Further details** на странице обзора **группы пользовательских устройств**. При создании дополнительных шин событий метрики пользовательских шин будут отображаться в разделе **Further details** на странице обзора **пользовательского устройства**.
...
`EventBusName` — основное измерение.

| Название | Описание | Единица | Статистика | Измерения | Рекомендовано |
| --- | --- | --- | --- | --- | --- |
| DeadLetterInvocations | Количество раз, когда цель правила не была вызвана в ответ на событие | Count | Sum | EventBusName, RuleName | Применимо |
| DeadLetterInvocations |  | Count | Sum | Region, RuleName | Применимо |
| DeadLetterInvocations |  | Count | Sum | Region | Применимо |
| FailedInvocations | Количество вызовов, завершившихся окончательной ошибкой | Count | Sum | EventBusName, RuleName | Применимо |
| FailedInvocations |  | Count | Sum | Region, RuleName | Применимо |
| FailedInvocations |  | Count | Sum | Region | Применимо |
| Invocations | Количество раз, когда цель была вызвана для правила в ответ на событие | Count | Sum | EventBusName, RuleName | Применимо |
| Invocations |  | Count | Sum | Region, RuleName | Применимо |
| Invocations |  | Count | Sum | Region | Применимо |
| MatchedEvents | Количество событий, совпавших с любым правилом | Count | Sum | Region | Применимо |
| ThrottledRules | Количество сработавших правил, которые ограничиваются по пропускной способности | Count | Sum | EventBusName, RuleName | Применимо |
| ThrottledRules |  | Count | Sum | Region, RuleName | Применимо |
| ThrottledRules |  | Count | Sum | Region | Применимо |
| TriggeredRules | Количество сработавших правил, совпавших с любым событием | Count | Sum | EventBusName, RuleName | Применимо |
| TriggeredRules |  | Count | Sum | Region, RuleName | Применимо |
| TriggeredRules |  | Count | Sum | Region | Применимо |

## Ограничения

Amazon EventBridge отправляет метрики `Invocations` в CloudWatch только при ненулевом значении. Для получения дополнительных сведений см. [документацию AWSï»¿](https://docs.aws.amazon.com/eventbridge/latest/userguide/eventbridge-monitoring-cloudwatch-metrics.html).
