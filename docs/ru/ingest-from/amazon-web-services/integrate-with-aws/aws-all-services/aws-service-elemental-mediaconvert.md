---
title: Мониторинг AWS Elemental MediaConvert
...
scraped: 2026-03-03T21:28:17.262438
---

# Мониторинг AWS Elemental MediaConvert
...
Dynatrace получает метрики для множества предварительно выбранных пространств имён, включая AWS Elemental MediaConvert. Вы можете просматривать графики для каждого экземпляра сервиса с набором измерений и создавать пользовательские графики, которые можно закрепить на дашбордах.
...
* Для развёртываний Dynatrace SaaS требуется Environment ActiveGate или Multi-environment ActiveGate.
  ...
  Для доступа на основе ролей в развёртывании [SaaS](../cloudwatch-metrics.md#role-based-access "Интеграция метрик из Amazon CloudWatch.") требуется [Environment ActiveGate](../../../dynatrace-activegate/installation.md "Узнайте, как настроить ActiveGate"), установленный на хосте Amazon EC2.
...
`Queue` является основным измерением.

| Название | Описание | Единица | Статистика | Измерения | Рекомендовано |
| --- | --- | --- | --- | --- | --- |
| AudioOutputDuration | Количество секунд аудиовыхода для очереди | Milliseconds | Multi | Queue |  |
| AudioOutputDuration |  | Milliseconds | Sum | Queue | Доступен |
| Errors | Количество ошибок, вызванных недопустимыми параметрами операции, например запрос статуса задания без указания идентификатора задания | Count | Sum | Region, Operation | Доступен |
| HDOutputDuration | Количество секунд выхода высокой чёткости (HD) для очереди | Milliseconds | Multi | Queue |  |
| HDOutputDuration |  | Milliseconds | Sum | Queue | Доступен |
| JobsCanceledCount | Количество отменённых заданий в этой очереди | Count | Sum | Queue | Доступен |
| JobsCompletedCount | Количество завершённых заданий в этой очереди | Count | Sum | Queue | Доступен |
| JobsErroredCount | Количество заданий, завершившихся с ошибкой из-за недопустимых входных данных, например запрос на перекодирование файла, которого нет в указанном входном бакете | Count | Sum | Queue | Доступен |
| 8KOutputDuration | Количество секунд выхода 8K для очереди | Milliseconds | Multi | Queue |  |
| 8KOutputDuration |  | Milliseconds | Sum | Queue | Доступен |
| SDOutputDuration | Количество секунд выхода стандартной чёткости (SD) для очереди | Milliseconds | Multi | Queue |  |
| SDOutputDuration |  | Milliseconds | Sum | Queue | Доступен |
| StandbyTime | Количество секунд до начала перекодирования задания в AWS Elemental MediaConvert | Milliseconds | Multi | Region | Доступен |
| StandbyTime |  | Milliseconds | Multi | Queue | Доступен |
| TranscodingTime | Количество секунд, затраченных AWS Elemental MediaConvert на завершение перекодирования | Milliseconds | Multi | Queue | Доступен |
| UHDOutputDuration | Количество секунд выхода сверхвысокой чёткости (UHD) для очереди | Milliseconds | Multi | Queue |  |
| UHDOutputDuration |  | Milliseconds | Sum | Queue | Доступен |

## Ограничения

Dynatrace в настоящее время не собирает атрибуты экземпляров для AWS Elemental MediaConvert.