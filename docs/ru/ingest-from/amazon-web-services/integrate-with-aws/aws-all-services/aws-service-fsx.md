...
Dynatrace собирает метрики для множества предварительно выбранных пространств имён, включая Amazon FSx. Вы можете просматривать метрики для каждого экземпляра сервиса, разделять метрики по нескольким измерениям и создавать пользовательские графики, которые можно закрепить на дашбордах.
...
Если вы не хотите добавлять разрешения для всех сервисов, а только для определённых, обратитесь к таблице ниже. Таблица содержит набор разрешений, необходимых для [всех облачных сервисов AWS](../aws-all-services.md "Мониторинг всех облачных сервисов AWS с помощью Dynatrace и просмотр доступных метрик."), а также, для каждого поддерживаемого сервиса, список необязательных разрешений, специфичных для этого сервиса.
...
`FileSystemId` является основным измерением.

| Название | Описание | Единица | Статистика | Измерения | Рекомендуется |
| --- | --- | --- | --- | --- | --- |
| DataReadBytes | DataReadBytes | Bytes | Multi | FileSystemId | Доступен |
| DataReadBytes |  | Bytes | Sum | FileSystemId |  |
| DataReadOperations | Количество операций чтения | Count | Sum | FileSystemId | Доступен |
| DataWriteBytes | Количество байтов для операций записи файловой системы | Bytes | Multi | FileSystemId | Доступен |
| DataWriteBytes |  | Bytes | Sum | FileSystemId |  |
| DataWriteOperations | Количество операций записи | Count | Sum | FileSystemId | Доступен |
| FreeStorageCapacity | Объём доступной ёмкости хранилища | Bytes | Multi | FileSystemId | Доступен |
| MetadataOperations | Количество операций с метаданными | Count | Sum | FileSystemId | Доступен |
