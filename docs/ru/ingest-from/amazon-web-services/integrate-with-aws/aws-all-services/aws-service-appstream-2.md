---
title: Мониторинг Amazon AppStream 2.0
...
scraped: 2026-03-04T21:32:13.031331
---
...
`Fleet` является основным измерением.

| Название | Описание | Единица | Статистика | Измерения | Рекомендуется |
| --- | --- | --- | --- | --- | --- |
| ActualCapacity | Общее количество экземпляров, доступных для потоковой передачи или выполняющих потоковую передачу в данный момент. `ActualCapacity` = `AvailableCapacity` + `InUseCapacity`. | Count | Multi | Fleet | Применимо |
| AvailableCapacity | Количество бездействующих экземпляров, доступных для пользовательских сеансов. `AvailableCapacity` = `ActualCapacity` - `InUseCapacity`. | Count | Multi | Fleet | Применимо |
| CapacityUtilization | Процент используемых экземпляров во флоте, рассчитанный по следующей формуле. `CapacityUtilization` = (`InUseCapacity`/`ActualCapacity`) Ã 100. | Percent | Multi | Fleet | Применимо |
| DesiredCapacity | Общее количество экземпляров, которые работают или ожидают. Это представляет общее количество одновременных сеансов потоковой передачи, которые ваш флот может поддерживать в стабильном состоянии. `DesiredCapacity` = `ActualCapacity` + `PendingCapacity`. | Count | Multi | Fleet | Применимо |
| InUseCapacity | Количество экземпляров, используемых в настоящее время для сеансов потоковой передачи. Одно значение `InUseCapacity` представляет один сеанс потоковой передачи. | Count | Multi | Fleet | Применимо |
| InsufficientCapacityError | Количество запросов сеансов, отклоненных из-за нехватки емкости. | Count | Sum | Fleet |  |
| PendingCapacity | Количество экземпляров, подготавливаемых AppStream 2.0. Представляет дополнительное количество сеансов потоковой передачи, которые флот сможет поддерживать после завершения подготовки. После начала подготовки обычно требуется 10-20 минут, чтобы экземпляр стал доступен для потоковой передачи. | Count | Multi | Fleet | Применимо |