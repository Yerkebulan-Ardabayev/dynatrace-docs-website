---
title: Мониторинг Amazon Route 53
...
Dynatrace собирает метрики для множества предварительно выбранных пространств имен, включая Amazon Route 53. Вы можете просматривать метрики для каждого экземпляра сервиса, разделять метрики по различным измерениям и создавать пользовательские графики, которые можно закрепить на дашбордах.
...
* Для развертываний Dynatrace требуется Environment ActiveGate или Multi-environment ActiveGate.

Для доступа на основе ролей в развертывании [Dynatrace](../cloudwatch-metrics.md#role-based-access "Интеграция метрик из Amazon CloudWatch.") вам необходим Environment ActiveGate, установленный на хосте Amazon EC2.

* Dynatrace версии 1.201+
* Обновленная [политика мониторинга AWS](../cloudwatch-metrics.md#monitoring-policy "Интеграция метрик из Amazon CloudWatch.") для включения дополнительных сервисов AWS.
...
Если вы не хотите добавлять разрешения для всех сервисов, а хотите выбрать разрешения только для определенных сервисов, обратитесь к таблице ниже. Таблица содержит набор разрешений, необходимых для всех облачных сервисов AWS, а также для каждого поддерживаемого сервиса список дополнительных разрешений, специфичных для этого сервиса.
...
| Название | Разрешения |
| --- | --- |
| Все отслеживаемые сервисы Amazon (обязательные) | `cloudwatch:GetMetricData`, `cloudwatch:GetMetricStatistics`, `cloudwatch:ListMetrics`, `sts:GetCallerIdentity`, `tag:GetResources`, `tag:GetTagKeys`, `ec2:DescribeAvailabilityZones` |
...
Route 53 является глобальным сервисом; метрики хостинговых зон (например, `DNSQueries`) доступны только в регионе `us-east-1`. Для получения метрик хостинговых зон учетные данные должны иметь как минимум следующие разрешения, установленные для региона `us-east-1`:
...
После добавления сервиса в мониторинг предустановленный дашборд, содержащий все рекомендуемые метрики, автоматически появляется на странице **Дашборды**. Для поиска конкретных дашбордов отфильтруйте по **Предустановленные**, а затем по **Название**.

![Предустановки AWS](https://dt-cdn.net/images/image-26-1645-389f58aa89.png)

Для существующих отслеживаемых сервисов может потребоваться повторное сохранение учетных данных, чтобы предустановленный дашборд появился на странице **Дашборды**. Чтобы повторно сохранить учетные данные, перейдите в **Settings** > **Cloud and virtualization** > **AWS**, выберите нужный экземпляр AWS, а затем нажмите **Save**.
...
Чтобы убрать дашборд со страницы дашбордов, вы можете скрыть его. Чтобы скрыть дашборд, откройте меню обзора (**...**) и выберите **Hide**.

Скрытие дашборда не влияет на других пользователей.

![Клонирование/скрытие AWS](https://dt-cdn.net/images/2020-12-10-15-04-09-1502-b899a29d73.png)
...
![53](https://dt-cdn.net/images/teo-02-3049-2f7be33072.png)

## Доступные метрики

`HostedZoneId` является основным измерением.

| Название | Описание | Единица измерения | Статистика | Измерения | Рекомендуется |
| --- | --- | --- | --- | --- | --- |
| ChildHealthCheckHealthyCount | Количество проверок работоспособности, которые являются здоровыми среди проверок работоспособности, отслеживаемых Route 53 | Количество | Sum | Region, HealthCheckId |  |
| ConnectionTime | Среднее время в миллисекундах, которое потребовалось средствам проверки работоспособности Route 53 для установления TCP-соединения с эндпоинтом | Миллисекунды | Multi | Region, HealthCheckId, Region |  |
| ConnectionTime |  | Миллисекунды | Multi | Region, HealthCheckId |  |
| DNSQueries | Общее количество DNS-запросов, на которые Route 53 отвечает для хостинговой зоны | Количество | Multi | HostedZoneId | Доступен |
| DNSQueries |  | Количество | Sum | HostedZoneId | Доступен |
| HealthCheckPercentageHealthy | Процент средств проверки работоспособности Route 53, которые считают выбранный эндпоинт здоровым | Проценты | Multi | Region, HealthCheckId |  |
| HealthCheckStatus | Статус эндпоинта проверки работоспособности, который проверяет CloudWatch. `1` означает здоровый, `0` означает нездоровый | Нет | Multi | Region, HealthCheckId |  |
| SSLHandshakeTime | Среднее время в миллисекундах, которое потребовалось средствам проверки работоспособности Route 53 для выполнения SSL-рукопожатия | Миллисекунды | Multi | Region, HealthCheckId, Region |  |
| SSLHandshakeTime |  | Миллисекунды | Multi | Region, HealthCheckId |  |
| TimeToFirstByte | Среднее время в миллисекундах, которое потребовалось средствам проверки работоспособности Route 53 для получения первого байта ответа на HTTP- или HTTPS-запрос | Миллисекунды | Multi | Region, HealthCheckId, Region |  |
| TimeToFirstByte |  | Миллисекунды | Multi | Region, HealthCheckId |  |
