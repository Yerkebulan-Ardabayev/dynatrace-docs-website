...
scraped: 2026-03-06T21:35:35.815594
---

# Мониторинг AWS Chatbot
...
Dynatrace собирает метрики для множества предварительно выбранных пространств имён, включая AWS Chatbot. Вы можете просматривать метрики для каждого экземпляра сервиса, разделять метрики по нескольким измерениям и создавать пользовательские диаграммы, которые можно закрепить на ваших дашбордах.
...
Для доступа на основе ролей в развёртывании [SaaS](/docs/ingest-from/amazon-web-services/integrate-with-aws/cloudwatch-metrics#role-based-access "Integrate metrics from Amazon CloudWatch.") требуется [Environment ActiveGate](/docs/ingest-from/dynatrace-activegate/installation "Learn how to configure ActiveGate"), установленный на хосте Amazon EC2.
...
Основным измерением является `ConfigurationName`.

| Название | Описание | Единица | Статистика | Измерения | Рекомендуемая |
| --- | --- | --- | --- | --- | --- |
| EventsProcessed | Количество уведомлений о событиях, полученных AWS Chatbot | Count | Sum | ConfigurationName | Доступен |
| EventsProcessed |  | Count | Sum | Region | Доступен |
| EventsThrottled | Количество отклонённых (throttled) уведомлений | Count | Sum | Region | Доступен |
| MessageDeliveryFailure | Количество сообщений, которые не удалось доставить в чат-клиент | Count | Sum | ConfigurationName | Доступен |
| MessageDeliveryFailure |  | Count | Sum | Region | Доступен |
| MessageDeliverySuccess | Количество сообщений, успешно доставленных в чат-клиент | Count | Sum | ConfigurationName |  |
| MessageDeliverySuccess |  | Count | Sum | Region |  |
| UnsupportedEvents | Количество неподдерживаемых событий или попыток отправки сообщений | Count | Sum | ConfigurationName | Доступен |
| UnsupportedEvents |  | Count | Sum | Region | Доступен |

## Ограничения

Метрики для отклонённых (throttled) событий являются общерегиональными и не имеют измерения для конкретной конфигурации.
