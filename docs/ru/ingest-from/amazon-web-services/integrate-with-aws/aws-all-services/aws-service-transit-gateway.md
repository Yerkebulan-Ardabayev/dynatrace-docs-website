---
title: Мониторинг AWS Transit Gateway
...
Dynatrace собирает метрики для множества предварительно выбранных пространств имён, включая AWS Transit Gateway. Вы можете просматривать метрики для каждого экземпляра сервиса, разделять метрики по нескольким измерениям и создавать пользовательские графики, которые можно закрепить на дашбордах.
...
Для доступа на основе ролей в развёртывании [Dynatrace](../cloudwatch-metrics.md#role-based-access "Интеграция метрик из Amazon CloudWatch.") вам потребуется Environment ActiveGate, установленный на хосте Amazon EC2.
...
Если вы не хотите добавлять разрешения для всех сервисов и хотите выбрать разрешения только для определённых сервисов, обратитесь к таблице ниже. Таблица содержит набор разрешений, необходимых для всех облачных сервисов AWS, а также для каждого поддерживаемого сервиса — список необязательных разрешений, специфичных для этого сервиса.
...
TransitGateway является межаккаунтным сервисом, что означает, что несколько аккаунтов могут отслеживать один TransitGateway. Dynatrace хранит метрики отдельно для каждого отслеживаемого аккаунта, который мониторит один и тот же экземпляр TransitGateway.
...
`TransitGateway` является основным измерением.

| Название | Описание | Единица | Статистика | Измерения | Рекомендуемая |
| --- | --- | --- | --- | --- | --- |
| BytesDropCountBlackhole | Количество байт, отброшенных из-за совпадения с маршрутом чёрной дыры | Count | Sum | TransitGateway, TransitGatewayAttachment |  |
| BytesDropCountBlackhole |  | Count | Sum | TransitGateway | Доступна |
| BytesDropCountNoRoute | Количество байт, отброшенных из-за отсутствия совпадения с маршрутом | Count | Sum | TransitGateway, TransitGatewayAttachment |  |
| BytesDropCountNoRoute |  | Count | Sum | TransitGateway | Доступна |
| BytesIn | Количество байт, полученных транзитным шлюзом | Байты | Sum | TransitGateway, TransitGatewayAttachment |  |
| BytesIn |  | Байты | Sum | TransitGateway | Доступна |
| BytesOut | Количество пакетов, отправленных транзитным шлюзом | Байты | Sum | TransitGateway, TransitGatewayAttachment |  |
| BytesOut |  | Байты | Sum | TransitGateway | Доступна |
| IGMPJoins | Количество сообщений `IGMP Join`, полученных транзитным шлюзом | Count | Sum | TransitGateway, TransitGatewayAttachment |  |
| IGMPQueries | Количество сообщений `IGMP Query`, полученных транзитным шлюзом | Count | Sum | TransitGateway, TransitGatewayAttachment |  |
| IGMPReceives | Количество сообщений `IGMP Leave`, полученных транзитным шлюзом | Count | Sum | TransitGateway, TransitGatewayAttachment |  |
| MulticastBytesIn | Количество мультикастовых байт, полученных транзитным шлюзом | Байты | Sum | TransitGateway, TransitGatewayAttachment |  |
| MulticastBytesOut | Количество мультикастовых байт, отправленных транзитным шлюзом | Байты | Sum | TransitGateway, TransitGatewayAttachment |  |
| MulticastPacketDropCount | Количество отброшенных мультикастовых пакетов | Count | Sum | TransitGateway, TransitGatewayAttachment |  |
| MulticastPacketsIn | Количество мультикастовых пакетов, полученных транзитным шлюзом | Count | Sum | TransitGateway, TransitGatewayAttachment |  |
| MulticastPacketsOut | Количество мультикастовых пакетов, отправленных транзитным шлюзом | Count | Sum | TransitGateway, TransitGatewayAttachment |  |
| NumberofMulticastDomains | Количество мультикастовых доменов, настроенных на транзитном шлюзе | Count | Sum | TransitGateway |  |
| NumberofMulticastGroupMembers | Количество участников мультикастовых групп, настроенных на транзитном шлюзе | Count | Sum | TransitGateway |  |
| PacketDropCountBlackhole | Количество пакетов, отброшенных из-за совпадения с маршрутом чёрной дыры | Count | Sum | TransitGateway, TransitGatewayAttachment |  |
| PacketDropCountBlackhole |  | Count | Sum | TransitGateway | Доступна |
| PacketDropCountNoRoute | Количество пакетов, отброшенных из-за отсутствия совпадения с маршрутом | Count | Sum | TransitGateway, TransitGatewayAttachment |  |
| PacketDropCountNoRoute |  | Count | Sum | TransitGateway | Доступна |
| PacketsIn | Количество пакетов, полученных транзитным шлюзом | Count | Sum | TransitGateway, TransitGatewayAttachment |  |
| PacketsIn |  | Count | Sum | TransitGateway | Доступна |
| PacketsOut | Количество пакетов, отправленных транзитным шлюзом | Count | Sum | TransitGateway, TransitGatewayAttachment |  |
| PacketsOut |  | Count | Sum | TransitGateway | Доступна |
