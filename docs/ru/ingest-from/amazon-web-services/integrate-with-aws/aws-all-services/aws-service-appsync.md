...
* Для развертываний Dynatrace вам необходим Environment ActiveGate или Multi-environment ActiveGate.

  Для ролевого доступа в развертывании [Dynatrace](../cloudwatch-metrics.md#role-based-access "Integrate metrics from Amazon CloudWatch.") вам необходим Environment ActiveGate установленный на хосте Amazon EC2.
...
Если вы не хотите добавлять разрешения для всех сервисов и хотите выбрать разрешения только для определенных сервисов, обратитесь к таблице ниже. Таблица содержит набор разрешений, необходимых для All AWS cloud services и для каждого поддерживаемого сервиса список необязательных разрешений, специфичных для этого сервиса.
...
`GraphQLAPIId` является основным измерением.

| Название | Описание | Единица | Статистика | Измерения | Рекомендуется |
| --- | --- | --- | --- | --- | --- |
| ActiveConnections | Количество одновременных WebSocket-подключений от клиентов к AWS AppSync за 1 минуту | Count | Sum | GraphQLAPIId | Применимо |
| ActiveSubscriptions | Количество одновременных подписок от клиентов за 1 минуту | Count | Sum | GraphQLAPIId | Применимо |
| ConnectClientError | Количество WebSocket-подключений, отклоненных AWS AppSync из-за ошибок на стороне клиента | Count | Sum | GraphQLAPIId | Применимо |
| ConnectServerError | Количество ошибок, возникших в AWS AppSync при обработке подключений | Count | Sum | GraphQLAPIId | Применимо |
| ConnectSuccess | Количество успешных WebSocket-подключений к AWS AppSync | Count | Sum | GraphQLAPIId | Применимо |
| ConnectionDuration | Время, в течение которого подключение остается открытым | Milliseconds | Multi | GraphQLAPIId | Применимо |
| DisconnectClientError | Количество ошибок, возникших в AWS AppSync при отключении WebSocket-подключений из-за ошибок на стороне клиента | Count | Sum | GraphQLAPIId | Применимо |
| DisconnectServerError | Количество ошибок, возникших в AWS AppSync при отключении WebSocket-подключений во время обработки подключений | Count | Sum | GraphQLAPIId | Применимо |
| DisconnectSuccess | Количество успешных отключений WebSocket от AWS AppSync | Count | Sum | GraphQLAPIId | Применимо |
| Latency | Время между получением запроса от клиента AWS AppSync и возвратом ответа клиенту | Milliseconds | Multi | GraphQLAPIId | Применимо |
| PublishDataMessageClientError | Количество сообщений о событиях подписки, которые не удалось опубликовать из-за ошибок на стороне клиента | Count | Sum | GraphQLAPIId | Применимо |
| PublishDataMessageServerError | Количество ошибок, возникших в AWS AppSync при публикации сообщений о событиях подписки | Count | Sum | GraphQLAPIId | Применимо |
| PublishDataMessageSize | Размер опубликованных сообщений о событиях подписки | Bytes | Multi | GraphQLAPIId | Применимо |
| PublishDataMessageSize |  | Bytes | Sum | GraphQLAPIId | Применимо |
| PublishDataMessageSuccess | Количество сообщений о событиях подписки, которые были успешно опубликованы | Count | Sum | GraphQLAPIId | Применимо |
| SubscribeServerError | Количество ошибок, возникших в AWS AppSync при обработке подписок | Count | Sum | GraphQLAPIId | Применимо |
| SubscribeSuccess | Количество подписок, которые были успешно зарегистрированы в AWS AppSync через WebSocket | Count | Sum | GraphQLAPIId | Применимо |
| UnsubscribeClientError | Количество отписок, отклоненных AWS AppSync из-за ошибок на стороне клиента | Count | Sum | GraphQLAPIId | Применимо |
| UnsubscribeServerError | Количество ошибок, возникших в AWS AppSync при обработке отписок | Count | Sum | GraphQLAPIId | Применимо |
| UnsubscribeSuccess | Количество отписок, успешно обработанных AWS AppSync | Count | Sum | GraphQLAPIId | Применимо |
| 4XXError | Количество ошибок на стороне клиента, зафиксированных за определенный период | Count | Sum | GraphQLAPIId | Применимо |
| 5XXError | Количество ошибок на стороне сервера, зафиксированных за определенный период | Count | Sum | GraphQLAPIId | Применимо |

## Ограничения

Хотя Dynatrace не поддерживает трафик WebSocket, интеграция AppSync отображает URL WebSocket как часть свойств создаваемых пользовательских устройств.