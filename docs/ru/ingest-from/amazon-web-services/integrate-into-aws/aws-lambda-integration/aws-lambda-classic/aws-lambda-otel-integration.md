...
Убедитесь, что вы используете [поддерживаемую среду выполнения AWS Lambda](/docs/ingest-from/amazon-web-services/integrate-into-aws/aws-lambda-integration#support-lifecycle "Возможности и варианты интеграции AWS Lambda") и что вы выполнили шаги **начальной настройки**, описанные в [Мониторинг AWS Lambda с помощью OpenTelemetry](/docs/ingest-from/amazon-web-services/integrate-into-aws/aws-lambda-integration/aws-lambda-classic/aws-lambda-otel-integration/aws-lambda-otel-setup "Предварительные требования для мониторинга AWS Lambda с помощью OpenTelemetry"), прежде чем использовать пакеты ниже.
...
Начиная с .NET 6, Lambda-функции могут [собираться и развёртываться как образы контейнеров](https://dt-url.net/yz038r7). Инициализация и трассировка для [управляемой среды выполнения .NET](/docs/ingest-from/technology-support/application-software/dotnet "Узнайте обо всех аспектах поддержки мониторинга .NET-приложений в Dynatrace.") применяются к функциям с образами контейнеров без каких-либо изменений.
