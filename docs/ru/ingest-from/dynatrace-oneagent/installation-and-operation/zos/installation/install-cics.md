...
Анализируйте производительность транзакций от начала до конца с помощью потока сервисов.

![Поток сервисов CICS](https://dt-cdn.net/images/cics-trace-2357-0a7717e199.png)

Используйте распределённые трассировки PurePath, чтобы детализировать информацию до уровня кода и оптимизировать свои программы.
...
Необходимо добавить модуль Java z/OS к каждому CICS Transaction Gateway, который вы хотите отслеживать.
...
Независимо от того, какой вариант вы выберете, необходимо настроить DSN, заменив `<hlq>` квалификатором высокого уровня, заданным при скачивании наборов данных продукта z/OS.
...
Этот шаг является необязательным для тестовых установок, поскольку вместо него можно использовать транзакцию DTAX для активации модуля после инициализации CICS. Мы рекомендуем помещать запись `ZDTPLT` непосредственно перед спецификацией `TYPE=FINAL`.
...
Для проверки подключения между модулем CICS и подсистемой zDC отправьте эхо-запрос.
...
Если вы включили группировку имён CICSPlex **после** запуска региона CICS, необходимо выполнить транзакцию DTAX `DTAX DISABLE` и `DTAX ENABLE`.
...
Управлять уровнем журнала модуля CICS можно с помощью транзакции DTAX или указав необязательный параметр `INITPARM` при запуске региона CICS.
...
2. Используйте транзакцию DTAX для отключения модуля CICS в регионе CICS командой `DISABLE`.
...
Убедитесь, что zDC с этим идентификатором подсистемы запущен. Если это так, попробуйте выполнить команду `ENABLE` транзакции DTAX для восстановления соединений.
