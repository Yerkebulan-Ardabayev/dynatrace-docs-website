...
Анализируйте производительность транзакций от начала до конца с помощью [потока сервисов](/docs/observe/application-observability/services-classic/service-backtrace "Отслеживайте последовательность вызовов сервисов вплоть до клика в браузере, который инициировал цепочку вызовов.").

![Поток сервисов CICS](https://dt-cdn.net/images/cics-trace-2357-0a7717e199.png)

Используйте [распределённые трассировки PurePath](/docs/observe/application-observability/distributed-traces "Получите наблюдаемость в высокодистрибутивных, облачно-нативных архитектурах для эффективной трассировки и анализа транзакций в реальном времени."), чтобы детализировать информацию до уровня кода и оптимизировать свои программы.
...
Необходимо добавить [модуль Java z/OS](/docs/ingest-from/dynatrace-oneagent/installation-and-operation/zos/installation/install-zos-java#middleware "Настройте мониторинг Java на z/OS с помощью модуля Java.") к каждому CICS Transaction Gateway, который вы хотите отслеживать.
...
Независимо от того, какой вариант вы выберете, необходимо настроить DSN, заменив `<hlq>` квалификатором высокого уровня, заданным при [скачивании наборов данных продукта z/OS](/docs/ingest-from/dynatrace-oneagent/installation-and-operation/zos/installation/zosmf-installer/download-zos-datasets "Скачайте и установите наборы данных продукта Dynatrace для z/OS.").
...
Этот шаг является необязательным для тестовых установок, поскольку вместо него можно использовать [транзакцию DTAX](/docs/ingest-from/dynatrace-oneagent/installation-and-operation/zos/operation/dtax-transaction "Управляйте модулем CICS с помощью транзакций DTAX.") для активации модуля после инициализации CICS. Мы рекомендуем помещать запись `ZDTPLT` непосредственно перед спецификацией `TYPE=FINAL`.
...
Для проверки подключения между модулем CICS и подсистемой zDC [отправьте эхо-запрос](/docs/ingest-from/dynatrace-oneagent/installation-and-operation/zos/operation/dtax-transaction#ping "Управляйте модулем CICS с помощью транзакций DTAX.").
...
Если вы включили группировку имён CICSPlex **после** запуска региона CICS, необходимо выполнить [транзакцию DTAX](/docs/ingest-from/dynatrace-oneagent/installation-and-operation/zos/operation/dtax-transaction "Управляйте модулем CICS с помощью транзакций DTAX.") `DTAX DISABLE` и `DTAX ENABLE`.
...
Управлять уровнем журнала модуля CICS можно с помощью [транзакции DTAX](/docs/ingest-from/dynatrace-oneagent/installation-and-operation/zos/operation/dtax-transaction "Управляйте модулем CICS с помощью транзакций DTAX.") или указав необязательный параметр `INITPARM` при запуске региона CICS.
...
Доступ к журналам CICS можно получить через [журналы zRemote](/docs/ingest-from/dynatrace-oneagent/installation-and-operation/zos/installation/install-zremote#logging "Подготовьте и установите zRemote для мониторинга z/OS.").

## Обновление без перезапуска региона

Для обновления модуля CICS до более новой версии без перезапуска региона:

1. [Скачайте наборы данных продукта z/OS](/docs/ingest-from/dynatrace-oneagent/installation-and-operation/zos/installation/zosmf-installer/download-zos-datasets#download-pax "Скачайте и установите наборы данных продукта Dynatrace для z/OS.") и [извлеките их](/docs/ingest-from/dynatrace-oneagent/installation-and-operation/zos/installation/zosmf-installer/download-zos-datasets#extract-datasets "Скачайте и установите наборы данных продукта Dynatrace для z/OS.").
2. Используйте [транзакцию DTAX](/docs/ingest-from/dynatrace-oneagent/installation-and-operation/zos/operation/dtax-transaction "Управляйте модулем CICS с помощью транзакций DTAX.") для отключения модуля CICS в регионе CICS командой `DISABLE`.
...
Убедитесь, что zDC с этим идентификатором подсистемы запущен. Если это так, попробуйте выполнить команду `ENABLE` [транзакции DTAX](/docs/ingest-from/dynatrace-oneagent/installation-and-operation/zos/operation/dtax-transaction "Управляйте модулем CICS с помощью транзакций DTAX.") для восстановления соединений.
