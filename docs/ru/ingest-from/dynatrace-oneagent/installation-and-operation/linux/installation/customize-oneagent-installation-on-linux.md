...
Параметр `INSTALL_PATH` не управляет каталогами [файлов логов и конфигурации](disk-space-requirements-for-oneagent-installation-and-update-on-linux.md "Узнайте о структуре каталогов OneAgent и требованиях к дисковому пространству для установки на Linux.") OneAgent. Для настройки пути к логам используйте параметр `LOG_PATH`.
...
Для изменения конечной точки после установки используйте `--set-server` в [интерфейсе командной строки OneAgent](../../../oneagent-configuration-via-command-line-interface.md "Узнайте, как выполнять некоторые задачи настройки OneAgent без необходимости переустановки.").

## Идентификатор окружения

**Значение по умолчанию**: `зависит от окружения`

[Идентификатор окружения](../../../../../discover-dynatrace/get-started/monitoring-environment.md "Узнайте о работе с окружениями мониторинга.") Dynatrace, полученный с письмом активации. Если вы устанавливаете OneAgent со страницы **Deploy** в Dynatrace, это значение уже установлено правильно. Если вы продаете услуги на основе Dynatrace, используйте эту опцию для установки идентификаторов ваших клиентов из пула идентификаторов, приобретенных у Dynatrace.
...
Для изменения tenant после установки используйте `--set-tenant` в [интерфейсе командной строки OneAgent](../../../oneagent-configuration-via-command-line-interface.md "Узнайте, как выполнять некоторые задачи настройки OneAgent без необходимости переустановки.").
...
См. [Токены доступа](../../../../../manage/identity-access-management/access-tokens-and-oauth-clients/access-tokens/rotate-tenant-token.md "Узнайте, что такое токен tenant и как его изменить."), чтобы узнать, как получить токен.

Для изменения токена tenant после установки используйте `--set-tenant-token` в [интерфейсе командной строки OneAgent](../../../oneagent-configuration-via-command-line-interface.md "Узнайте, как выполнять некоторые задачи настройки OneAgent без необходимости переустановки.").

## Сетевые зоны

**Значение по умолчанию**: `не задано`

Чтобы узнать о правилах именования сетевых зон и другую справочную информацию, см. [Сетевые зоны](../../../../../manage/network-zones.md "Узнайте, как работают сетевые зоны в Dynatrace.").
...
Для изменения или очистки назначения сетевой зоны после установки используйте [Удаленное управление конфигурацией](../../../../bulk-configuration.md#configure-oneagents "Выполняйте настройку OneAgent и ActiveGate на хостах со страницы статуса развертывания или в масштабе с помощью Dynatrace API.") (выберите действие **изменить сетевую зону**).
...
Для изменения или очистки адреса прокси после установки используйте `--set-proxy` в [интерфейсе командной строки OneAgent](../../../oneagent-configuration-via-command-line-interface.md "Узнайте, как выполнять некоторые задачи настройки OneAgent без необходимости переустановки.").
...
* `oneagentextensions` -- мониторинг с использованием локальных [Extensions](../../../../extensions.md "Узнайте, как создавать и управлять Dynatrace Extensions.")
* `oneagentloganalytics` -- [Log Monitoring](../../../../../analyze-explore-automate/log-monitoring.md "Узнайте, как включить Log Monitoring, какие возможности он предоставляет, и многое другое.")
* `oneagentnetwork` -- [мониторинг сети](../../../../../observe/infrastructure-observability/networks-classic.md "Узнайте, как отслеживать сетевые коммуникации.")
...
Имя группы, к которой вы хотите назначить хост. Подробности см. в разделе [Организация окружения с помощью групп хостов](../../../../../observe/infrastructure-observability/hosts/configuration/organize-your-environment-using-host-groups.md "Узнайте, как Dynatrace позволяет организовать хосты, процессы и сервисы с помощью групп хостов."). Требования к строке Host Group:
...
Для удаления хоста из группы необходимо [удалить OneAgent](../operation/uninstall-oneagent-on-linux.md "Узнайте, как удалить OneAgent из вашей системы на базе Linux.") или передать пустое значение `--set-host-group=""` при обновлении OneAgent. Вы не можете удалить хост из группы, используя параметр `HOST_GROUP` при обновлении OneAgent.

Для изменения или очистки назначения группы хостов после установки используйте [Удаленное управление конфигурацией](../../../../bulk-configuration.md#configure-oneagents "Выполняйте настройку OneAgent и ActiveGate на хостах со страницы статуса развертывания или в масштабе с помощью Dynatrace API.") (выберите действие **изменить группу хостов**).
...
Для изменения режима мониторинга после установки используйте `--set-monitoring-mode` в [интерфейсе командной строки OneAgent](../../../oneagent-configuration-via-command-line-interface.md#infrastructure-monitoring "Узнайте, как выполнять некоторые задачи настройки OneAgent без необходимости переустановки.") или настройте его на странице [настроек хоста](../../../../../platform/oneagent/monitoring-modes/monitoring-modes.md "Узнайте больше о доступных режимах мониторинга при использовании OneAgent.").
...
Эта команда добавляет пользовательское имя хоста для отображения в интерфейсе, но обнаруженное имя хоста не изменяется. Подробности см. в разделе [Установка пользовательских имен хостов](../../../../../observe/infrastructure-observability/hosts/configuration/set-custom-host-names-in-dynamic-environments.md "Узнайте, как изменить имя отслеживаемого хоста.").
...
Для изменения имени хоста после установки используйте `--set-host-name` в [интерфейсе командной строки OneAgent](../../../oneagent-configuration-via-command-line-interface.md "Узнайте, как выполнять некоторые задачи настройки OneAgent без необходимости переустановки.").
...
Для изменения метаданных хоста после установки используйте [Удаленное управление конфигурацией](../../../../bulk-configuration.md#configure-oneagents "Выполняйте настройку OneAgent и ActiveGate на хостах со страницы статуса развертывания или в масштабе с помощью Dynatrace API.") (выберите действие **изменить свойства хоста**).

В качестве альтернативы вы можете использовать `--set-host-property` в [интерфейсе командной строки OneAgent](../../../oneagent-configuration-via-command-line-interface.md "Узнайте, как выполнять некоторые задачи настройки OneAgent без необходимости переустановки.").
...
Для изменения тегов хоста после установки используйте [Удаленное управление конфигурацией](../../../../bulk-configuration.md#configure-oneagents "Выполняйте настройку OneAgent и ActiveGate на хостах со страницы статуса развертывания или в масштабе с помощью Dynatrace API.") (выберите действие **изменить теги хоста**).

В качестве альтернативы вы можете использовать `--set-host-tag` в [интерфейсе командной строки OneAgent](../../../oneagent-configuration-via-command-line-interface.md "Узнайте, как выполнять некоторые задачи настройки OneAgent без необходимости переустановки.").
...
Если вам нужно изменить этот доступ после установки, используйте [интерфейс командной строки OneAgent](../../../oneagent-configuration-via-command-line-interface.md "Узнайте, как выполнять некоторые задачи настройки OneAgent без необходимости переустановки."):

Обратите внимание, что это настройка самодиагностики, которая не связана с [Log Monitoring](../../../../../analyze-explore-automate/log-monitoring.md "Узнайте, как включить Log Monitoring, какие возможности он предоставляет, и многое другое.").

## Log Monitoring

**Значение по умолчанию**: `true`

Если установлено в `true`, позволяет OneAgent обращаться к файлам логов для целей Log Monitoring. Допустимые значения: (`true`, `false`) или (`1`, `0`). Эта опция также может быть [включена/отключена через веб-интерфейс](../../../../../analyze-explore-automate/log-monitoring.md "Узнайте, как включить Log Monitoring, какие возможности он предоставляет, и многое другое.").

Например:
`--set-app-log-content-access=true`

Если вам нужно включить или отключить Log Monitoring после установки, используйте `-set-app-log-content-access` в [интерфейсе командной строки OneAgent](../../../oneagent-configuration-via-command-line-interface.md "Узнайте, как выполнять некоторые задачи настройки OneAgent без необходимости переустановки.").
...
Вы можете использовать параметр `--set-extensions-ingest-port=<arg>` для изменения порта связи по умолчанию, используемого для приема локальных метрик. Порт используется [OneAgent REST API](../../../../extend-dynatrace/extend-metrics/ingestion-methods/oneagent-metric-api.md "Используйте Dynatrace API для получения метрик отслеживаемых сущностей."), [интеграцией скриптов](../../../../extend-dynatrace/extend-metrics/ingestion-methods/oneagent-pipe.md "Узнайте, как принимать метрики с помощью локальной интеграции скриптов.") (`dynatrace_ingest`) и [Telegraf](../../../../extend-dynatrace/extend-metrics/ingestion-methods/telegraf.md "Принимайте метрики Telegraf в Dynatrace.").

Для получения дополнительной информации см. [Прием метрик](../../../../extend-dynatrace/extend-metrics.md "Узнайте, как расширить наблюдаемость метрик в Dynatrace.").

## Прием метрик StatsD

**Значение по умолчанию**: `18125`

Вы можете использовать параметр `--set-extensions-statsd-port=<arg>` для изменения UDP-порта прослушивания [DynatraceStatsD](../../../../extend-dynatrace/extend-metrics/ingestion-methods/statsd.md "Принимайте метрики в Dynatrace с помощью OneAgent и клиента ActiveGate StatsD.") по умолчанию.
...
* Ядро Linux версии 4.3+ (рекомендуемая версия systemd 221+) для автоматических обновлений OneAgent и полноценной работы без привилегий root.
  Для получения дополнительной информации см. [Непривилегированный режим Linux](linux-non-privileged.md "Узнайте, когда Dynatrace OneAgent требует привилегии root в Linux.")
...
Для получения дополнительной информации см. [требования к разрешениям для установки и работы OneAgent в Linux](linux-non-privileged.md "Узнайте, когда Dynatrace OneAgent требует привилегии root в Linux.").
...
Информацию о механизме автообновления OneAgent см. в разделе [Обновление OneAgent в Linux](../operation/update-oneagent-on-linux.md "Узнайте о различных способах обновления OneAgent в Linux.").
