...
Параметр `INSTALL_PATH` не управляет каталогами [файлов логов и конфигурации](/docs/ingest-from/dynatrace-oneagent/installation-and-operation/linux/installation/disk-space-requirements-for-oneagent-installation-and-update-on-linux "Узнайте о структуре каталогов OneAgent и требованиях к дисковому пространству для установки на Linux.") OneAgent. Для настройки пути к логам используйте параметр `LOG_PATH`.
...
Для изменения конечной точки после установки используйте `--set-server` в [интерфейсе командной строки OneAgent](/docs/ingest-from/dynatrace-oneagent/oneagent-configuration-via-command-line-interface "Узнайте, как выполнять некоторые задачи настройки OneAgent без необходимости переустановки.").

## Идентификатор окружения

**Значение по умолчанию**: `зависит от окружения`

[Идентификатор окружения](/docs/discover-dynatrace/get-started/monitoring-environment "Узнайте о работе с окружениями мониторинга.") Dynatrace, полученный с письмом активации. Если вы устанавливаете OneAgent со страницы **Deploy** в Dynatrace, это значение уже установлено правильно. Если вы продаете услуги на основе Dynatrace, используйте эту опцию для установки идентификаторов ваших клиентов из пула идентификаторов, приобретенных у Dynatrace.
...
Для изменения tenant после установки используйте `--set-tenant` в [интерфейсе командной строки OneAgent](/docs/ingest-from/dynatrace-oneagent/oneagent-configuration-via-command-line-interface "Узнайте, как выполнять некоторые задачи настройки OneAgent без необходимости переустановки.").
...
См. [Токены доступа](/docs/manage/identity-access-management/access-tokens-and-oauth-clients/access-tokens/rotate-tenant-token "Узнайте, что такое токен tenant и как его изменить."), чтобы узнать, как получить токен.

Для изменения токена tenant после установки используйте `--set-tenant-token` в [интерфейсе командной строки OneAgent](/docs/ingest-from/dynatrace-oneagent/oneagent-configuration-via-command-line-interface "Узнайте, как выполнять некоторые задачи настройки OneAgent без необходимости переустановки.").

## Сетевые зоны

**Значение по умолчанию**: `не задано`

Чтобы узнать о правилах именования сетевых зон и другую справочную информацию, см. [Сетевые зоны](/docs/manage/network-zones "Узнайте, как работают сетевые зоны в Dynatrace.").
...
Для изменения или очистки назначения сетевой зоны после установки используйте [Удаленное управление конфигурацией](/docs/ingest-from/bulk-configuration#configure-oneagents "Выполняйте настройку OneAgent и ActiveGate на хостах со страницы статуса развертывания или в масштабе с помощью Dynatrace API.") (выберите действие **изменить сетевую зону**).

В качестве альтернативы вы можете использовать `--set-network-zone` в [интерфейсе командной строки OneAgent](/docs/ingest-from/dynatrace-oneagent/oneagent-configuration-via-command-line-interface#nz "Узнайте, как выполнять некоторые задачи настройки OneAgent без необходимости переустановки.").
...
Для изменения или очистки адреса прокси после установки используйте `--set-proxy` в [интерфейсе командной строки OneAgent](/docs/ingest-from/dynatrace-oneagent/oneagent-configuration-via-command-line-interface "Узнайте, как выполнять некоторые задачи настройки OneAgent без необходимости переустановки.").
...
* `oneagentplugin` -- мониторинг с использованием [расширений OneAgent](/docs/ingest-from/extensions/develop-your-extensions#oneagent-extensions "Разрабатывайте собственные Extensions в Dynatrace.")
* `oneagentextensions` -- мониторинг с использованием локальных [Extensions](/docs/ingest-from/extensions "Узнайте, как создавать и управлять Dynatrace Extensions.")
* `oneagentloganalytics` -- [Log Monitoring](/docs/analyze-explore-automate/log-monitoring "Узнайте, как включить Log Monitoring, какие возможности он предоставляет, и многое другое.")
* `oneagentnetwork` -- [мониторинг сети](/docs/observe/infrastructure-observability/networks-classic "Узнайте, как отслеживать сетевые коммуникации.")
...
Для изменения диапазона портов после установки используйте `--set-watchdog-portrange` в [интерфейсе командной строки OneAgent](/docs/ingest-from/dynatrace-oneagent/oneagent-configuration-via-command-line-interface#portrange "Узнайте, как выполнять некоторые задачи настройки OneAgent без необходимости переустановки.").
...
Имя группы, к которой вы хотите назначить хост. Подробности см. в разделе [Организация окружения с помощью групп хостов](/docs/observe/infrastructure-observability/hosts/configuration/organize-your-environment-using-host-groups "Узнайте, как Dynatrace позволяет организовать хосты, процессы и сервисы с помощью групп хостов."). Требования к строке Host Group:
...
Для удаления хоста из группы необходимо [удалить OneAgent](/docs/ingest-from/dynatrace-oneagent/installation-and-operation/linux/operation/uninstall-oneagent-on-linux "Узнайте, как удалить OneAgent из вашей системы на базе Linux.") или передать пустое значение `--set-host-group=""` при обновлении OneAgent. Вы не можете удалить хост из группы, используя параметр `HOST_GROUP` при обновлении OneAgent.

Для изменения или очистки назначения группы хостов после установки используйте [Удаленное управление конфигурацией](/docs/ingest-from/bulk-configuration#configure-oneagents "Выполняйте настройку OneAgent и ActiveGate на хостах со страницы статуса развертывания или в масштабе с помощью Dynatrace API.") (выберите действие **изменить группу хостов**).

В качестве альтернативы вы можете использовать `--set-host-group` в [интерфейсе командной строки OneAgent](/docs/ingest-from/dynatrace-oneagent/oneagent-configuration-via-command-line-interface#host-groups "Узнайте, как выполнять некоторые задачи настройки OneAgent без необходимости переустановки.").
...
Для изменения режима мониторинга после установки используйте `--set-monitoring-mode` в [интерфейсе командной строки OneAgent](/docs/ingest-from/dynatrace-oneagent/oneagent-configuration-via-command-line-interface#infrastructure-monitoring "Узнайте, как выполнять некоторые задачи настройки OneAgent без необходимости переустановки.") или настройте его на странице [настроек хоста](/docs/platform/oneagent/monitoring-modes/monitoring-modes "Узнайте больше о доступных режимах мониторинга при использовании OneAgent.").
...
Эта команда добавляет пользовательское имя хоста для отображения в интерфейсе, но обнаруженное имя хоста не изменяется. Подробности см. в разделе [Установка пользовательских имен хостов](/docs/observe/infrastructure-observability/hosts/configuration/set-custom-host-names-in-dynamic-environments "Узнайте, как изменить имя отслеживаемого хоста.").
...
Для изменения имени хоста после установки используйте `--set-host-name` в [интерфейсе командной строки OneAgent](/docs/ingest-from/dynatrace-oneagent/oneagent-configuration-via-command-line-interface "Узнайте, как выполнять некоторые задачи настройки OneAgent без необходимости переустановки.").
...
Для изменения метаданных хоста после установки используйте [Удаленное управление конфигурацией](/docs/ingest-from/bulk-configuration#configure-oneagents "Выполняйте настройку OneAgent и ActiveGate на хостах со страницы статуса развертывания или в масштабе с помощью Dynatrace API.") (выберите действие **изменить свойства хоста**).

В качестве альтернативы вы можете использовать `--set-host-property` в [интерфейсе командной строки OneAgent](/docs/ingest-from/dynatrace-oneagent/oneagent-configuration-via-command-line-interface "Узнайте, как выполнять некоторые задачи настройки OneAgent без необходимости переустановки.").
...
Для изменения тегов хоста после установки используйте [Удаленное управление конфигурацией](/docs/ingest-from/bulk-configuration#configure-oneagents "Выполняйте настройку OneAgent и ActiveGate на хостах со страницы статуса развертывания или в масштабе с помощью Dynatrace API.") (выберите действие **изменить теги хоста**).

В качестве альтернативы вы можете использовать `--set-host-tag` в [интерфейсе командной строки OneAgent](/docs/ingest-from/dynatrace-oneagent/oneagent-configuration-via-command-line-interface "Узнайте, как выполнять некоторые задачи настройки OneAgent без необходимости переустановки.").
...
OneAgent загружает системные логи Linux для диагностики проблем, которые могут быть вызваны условиями в вашем окружении. Подробности см. в разделе [Системные логи, загружаемые OneAgent](/docs/ingest-from/dynatrace-oneagent/installation-and-operation/linux/installation/oneagent-security-linux#system-logs "Узнайте о безопасности Dynatrace OneAgent и изменениях в вашей системе на базе Linux").
...
Если вам нужно изменить этот доступ после установки, используйте [интерфейс командной строки OneAgent](/docs/ingest-from/dynatrace-oneagent/oneagent-configuration-via-command-line-interface "Узнайте, как выполнять некоторые задачи настройки OneAgent без необходимости переустановки."):

Обратите внимание, что это настройка самодиагностики, которая не связана с [Log Monitoring](/docs/analyze-explore-automate/log-monitoring "Узнайте, как включить Log Monitoring, какие возможности он предоставляет, и многое другое.").

## Log Monitoring

**Значение по умолчанию**: `true`

Если установлено в `true`, позволяет OneAgent обращаться к файлам логов для целей Log Monitoring. Допустимые значения: (`true`, `false`) или (`1`, `0`). Эта опция также может быть [включена/отключена через веб-интерфейс](/docs/analyze-explore-automate/log-monitoring "Узнайте, как включить Log Monitoring, какие возможности он предоставляет, и многое другое.").

Например:
`--set-app-log-content-access=true`

Если вам нужно включить или отключить Log Monitoring после установки, используйте `-set-app-log-content-access` в [интерфейсе командной строки OneAgent](/docs/ingest-from/dynatrace-oneagent/oneagent-configuration-via-command-line-interface "Узнайте, как выполнять некоторые задачи настройки OneAgent без необходимости переустановки.").
...
Для получения дополнительной информации см. [Автоматическое внедрение](/docs/ingest-from/dynatrace-oneagent/oneagent-configuration-via-command-line-interface#injection-toggle "Узнайте, как выполнять некоторые задачи настройки OneAgent без необходимости переустановки.").

## Прием локальных метрик

**Значение по умолчанию**: `14449`

Вы можете использовать параметр `--set-extensions-ingest-port=<arg>` для изменения порта связи по умолчанию, используемого для приема локальных метрик. Порт используется [OneAgent REST API](/docs/ingest-from/extend-dynatrace/extend-metrics/ingestion-methods/oneagent-metric-api "Используйте Dynatrace API для получения метрик отслеживаемых сущностей."), [интеграцией скриптов](/docs/ingest-from/extend-dynatrace/extend-metrics/ingestion-methods/oneagent-pipe "Узнайте, как принимать метрики с помощью локальной интеграции скриптов.") (`dynatrace_ingest`) и [Telegraf](/docs/ingest-from/extend-dynatrace/extend-metrics/ingestion-methods/telegraf "Принимайте метрики Telegraf в Dynatrace.").

Для получения дополнительной информации см. [Прием метрик](/docs/ingest-from/extend-dynatrace/extend-metrics "Узнайте, как расширить наблюдаемость метрик в Dynatrace.").

## Прием метрик StatsD

**Значение по умолчанию**: `18125`

Вы можете использовать параметр `--set-extensions-statsd-port=<arg>` для изменения UDP-порта прослушивания [DynatraceStatsD](/docs/ingest-from/extend-dynatrace/extend-metrics/ingestion-methods/statsd "Принимайте метрики в Dynatrace с помощью OneAgent и клиента ActiveGate StatsD.") по умолчанию.
...
* Ядро Linux версии 4.3+ (рекомендуемая версия systemd 221+) для автоматических обновлений OneAgent и полноценной работы без привилегий root.
  Для получения дополнительной информации см. [Непривилегированный режим Linux](/docs/ingest-from/dynatrace-oneagent/installation-and-operation/linux/installation/linux-non-privileged "Узнайте, когда Dynatrace OneAgent требует привилегии root в Linux.")
...
Для получения дополнительной информации см. [требования к разрешениям для установки и работы OneAgent в Linux](/docs/ingest-from/dynatrace-oneagent/installation-and-operation/linux/installation/linux-non-privileged "Узнайте, когда Dynatrace OneAgent требует привилегии root в Linux.").
...
Информацию о механизме автообновления OneAgent см. в разделе [Обновление OneAgent в Linux](/docs/ingest-from/dynatrace-oneagent/installation-and-operation/linux/operation/update-oneagent-on-linux "Узнайте о различных способах обновления OneAgent в Linux.").
...
Для поддерживаемых платформ см. [Поддержка технологий](/docs/ingest-from/technology-support#linux "Узнайте технические подробности поддержки Dynatrace для конкретных платформ и фреймворков разработки.").
