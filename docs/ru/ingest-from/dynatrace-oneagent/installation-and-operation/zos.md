...
С помощью Dynatrace вы можете получить Full-Stack Monitoring с мониторингом хостов (DPS) для Linux на Z, используя OneAgent на Linux. Чтобы узнать больше о поддерживаемых технологиях на архитектуре s390, см. [Поддержка технологий](../../technology-support.md#linux "Технические подробности о поддержке Dynatrace для конкретных платформ и фреймворков разработки.").
