...
В Dynatrace существует [модель разрешений для Grail](/docs/platform/grail/organize-data/assign-permissions-in-grail "Find out how to assign permissions to buckets and tables in Grail."). Она применяется ко всем данным телеметрии: метрикам, событиям, спанам и журналам.
...
Для развёртываний на базе Kubernetes убедитесь, что оператор Dynatrace имеет включённое [обогащение метаданными](/docs/ingest-from/setup-on-k8s/guides/metadata-automation/metadata-enrichment "Metadata enrichment in the Dynatrace Operator adds context to Kubernetes pods by attaching relevant metadata to entities like pods, hosts, and processes for better observability.").
...
* `k8s.cluster.name`
* `k8s.namespace.name`
* Любой другой атрибут, перечисленный в [модели разрешений](/docs/platform/grail/organize-data/assign-permissions-in-grail#grail-permissions-table-record "Find out how to assign permissions to buckets and tables in Grail.")
...
Атрибут `dt.security_context` используется многими функциями Dynatrace и доступен для всех данных телеметрии. Его можно применять для [разрешений на уровне записей](/docs/platform/grail/organize-data/assign-permissions-in-grail#field-permissions "Find out how to assign permissions to buckets and tables in Grail.").
