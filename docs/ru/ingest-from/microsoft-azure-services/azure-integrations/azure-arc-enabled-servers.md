...
* Поддерживаются также классические виртуальные машины
...
* При необходимости определите URL-адрес сервера.
  ...
  URL-адрес сервера требуется только при использовании ActiveGate для конечной точки Dynatrace. URL-адрес автоматически генерируется из идентификатора среды.
...
| hostGroup | Необязательный | Определите группу хостов, к которой принадлежит виртуальная машина. |
