...
В качестве альтернативы вы можете настроить среду Dynatrace SaaS с помощью Azure Marketplace.
...
* Облачные сервисы Azure, включённые по умолчанию, и метрики
...
* Каждый сервис, отслеживаемый Dynatrace через Azure Monitor, а также обработка и анализ логов, приводят к потреблению единиц данных Davis.").
...
* Подробнее о стоимости сбора метрик см. Обзор метрик на основе Grail (DPS).
...
Для управления конфигурацией мониторинга Azure необходимо разрешение **Изменение настроек мониторинга** в Dynatrace. Вы можете настроить разрешение **Изменение настроек мониторинга** на уровне разрешений среды или зоны управления.
...
Чтобы добавить ActiveGate с поддержкой облачного мониторинга, следуйте руководству по установке ActiveGate и вернитесь к этому руководству после завершения.
...
После подключения Dynatrace к вашей среде Azure он немедленно начинает мониторинг встроенных сервисов Azure для определённого субъекта-службы. Классические (ранее «встроенные») метрики Azure содержат список метрик облачных сервисов Azure, отслеживаемых по умолчанию.
...
Чтобы увидеть полный список облачных сервисов Azure и узнать о метриках, собираемых для каждого из них, см. Все облачные сервисы Azure.
...
Вы также можете создать собственный дашборд из метрик, собранных для ваших экземпляров Azure. Подробнее о создании дашбордов см. Дашборды (классический вид).

Виртуальные машины, контейнеры и глубокий мониторинг кода с помощью Dynatrace OneAgent

Dynatrace OneAgent обеспечивает непревзойдённую глубину анализа хостов, контейнеров и кода. Подробнее см. Интеграции Microsoft Azure.

Дополнительная настройка уведомлений и оповещений

После настройки мониторинга Azure вы можете:

* Настроить уведомления мониторинга с помощью Azure Alerts. Это позволяет применять оповещения и автоматически преобразовывать их в события, которые используются Dynatrace Intelligence для более глубокого анализа.
...
Вы также можете мониторить логи Azure. Подробнее см. Логи Azure.