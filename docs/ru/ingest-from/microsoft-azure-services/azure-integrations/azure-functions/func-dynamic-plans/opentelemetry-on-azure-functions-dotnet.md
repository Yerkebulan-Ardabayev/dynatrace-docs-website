...
Убедитесь, что вы выполнили **начальную настройку**, описанную в [Настройка мониторинга OpenTelemetry для Azure Functions на плане потребления](/docs/ingest-from/microsoft-azure-services/azure-integrations/azure-functions/func-dynamic-plans/opentelemetry-on-azure-functions "Мониторинг Azure Functions на плане потребления с помощью OpenTelemetry и Dynatrace."), прежде чем использовать пакеты, описанные ниже.
...
Обратите внимание, что это не включает логирование, если оно не [настроено явно](/docs/ingest-from/microsoft-azure-services/azure-integrations/azure-functions/func-dynamic-plans/opentelemetry-on-azure-functions "Мониторинг Azure Functions на плане потребления с помощью OpenTelemetry и Dynatrace."). См. [InitializeLogging](#initializelogging).
...
* Вызов `InitializeLogging` обязателен, даже если вы не планируете включать логирование, и фактические сообщения логов не будут записываться даже после вызова этого метода, если не выполнена [настройка](/docs/ingest-from/microsoft-azure-services/azure-integrations/azure-functions/func-dynamic-plans/opentelemetry-on-azure-functions "Мониторинг Azure Functions на плане потребления с помощью OpenTelemetry и Dynatrace.").
