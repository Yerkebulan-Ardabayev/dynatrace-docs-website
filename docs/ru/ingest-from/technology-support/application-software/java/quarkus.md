...
* OneAgent или Dynatrace Operator установлен на машине, где будет выполняться приложение.
  ...
  | --- | --- |
  | на виртуальной машине или физическом сервере | OneAgent |
...
Для AOT-компилированных приложений Quarkus, работающих на GraalVM, см. GraalVM Native Image для начала работы.

#### OpenTelemetry

Вы можете экспортировать информацию о трассировке Quarkus с помощью OpenTelemetry в Dynatrace.").
...
Подробнее о процедуре приёма см. Отправка метрик Micrometer в Dynatrace.
...
Используйте платформу Extensions 2.0 Dynatrace для приёма метрик Micrometer, полученных из источника данных Prometheus — для этого необходимо создать пользовательское расширение.
...
Dynatrace предлагает различные варианты для сбора логов из ваших приложений и сред.
...
6. Введите полный путь к вашему файлу логов. Обязательно соблюдайте требования к пути лога.
7. Выберите **Save changes**.
8. Включите добавленные файлы логов в хранилище логов.
