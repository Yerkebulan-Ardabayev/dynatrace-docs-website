...
Полная документация по API доступна в разделе [Events API v2](/docs/dynatrace-api/environment-api/events-v2 "Узнайте, что можно сделать с помощью Dynatrace Events API v2.").
...
Обязательно укажите ваш [идентификатор среды](/docs/discover-dynatrace/get-started/monitoring-environment "Изучите и узнайте, как работать со средами мониторинга.") в правильном месте URL.
...
Для получения дополнительной информации о токенах доступа см. [Dynatrace API — Токены и аутентификация](/docs/dynatrace-api/basics/dynatrace-api-authentication "Узнайте, как пройти аутентификацию для использования Dynatrace API.").
...
Полный список примеров ActiveGate для различных типов запросов см. в [Events API v2](/docs/dynatrace-api/environment-api/events-v2 "Узнайте, что можно сделать с помощью Dynatrace Events API v2.").
...
OneAgent по умолчанию использует TCP-порт 14499 для этой конечной точки. Вы можете изменить порт с помощью [`oneagentctl`](/docs/ingest-from/extend-dynatrace/extend-metrics/ingestion-methods/oneagent-metric-api#communication-port "Используйте Dynatrace API для получения метрик отслеживаемых сущностей.").
...
Следующая команда curl отправляет запрос `POST` на локальную конечную точку OneAgent по пути `/v2/events/ingest`, указывает тип контента JSON и предоставляет [данные события в формате JSON](/docs/dynatrace-api/environment-api/events-v2/post-event#request-body-objects "Загрузка события через Dynatrace API.") с помощью параметра `--data`.
