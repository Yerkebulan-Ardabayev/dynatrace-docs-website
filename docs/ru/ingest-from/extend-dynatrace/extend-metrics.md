...
Расширение наблюдаемости метрик через скриптовую интеграцию Dynatrace.](extend-metrics/ingestion-methods/oneagent-pipe.md "Узнайте, как принимать метрики с помощью локальной скриптовой интеграции.")[### API приёма метрик

Расширение наблюдаемости метрик через открытые API метрик Dynatrace.](extend-metrics/ingestion-methods/oneagent-metric-api.md "Используйте API Dynatrace для получения метрик отслеживаемых сущностей.")
...
Используйте вызов [GET metric data points](../../dynatrace-api/environment-api/metric-v2/get-data-points.md "Чтение точек данных одной или нескольких метрик через Metrics v2 API.") из Metrics API v2 для получения принятых точек данных.

### Data Explorer

Выберите **Create custom chart**, а затем **Try it out** в верхнем баннере. Подробнее см. в разделе [Data Explorer](../../analyze-explore-automate/explorer.md "Запрос метрик и преобразование результатов для получения нужных данных.").
...
* **Кортежи**: уникальные комбинации пар метрика-измерение (см. примеры ниже).
  ...
  + Метрики на базе Grail
    Каждый кортеж учитывается в [лимите кардинальности](../../analyze-explore-automate/metrics/limits.md "Справочник по метрикам на базе Grail") вашей среды.
...
Каждое значение измерения (в данном примере — каждая сетевая карта) генерирует отдельный временной ряд в графике. Поэтому при [расчёте потребления пользовательских метрик](../../license/monitoring-consumption-classic/davis-data-units/metric-cost-calculation.md "Узнайте, как рассчитать потребление единиц данных Davis и затраты, связанные с мониторингом метрик.") каждое значение измерения учитывается как отдельная пользовательская метрика.
