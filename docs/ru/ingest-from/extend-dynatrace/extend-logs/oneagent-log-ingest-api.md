...
Конечная точка загрузки логов OneAgent повторяет поведение публичной конечной точки Log Monitoring API v2 - POST ingest logs.
...
Запрос принимает полезную нагрузку `application/json` с набором символов `charset=utf-8`. Для получения дополнительной информации о формате см. Log Monitoring API v2 - POST ingest logs.
...
Обратите внимание, что изменение порта для API загрузки логов OneAgent также влияет на OneAgent metric API, Metric scripting integration и Telegraf metrics integration.