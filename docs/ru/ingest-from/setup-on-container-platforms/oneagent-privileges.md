...
* Docker вне контейнерной платформы
  ...
  + OneAgent в качестве Docker-контейнера

### OneAgent на хосте Docker

В качестве альтернативы вы также можете развернуть OneAgent на хосте Docker под управлением Linux. В этом случае OneAgent не запускается в контейнере, а работает непосредственно на хосте, поэтому изоляция пространства имён Linux отсутствует. Подробнее см. в OneAgent на Linux.
