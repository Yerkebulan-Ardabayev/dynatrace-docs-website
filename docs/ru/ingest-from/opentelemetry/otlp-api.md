...
В качестве альтернативы вы можете развернуть [Dynatrace Collector](/docs/ingest-from/opentelemetry/collector "Узнайте о Dynatrace OTel Collector.") как промежуточный сервис для пакетирования запросов и повышения сетевой производительности или для преобразования запросов перед их отправкой в Dynatrace (например, [маскирование конфиденциальных данных](/docs/ingest-from/opentelemetry/collector/use-cases/redact "Настройте OpenTelemetry Collector для маскирования конфиденциальных данных перед отправкой в Dynatrace.")).
...
В зависимости от конфигурации вам может потребоваться добавить эти пути отдельно к базовым URL-адресам следующих конечных точек сервисов при указании URL-адресов экспорта. Это может происходить как в коде при использовании [ручного инструментирования](/docs/ingest-from/opentelemetry/walkthroughs "Узнайте, как интегрировать и передавать данные OpenTelemetry (трассировки, метрики и логи) в Dynatrace."), так и с использованием стандартных [переменных окружения](https://opentelemetry.io/docs/languages/sdk-configuration/otlp-exporter/#endpoint-configuration).

## Экспорт в Dynatrace

### Базовые URL-адреса

Следующие адреса предоставляют базовые URL-адреса для конфигурации приёма OTLP. Используйте URL-адрес, соответствующий типу вашей среды, и замените соответствующую часть вашим [идентификатором среды](/docs/discover-dynatrace/get-started/monitoring-environment "Узнайте, как работать со средами мониторинга.").
...
Для этой конфигурации требуется [PVC](/docs/ingest-from/setup-on-k8s/guides/deployment-and-configuration/activegate-pvc "Настройте постоянное хранилище для контейнеризированного ActiveGate, используемого в качестве временного хранилища для принимаемых данных.").
...
Для этого убедитесь, что ваши трассировки имеют правильные атрибуты ресурсов для сопоставления. Список применимых атрибутов можно найти (или импортировать) в [файлах обогащения](/docs/ingest-from/extend-dynatrace/extend-data "Узнайте, как автоматически обогащать телеметрические данные специфичными для Dynatrace полями.").
...
* gRPC не поддерживается.
  ...
  Вы можете использовать Collector для преобразования gRPC OTLP-запроса в его HTTP-аналог, см. [Преобразование OTLP gRPC в HTTP с помощью OpenTelemetry Collector](/docs/ingest-from/opentelemetry/collector/use-cases/grpc "Настройте OpenTelemetry Collector для преобразования gRPC OTLP-запроса в HTTP.").
...
Подробнее о конфигурации для конкретных языков см. [Инструментирование приложения](/docs/ingest-from/opentelemetry/walkthroughs "Узнайте, как интегрировать и передавать данные OpenTelemetry (трассировки, метрики и логи) в Dynatrace.").
...
Подробнее о токенах доступа см. [Dynatrace API — токены и аутентификация](/docs/dynatrace-api/basics/dynatrace-api-authentication "Узнайте, как пройти аутентификацию для использования Dynatrace API.").
...
Использование Collector в качестве промежуточного шлюза позволяет централизованно оптимизировать телеметрические данные и запросы. Подробнее см. [Сценарии использования OpenTelemetry Collector](/docs/ingest-from/opentelemetry/collector/use-cases "Настройте экземпляр Collector для различных сценариев использования.") и примеры конфигураций для популярных сценариев использования Collector.

Подробнее о настройке экземпляра Collector см. [Dynatrace OTel Collector](/docs/ingest-from/opentelemetry/collector "Узнайте о Dynatrace OTel Collector.").
...
Подробнее см. [Преобразование OTLP gRPC в HTTP с помощью OpenTelemetry Collector](/docs/ingest-from/opentelemetry/collector/use-cases/grpc "Настройте OpenTelemetry Collector для преобразования gRPC OTLP-запроса в HTTP.").
...
* [OpenTelemetry Protocol (OTLP) ingest API](/docs/dynatrace-api/environment-api/opentelemetry "Используйте Dynatrace API в качестве целевой точки для экспортёров OpenTelemetry для приёма метрик, логов и трассировок OpenTelemetry.")
