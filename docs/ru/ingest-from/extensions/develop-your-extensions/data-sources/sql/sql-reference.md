...
* Убедитесь, что строка подключения, используемая в конфигурации мониторинга JDBC, не раскрывает конфиденциальных данных. Подробнее см. [Конфигурация мониторинга JDBC](/docs/ingest-from/extensions/develop-your-extensions/data-sources/sql/jdbc-monitoring "Расширения JDBC в фреймворке расширений.").
...
В зависимости от провайдера, определение области мониторинга SQL начинается с выделенного узла YAML. Для Oracle Database это `sqloracle`. Все настройки под этим узлом относятся к объявленному [типу источника данных](/docs/ingest-from/extensions/concepts#data-source-type "Узнайте больше о концепции расширений Dynatrace.") (в данном случае SQL).
...
Пользователи, запускающие ваше расширение, также должны будут загрузить соответствующий драйвер JDBC на ActiveGate, принадлежащий группе, назначенной для запуска вашего расширения. Подробнее см. [Конфигурация мониторинга JDBC](/docs/ingest-from/extensions/develop-your-extensions/data-sources/sql/jdbc-monitoring#upload "Расширения JDBC в фреймворке расширений.").
...
2. Затем используйте ссылку на переменную вместо фиксированного значения в YAML-файле расширения
   ...
   Подробнее об использовании переменных см. [YAML-файл расширения](/docs/ingest-from/extensions/develop-your-extensions/extension-yaml#variables "Узнайте, как создать YAML-файл расширения с помощью фреймворка расширений.")
...
Вы также можете использовать ссылку на переменную для указания таймаута. Подробнее об использовании переменных см. [YAML-файл расширения](/docs/ingest-from/extensions/develop-your-extensions/extension-yaml#variables "Узнайте, как создать YAML-файл расширения с помощью фреймворка расширений.").
...
Строка ключа измерения должна соответствовать [протоколу приёма метрик](/docs/ingest-from/extend-dynatrace/extend-metrics/reference/metric-ingestion-protocol#dimension-optional "Узнайте, как работает протокол приёма данных для Dynatrace Metrics API.").
...
Фильтры могут быть заданы как константное значение или как [переменная](/docs/ingest-from/extensions/develop-your-extensions/extension-yaml#variables "Узнайте, как создать YAML-файл расширения с помощью фреймворка расширений.").
...
Строка ключа метрики должна соответствовать [протоколу приёма метрик](/docs/ingest-from/extend-dynatrace/extend-metrics/reference/metric-ingestion-protocol#metric-key-required "Узнайте, как работает протокол приёма данных для Dynatrace Metrics API.").
...
Формат конфигурации мониторинга зависит от поставщика базы данных. Подробнее см. [Конфигурация мониторинга Oracle Database](/docs/ingest-from/extensions/develop-your-extensions/data-sources/sql/oracle-monitoring "Создание и активация конфигурации мониторинга для расширения на основе источника данных SQL для Oracle Database.").
