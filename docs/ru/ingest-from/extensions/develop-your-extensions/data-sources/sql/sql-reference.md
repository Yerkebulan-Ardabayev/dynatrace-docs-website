...
* Убедитесь, что строка подключения, используемая в конфигурации мониторинга JDBC, не раскрывает конфиденциальных данных. Подробнее см. Конфигурация мониторинга JDBC.
...
Расширение может определять метаданные для каждой метрики, доступной в Dynatrace. Например, вы можете добавить отображаемое имя метрики и единицу измерения, которые можно использовать для фильтрации в Обозревателе метрик.
...
Формат конфигурации мониторинга зависит от поставщика базы данных. Подробнее см. Конфигурация мониторинга Oracle Database.
