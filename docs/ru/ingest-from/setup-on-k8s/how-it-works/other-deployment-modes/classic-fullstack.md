...
Дополнительную информацию см. в разделе `.spec.oneAgent.classicFullStack` документации DynaKube.
...
* Dynatrace Operator управляет автоматическим развёртыванием, конфигурацией и жизненным циклом компонентов Dynatrace в вашей среде Kubernetes.
* Веб-хук Dynatrace Operator проверяет определения DynaKube, конвертирует определения со старыми версиями API и внедряет конфигурации в поды.
...
* Dynatrace OneAgent собирает метрики хостов с узлов Kubernetes.
* Dynatrace ActiveGate маршрутизирует данные наблюдаемости в кластер Dynatrace и осуществляет мониторинг Kubernetes API.
