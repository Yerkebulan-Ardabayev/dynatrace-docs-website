...
* [Введение в сетевые зоны](/docs/manage/network-zones "Find out how network zones work in Dynatrace.") и [базовая информация](/docs/manage/network-zones/network-zones-basic-info#activate "Learn how to get started with network zones.")
* Подключение [OneAgent](/docs/manage/network-zones/oneagent-connectivity "Find out how network zones prioritize ActiveGates for OneAgent connectivity.") и [ActiveGate](/docs/manage/network-zones/activegate-connectivity "Find out how network zones prioritize ActiveGates for Environment ActiveGate connectivity.")
...
Кроме того, может потребоваться [настроить прокси](/docs/ingest-from/setup-on-k8s/guides/networking-security-compliance/network-configurations#configure-proxy "Configure Dynatrace in network-restricted environments, network-related settings and proxy configurations.") для обеспечения контролируемого сетевого доступа в кластерах Kubernetes с ограниченным исходящим трафиком.

1. Выполните следующую команду для создания сетевой зоны в резервном режиме *None* с помощью [Dynatrace API](/docs/dynatrace-api/environment-api/network-zones/put-network-zone "Update a network zone via the Dynatrace API.").
...
* [Начало работы с сетевыми зонами](/docs/manage/network-zones/network-zones-basic-info "Learn how to get started with network zones.")
