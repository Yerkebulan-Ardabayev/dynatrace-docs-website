...
Dynatrace может автоматически создавать непрерывный поток сервисов для IBM MQ, когда сервисы-производители и сервисы-потребители используют одно и то же имя очереди или темы. Если сервисы-производители и потребители обращаются к разным именам очередей или тем, для создания непрерывного потока сервисов может потребоваться настройка IBM MQ.
...
* Согласованное адаптивное управление трафиком во всей среде мониторинга, снижающее объём трассировок IBM MQ.
...
Вы можете управлять конфигурацией IBM MQ автоматически, установив расширение IBM MQ и активировав **Retrieve topology for improved transaction tracing** для получения конфигурации IBM MQ вашей среды и отправки её в Settings API. Это также можно сделать вручную через веб-интерфейс или Settings API.
...
Вы можете управлять конфигурацией IBM MQ через Settings API Dynatrace.
...
Следуйте приведённым ниже процедурам для создания или обновления элемента конфигурации. Обратите внимание, что область действия этих элементов всегда является средой. Перед началом изучите формат объекта настроек, запросив его схему через вызов GET a schema.
...
1. Создайте JSON-объект для ваших настроек.
   ...
   ```
2. Используйте эндпоинт POST an object для отправки вашей конфигурации.
...
1. Запросите текущую конфигурацию через вызов GET objects.
2. Создайте JSON для обновления.
   ...
   * Измените значения по мере необходимости.
     ...
     ```
3. Используйте эндпоинт PUT an object для отправки вашей конфигурации.
