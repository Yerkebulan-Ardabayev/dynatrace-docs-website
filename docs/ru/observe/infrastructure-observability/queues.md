...
Очереди сообщений в форме очереди или топика предоставляют легковесное хранилище для сообщений. Они предлагают конечные точки, которые позволяют приложениям отправлять в них сообщения, а также конечные точки, позволяющие приложениям асинхронно получать сообщения или подписываться на топики. Подробную информацию см. в разделе [Концепции очередей](/docs/observe/infrastructure-observability/queues/queue-concepts "Основные концепции мониторинга очередей сообщений в Dynatrace.").
...
Изучите основные концепции мониторинга очередей.](/docs/observe/infrastructure-observability/queues/queue-concepts "Основные концепции мониторинга очередей сообщений в Dynatrace.")[### Настройка

Настройте мониторинг, трассировку для IBM MQ, теги и зоны управления.](/docs/observe/infrastructure-observability/queues/configuration "Настройка Dynatrace для мониторинга очередей сообщений.")[### Анализ

Анализируйте очереди и топики в вашей среде.](/docs/observe/infrastructure-observability/queues/analyze-queues "Получите информацию об аномалиях, связанных с очередями сообщений, с помощью аналитических представлений.")

## Очереди и топики в Dynatrace

OneAgent автоматически обнаруживает очереди и топики в рамках распределённых трассировок, когда мониторируемые приложения используют конечные точки совместимых клиентов обмена сообщениями для отправки или получения сообщений. Список совместимых клиентов см. в разделе [Поддержка технологий](/docs/ingest-from/technology-support "Техническая информация о поддержке Dynatrace для конкретных платформ и фреймворков разработки.").
...
* Выберите **Имя** конкретной очереди или топика, чтобы отобразить его [аналитическое представление](/docs/observe/infrastructure-observability/queues/analyze-queues "Получите информацию об аномалиях, связанных с очередями сообщений, с помощью аналитических представлений.") с расширенными возможностями диагностики для получения дополнительной информации о связанных аномалиях.
...
Нет. Очереди и топики обнаруживаются в рамках распределённых трассировок, когда OneAgent работает в режиме [Full-Stack Monitoring](/docs/platform/oneagent/monitoring-modes/monitoring-modes "Узнайте больше о доступных режимах мониторинга при использовании OneAgent.").

Может ли OneAgent обнаруживать очереди и топики в режиме Infrastructure Monitoring?

Нет. Очереди и топики обнаруживаются в рамках распределённых трассировок только тогда, когда OneAgent работает в режиме [Full-Stack Monitoring](/docs/platform/oneagent/monitoring-modes/monitoring-modes "Узнайте больше о доступных режимах мониторинга при использовании OneAgent.").
...
OneAgent поддерживает различные клиенты обмена сообщениями. Список совместимых клиентов см. в разделе [Поддержка технологий](/docs/ingest-from/technology-support "Техническая информация о поддержке Dynatrace для конкретных платформ и фреймворков разработки.").
...
Обратитесь к соответствующему разделу на странице [Теги и зоны управления](/docs/observe/infrastructure-observability/queues/configuration/tags-and-management-zones#tags "Автоматическое применение тегов к очередям и их организация в зоны управления.").

Как добавить сущности очередей в существующие зоны управления?

Обратитесь к соответствующему разделу на странице [Теги и зоны управления](/docs/observe/infrastructure-observability/queues/configuration/tags-and-management-zones#management-zones "Автоматическое применение тегов к очередям и их организация в зоны управления.").
