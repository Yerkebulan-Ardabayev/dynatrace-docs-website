---
title: Использование свойств действий пользователя и пользовательских сессий для веб-приложений
...
Прежде чем использовать свойства действий и сеансов пользователя, необходимо определить эти свойства в настройках приложения. Подробнее см. Define user action and user session properties for web applications.

## Анализ сеансов пользователей

Страница **User sessions** позволяет фильтровать сеансы пользователей по свойствам действий и сеансов. Например, если вы проводите программу лояльности, вы можете добавить свойство `loyalty_status`, чтобы узнать, является ли пользователь в отслеживаемом сеансе участником уровня `Silver`, `Gold` или `Platinum`. Затем вы можете отфильтровать клиентов `Platinum` или `Gold` на странице **User sessions**.
...
User sessions API в сочетании с User Sessions Query Language (USQL) позволяет получить доступ ко всем данным действий и сеансов пользователя, включая свойства и значения.
...
Для веб-приложений вы можете дополнительно создать вычисляемые метрики на основе пользовательских свойств, использовать эти метрики для создания диаграммы, а затем закрепить её на дашборде. Это можно сделать в Data Explorer.
...
Подробнее см. Export user sessions.

## Многомерный анализ

Вы также можете использовать пользовательские свойства в Multidimensional analysis. Например, вы можете анализировать действия пользователей на основе ваших пользовательских свойств или фильтровать данные по значениям свойств.
...
5. Необязательно: выберите **Create a chart** или **Create alert**. Подробнее см. Data Explorer и Metric events.
...
8. Необязательно: из списка метрик разверните созданную метрику и выберите **Create a chart** или **Create alert**. Подробнее см. Data Explorer и Metric events.
