...
* Предоставлять необходимую информацию для связи пользовательских действий с [серверными распределёнными трассировками](/docs/observe/application-observability/distributed-traces "Gain observability into highly distributed, cloud-native architectures to effectively trace and analyze transactions in real time.")
...
* Если вы предпочитаете вставлять RUM JavaScript вручную, не подавляйте внедрение, отключая Real User Monitoring для своих групп процессов. Это подавляет не только внедрение, но и связь пользовательских действий с распределёнными трассировками. Вместо этого используйте пользовательское правило внедрения, как описано в разделе [Configure automatic injection](/docs/observe/digital-experience/web-applications/initial-setup/rum-injection#manual-insertion-using-oneagent "Configure automatic injection of the RUM JavaScript into the pages of your applications").
