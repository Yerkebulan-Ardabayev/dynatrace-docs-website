...
Dynatrace связывает действия пользователей и их соответствующие распределённые трассировки, обеспечивая непрерывную сквозную видимость. Например, это позволяет определить [топ-3 веб-запроса-контрибьютора](../analyze-and-use/analyze-individual-user-actions.md#top-3-web-request-contributors "Understand how you can access user action detail pages and analyze user actions.") на странице деталей действия пользователя или перейти из каскадной диаграммы анализа к распределённым трассировкам.
...
* Два задействованных домена имеют как минимум общий [эффективный домен верхнего уровня плюс один (eTLD+1)](https://web.dev/same-site-same-origin/), что позволяет указать общий домен cookie.
...
Если заголовок `x-dtc` не добавляется к запросу после выполнения приведённых выше инструкций, проверьте ваше регулярное выражение и убедитесь, что RUM настроен для захвата XHR-действий.
