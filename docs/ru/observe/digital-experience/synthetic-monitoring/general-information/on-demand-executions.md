...
Помимо запуска синтетических мониторов по расписанию с заданными интервалами (в настройках **Частота и локации**), вы также можете запускать выполнение по требованию для мониторов [браузера](../browser-monitors/configure-browser-monitors.md "Узнайте о настройке браузерных мониторов и clickpath-сценариев."), а также [HTTP](../http-monitors-classic/configure-http-monitors-classic.md "Узнайте о настройке HTTP-мониторов.") мониторов. Вы даже можете запускать выполнение по требованию для [автоматически созданных HTTP-мониторов для синхронизации учётных данных](external-vault-integration.md "Синхронизируйте учётные данные Synthetic Monitoring с внешними хранилищами."). В [веб-интерфейсе Dynatrace](#ui) выберите **Выполнение по требованию** в верхней части страницы сведений о мониторе.
...
Благодаря возможности включения или исключения из общих результатов монитора и обнаружения проблем, выполнения по требованию предлагают значительно больше возможностей, чем [локальное воспроизведение браузерных clickpath-сценариев](../browser-monitors/record-a-browser-clickpath.md#playback "Узнайте, как записать браузерный clickpath-сценарий для мониторинга доступности и производительности приложения.").
...
Даже если вы [отключите запланированное выполнение синтетического монитора в течение окон обслуживания](../../../../analyze-explore-automate/notifications-and-alerting/maintenance-windows/define-maintenance-window.md#disable-synthetic "Создавайте окна обслуживания и определяйте их область."), вы всё равно можете запускать мониторы по требованию.
...
3. Выберите **Режим обработки** — обратите внимание, что он применяется ко всем выполнениям при выборе всех локаций.
   ...
   * **Стандартный** (по умолчанию) — выполнения учитываются при обнаружении проблем, а также в статистике доступности и производительности. Для браузерных мониторов данные можно просматривать на [странице **Многомерный анализ**](../analysis-and-alerting/multidimensional-analysis-for-browser-monitors.md "Узнайте, как анализировать точки данных браузерного монитора."). Для HTTP-мониторов результаты выполнения доступны при [**Анализе деталей выполнения**](../analysis-and-alerting/synthetic-details-for-http-monitors-classic.md "Узнайте о странице сведений Synthetic для HTTP-мониторов.").
...
5. Только для HTTP-мониторов **Сбой при отсутствующем или истекающем SSL-сертификате** — сбой монитора, если один или несколько запросов обнаруживают истёкший, отсутствующий или истекающий SSL-сертификат.
   ...
   Этот параметр работает только в том случае, если **Проверка даты истечения SSL** уже включена для HTTP-запроса в [настройках монитора](../http-monitors-classic/configure-http-monitors-classic.md "Узнайте о настройке HTTP-мониторов."); он использует настройку на уровне запроса для проверки действительности SSL-сертификата в течение указанного количества дней.
...
* При запуске нескольких выполнений по требованию через [API](on-demand-executions.md#api "Запускайте синтетические мониторы по требованию из публичных или частных локаций") действует ограничение в 100 выполнений на пакет.
* Также существует ограничение в 5000 выполнений по требованию в минуту для среды Dynatrace.
* Вы можете определить до 64 пар ключ-значение [метаданных на пакет](on-demand-executions.md#api "Запускайте синтетические мониторы по требованию из публичных или частных локаций"), где ключи и значения могут иметь длину до 1024 символов каждый.
* При указании [повторных выполнений по требованию на локацию](on-demand-executions.md#api "Запускайте синтетические мониторы по требованию из публичных или частных локаций") максимальное количество выполнений равно 10.
...
Для браузерных мониторов вы будете перенаправлены на страницу **Многомерный анализ** с выбранной точкой данных на диаграмме рассеяния. Выполнения по требованию можно идентифицировать по форме на [диаграмме рассеяния](../analysis-and-alerting/multidimensional-analysis-for-browser-monitors.md#scatter-plot "Узнайте, как анализировать точки данных браузерного монитора."), а также по аннотации в списке [точек данных](../analysis-and-alerting/multidimensional-analysis-for-browser-monitors.md#data-points "Узнайте, как анализировать точки данных браузерного монитора.").

![Диаграмма рассеяния](https://dt-cdn.net/images/scatterplotondemandexecution-2164-393f41dda4.jpg)

Создание снимков экрана для браузерных мониторов при выполнении по требованию аналогично [запланированным выполнениям](../analysis-and-alerting/synthetic-details-for-browser-monitors.md#screenshots "Анализируйте результаты браузерных мониторов и clickpath-сценариев на странице сведений Synthetic."). Однако вы можете включить создание снимков экрана при успехе [через веб-интерфейс](#trigger-ui) и через POST-запросы [через API](#api).

Для HTTP-мониторов вы будете перенаправлены на вкладку **Выполнение по требованию** страницы **Анализ деталей выполнения**. Также вы можете перейти на эту вкладку, выбрав **Анализ деталей выполнения** на [странице сведений HTTP-монитора](../analysis-and-alerting/synthetic-details-for-http-monitors-classic.md "Узнайте о странице сведений Synthetic для HTTP-мониторов."). Выберите выполнение по требованию **Выполнение** из выпадающего списка для просмотра его деталей.
...
Дополнительную информацию см. в документации по [API v2 выполнения синтетических мониторов](../../../../dynatrace-api/environment-api/synthetic-v2/synthetic-monitor-execution.md "Просматривайте результаты выполнения синтетических мониторов через Synthetic v2 API."). Вы можете [запустить пакетное выполнение](../../../../dynatrace-api/environment-api/synthetic-v2/synthetic-monitor-execution/post-batch-execution.md "Запустите пакетное выполнение синтетических мониторов через Dynatrace API."), [получить список выполнений по требованию](../../../../dynatrace-api/environment-api/synthetic-v2/synthetic-monitor-execution/get-all-executions.md "Получите список всех выполнений синтетических мониторов по требованию через Dynatrace API."), проверить [основные](../../../../dynatrace-api/environment-api/synthetic-v2/synthetic-monitor-execution/get-execution-basic.md "Просматривайте основную информацию о выполнении синтетического монитора по требованию через Dynatrace API.") и [подробные результаты](../../../../dynatrace-api/environment-api/synthetic-v2/synthetic-monitor-execution/get-execution-full.md "Просматривайте полную информацию о выполнении синтетического монитора по требованию через Dynatrace API.") отдельного выполнения, а также проверить [результаты пакетного выполнения](../../../../dynatrace-api/environment-api/synthetic-v2/synthetic-monitor-execution/get-batch-summary.md "Просматривайте сводку пакетного выполнения синтетических мониторов через Dynatrace API.").
...
* [API v2 выполнения синтетических мониторов](../../../../dynatrace-api/environment-api/synthetic-v2/synthetic-monitor-execution.md "Просматривайте результаты выполнения синтетических мониторов через Synthetic v2 API.")
