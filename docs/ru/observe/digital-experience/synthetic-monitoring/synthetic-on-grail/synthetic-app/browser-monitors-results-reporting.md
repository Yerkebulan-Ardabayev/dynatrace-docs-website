...
В верхнем правом углу страницы вы можете получить доступ к [**Анализу выполнений**](#analyze-executions), [**Выполнению по запросу**](on-demand-executions.md "Узнайте о выполнении по запросу.") и дополнительным настройкам монитора браузера:
...
Также, если монитор не работает (недоступен) в одной или нескольких локациях, продолжительность сбоя (простоя) и затронутые локации отображаются в [визуализациях метрик](browser-monitors-results-reporting.md#metric-visualizations "Узнайте о странице деталей браузера для мониторов браузера.") над карточкой **Доступность**.
...
* **Окно обслуживания** — в данном временном периоде было активно окно обслуживания.
  ...
  + Когда опция **Отключить выполнение синтетического монитора** включена в окне обслуживания, запланированные автоматические тесты подавляются в период обслуживания, поэтому для этого периода нет результатов тестов. Только тесты [по запросу](on-demand-executions.md "Узнайте о выполнении по запросу.") не подавляются.
...
2. Найдите нужное выполнение в таблице **Выполнения**. Каждая строка таблицы — это запись о выполнении. Вы можете использовать фильтр для сужения списка выполнений. Фильтровать можно по локации или временному периоду.
   ...
   Если включён [автоматический повтор при ошибке](create-configure-browser-monitors.md#outage-and-performance "Узнайте, как создать и настроить монитор браузера для проверки производительности и доступности вашего сайта."), монитор выполняет вторую попытку в случае неудачи первой. В таблице **Выполнения** вторая попытка отображается по умолчанию, и в записи о выполнении появляется значок — выберите его, чтобы увидеть также первую попытку.
