...
Верхняя панель показывает общую [доступность](#availability) монитора или инфографику [производительности](#performance) для выбранного временного диапазона. Выберите любую инфографику, чтобы перейти к соответствующей карточке. Инфографика **Доступности** показывает процент доступности и время простоя за выбранный временной диапазон. Инфографика производительности настроена для отображения выбранных вами [**ключевых показателей производительности**](../browser-monitors/configure-browser-monitors.md#kpm "Узнайте о настройке мониторов браузера и кликпутей.") вместе с **Общей длительностью**.

Вы также можете пролистать справочные [скриншоты](#screenshots) для каждого из [событий скрипта](../browser-monitors/browser-clickpath-events.md "Узнайте о типах событий, создаваемых при записи кликпути браузера.") вашего монитора.
...
* Появление проблем и получение уведомлений во время окон обслуживания зависит от того, как вы [настроите окна обслуживания](../../../../analyze-explore-automate/notifications-and-alerting/maintenance-windows/define-maintenance-window.md "Создание окон обслуживания и определение их области действия.").
* [Окна обслуживания могут быть исключены из расчетов доступности](../general-information/synthetic-calculations.md#m-windows-availability "Понимание расчетов метрик Synthetic Monitoring.") с помощью глобальной настройки.

### Скриншоты

Справочные скриншоты, показанные на верхней панели страницы деталей Synthetic, делаются при успешном запуске монитора после его создания или редактирования, а затем раз в 24 часа из случайной локации мониторинга (включая [приватные локации](../private-synthetic-locations/create-a-private-synthetic-location.md "Узнайте, как создать приватную локацию для синтетического мониторинга.")). Скриншоты делаются в конце каждого события скрипта (даже тех, у которых нет таймингов). Вы можете пролистать скриншоты здесь или выбрать изображение для просмотра увеличенных версий.
...
Когда монитор завершается неудачей, скриншоты для каждого неудачного выполнения (SCoE) доступны на [странице многомерного анализа](multidimensional-analysis-for-browser-monitors.md "Узнайте, как анализировать точки данных мониторов браузера.").
...
Обратите внимание, что монитор может быть недоступен в одной или всех локациях, даже если не установлены [пороги сбоев (глобальные или локальные)](../browser-monitors/configure-browser-monitors.md#outage-handling "Узнайте о настройке мониторов браузера и кликпутей."). Длительность сбоя — это сумма всех простоев за выбранный временной диапазон без учета пересекающихся простоев. См. [Расчеты Synthetic](../general-information/synthetic-calculations.md "Понимание расчетов метрик Synthetic Monitoring.") для подробностей о том, как рассчитываются доступность и простой.
...
Разверните **Все локации**, чтобы просмотреть графики доступности для каждой локации. Выберите блок простоя монитора (красный) или доступности (фиолетовый) на временной шкале любой локации, а затем выберите **Анализировать**, чтобы увидеть [Многомерный анализ](multidimensional-analysis-for-browser-monitors.md "Узнайте, как анализировать точки данных мониторов браузера.") для доступности, отфильтрованный по этой локации. Вы также можете выбрать имя локации или **Анализировать доступность**, чтобы просмотреть точки данных для всех локаций.
...
Для каждого монитора браузера или кликпути вы можете выбрать другой ключевой показатель производительности для действий загрузки и XHR-действий в [режиме редактирования](../browser-monitors/configure-browser-monitors.md#kpm "Узнайте о настройке мониторов браузера и кликпутей.") после создания монитора.
...
Карточка **Синтетические события и действия** помогает различать [события](../browser-monitors/browser-clickpath-events.md "Узнайте о типах событий, создаваемых при записи кликпути браузера.") скрипта с таймингами и без них. Выберите **все события** или **события с таймингами** и тип действия (**действия загрузки** или **XHR-действия**) для фильтрации карточки. При просмотре всех событий вы видите среднюю Общую длительность составляющих действий. При просмотре событий по типу действия вы видите выбранный [ключевой показатель производительности](../../web-applications/analyze-and-use/work-with-key-performance-metrics.md "Узнайте, как использовать правильные ключевые показатели производительности для оптимизации данных о пользовательском опыте для каждого приложения.").
...
Карточка **Проблемы** показывает проблемы производительности (нарушение порога) и доступности (локальный или глобальный сбой), когда вы включаете соответствующие пороги в [настройках монитора](../browser-monitors/configure-browser-monitors.md "Узнайте о настройке мониторов браузера и кликпутей."). Разверните карточку, чтобы увидеть активные и решенные проблемы за выбранный временной диапазон.
...
Любые [пользовательские оповещения](../../../../dynatrace-intelligence/root-cause-analysis/event-analysis-and-correlation/event-categories/custom-alerts.md "Узнайте больше о пользовательских оповещениях и логике их создания.") на основе пользовательских порогов также отображаются здесь.

![Карточка проблем](https://dt-cdn.net/images/clickpathdetailsproblemscard-1094-0de0568223.png)

См. [Расчеты Synthetic](../general-information/synthetic-calculations.md "Понимание расчетов метрик Synthetic Monitoring.") для информации о том, как рассчитываются доступность и производительность и как проблемы генерируются и закрываются.
...
Карточка **События** показывает все события, которые могут привести к проблемам. [Пользовательские события](../../../../dynatrace-api/environment-api/events-v1.md "Узнайте, что вы можете делать с Dynatrace Events API."), которые вы отправляете в Dynatrace, также отображаются на этой карточке. События для активных и решенных проблем отображаются в списке и на временной шкале.
...
Выберите **Анализировать ошибки**, чтобы просмотреть выполнения с этими ошибками на [странице многомерного анализа](multidimensional-analysis-for-browser-monitors.md "Узнайте, как анализировать точки данных мониторов браузера."). Выберите **Анализировать** ![Анализировать](https://dt-cdn.net/images/analyze-icon-e97b8dab47.svg "Анализировать") рядом с отдельной ошибкой, чтобы просмотреть многомерный анализ, отфильтрованный по этой ошибке.
