...
Вы можете управлять локациями Kubernetes/OpenShift через веб-интерфейс Dynatrace и существующий [API v2 Synthetic — Локации, узлы и конфигурация](../../../../dynatrace-api/environment-api/synthetic-v2/synthetic-locations-v2.md "Управляйте синтетическими локациями через Synthetic v2 API."). Дополнительные конечные точки раннего доступа в этом API облегчают развёртывание локаций Kubernetes; новые конечные точки помогают генерировать команды, которые необходимо выполнить в кластере Kubernetes.

Вы можете выполнять как запланированные, так и [внеплановые](../general-information/on-demand-executions.md "Запускайте синтетические мониторы по требованию из публичных или частных локаций") выполнения всех [типов синтетических мониторов](../general-information/types-of-synthetic-monitors.md "Узнайте о типах синтетических мониторов Dynatrace.") в контейнеризованных локациях.
...
* Если локация отслеживается с помощью [OneAgent](../../../../ingest-from/dynatrace-oneagent/oa-requirements.md "Требования к кодовым модулям OneAgent") или другого решения глубокого мониторинга, требования к памяти (RAM) возрастут.
...
* Если вы хотите установить локацию в том же пространстве имён, что и другие ресурсы Dynatrace, например [Dynatrace Operator](../../../../ingest-from/setup-on-k8s/guides/deployment-and-configuration/resource-management/dto-resource-limits.md "Задайте лимиты ресурсов для компонентов Dynatrace Operator."), обратите внимание на более высокие [требования к оборудованию и системе](#requirements) для контейнеризованных Synthetic-совместимых ActiveGate.
...
1. Переведите локацию в режим FIPS
   ...
   В настоящее время это возможно только с помощью [REST API](../../../../dynatrace-api/environment-api/synthetic-v2/synthetic-locations-v2.md "Управляйте синтетическими локациями через Synthetic v2 API."), устанавливая свойство `fipsMode` в запросном JSON.

   * Для создания новой локации используйте вызов [POST location](../../../../dynatrace-api/environment-api/synthetic-v2/synthetic-locations-v2/post-a-location.md "Создайте частную синтетическую локацию через Synthetic v2 API.").
   * Для обновления существующей локации используйте вызов [PUT location](../../../../dynatrace-api/environment-api/synthetic-v2/synthetic-locations-v2/put-a-location.md "Обновите частную синтетическую локацию через Synthetic v2 API.").
2. Выполните дополнительную настройку
   ...
   Предоставьте сертификат для повторного подписания HTTPS-запросов (описано в разделе [Режим FIPS](setting-up-proxy-for-private-synthetic.md#fips-proxy "Узнайте, как настроить свойства ActiveGate для настройки прокси для частного синтетического мониторинга.")):

   ```
   kubectl -n $NAMESPACE create secret tls synthetic-fips-proxy-cert --cert=squid.crt --key=squid.key
   ```

   1. Предоставьте сертификат для повторного подписания HTTPS-запросов (описано в разделе [Режим FIPS](setting-up-proxy-for-private-synthetic.md#fips-proxy "Узнайте, как настроить свойства ActiveGate для настройки прокси для частного синтетического мониторинга.")):

      ```
      kubectl -n $NAMESPACE create secret tls synthetic-fips-proxy-cert --cert=squid.crt --key=squid.key
      ```
   2. Предоставьте настройку корпоративного прокси для Squid (описано в разделе [Режим FIPS с корпоративным прокси](setting-up-proxy-for-private-synthetic.md#fips-corporate-proxy "Узнайте, как настроить свойства ActiveGate для настройки прокси для частного синтетического мониторинга.")):
...
Вы можете автоматизировать развёртывание и управление контейнеризованными локациями через существующий [API v2 Synthetic — Локации, узлы и конфигурация](../../../../dynatrace-api/environment-api/synthetic-v2/synthetic-locations-v2.md "Управляйте синтетическими локациями через Synthetic v2 API."). В этот API добавлены конечные точки раннего доступа для упрощения развёртывания локаций Kubernetes. Новые конечные точки помогают генерировать команды, которые необходимо выполнить в кластере Kubernetes.
...
* [API v2 синтетических локаций](../../../../dynatrace-api/environment-api/synthetic-v2/synthetic-locations-v2.md "Управляйте синтетическими локациями через Synthetic v2 API.")
* [Настройка Dynatrace на Kubernetes](../../../../ingest-from/setup-on-k8s.md "Способы развёртывания и настройки Dynatrace на Kubernetes")
* [Kubernetes Classic](../../../infrastructure-observability/container-platform-monitoring/kubernetes-monitoring.md "Мониторинг Kubernetes/OpenShift с помощью Dynatrace.")
