...
В этом руководстве мы будем использовать интернет-магазин в качестве примера. Магазин инструментирован с помощью RUM JavaScript, а собранные данные привязаны к фронтенду с именем `webshop`.

Инструментация была настроена для отправки свойства пользовательской сессии `successful_checkout` каждый раз, когда клиент успешно завершает процесс оформления заказа. Свойство было сконфигурировано, как описано в Захват свойств событий и сессий для веб-фронтендов, а затем отправлено через JavaScript API с помощью [`sendSessionPropertyEvent`](https://docs.dynatrace.com/javascriptapi/doc-latest/functions/Types.dynatrace.sendSessionPropertyEvent.html):
...
Убедитесь, что у вас есть разрешения, описанные в Разрешения New RUM Experience.
