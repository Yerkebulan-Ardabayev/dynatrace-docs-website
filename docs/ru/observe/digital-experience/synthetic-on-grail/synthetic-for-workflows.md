...
Также возможно использовать выражения рабочих процессов для извлечения списка мониторов и локаций из входящих событий, что позволяет создавать повторно используемые рабочие процессы.
...
После установки действия `synthetic_for_workflows` автоматически появятся в разделе **Choose action** в **Workflows**.
...
3. Для **From event** вы можете определить список синтетических мониторов, которые хотите запускать в рабочем процессе, на основе входящего события. Для этого необходимо создать выражение, которое позволит рабочему процессу извлечь эту информацию из события и запустить выбранные мониторы.
...
2. Заполните поля **Key** и **Value**. В этих полях можно использовать выражения рабочих процессов.
...
Обратите внимание, что Synthetic **не** генерирует события Grail для тенантов без DPS. Вся обработка будет основываться исключительно на данных API. Чтобы узнать больше о лицензировании DPS, см. Лицензирование Dynatrace, модели лицензирования для всех возможностей Dynatrace.").
...
Если вы хотите оценивать цели на основе результатов синтетических мониторов, вы можете интегрировать Site Reliability Guardian (SRG) в качестве следующего действия в вашем рабочем процессе.
...
9. Нажмите ![Add](https://dt-cdn.net/images/dashboards-app-menu-plus-7e9b7c3547.svg "Add") **Add new objective** и укажите один из следующих запросов DQL.
...
* Обновления имён атрибутов Synthetic
