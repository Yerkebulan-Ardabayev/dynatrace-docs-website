...
В вашей среде существуют тысячи запросов, каждый со своими связями и контекстом. Чтобы определить корневую причину неэффективности, необходимо сузить анализ до нужных запросов. Вы можете сегментировать запросы, применяя фильтрацию в сервисном потоке, или через анализ выбросов.
...
* Чтобы увидеть, откуда пришёл запрос, выберите **More** (**...**) > **Service backtrace**.
