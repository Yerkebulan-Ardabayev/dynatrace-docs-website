...
* Для запуска создания дампов памяти вам необходимо разрешение пользователя [**View sensitive request data**](/docs/manage/identity-access-management/permission-management/role-based-permissions#environment "Role-based permissions").
* Для хранения дампов памяти на сервере приложений должно быть достаточно свободного места.
* Для доступа к сохраненным дампам памяти через веб-интерфейс Dynatrace [настройте ActiveGate для хранения дампов памяти в централизованном расположении](/docs/observe/application-observability/profiling-and-optimization/memory-dump-analysis/configure-an-activegate-for-memory-dump-storage "Learn how to enable storage of memory dumps on an ActiveGate."). Подробнее о сроках хранения дампов памяти см. в разделе [Сроки хранения данных](/docs/manage/data-privacy-and-security/data-privacy/data-retention-periods#memory-dumps "Check retention times for various data types.").
...
* Вы можете включить удаленный доступ к ActiveGate, изменив публичные конечные точки.
  ...
  + [Включение модуля дампов памяти](/docs/ingest-from/dynatrace-activegate/configuration/configure-activegate#mem_dump_mod "Learn which ActiveGate properties you can configure based on your needs and requirements.")
  + [Настройка ActiveGate для хранения дампов памяти](/docs/observe/application-observability/profiling-and-optimization/memory-dump-analysis/configure-an-activegate-for-memory-dump-storage "Learn how to enable storage of memory dumps on an ActiveGate.")
* Если удаленный доступ к ActiveGate невозможен, вы можете скачать файл дампа памяти вручную с хоста ActiveGate.
  ...
  + Чтобы скачать файл дампа памяти, вам необходимо [узнать расположение файла](/docs/observe/application-observability/profiling-and-optimization/memory-dump-analysis/configure-an-activegate-for-memory-dump-storage#specify-dedicated-dump-directory "Learn how to enable storage of memory dumps on an ActiveGate.").
...
* [Разрешения на основе ролей](/docs/manage/identity-access-management/permission-management/role-based-permissions "Role-based permissions")