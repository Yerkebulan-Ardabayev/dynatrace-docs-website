...
Расширенная аналитика трассировки на базе Grail предназначена для опытных пользователей, которым требуется продвинутый анализ трассировки, доступный через [Дашборды](/docs/analyze-explore-automate/dashboards-and-notebooks/dashboards-new "Создавайте интерактивные, настраиваемые представления для визуализации, анализа и обмена данными наблюдаемости в реальном времени."), [Ноутбуки](/docs/analyze-explore-automate/dashboards-and-notebooks/notebooks "Анализируйте, визуализируйте и делитесь информацией из данных наблюдаемости — всё в одном совместном настраиваемом рабочем пространстве."), [Workflows](/docs/analyze-explore-automate/workflows "Автоматизируйте ИТ-процессы с помощью Dynatrace Workflows — реагируйте на события, планируйте задачи и подключайте сервисы.") и через [Dynatrace API](/docs/dynatrace-api "Узнайте, что вам нужно для использования Dynatrace API.").

## Перед началом работы

Необходимые условия

* Убедитесь, что у вашего пользователя есть [разрешения](/docs/observe/application-observability/distributed-tracing/permissions "Управление разрешениями для распределённой трассировки на базе Grail."), необходимые для доступа к данным трассировки.
* [Обзор трассировок на базе Grail (DPS)](/docs/license/capabilities/traces "Узнайте, как рассчитывается потребление трассировок Dynatrace на базе Grail в модели подписки Dynatrace Platform Subscription (DPS).").

Предварительные знания

* [Концепции распределённой трассировки](/docs/observe/application-observability/distributed-tracing#main-content--concepts "Отслеживайте и анализируйте в реальном времени высокораспределённые системы с Grail.")
* [Семантическая модель для трассировок](/docs/semantic-dictionary/model/trace "Познакомьтесь с моделями семантического словаря, связанными с трассировками.")

## Доступ к спанам и атрибутам спанов через DQL

Спан представляет собой логическую единицу работы внутри трассировки и описывается атрибутами спана. Атрибуты спана устанавливаются инструментацией, которая создаёт спан, и предоставляют подробную информацию о спане, включая тип выполняемой операции и контекст его происхождения. Семантика всех полей документирована в [Семантическом словаре Dynatrace](/docs/semantic-dictionary "Семантический словарь определяет стандартизированные имена полей, используемые в разных типах данных мониторинга, таких как логи, события, спаны, метрики и сущности."). С помощью DQL вы можете:
...
* Выполнять полнотекстовый поиск по любому атрибуту спана типа `string` с помощью [строковых функций, доступных в DQL](/docs/platform/grail/dynatrace-query-language/functions/string-functions "Список строковых функций DQL."), таких как `startsWith`, `endsWith`, `contains` или `matchesPhrase`.
...
* [Концепции распределённых трассировок](/docs/observe/application-observability/distributed-traces/concepts "Узнайте больше об основных концепциях и терминологии распределённой трассировки.")
