...
Расширенная аналитика трассировки на базе Grail предназначена для опытных пользователей, которым требуется продвинутый анализ трассировки, доступный через [Дашборды](../../../analyze-explore-automate/dashboards-and-notebooks/dashboards-new.md "Создавайте интерактивные, настраиваемые представления для визуализации, анализа и обмена данными наблюдаемости в реальном времени."), [Ноутбуки](../../../analyze-explore-automate/dashboards-and-notebooks/notebooks.md "Анализируйте, визуализируйте и делитесь информацией из данных наблюдаемости — всё в одном совместном настраиваемом рабочем пространстве."), [Workflows](../../../analyze-explore-automate/workflows.md "Автоматизируйте ИТ-процессы с помощью Dynatrace Workflows — реагируйте на события, планируйте задачи и подключайте сервисы.") и через [Dynatrace API](../../../dynatrace-api.md "Узнайте, что вам нужно для использования Dynatrace API.").

## Перед началом работы

Необходимые условия

* Убедитесь, что у вашего пользователя есть [разрешения](permissions.md "Управление разрешениями для распределённой трассировки на базе Grail."), необходимые для доступа к данным трассировки.
* [Обзор трассировок на базе Grail (DPS)](../../../license/capabilities/traces.md "Узнайте, как рассчитывается потребление трассировок Dynatrace на базе Grail в модели подписки Dynatrace Platform Subscription (DPS).").
...
* [Семантическая модель для трассировок](../../../semantic-dictionary/model/trace.md "Познакомьтесь с моделями семантического словаря, связанными с трассировками.")

## Доступ к спанам и атрибутам спанов через DQL

Спан представляет собой логическую единицу работы внутри трассировки и описывается атрибутами спана. Атрибуты спана устанавливаются инструментацией, которая создаёт спан, и предоставляют подробную информацию о спане, включая тип выполняемой операции и контекст его происхождения. Семантика всех полей документирована в [Семантическом словаре Dynatrace](../../../semantic-dictionary.md "Семантический словарь определяет стандартизированные имена полей, используемые в разных типах данных мониторинга, таких как логи, события, спаны, метрики и сущности."). С помощью DQL вы можете:
...
* Выполнять полнотекстовый поиск по любому атрибуту спана типа `string` с помощью [строковых функций, доступных в DQL](../../../platform/grail/dynatrace-query-language/functions/string-functions.md "Список строковых функций DQL."), таких как `startsWith`, `endsWith`, `contains` или `matchesPhrase`.
...
* [Концепции распределённых трассировок](../distributed-traces/concepts.md "Узнайте больше об основных концепциях и терминологии распределённой трассировки.")
