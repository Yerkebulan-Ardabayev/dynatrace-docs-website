...
Атрибуты, используемые для обнаружения, отмечены звёздочкой * на странице обзора сервиса, в разделе **Properties and tags**.
...
1. Запросите схему настроек через вызов GET a schema — он содержит информацию о параметрах, включённых в объект настроек.
...
2. Создайте JSON-объект для ваших настроек.
   ...
   ```
3. Используйте вызов POST an object для отправки вашей конфигурации.
...
3. Создайте JSON-объект для обновления. Мы рекомендуем использовать **updateToken** из предыдущего объекта — это обеспечивает корректное версионирование ваших настроек.
   ...
   ```
4. Используйте вызов PUT an object для отправки вашей конфигурации.
...
1. Запросите список существующих правил через вызов GET objects. Укажите схему вашего типа запроса в параметре запроса **schemaIds**.
   ID схемы зависит от типа запроса, как указано в таблице ниже.
2. Найдите правило, которое хотите удалить, и скопируйте его **objectId**.
3. Удалите правило через вызов DELETE an object. Используйте ID объекта, полученный на предыдущем шаге.
