...
[Список метрик](/docs/dynatrace-api/environment-api/metric-v2/get-all-metrics "Список всех метрик, доступных в вашей среде мониторинга, через конечную точку Метрики v2 API.")

[Получение точек данных](/docs/dynatrace-api/environment-api/metric-v2/get-data-points "Чтение точек данных одной или нескольких метрик через конечную точку Метрики v2 API.")

[Принятие точек данных](/docs/dynatrace-api/environment-api/metric-v2/post-ingest-metrics "Принятие пользовательских метрик в Dynatrace через конечную точку Метрики v2 API.")

[Селектор метрик](/docs/dynatrace-api/environment-api/metric-v2/metric-selector "Настройка селектора метрик для конечной точки Метрики v2 API.")

[Выражения метрик](/docs/dynatrace-api/environment-api/metric-v2/metric-expressions "Использование выражений метрик для применения арифметических операций в запросе точек данных через конечную точку Метрики API v2.")
...
[Контролируемые сущности](/docs/dynatrace-api/environment-api/entity-v2 "Узнайте о конечной точке Контролируемые сущности Dynatrace API.")
...
[Проблемы v2](/docs/dynatrace-api/environment-api/problems-v2 "Узнайте, что предлагает конечная точка Проблемы v2 Dynatrace API.")

### Релизы

[Релизы](/docs/dynatrace-api/environment-api/releaseapi "Узнайте, что предлагает конечная точка Релизы Dynatrace API.")
...
[Географические регионы](/docs/dynatrace-api/environment-api/rum/geographic-regions "Просмотр запросов, доступных через конечную точку Географические регионы Dynatrace.")

[Сеансы пользователей](/docs/dynatrace-api/environment-api/rum/user-sessions "Узнайте, что предлагает язык запросов Сеансов пользователей Dynatrace API.")
...
[Настройки](/docs/dynatrace-api/environment-api/settings "Узнайте, что предлагает конечная точка Настройки Dynatrace API.")

### SLO

[Цели обслуживания](/docs/dynatrace-api/environment-api/service-level-objectives "Откройте для себя функциональные возможности конечной точки Целей обслуживания, работающих на Grail.")

### Синтетический

[Мониторы](/docs/dynatrace-api/environment-api/synthetic/synthetic-monitors "Управление синтетическими мониторами через конечную точку Синтетический v1 API.")

[Выполнения мониторов v2](/docs/dynatrace-api/environment-api/synthetic-v2/synthetic-monitor-execution "Просмотр результатов выполнения Синтетических мониторов через конечную точку Синтетический v2 API.")

[Местоположения v1](/docs/dynatrace-api/environment-api/synthetic/synthetic-locations "Управление синтетическими местоположениями через конечную точку Синтетический v1 API.")

[Местоположения v2](/docs/dynatrace-api/environment-api/synthetic-v2/synthetic-locations-v2 "Управление синтетическими местоположениями через конечную точку Синтетический v2 API.")

[Узлы v1](/docs/dynatrace-api/environment-api/synthetic/synthetic-nodes "Получение информации о синтетических узлах через конечную точку Синтетический v1 API.")

[Узлы v2](/docs/dynatrace-api/environment-api/synthetic-v2/synthetic-nodes-v2 "Управление синтетическими узлами через конечную точку Синтетический v2 API.")
...
Просмотр справочной информации о вашем аккаунте. (/docs/dynatrace-api/account-management-api/reference-data "Просмотр справочной информации аккаунта через API справочных данных API.")
...
Как только вы введете свой OAuth 2.0-токен, вы можете直接 выполнить вызовы API внутри API Explorer. Просто выберите **Попробовать**, чтобы открыть раздел параметров выбранной конечной точки API, где вы можете ввести дополнительные параметры и изменить полезную нагрузку запроса перед выполнением его, выбрав **Выполнить**.
